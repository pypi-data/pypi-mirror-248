...
import cdh_dav_python.cdc_metadata_service.logging_metadata as davt_log_metadata
import cdh_dav_python.cdc_tech_environment_service.job_core as davt_job_core
import cdh_dav_python.cdc_tech_environment_service.repo_core as davt_repo_core
...
import cdh_dav_python.cdc_tech_environment_service.environment_file as davt_env_file
...
if running_local and IPY_ENABLED:
...
class JobMetaData:
    ...
    def get_cluster_library_status(config) -> str:
        ...
        library_file = f"{library_folder}{cdh_dav_python_version}-py3-none-any.whl"
        obj_repo = davt_repo_core.RepoCore()
        results = obj_repo.get_cluster_library_status(
    ...
    def download_config(
        ...
        source_path_1 = obj_file.convert_abfss_to_https_path(cdh_folder_config)
    ...
    def get_parameter_menu(config, spark, dbutils=None):
        ...
        with tracer.start_as_current_span("get_parameter_menu"):
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"SPARK_DIST_CLASSPATH:{os.getenv('SPARK_DIST_CLASSPATH')}"
                    )
                    df_jobs = (
                    ...
                    if dbutils is not None:
                        ...
                        if widgets is not None:
                            dbutils.widgets.dropdown(
                                "report_yyyy", report_yyyy, report_yyyy_values
                            )
                            dbutils.widgets.dropdown(
                                "report_mm", report_mm, report_mm_values
                            )
                            dbutils.widgets.dropdown(
                                "report_dd", report_dd, report_dd_values
                            )
    ...
    def get_standard_parameters(
        environment: str, dbutils, spark, config_jobs_path
    ) -> dict:
    ...
    def run_publish_release(
        obj_env_metadata: davt_env_metadata.EnvironmentMetaData, config
    ) -> str:
        """Publishes a release to the appropriate environment in Databricks.  This function is not complete.

        Args:
            obj_env_metadata (davt_env_metadata.EnvironmentMetaData): The environment metadata object containing information about the environment.
                ...
                for dataset_metadata in data_collect:
                    ...
                    if config_dataset["is_active"] is True:
                        return_text = obj_dataset.save_dataset(
                            config, spark, dbutils, df_columns, config_dataset
                        )
    ...
    def run_analytics_processing(
        ...
        with tracer.start_as_current_span("run_analytics_processing"):
            try:
                ...
                cdh_databricks_org = config["cdh_databricks_org"]
                ...
                for row in data_collect:
                    ...
                    if environment == "dev":
                        ...
                        response_text = ""
                        response_text = obj_pipeline.save_pipeline_sql(
                        ...
                        save_flag = "skip_save"
                        logger.info(f"skip save requested: {pipeline_name}")
                    ...
                    if execute_flag is None:
                        ...
                        if execute_flag == "skip_execute":
                            ...
                            if pipeline_type == "databricks_sql":
                                ...
                                config_pipeline = (
                                    obj_pipeline.get_execute_pipeline_parameters(
                                        config, config_pipeline
                                    )
                                )
                                ...
                                if running_local is True:
                                    ...
                                    az_kv_key_vault_name = config.get(
                                    ...
                                    az_sub_client_secret_key = (
                                        az_sub_client_secret_key.replace("-", "_")
                                    )
                                    ...
                                    dbutils.notebook.run(
                                        path_to_execute, 900, arg_dictionary
                                    )
                                ...
                                davt_parquet_folder = config[
                                    "cdh_folder_database"
                                ].replace("/delta", "/" + export_format)
                                davt_parquet_path = (
                                ...
                                df_export.repartition(1).write.format(
                                    export_format
                                ).mode("overwrite").save(davt_parquet_path)
    ...
    def run_job_name(cls, obj_env, spark, job_name, config, dbutils) -> str:
        ...
        Args:
            obj_env (_type_): The environment object.
            spark (_type_): The Spark session.
            job_name (_type_): The name of the job to run.
    ...
    def run_ingress_processing(
        ...
        with tracer.start_as_current_span("run_ingress_processing"):
            try:
                ...
                # In the future, make sure to apply table filters to the columns dataframe
                if filter_column_name is not None and filter_column_name != "all":
                    filter_text = f"(data_product_id == '{data_product_id}') and {filter_column_name} == '{filter_value}'"
                    filter_text = (
                        filter_text
                        + " and is_active not in ('False', 'FALSE','false','0')"
                    )
                    df_datasets = df_datasets.filter(filter_text)
                    # filter for current project
                    count_string = str(df_datasets.count())
                ...
                for dataset_metadata in data_collect:
                    ...
                    if config_dataset.get("source_dataset_name"):
                        ...
                        return_text = "source_dataset_name is empty"
