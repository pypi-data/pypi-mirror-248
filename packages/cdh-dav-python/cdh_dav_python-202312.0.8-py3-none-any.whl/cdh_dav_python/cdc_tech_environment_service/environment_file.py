...
import cdh_dav_python.cdc_tech_environment_service.repo_core as cdh_repo
...
from cdh_dav_python.az_storage_service import az_storage_file as cdc_az_storage_file
...
class EnvironmentFile:
    ...
    def prepend_line_to_file(
        cls, source_path: str, destination_path: str, line_to_prepend: str
    ) -> str:
        ...
        with tracer.start_as_current_span("get_file_size"):
            try:
                file_exists = cls.file_exists(
                    running_local, path, dbutils, client_id, client_secret, tenant_id
                )
                ...
                if file_exists is True:
                    ...
                    if running_local is True:
                        ...
                        https_path = cls.convert_abfss_to_https_path(path)
                        ...
                        obj_az_storage_file = cdc_az_storage_file.AzStorageFile()
                        file_size = obj_az_storage_file.get_file_size(
                            account_url, credential, storage_container, file_path
                        )
                    else:
    ...
    def copy_url_to_blob(
        cls, config: dict, src_url: str, dest_path: str, file_name: str
    ) -> str:
        """
        ...
        with tracer.start_as_current_span("copy_url_to_blob"):
            try:
                dest_path = cls.fix_url(dest_path)
    ...
    def get_latest_file(path, file_type=None, prefix=None):
        ...
        if prefix:
            files = [
                file for file in files if os.path.basename(file).startswith(prefix)
            ]
