...
def plot_ts(t, latency, outbase, title, summary):
    ts = TimeSeries(latency, name='Latency', unit='s', times=t)
    out_file = Path(outbase + '-latency.png')
    plot = ts.plot(figsize=[12.0, 10.0], dpi=100)
    ...
    if args.frame_type and args.gps:
        ...
        for f in flist:
            ...
            if age >= 0:
                res.append(age)
                gps_times.append(egps)
                logger.debug(f'{f}: age: {age}, egps: {egps}')
