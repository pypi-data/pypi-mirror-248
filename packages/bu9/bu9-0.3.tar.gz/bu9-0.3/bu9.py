...
banner_lines = [
    ...
    OKPURPLE + "version@ 0.9.8 ®" + ENDC,
    ORANGE + "All rights reserved 2022-2023 ♛: ®" + ENDC,      
    OKYELLOW + "Programmed by King  t.me/ssskingsss ☏: " + OKYELLOW + "®" + ENDC,
    ORANGE + "(¯`·.¸¸.·´¯`·.¸¸.-> 😁😊😭 <-.¸¸.·´¯`·.¸¸.·´¯)" + ENDC,
    ORANGE + "This Script is a Tool used for Scanning" + ENDC,
    ORANGE + "and Retrieving Info on Websites & IP" + ENDC,
    ORANGE + "Addresses." + ENDC,
    FAIL + "!!!WARNING!!!USAGE OF THIS TOOL IS" + ENDC,
    FAIL + "SOLELY UP TO USER DISCRETION, THIS TOOL" + ENDC,
    FAIL + "WAS MADE FOR WEBHOST/BUGBOUNTY HUNTING" + ENDC
]

# Show specific lines during the disappearing and reappearing effect
specific_lines = [
...
# Blink function to make the lines blink three times
def blink_line(line):
    for _ in range(1):
        print(line)
        time.sleep(0.5)
...
# Display specific lines with blinking effect
for line in banner_lines:
    if line in specific_lines:
        blink_line(line)
    else:
        print()

# Print the full banner again without the specified lines
for line in banner_lines:
...
def script2():
    ...
    # The user is prompted to enter a domain name for which they want to find subdomains and IPs #
    # The user is then prompted to enter a domain name e.g google.com, the script will then      #
    # prompt the user to save the results (y/n). Then it will ask the user to input the name of  #
    # txt file they want to save their results as...                                             #
        ...
        with open(save_filename, 'a+') as file:
            for cidr in cidrs_list:
                ...
                for index, ip in enumerate(iprange):
                    try:
                        with sem:
                            print("{}[INFO] Probing... ({}/{}) [{}]{}".format(R, index + 1, len(iprange), ip, Blu))
                            ...
                            threading.Thread(target=scan_server, args=(ip, 80, user_agent, save_filename)).start()
                            threading.Thread(target=scan_server, args=(ip, 443, user_agent, save_filename)).start()
    ...
    def nslookup_single(target):
        try:
            ...
            if process.returncode == 0:
                ...
                return f"Error executing nslookup for {target}:\n{stderr.decode()}"
        except Exception as e:
            return f"An error occurred: {str(e)}"
    ...
    def establish_websocket_connection(websocket_domain, ip_or_domain, port, output_file_name):
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                while ip_addresses:
                    current_batch = ip_addresses[:10]
                    ip_addresses = ip_addresses[10:]

                    for i, ip in enumerate(current_batch):
                        with print_lock:
                            print(f"Attempting WebSocket connection to {ip} ({i + 1}/{len(current_batch)})")

                        result = attempt_connection(ip, port, output_file_name)

                        if result:
                            successful_connections += 1

                    # Timeout for 5 seconds after attempting a batch
                    time.sleep(5)
...
def script17():
    ...
    if output_to_file:
        ...
        print(f"Output saved to 'output.txt' in the same directory.")
    else:
        print(output_data)
...
def script19(w):
    try:
        ...
        def set_custom_resolver():
            ...
            # Hard code the name servers
            custom_resolver.nameservers = ['8.8.8.8', '8.8.4.4']

            dns.resolver.override_system_resolver(custom_resolver)
        ...
        def get_user_choice():
            return input("single domain or a list from txt file (1 or 2): ").lower()

        def get_single_domain():
            return input("Enter the single domain: ")

        def get_domain_list_from_file():
            file_name = input("Enter the file name containing the list of domains (should be in the same directory): ")
        ...
        def get_output_file_name():
            return input("Enter the output file name (should be in the same directory): ")

        def process_domain(host, output_file):
            cdn_result = find_cdn_from_host(host, output_file)
            if cdn_result:
                print(f"\nThe CDN provider for {host} is: {cdn_result}")
                tls_ssl_certificates = fetch_tls_ssl_certificate(host)
                save_to_file(output_file, f"Domain: {host}", f"CDN Provider: {cdn_result}")
                if tls_ssl_certificates:
                    save_to_file(output_file, f"TLS/SSL Certificate:\n{tls_ssl_certificates}")
                try:
                    # Wrap the potentially problematic code in a try block
                    check_dns_records(host, output_file)
                except Exception as e:
                    print(f"An error occurred: {e}")
                cdn_result_from_headers = check_http_headers(f"http://{host}", host, output_file)
                if cdn_result_from_headers:
                    save_to_file(output_file, f"HTTP Headers:\n{cdn_result_from_headers}")
            else:
                print(f"\nUnable to determine CDN provider for {host}.")
        ...
        def find_cdn_from_host(host, output_file):
            ip_address = resolve_host_ip(host)
            if ip_address:
                tls_ssl_certificates = fetch_tls_ssl_certificate(host)
                dns_cdn_result = find_cdn_from_host_dns(host)

                if tls_ssl_certificates is not None:
                    for record_type in ['A', 'AAAA', 'SRV', 'CAA', 'PTR', 'NAPTR', 'HINFO', 'LOC', 'SPF', 'SSHFP', 'TLSA', 'SOA', 'MX', 'DNAME', 'DNSKEY', 'DS', 'CDS', 'URI', 'HIP', 'KEY', 'TXT']:
                        check_dns_records(host, record_type, output_file)

                    print(f"\nTLS/SSL Certificate for {host}:\n{tls_ssl_certificates}")
                    cdn_result_from_headers = check_http_headers(f"http://{host}", host, output_file)

                    if cdn_result_from_headers:
                        return cdn_result_from_headers

                    cdn_result = find_cdn_from_certificate(tls_ssl_certificates, dns_cdn_result)
                    return cdn_result
                else:
                    print(f"Unable to fetch TLS/SSL certificate for {host}")
            return None

        def find_cdn_from_host_dns(host):
            newhost = host
            try:
                ...
                return find_cdn(newhost)
            return find_cdn(newhost)

        def find_cdn_from_certificate(cert, dns_cdn_result):
            if dns_cdn_result:
                return dns_cdn_result
        ...
        def check_dns_records(host, record_type, output_file):
            try:
                records = dns.resolver.resolve(host, record_type)
                for record in records:
                    print_dns_record(host, record_type, record.to_text())
                    save_to_file(output_file, host, "DNS Record", record_type, record.to_text())
            except dns.resolver.NoAnswer:
                pass
            except dns.resolver.NXDOMAIN:
                print(f"No {record_type} Record found for {host}")
                save_to_file(output_file, host, "DNS Record", record_type, "No Record Found")
            except dns.resolver.NoNameservers:
                print(f"No nameservers found for {host}")
                save_to_file(output_file, host, "DNS Record", record_type, "No Nameservers Found")
            except dns.resolver.Timeout:
                print(f"Timeout while resolving {record_type} Record for {host}")
                save_to_file(output_file, host, "DNS Record", record_type, "Timeout")
            except Exception as e:
                print(f"An error occurred: {e}")


        def check_http_headers(url, host, output_file):
            try:
                response = requests.head(url)
                headers = response.headers
                print(f"\nHTTP Headers for {url}:\n{headers}")

                # Check if headers have already been saved to the file
                if 'HTTP Headers' not in open(output_file).read():
                    save_to_file(output_file, host, "HTTP Headers", str(headers))

                cdn_result = None
                for cdn in CDN_PROVIDER:
                    if cdn[0] in headers.get('Server', ''):
                        cdn_result = cdn[1]
                        break
        ...
        def print_dns_record(host, record_type, records):
            print(f"\n{record_type} Record for {host}:\n{records}")

        def save_to_file(output_file, *lines):
            try:
                with open(output_file, 'a') as file:
                    for line in lines:
                        file.write(f"{line}\n")
                ...
                print(f"Error while saving results to {output_file}: {e}")
        ...
        def w():
            ...
            if user_choice == '1':
                host = get_single_domain()
                output_file = get_output_file_name()
                process_domain(host, output_file)
            elif user_choice == '2':
                domain_list = get_domain_list_from_file()
        ...
        print("Error: Module not found. Please follow the instructions below:")
        print('''
            pkg install dns
            pip install dnspython
            if error cant write to path ....
            Go to the directory that the error is telling you to go.
            From Termux, change directory (cd) to that directory.
            Then, edit the resolver.py file using nano, like so: nano resolver.py.
            Look for the path it's saying it can't write to in the resolver.py code 
            and update it to /data/data/com.termux/files/usr/etc/resolv.conf.
            Part one is done! Now, go back to Termux home and change directory to 
            /data/data/com.termux/files/usr/etc.
            Update the resolv.conf file in that directory using nano: nano resolv.conf.
            Add these two lines:
            nameserver 8.8.4.4
            nameserver 8.8.8.8
            Press Ctrl and X, then Y to save. That should be it. Happy hunting!
        ''')
...
def script20():
    ...
    if choice == '1':
        ...
        script19(w=1)  # Assuming script19() is defined elsewhere
    elif choice == '18':
        script21()  # Assuming script21() is defined elsewhere
    else:
        print("Returning to BUGHUNTERS PRO...")
        time.sleep(2)
        os.system('cls' if os.name == 'nt' else 'clear')
