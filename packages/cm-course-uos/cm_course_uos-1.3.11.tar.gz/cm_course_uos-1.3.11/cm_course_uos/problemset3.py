...
class StroopNetwork:
    ...
    def forward(color_input, word_input,task_input, return_activations=False, weights=None):
      ...
      activation_output = StroopNetwork.output_activation(activation_ch, activation_wh,weights, bias)
    ...
    def simulate_stroop(color_input, word_input, task_input):
        ...
        response_probabilities = StroopNetwork.forward(color_input, word_input, task_input, weights=problemset3.StroopNetwork.TweakedWeights)
    ...
    def train_stroop(traning_data, n_epochs=100, learning_rate=0.01):
        ...
        Weights = problemset3.StroopNetwork.TweakedWeights
...
def test_conditions(conditions):
    ...
    if not (np.array_equal(cn_neutral[0], neutral) != np.array_equal(cn_neutral[1],neutral)) or not np.array_equal(cn_neutral[2],color_naming):
        print(err_msg)
        return
    ...
    if not (np.array_equal(cn_neutral[0], neutral)  != np.array_equal(cn_neutral[1],neutral)) or not np.array_equal(cn_neutral[2],word_reading):
        print(err_msg)
        return    
