...
class TestFetchNested(object):
    ...
    def test_struct_df(self,duckdb_cursor):
        ...
        compare_results("SELECT a from (SELECT STRUCT_PACK(a := i, b := i) as a FROM range(10) tbl(i)) as t",[{'a': 0, 'b': 0}, {'a': 1, 'b': 1}, {'a': 2, 'b': 2}, {'a': 3, 'b': 3}, {'a': 4, 'b': 4}, {'a': 5, 'b': 5}, {'a': 6, 'b': 6}, {'a': 7, 'b': 7}, {'a': 8, 'b': 8}, {'a': 9, 'b': 9}])

        compare_results("SELECT a from (SELECT STRUCT_PACK(a := LIST_VALUE(1,2,3), b := i) as a FROM range(10) tbl(i)) as t",[{'a': [1, 2, 3], 'b': 0}, {'a': [1, 2, 3], 'b': 1}, {'a': [1, 2, 3], 'b': 2}, {'a': [1, 2, 3], 'b': 3}, {'a': [1, 2, 3], 'b': 4}, {'a': [1, 2, 3], 'b': 5}, {'a': [1, 2, 3], 'b': 6}, {'a': [1, 2, 3], 'b': 7}, {'a': [1, 2, 3], 'b': 8}, {'a': [1, 2, 3], 'b': 9}])
    ...
    def test_nested_mix(self,duckdb_cursor):
        ...
        compare_results("SELECT [{'i':1,'j':2},NULL,{'i':2,'j':NULL}] as a", [[{'i': 1, 'j': 2}, None, {'i': 2, 'j': None}]])

        # Lists of structs with lists
        compare_results("SELECT [{'i':1,'j':[2,3]},NULL] as a", [[{'i': 1, 'j': [2, 3]}, None]])
                
        # Maps embedded in a struct
        compare_results("SELECT {'i':mp,'j':mp2} as a FROM (SELECT MAP(LIST_VALUE(1, 2, 3, 4),LIST_VALUE(10, 9, 8, 7)) as mp, MAP(LIST_VALUE(1, 2, 3, 5),LIST_VALUE(10, 9, 8, 7)) as mp2) as t", [{'i': {'key': [1, 2, 3, 4], 'value': [10, 9, 8, 7]}, 'j': {'key': [1, 2, 3, 5], 'value': [10, 9, 8, 7]}}])
        ...
        compare_results("SELECT [{'i':1,'j':[2,3]},NULL,{'i':1,'j':[2,3]}] as a",[[{'i': 1, 'j': [2, 3]}, None, {'i': 1, 'j': [2, 3]}]])
        ...
        df_duck = duckdb.query("SELECT col0 as a FROM (VALUES (MAP(LIST_VALUE(1,2),LIST_VALUE(3,4))),(NULL), (MAP(LIST_VALUE(1,2),LIST_VALUE(3,4))), (NULL))").df()
        duck_values = df_duck['a']
