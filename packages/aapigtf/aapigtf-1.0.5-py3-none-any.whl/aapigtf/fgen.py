...
def create_excel_sheet_for_testcase(file_path):
    ...
    # Remove the default sheet created and create "Configuration" sheet
    default_sheet = workbook.active
    workbook.remove(default_sheet)
    config_sheet = workbook.create_sheet(title="Configuration")

    config_headers = ['Header', 'Key', 'Value']
    config_sheet.append(config_headers)
    config_headers = ['Cookie_Headers', 'xheaders', '{"Content-Type": "application/x-www-form-urlencoded", "accept": "application/json", "X-User-ID": "xxxxxx"}']
    config_sheet.append(config_headers)
    config_headers = ['test_data_xl', 'test_case_data_xl', 'api_test_case_and_data.xlsx']
    config_sheet.append(config_headers)
    config_headers = ['App_Sys', 'xapireport', 'apiReport']
    config_sheet.append(config_headers)
    config_headers = ['Tags', 'Run_Type', 'Smoke']
    config_sheet.append(config_headers)
    config_headers = ['bURL', 'burl', 'https://<api_base_url>/']
    config_sheet.append(config_headers)
    config_headers = ['Reporting', 'title', '<Project Title>']
    config_sheet.append(config_headers)
    config_headers = ['Reporting', 'tester', '<Tester name>']
    config_sheet.append(config_headers)
    config_headers = ['Reporting', 'description', '<Project Description>']
    config_sheet.append(config_headers)
    config_headers = ['Header', 'Key', 'Value']
    config_sheet.append(config_headers)

    config_header_font = Font(bold=True, color="FFFFFF", underline="single")
    config_header_fill = PatternFill(start_color="FF0000", end_color="FF0000", fill_type="solid")

    for cell in config_sheet[1]:
        cell.font = config_header_font
        cell.fill = config_header_fill
    for cell in config_sheet[10]:
        cell.fill = config_header_fill
        cell.font = config_header_font

    config_row_border = Border(top=Side(border_style="thin", color="000000"),
                               left=Side(border_style="thin", color="000000"),
                               right=Side(border_style="thin", color="000000"),
                               bottom=Side(border_style="thin", color="000000"))

    for row in config_sheet.iter_rows(min_row=2, max_row=10):
        for cell in row:
            cell.border = config_row_border

    sheet = workbook.create_sheet(title="FMS_Login")
    ...
    sheet.append(headers)
    headers = ['Login with valid credentials', 'LGN001', 'api/login', 'POST', '{"username": "admin", "password": "admin"}', 'None', 'None',
               'statuscode', 'ST4', '200', 'Smoke,Sanity,Regression', 'Critical', '', '', '']

    # Set header values in the first row for "FMS_Login" sheet
    sheet.append(headers)

    # Apply formatting to the header row for "FMS_Login" sheet
    header_font = Font(bold=True, color="FFFFFF", underline="single")
    header_fill = PatternFill(start_color="FF0000", end_color="FF0000", fill_type="solid")
    ...
    # Apply borders to the first two rows except the header row for "FMS_Login" sheet
    row_border = Border(top=Side(border_style="thin", color="000000"),
                        left=Side(border_style="thin", color="000000"),
                        right=Side(border_style="thin", color="000000"),
                        bottom=Side(border_style="thin", color="000000"))
...
from aapigtf.plite import get_create_config
urllib3.disable_warnings()
from pathlib import Path
from aapigtf.common import generate_test_api
...
if __name__ == '__main__':
    ...
    for xfilepath in gen_file_path:
        if xfilepath.is_file():
            sheetname = args.s
            exeLine = args.l
            ePointsapi = mypath / 'utils' / 'endpointapi.ini'
            confxx = mypath / 'config.ini'
            fconNames = [confxx, ePointsapi]
            get_create_config(xfilepath, sheetname='Configuration', confnames=fconNames)
...
6.	To install dependencies, Type Command: ‘pip install aapigtf -U’
```
**Start new project (follow the steps to create project, run tests and see results)**
```
1.	Go to root or the directory where to create project : aapigtf <project_name> apix
...
from pathlib import Path
from configparser import ConfigParser
from aapigtf.common import get_tags, Create_New_Report, get_results, getepoint, get_sheetnames_excel, excel_row_data
from aapigtf.apiexecutor import apiSteps
