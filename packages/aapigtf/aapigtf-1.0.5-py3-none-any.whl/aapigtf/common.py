    ...
    if trig == 'YES':
        ...
        with open(pathdata_file, 'w') as f:
            ...
            for getxl in getallxm:
                if getxl != 'Configuration':
                    f.write('class Api_Onefms_' + str(getxl) + '(apicall):\n\n')
                    f.write('\tdef start(self):\n')
                    f.write('\t\tself.maxDiff = None\n\n')
                    f.write(
                        '\t@file_data(str(r"'+str(generate_test_api)+'"), line=line, end_line=end_line, sheet="' + str(getxl) + '")\n')
                    f.write(
                    ...
                    f.write("\t\t\tgtdata = get_sheetnames_excel(r'"+str(generate_test_api)+"', itemdata=depend, ustory=tcdef)\n")
