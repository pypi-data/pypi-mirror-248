...
def create_config(config_data, config_number):
    config_file = f"{config_number}"
    with open(config_file, "w") as f:
        for header, values in config_data.items():
            f.write(f"[{header}]\n")
...
def get_create_config(filepath, sheetname, confnames):
    ...
    for index, row in df.iterrows():
        if row['Header'] == 'Header' and row['Key'] == 'Key' and row['Value'] == 'Value':
            if config_data:
                create_config(config_data, filenames[config_number])
                config_number += 1
                config_data = {}
        else:
            header = row['Header']
            key = row['Key']
            value = row['Value']
            if header not in config_data:
                config_data[header] = {}
            config_data[header][key] = value
