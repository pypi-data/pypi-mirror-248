...
class Collection:
    ...
    def upsert(
        ...
        Parameters:
            ...
            max_processes (Optiona[int]): max number of processes if multiproc is selected, multiproc must be enabled
        ...
        if bool(natural_inputs):
            _upsert_natural_input(
                vector_tuples=list(zip(ids, natural_inputs, properties)),
            ...
            _upsert_vectors(
                vector_tuples=list(zip(ids, vectors, properties)),
                chunk_size=batch_size,
                collection_name=self.name,
                edge_services_url=self._vector_serving_client._edge_services_url,
                multiproc=multiproc,
    ...
    @typechecked
    def delete(self, vector_ids: List[str], batch_size: int = 10000) -> int:
        """
        ...
        return sum(
            ...
            for ids_chunk in _divide_chunks(vector_ids, batch_size)
        )
