...
class VectorServingServiceMock(VectorServiceServicer):
    ...
    def FetchVector(self, request, context):
        ...
        collection_store = self._vector_collections[request.collection_name]
        if request.vector_id not in collection_store:
            context.set_details(f"Vector with id {request.vector_id} doesn't exist'")
            context.set_code(grpc.StatusCode.NOT_FOUND)
            return

        stored_vector_dict = collection_store[request.vector_id]
        return FetchVectorResponse(
            vector=StoredVector(
                id=stored_vector_dict["id"],
