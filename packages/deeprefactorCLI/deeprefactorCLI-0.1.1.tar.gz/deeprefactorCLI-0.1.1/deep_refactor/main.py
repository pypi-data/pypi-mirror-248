...
def login():
    ...
    response = requests.post(
        "https://api.deeprefactor.dev/cli/validate-api-key", json={"api_key": api_key}
    )
...
def repos():
    ...
    response = requests.get("https://api.deeprefactor.dev/cli/repos", headers=headers)
...
def repo(project_name: str):
    ...
    response = requests.get(
        f"https://api.deeprefactor.dev/cli/repo-details/{project_name}", headers=headers
    )
...
def feedback(project_name: str, file_path: str):
    ...
    url = f"https://api.deeprefactor.dev/cli/repo-feedback/{project_name}"
...
def send_analysis_request(
    ...
    url = "https://api.deeprefactor.dev/cli/analysis"
    headers = {"Authorization": f"Bearer {api_key}"}
    payload = {
        "file_path": file_path,
...
# Delete all old builds with rm -rf dist/*
