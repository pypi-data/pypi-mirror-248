...
class DateGo:
    ...
    def youku_date(raw):
        ...
        优酷时间处理方法
        """
        if raw == '昨天' or raw == '一天前':
            return DateGo.date_befor_days(1)
        elif raw == '前天' or raw == '两天前':
            return DateGo.date_befor_days(2)
        elif raw == '三天前':
            return DateGo.date_befor_days(3)
        elif raw == '四天前':
            return DateGo.date_befor_days(4)
        elif raw == '五天前':
            return DateGo.date_befor_days(5)
        elif raw == '六天前':
            return DateGo.date_befor_days(6)
    ...
    @staticmethod
    def get_year_date(year:int):
        """
        获取一年中所有的日期  YYYY-MM-DD
        :param year: 年份
        :return:
        """
    ...
    def gen_date(days:int):
        ...
        # 生成近100天的日期
        for i in range(days):
            # 将日期对象格式化成字符串，然后添加到列表中
            dates.append(start_date.strftime("%Y-%m-%d"))
            # 将日期递减一天，以便获取前一天的日期
            start_date -= datetime.timedelta(days=1)
        # 反转列表，以便按照从早到晚的顺序排列日期
        dates.reverse()
        return dates