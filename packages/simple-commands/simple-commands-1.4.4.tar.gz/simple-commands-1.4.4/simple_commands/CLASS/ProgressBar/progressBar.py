...
class __ProgressBar:
    ...
    def set_start(self, n="[current_step/total_steps]", y=':', start='[', end=']'):
        ...
        self.__start = n
        self.__y = y
        self.__start_n = start
        self.__end = end
    
    def __n(self):
        starts = {"[current_step/total_steps]":str(self.__current_step)+"/"+str(self.__total_steps+1),
        ...
        try:
            return str(str(self.__start_n) + str(starts[self.__start]) + str(self.__end))
        except (KeyError, TypeError):
            try:
                if isinstance(self.__start, tuple) or isinstance(self.__start, list):
                    n =''
                    m = 0
                    for i in self.__start:
                        print(starts["time"][str(i)])
                        s = '' if m - (len(self.__start) -1) == 0 else str(self.__y)
                        n += str(starts["time"][str(i)]) + s
                        m += 1
                    return str(str(self.__start_n) + n + str(self.__end))
                else:
                    return str(str(self.__start_n) + str(starts["time"][self.__start]) + str(self.__end))
            except KeyError:
                return self.__start

    def generate(self, iterable, description:str=None, length=50, bar_character="#", empty_character="-"):
        ...
        for value in iterable:
            ...
            self.start = self.__n()
            print(f"\r{self.start} [{self.__bar}] {self.__progress_percentage:.2f}%{' description:' + self.__description if self.__description is not None else ''}", end='', flush=True)
            yield value
            
    def Progress_print(self, value):
        print(f"\r{self.start} [{self.__bar}] {self.__progress_percentage:.2f}%{' description: ' + self.__description if self.__description is not None else ''} value: {value}", end='', flush=True)
