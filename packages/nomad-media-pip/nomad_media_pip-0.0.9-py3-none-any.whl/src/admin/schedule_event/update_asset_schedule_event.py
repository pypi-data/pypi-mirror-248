...
from src.admin.schedule_event.event_types import _EVENT_TYPES
from src.admin.schedule_event.get_asset_schedule_event import _get_asset_schedule_event

import requests, json

def _update_asset_schedule_event(AUTH_TOKEN, URL, ID, CHANNEL_ID, ASSET, IS_LOOP, 
                                 DURATION_TIME_CODE, DEBUG):
        
        API_URL = f"{URL}/liveChannel/{CHANNEL_ID}/liveScheduleEvent/{ID}"

        SCHEDULE_EVENT_INFO = _get_asset_schedule_event(AUTH_TOKEN, URL, CHANNEL_ID, 
                                                        ID, DEBUG)
        HEADERS = {
