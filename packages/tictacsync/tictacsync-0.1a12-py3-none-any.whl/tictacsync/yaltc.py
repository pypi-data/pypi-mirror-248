...
# DEFAUT_LEVEL_MODE = 'over_noise_silence' # one or two sample more precise
DEFAUT_LEVEL_MODE = 'mean_silence_AFSK' # more robust for audio coupling
OVER_NOISE_FACTOR = 1.1
...
class Decoder:
    ...
    Attributes:
        ...
        cached_convolution_fit : dict
            if _fit_triangular_signal_to_convoluted_env() has already been
            called, will use cached values if sound_extract_position is the
            same.

        level_mode : str
            one of 'mean_silence_AFSK', 'over_noise_silence': how is determined
            self.pulse_detection_level. See _get_pulse_detection_level()

    """

    def __init__(self, aRec, level_mode):
        """
        Initialises Decoder
    ...
    #     """
    #     Returns the already detected position previously set
    #     by _detect_sync_pulse_position(). The position is in samples number
    #     relative to extract beginning
    #     """
    #     if self.detected_pulse_position:
    #         logger.debug('returning detected value')
    #         fct_trace = ' => '.join([e[-2] for e in traceback.extract_stack()])
    #         print(fct_trace)
    ...
    def make_silence_analysis_plot(self, title=None, filename=None):
        ...
        pulse_pos_in_file = approx_pulse_x + self.sound_extract_position
        ...
        ax.legend(
            custom_lines,
            'sqr convolution,silence zone,pulse trig level,lower FSK ampl., approx puls.'.split(','),
            loc='lower right')
        ax.plot(
            x_signal, signal,
    ...
    def _detect_sync_pulse_position(self):
        ...
        detected_point += search_pulse_start_point
        logger.debug('found sync pulse at %i in extract'%detected_point)
        distance_to_end = len(abs_signal) - detected_point
        if distance_to_end/self.samplerate < 0.5: # sec
            logger.debug('detected_point too near the end')
            # go back one second
            detected_point -= self.samplerate
            logger.debug('new detected_point %i'%detected_point)
        self.detected_pulse_position = detected_point
        return detected_point

    def _get_word_width_parameters(self):
        abs_signal = abs(self.sound_extract)
        pulse_position = self.detected_pulse_position
    ...
    def _get_BFSK_word_boundaries(self):
        ...
        # mono_1bit = mono_data>20000
        # closed = closing(mono_1bit, np.ones(400))
        # labels = label(closed)
        # n_clics = max(labels)
        ...
        word_extract = self.sound_extract[pulse_position :
            search_end_position]
        self.sound_extract[search_end_position] # out of bounds?
...
class Recording:
    ...
    Attributes:
        ...
        YaLTC_channel : int
            ...
            Set in _read_sound_and_find_YaLTC().
    ...
    def __init__(self, media, level_mode=DEFAUT_LEVEL_MODE):
        """
        ...
        if recording_init_fail == '': # success
            self.decoder = Decoder(self, level_mode)
    ...
    def _find_time_around(self, time, plots):
        ...
        if time < 0: # if negative, time from end
            there = self.get_duration() + time
        else:
            there = time
        found = self._read_sound_and_find_YaLTC(there, SOUND_EXTRACT_LENGTH)
        if not found:
            return None
        else:
    ...
    def get_start_time(self, plots=False, progress=None, task=None):
        # returns a python datetime object
        if self.start_time is not None:
            return self.start_time
        cached_times = {}
        def _find_cached_time_around(t_sec, plots=False):
            # wrapper around _find_time_around() for querying any cached values
            logger.debug('cache hit for time t=%s s?'%t_sec)
            time_k = int(t_sec)
            # if cached_times.has_key(time_k):
            if CACHING and time_k in cached_times:
                logger.debug('YES, cache hit for t=%s s'%t_sec)
                logger.debug('cached values \n%s'%pprint.pformat(cached_times))
                return cached_times[time_k]
            else:
                logger.debug('NO, for t=%s s not cached, will compute'%time_k)
        ...
        for i, pair in enumerate(TRIAL_TIMES):
            ...
            time_around_beginning = _find_cached_time_around(near_beg, plots)
            # if self.YaLTC_channel is None:
            #     logger.debug('self.YaLTC_channel is None')
            #     return None
            logger.debug('Trial #%i, end at %f'%(i+1, near_end))
            # time_around_end = self._find_time_around(near_end)
            time_around_end = _find_cached_time_around(near_end, plots)
            ...
            if coherence:
                logger.debug('Succeeded at trial: #%i/%i'%(i+1,
                        len(TRIAL_TIMES)))
                break
        if not coherence:
        ...
        if None in [time_around_beginning, time_around_end]:
            ...
            self.start_time = None
            return None
        # if here, coherent time has been found.
        ...
        start_UTC += in_cam_correction
        DAC_delay_correction = timedelta(microseconds=SAMD21_LATENCY*1e-6)
        start_UTC -= DAC_delay_correction
    ...
    def _read_sound_and_find_YaLTC(self, time_where, chunk_length):
        """
        ...
        Returns:
            True if YaLTC signal found, False if not.
        ...
        self.YaLTC_channel = None
        logger.warning('found no YaLTC signal around %.2fs'%
            (time_where-self.get_duration()))
        return False
    
    def seems_to_have_YaLTC_at_beginning(self):
        if self.probe is None:
            return False
        return self._read_sound_and_find_YaLTC(TRIAL_TIMES[0][0],
