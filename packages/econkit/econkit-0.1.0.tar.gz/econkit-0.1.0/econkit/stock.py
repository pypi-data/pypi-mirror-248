...
def stock(ticker_symbol: str, start_date: str, end_date: str, interval: str):
    """
    Download stock data and return as a pandas DataFrame.
    The data is saved in a CSV file within a specified folder named 'Portfolio-Optimization'.
    ...
    stock = yf.download(ticker_symbol, start=start_date, end=end_date, interval=interval)

    # Calculate % growth of Adjusted Close price
    stock['Returns'] = stock['Adj Close'].pct_change() * 100
    ...
    # Read and return the data from the saved CSV file
    dataframe = pd.read_csv(file_name)

    # Create a global variable based on the ticker symbol
