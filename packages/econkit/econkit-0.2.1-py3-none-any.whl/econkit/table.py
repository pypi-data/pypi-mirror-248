...
def table(column_name, *dataframes):
    ...
    starting from the first non-zero row for each column.
    ...
    for df, name in zip(dataframes, df_names):
        if column_name in df.columns:
            # Find the first non-zero row in the specified column
            first_non_zero_row = df[df[column_name] != 0].first_valid_index()
            if first_non_zero_row is not None:
                combined_df[name] = df.loc[first_non_zero_row:, column_name]
            else:
                # If all values are zero, insert missing values
