...
def plot_population_grouped(df_pct, sample_group_map, sort_by = [], bar_width = 0.6, 
    ...
    for j, g in enumerate(glst):
        ...
        ax = dft.plot.bar(width = bar_width, stacked = True, ax = axes[j+cnt], 
...
def cci_get_means( cci_df_dct, cci_idx_lst = None, pval_cutoff = 0.05 ):
    ...
    if cci_idx_lst is None:
        ...
        for j, key in enumerate(cci_df_dct.keys()):
            ...
            if j == 0:
                ...
                idx_union = list(set(idx_common).union(list(cci_df_dct[key].index.values[b])))
...
def plot_pct_box( df_pct, sg_map, ncols, figsize = None, dpi = 100,
    ...
    for k, item in enumerate(items):
        ...
        if stripplot:
            sns.stripplot(x='Group', y=item, data=df, jitter=True, color="black")
        if k%nc == 0: 
...
def plot_cci_dot( dfp, legend_fs = 11, legend_mkr_sz = 6, 
                  tick_fs = 6, xtick_rot = 90, xtick_ha = 'center', 
                  title = None, title_fs = 14,
                             dpi = 120, swap_ax = False ):
    if swap_ax == False:
        a = 'gene_pair'
        b = 'cell_pair'
    else:
...
def plot_cci_circ_group( df_lst, ncol = 3, figsize = (8,7), 
    ...
    for k in df_lst.keys():
        n_max = max( df_lst[k].max().max(), n_max )
    linewidth_scale = linewidth_max/(n_max + 0.01)
    lw_max_dict = {}
    for k in df_lst.keys():
        lw_max_dict[k] = df_lst[k].max().max()*linewidth_scale
