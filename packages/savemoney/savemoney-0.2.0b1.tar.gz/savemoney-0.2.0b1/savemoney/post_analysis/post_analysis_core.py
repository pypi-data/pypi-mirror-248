...
def execute_alignment(ref_seq_list:mc.MyRefSeq, my_fastq:mc.MyFastQ, param_dict, save_dir:Path=None):
    ...
    if save_dir is not None:
        intermediate_results_save_path = save_dir.parent / f"{my_fastq.combined_name_stem}.intermediate_results.txt"
        if intermediate_results_save_path.exists():
            ...
            if ref_seq_list_tmp is not None:
                ...
                skip = True
    intermediate_results_save_path = save_dir / f"{my_fastq.combined_name_stem}.intermediate_results.txt"
    if not skip:
        ###########
        # Execute #
        ###########
