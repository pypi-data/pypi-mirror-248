...
def get_common_portions(string, compare_string):
    return {get_longest_common_portion(string[i:], compare_string) for i in range(len(string))}

def get_shared_characters_count(string, compare_string):
    for char in string:
        compare_string = compare_string.replace(char, '', 1)
    return len(compare_string)
...
def get_closest_match_from_list(comp_obj, total_list, case_sensitive=False,highest_out=True,sorted_out=False):
    ...
    for obj in total_list:
        obj_og = obj
        if isinstance(comp_obj,type(obj)):
            if obj and isinstance(comp_obj,str):
                if not case_sensitive:
                    comp_obj,obj=comp_obj.lower(),obj.lower()
                    total_list = [str_item.lower() for str_item in total_list if isinstance(str_item,type(comp_obj))]
                else:
                    total_list = [str_item for str_item in total_list if isinstance(str_item,type(comp_obj))]
                ...
                if common_portions:
                    ...
                    for portion in common_portions:
                        if comp_obj[:len(portion)] == portion:
                            beggins_with = len(portion)/len(comp_obj)
                    comp_curr_js = {
                        "obj": obj,
                ...
                if comp_obj==obj:
                    return get_out(sorted_out,highest_out,comparison_data,obj)
