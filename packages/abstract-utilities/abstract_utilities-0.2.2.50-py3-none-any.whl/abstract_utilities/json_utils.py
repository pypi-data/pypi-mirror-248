...
import re
import os
from .read_write_utils import check_read_write_params
...
def create_and_read_json(*args,file_path: str=None, json_data: dict = None, error=False, error_msg=True, **kwargs) -> dict:
    """
    ...
    """
    params = check_read_write_params(*args, file_path=file_path,contents=json_data, **kwargs)
    if params:
        if error_msg == True:
            error_msg = f"{file_path} does not exist; creating file with default_data"
        try_read = safe_read_from_json(params[0], default_value=None)
        if try_read is None and json_data is not None:
            safe_dump_to_file(file_path=params[0], data=params[1])
        return safe_read_from_json(file_path=params[0])
...
def safe_dump_to_file(*args,file_path=None,data=None,ensure_ascii=False, indent=4, **kwargs):
    params = check_read_write_params(*args, file_path=file_path,contents=data, **kwargs)
    if params:
...
def safe_write_to_json(*args,file_path:str=None, data:(dict or list or str)=None, ensure_ascii=False, indent=4, error=False, error_msg=None, **kwargs):
    params = check_read_write_params(*args, file_path=file_path,contents=data, **kwargs)
    if params:
...
def safe_read_from_json(*args,file_path: str=None, default_value: Any = None, error: bool = False, error_msg: str = None, **kwargs) -> Any:
    """
    ...
    """
    params = check_read_write_params(*args, file_path=file_path,contents=False, **kwargs)
    if params:
