...
def check_read_write_params(*args, file_path=None, contents=None, **kwargs):
    if not file_path:
        file_path = string_in_keys(['file_path', 'path'], kwargs) or get_path(args)
    if not contents:
        contents_key = string_in_keys(['contents', 'data', 'text', 'content', 'string'], kwargs)
...
def write_to_file(*args, file_path=None, contents=None, **kwargs):
    """
    ...
    """
    params = check_read_write_params(*args, file_path=file_path, contents=contents, **kwargs)
...
def determine_path_and_content(*args,**kwargs):
    ...
    if file_path is None:
        ...
        if os.path.isdir(path) and ext:
            ...
            contents = args[0]
    return file_path,contents
def create_and_read_file(*args, file_path=None, contents:str='', **kwargs) -> str:
    """
    ...
    if not os.path.isfile(params[0]):
        write_to_file(file_path=params[0], contents=params[1])
    return read_from_file(params[0])
