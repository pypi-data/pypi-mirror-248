...
class GLMEmbedding(Embeddings):
    ...
    #这里实现了并发embedding获取
    def aembed_documents(self, texts: List[str],thread_num = 5,thread_sec = 0.3) -> List[List[float]]:
        import threading
        ...
        for i in range(thread_batch):
            ...
            for thread in threads:
                thread.join(thread_sec)  # 设置超时时间为0.3秒
        return self.aembeddings
...
if __name__ == '__main__':
    ...
    aembeddings = (GLMEmbedding().aembed_documents(inputs))
    print(aembeddings)
