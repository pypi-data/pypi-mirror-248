...
from spider.elements.Box import TrackingBoxList, obb2vertices
...
class BoxCollisionChecker(CollisionChecker):
    def __init__(self, ego_veh_length, ego_veh_width, methodflag=COLLISION_CHECKER_SAT):
        super(BoxCollisionChecker, self).__init__(methodflag)
        # self.method = methodflag
    ...
    def setObstacles(self, bboxes_vertices=None, ogm=None):
        # TODO: 考虑ogm位姿是否校正
    ...
    def check(self, ego_box_vertices=None, bboxes_vertices=None):
        ...
        collision = False
        if self.method == COLLISION_CHECKER_SAT:
            for bbox_vertices in self.bboxes_vertices:
                if SAT_check(self.ego_box_vertices, bbox_vertices):
                    collision = True
                    break
        elif self.method == COLLISION_CHECKER_DISK:
            pass
        elif self.method == COLLISION_CHECKER_AABB:
            pass
        else:
