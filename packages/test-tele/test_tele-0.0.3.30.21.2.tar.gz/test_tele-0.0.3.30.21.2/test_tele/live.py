...
from test_tele.pyrogram.vip_vanillarock import inline_vanillarock, get_vr_file, upload_vr_batch
...
async def run_pyrogram():
        ...
        try:
            ...
            for key, handler in handlers.items():
                if message.text.lower().startswith(key):
                    return await handler(app, message)
            else:
                if match:
                    telepages = await cari_konten(match.group(1))
                    if not telepages:
                        return await generate_telegraph(match.group(1))
    ...
    async def callback_query_handler(app, callback_query: CallbackQuery):
        ...
        for prefix, handler in handlers.items():
            ...
            # Handle case when no prefix matches
            pass
