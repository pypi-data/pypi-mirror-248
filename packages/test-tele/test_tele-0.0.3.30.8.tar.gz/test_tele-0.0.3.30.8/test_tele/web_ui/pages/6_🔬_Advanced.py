...
if check_password(st):
    ...
    if st.checkbox("I agree"):
        ...
        with st.expander("Bot Configuration"):
            tab1, tab2, tab3 = st.tabs(['Bot', 'Telegraph', 'Utils'])
            with tab1:
                st.info(
            ...
            with tab3:
                BOT_CONFIG.all_tags.tags = st.text_area(
                    "All available tags", value=BOT_CONFIG.all_tags.tags
                )
                try:
                    all_tags = yaml.safe_load(
                        BOT_CONFIG.all_tags.tags
                    )
                    BOT_CONFIG.all_tags.tags = all_tags
                except:
                    BOT_CONFIG.all_tags.tags = {}
