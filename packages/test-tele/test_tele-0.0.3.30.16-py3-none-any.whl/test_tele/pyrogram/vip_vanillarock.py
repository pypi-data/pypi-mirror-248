        ...
        if response.status_code == 200:
            ...
            for div_post in div_posts[post_number:]:
                ...
                if h2_title: 
                    ...
                    logging.warning(f"hal: {page_number}, post: {post_number}, url: {id_img}")
...
async def set_url(query: str) -> str:
    ...
    for tag in input:
        ...
        logging.warning(f"tag: {gen_tag}")

        if gen_tag:
            query = query.replace(tag, gen_tag)
    ...
    if lists:
        try:
            while True:
                ...
                logging.warning(f"time: {waktu_sekarang}")
                if waktu_sekarang < 8:
                    ...
                    return await exception_msg(client, inline_query)
