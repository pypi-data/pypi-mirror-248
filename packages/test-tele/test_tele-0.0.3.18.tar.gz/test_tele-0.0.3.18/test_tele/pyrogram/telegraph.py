...
class TelegraphManager:
    ...
    @classmethod
    async def get_telegraph(cls):
        if cls._telegraph is None:
            access_token = BOT_CONFIG.telegraph.access_token
            if access_token:
                cls._telegraph = Telegraph(access_token)
            else:
                cls._telegraph = await Telegraph().create_account(short_name=BOT_CONFIG.telegraph.short_name)
                access_token = cls._telegraph['access_token']
                BOT_CONFIG.telegraph.access_token = access_token
                BOT_CONFIG.telegraph.auth_url = cls._telegraph['auth_url']
                write_bot_config(BOT_CONFIG)
        return cls._telegraph


async def generate_new_telepage(images: list, title: str, author_name: str):
    telegraph = await TelegraphManager.get_telegraph()
    content = []
