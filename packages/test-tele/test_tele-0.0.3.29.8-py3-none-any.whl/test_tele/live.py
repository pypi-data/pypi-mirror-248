...
from test_tele.pyrogram.hentaihand import inline_hentaihand, generate_telegraph, cari_konten, slugify
...
async def run_pyrogram():
    ...
    async def incoming_message_handler(app, message: Message):
        ...
        try:
            ...
            if match:
                ...
                if not telepages:
                    link_doujins = await generate_telegraph(await slugify(match.group(1)))
        except:
            pass
    ...
    async def inline_handler(app, inline_query: InlineQuery):
        ...
        if inline_query.query.startswith('.px'):
            ...
            await inline_hentaihand(app, inline_query)
    ...
    @app.on_callback_query(filters.regex(r"(?:gb|px|rp|md|fur)"))
