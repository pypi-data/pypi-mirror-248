...
async def autocomplete(keywords: dict, input_text: str, ret_key=True):
    ...
    if ret_key:
        suggestions = [key for key in keywords.keys() if input_text in key]
    else:
        suggestions = [keywords[key] for key in keywords.keys() if input_text in key]
...
async def exception_msg(client, inline_query):
    err_result = [
        InlineQueryResultArticle(
            ...
            id='requesttimeout', description='Unfortunately, the server is unable to send the results at this time')
    ]
