...
from .utils import OFFSET_PID, BOT_CONFIG, autocomplete, not_found_msg, is_japanese
            ...
            for div_post in div_posts[post_number:]:
                ...
                if h2_title:
                    ...
                    if is_first:
                        ...
                        logging.warning(f"hal: {page_number}, post: {post_number}, id: {id_img}")
                        is_first = False
...
async def upload_vr_batch(app, message):
    ...
    try:
        ...
        for i in range(0, len(lists[-1]['img_urls']), img_slice):
            logging.warning(lists[-1]['img_urls'][i:i + img_slice])
            if asfile:
                await app.send_media_group(
                    message.chat.id, 
                    [InputMediaDocument(media=url) for url in lists[-1]['img_urls'][i:i + img_slice]],
                    disable_notification=True
                )
            else:
                await app.send_media_group(
                    message.chat.id, 
                    [InputMediaPhoto(media=url) for url in lists[-1]['img_urls'][i:i + img_slice]],
                    disable_notification=True
                )
            time.sleep(3)
    except Exception as err:
