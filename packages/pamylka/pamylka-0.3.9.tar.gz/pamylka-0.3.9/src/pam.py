...
class App:
    ...
    def __init__(
        ...
        try:
            with open(f"{self.name}.{extension}", "r", encoding="utf-8") as file:
                pattern = compile("[_ ]{0,2}?version[_ ]{0,2}?")
                file_lines = file.readlines()
                for i in range(len(file_lines)):
                    if pattern.search(file_lines[i]):
                        file_lines[i] = f'__version__ = "{self.version}"\n'
                        with open(f"{self.name}.{extension}", "w", encoding="utf-8") as file:
                            file.writelines(file_lines)
                        if not execute:
                            return
                        with open(f"{self.name}.{extension}pam", "w", encoding="utf-8") as file:
                            file.writelines(file_lines)
                        return
...
class BaseApp(AlphaApp):
    ...
    def __init__(
        ...
        super().__init__(
            name, version, type, date, creator, extension=extension, app="Base App", execute=execute
        )
        """
        Parameters
        -
...
class HeroApp(AlphaApp):
    ...
    def __init__(
        ...
        super().__init__(
            name, version, type, date, creator, extension=extension, app="Hero App", execute=execute
        )
        """
        Parameters
        -
...
class InApp(App):
    ...
    def __init__(
        ...
        super().__init__(
            name, ver, type, date, creator, extension=extension, app="Inside App", execute=execute
        )
        """
        Parameters
        -
...
if __name__ == "__main__":
    pam = AlphaApp(
        "pam", "00.03.09", "Programming", "13/3/1445 Higri", app="python module", execute=False
    )
