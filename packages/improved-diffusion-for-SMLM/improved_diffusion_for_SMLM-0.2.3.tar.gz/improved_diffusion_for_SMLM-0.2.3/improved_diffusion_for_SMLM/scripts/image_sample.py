...
def main(model_path, output_path, image_size, num_samples, num_channels = 64, num_res_blocks = 1, num_heads = 4, num_heads_upsample = -1,
    ...
    while len(all_images) * batch_size < num_samples:
        ...
        if(conditioning):
            ...
            cond_batch = []
            for cond_img in cond_data:
                cond_batch.append(io.imread(os.path.join(cond_dir, cond_img)))
            model_kwargs["conditioning"] = th.tensor(cond_batch, device=dist_util.dev())
        else:
            sample_fn = (
        ...
        if(conditioning):
            sample = sample_fn(
                model,
                (batch_size, 6, image_size, image_size),
