...
def run_manual_exclusion_task(cmd: commands.RunManualExclusion,
                              ...
                              ads_api_client: GoogleAdsApiClient) -> int:
    with uow:
...
def run_task(cmd: commands.RunTask,
    ...
    with uow:
        ...
        if to_be_excluded_placements and task_obj.output in (
            ...
            n_excluded_placements = len(to_be_excluded_placements)
        else:
            end_time = datetime.now()
        ...
        if hasattr(cmd, "task_id") and cmd.task_id:
            ...
            if cmd.schedule:
                uow.published_events.append(
                    events.TaskUpdated(cmd.task_id, cmd.schedule))
            else:
            ...
            if cmd.schedule:
                uow.published_events.append(
                    events.TaskCreated(task_id, cmd.name, cmd.schedule))
        uow.commit()
        task_id = task_obj.id
        return str(task_id)
