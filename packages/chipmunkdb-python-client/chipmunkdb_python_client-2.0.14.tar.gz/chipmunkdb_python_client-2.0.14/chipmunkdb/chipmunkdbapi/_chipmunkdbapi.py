...
class Cursor(object):
    ...
    def _gen_description(self, col_types=None):
        blank = [None] * len(self.columns)
    ...
    def execute(self, operation, parameters=()):
        ...
        self.real_result = resp.json()['result']


        json = resp.json()
        self.data = json['result']
        self._row_stream = iter(self.data)
        self.rowcount = len(self.data)
        self.columns = [d["name"] for d in json["columns"]]
        self.returns_rows = True
...
class Connection(object):
    ...
    def submit_query(self, query: str):
        ...
        resp = self._session.get(
            f'{self._base_url}/query?q='+query,
