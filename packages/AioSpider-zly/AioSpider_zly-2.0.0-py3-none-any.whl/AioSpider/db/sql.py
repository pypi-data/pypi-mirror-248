...
operator = ('>=', '<=', '=', '>', '<')
logical = ('is null', 'is not null', 'in', 'not in')
...
def logical_compare(k, x, y):
    if x in ('in', 'not in'):
        return f'`{k}` {x} {tuple(y) if y else tuple()}'
    else:
        return f'`{k}` {x} {escape_strings(y) if y else y}'
...
class MysqlAlterTable(AlterTableBase):
    ...
    def build_modify_cols_unique_uniun_index_sql(self, indexes):
        ...
        if index_name not in [i['name'] for i in indexes]:
            return f'CREATE UNIQUE INDEX {index_name} ON {self.model.Meta.tb_name} ({", ".join(new_index_fields)});'
        ...
        for i in indexes:
            ...
            if set(i['field']) == set(index_fields) and i['unique']:
                continue
            sql += f'ALTER TABLE {self.model.Meta.tb_name} DROP INDEX {index_name};\n'
            sql += f'CREATE UNIQUE INDEX {index_name} ON {self.model.Meta.tb_name} ({", ".join(new_index_fields)});'
    ...
    def build_modify_cols_uniun_index_sql(self, indexes):
        ...
        # 创建索引
        if index_name not in [i['name'] for i in indexes]:
            return f'CREATE INDEX {index_name} ON {self.model.Meta.tb_name} ({", ".join(new_index_fields)});'
        ...
        for i in indexes:
            ...
            if set(i['field']) == set(index_fields) and not i['unique']:
                continue
            sql += f'ALTER TABLE {self.model.Meta.tb_name} DROP INDEX {index_name};\n'
            sql += f'CREATE INDEX {index_name} ON {self.model.Meta.tb_name} ({", ".join(new_index_fields)});'
...
class SqliteAlterTable(AlterTableBase):
    ...
    def build_modify_cols_uniun_index_sql(self, indexes):
        ...
        index_fields = [i for i in self.model.Meta.union_index if i in self.model.fields]
        new_index_fields = [f'`{i}`' for i in index_fields]
        index_name = f'union_idx_{self.model.Meta.tb_name}_{"_".join(index_fields)}'
        ...
        for i in indexes:
            ...
            sql.append(f'CREATE INDEX {index_name} ON {self.model.Meta.tb_name} ({", ".join(new_index_fields)});')
    ...
    def build_modify_cols_unique_uniun_index_sql(self, indexes):
        ...
        index_fields = [i for i in self.model.Meta.union_unique_index if i in self.model.fields]
        new_index_fields = [f'`{i}`' for i in index_fields]
        index_name = f'unique_idx_{self.model.Meta.tb_name}_{"_".join(index_fields)}'
        ...
        for i in indexes:
            ...
            sql.append(f'DROP INDEX {index_name};')
            sql.append(
                f'CREATE UNIQUE INDEX {index_name} ON {self.model.Meta.tb_name} ({", ".join(new_index_fields)});'
            )
