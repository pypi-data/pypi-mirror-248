...
class RedisLock:
    ...
    def __init__(self, spider_key, lock_key, conn, wait_timeout=0, lock_timeout=12 * 60 * 60):
        """
        redis超时锁
        Args:
            key: 存储锁的key
    ...
    async def acquire(self):
        ...
        while True:
            try:
                ...
                timeout = await self.conn.hash.hget(self.redis_spider_key, self.lock_key)
                timeout = int(timeout) if timeout is not None else 0

                if timeout + self.lock_timeout < time.time():
                    logger.info("锁时间已超时...")
                    break

                logger.debug(f"等待加锁 wait: {round(time.time() - start, 2)}")
                await asyncio.sleep(5 if self.wait_timeout >= 10 else 1)

            except Exception as e:
                logger.error(f"Error in acquiring lock: {e}")
                break
