...
class QuerySet:
    ...
    def only(self, *args):
        ...
        if self._field is None:
            ...
            for i in args:
                if i in self._field:
                    continue
                self._field = self._field.append(i)
    ...
    def get(self, flat=False, callback=None, **kwargs):
        ...
        if callback is None:
            ...
            return callback(data if isinstance(data, tuple) else (data,))
    ...
    def all(self, flat=False, callback=None, **kwargs):
        ...
        if callback is None:
            return [
                tuple(i[k] for k in (self._field or i)) if len(i.values()) > 1 else next(
