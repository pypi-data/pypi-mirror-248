...
class RequestRedisDB(RequestBaseDB):
    ...
    def __init__(self, spider, connector):
        super().__init__()
        self.success_key = f'{spider.name}:{self.success_status}'
        self.failure_key = f'{spider.name}:{self.failure_status}'
    ...
    async def clear_success(self):
        self.success_count = 0
        await self.conn.delete(self.success_key)
        self.success_hash.clear()

    async def clear_failure(self):
        self.failure_count = 0
        await self.conn.delete(self.failure_key)
        self.failure_hash.clear()

    async def remove_failure(self, request: BaseRequest):
        await self.conn.set.srem(self.failure_key, request.hash)
        self.failure_hash.discard(request.hash)
    ...
    async def has_request(self, request: BaseRequest):
        ...
        if request.hash in self.success_hash or request.hash in self.failure_hash:
            return True
        elif await self.conn.set.sismember(self.success_key, request.hash):
            return True
        elif await self.conn.set.sismember(self.failure_key, request.hash):
            return True
        else:
            return False
...
class RequestDB:
    ...
    def __init__(self, spider, settings, connector, backend=BackendEngine.queue):
        ...
        self.done = self._init_done(connector)

        Signal().connect(SignalType.done_close, self.close)

    def _init_done(self, connector):

        if self.backend == BackendEngine.queue:
            return RequestQueueDB()

        if self.backend == BackendEngine.redis:
            return RequestRedisDB(self.spider, connector)
    ...
    async def request_failure_size(self):
        return self.done.failure_count
