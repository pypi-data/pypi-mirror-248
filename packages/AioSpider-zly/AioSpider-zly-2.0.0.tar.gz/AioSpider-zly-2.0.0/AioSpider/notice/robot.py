...
from AioSpider.tools.tools import before_day
from AioSpider.tools.program_tools import get_ipv4
from AioSpider.constants import NoticeType

from ._email import Email
from .wechat import GroupRobot as wRobot
from .dingding import GroupRobot as dRobot
from ._email import Email
...
class Drawing:
    ...
    def __init__(self, spider, type, level, msg, size: tuple = None, bgc=(255, 255, 255)):
        ...
        self.font_path = Path(__file__).parent.parent / 'resource/font/微软雅黑.ttf'
        
        if not self.font_path.exists():
            raise Exception('字体不存在，请执行：“AioSpider install” 安装')
        
        self.font_path = str(self.font_path)
    ...
    def draw(self):
        ...
        text = [
            self.spider,
            get_ipv4(),
            before_day().strftime("%Y-%m-%d %H:%M:%S"),
            self.level,
        ]
...
class RobotBase(EmptyRobot):
    ...
    def __init__(self, spider, platform, config):
        ...
        if platform == NoticeType.wechat:
            self.robot = wRobot(config['token'])
        elif platform == NoticeType.dingding:
            self.robot = dRobot(config['token'])
...
class WechatRobot:
    ...
    def add_robot(self, nmae, spider, config):
        self.spider = spider
        self.robots[nmae] = wRobot(config['token'])
    
    def send(self, level, name, msg, warn: bool = False):

        notice_type = '预警' if warn else '通知'

        content = Drawing(self.spider, notice_type, level, msg).draw()
        self[name].send_image(content=content)
    
    def remove_robot(self, nmae):
        self.robots.pop(nmae)

    def debug(self, msg, name):
        self.send('DEBUG', name, msg, warn=False)

    def info(self, msg, name='system'):
        self.send('INFO', name, msg, warn=False)

    def warning(self, msg, name='system'):
        self.send('WARNING', name, msg, warn=True)

    def error(self, msg, name='system'):
        self.send('ERROR', name, msg, warn=True)

    def critical(self, msg, name='system'):
        self.send('CRITICAL', name, msg, warn=True)
...
class DingdingRobot:
    ...
    def add_robot(self, nmae, spider, config):
        self.spider = spider
        self.robots[nmae] = dRobot(config['token'])
    
    def send(self, level, name, msg, warn: bool = False):

        notice_type = '预警' if warn else '通知'

        content = f'## {notice_type}\n\n**爬虫**: {self.spider}\n\n**主机**: {get_ipv4()}\n\n**时间**: ' \
                  f'{before_day().strftime("%Y-%m-%d %H:%M:%S")}\n\n**级别**: {level}\n\n**内容**: {msg}'

        self[name].send_markdown(title='爬虫预警通知', text=content)
...
class EmailRobot:
    ...
    def send(self, level, name, msg, warn: bool = False):
        ...
        self[name].send_text(subject='爬虫预警通知', body=content)
...
class Robot:
    ...
    def debug(self, msg):
        self['system'].debug(msg, warn=False)

    def info(self, msg):
        self['system'].info(msg, warn=False)

    def warning(self, msg):
        self['system'].warning(msg, warn=True)

    def error(self, msg):
        self['system'].error(msg, warn=True)

    def critical(self, msg):
        self['system'].critical(msg, warn=True)
