...
from AioSpider.models.models import SQLiteModel, MySQLModel, CSVModel, TablesModel
...
class TableSync:
    ...
    async def _handle_sqlite_table(self, model):
        ...
        if not model.Meta.read_only:
            ...
            if model.Meta.union_index:
                indexes = await self._connector['sqlite'][model.Meta.db].get_index(table)
    ...
    async def _handle_mysql_table(self, model):
        ...
        if not model.Meta.read_only:
            ...
            if model.Meta.union_index:
                indexes = await self._connector['mysql'][model.Meta.db].get_index(table)
    ...
    async def _execute_sqlite_alter_sql(
        ...
        if modify_uniun_index_sql:
            await self._connector['mysql'][model.Meta.db]._execute(modify_unique_union_index_sql)
            logger.info(f'{table}表普通联合索引添加或修改成功')

        # 修改唯一联合索引
        if modify_unique_union_index_sql:
            await self._connector['sqlite'][model.Meta.db]._execute(modify_unique_union_index_sql)
...
class LoadingTableData:
    ...
    async def load_data(self):
        ...
        data_loaders = {
            'mysql': self.load_sql_data,
            'sqlite': self.load_sql_data,
            'csv': self.load_csv_data,
        }
