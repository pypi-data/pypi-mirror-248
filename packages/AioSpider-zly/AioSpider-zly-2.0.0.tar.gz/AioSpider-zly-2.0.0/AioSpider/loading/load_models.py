...
class LoadModels:
    ...
    def load_models(self) -> List[Type[Model]]:
        ...
        for model in self.models:
            if model.Meta.engine == 'file':
                if hasattr(self.db_config, 'File'):
                    model.Meta.base_path = self.db_config.File['CONNECT'][model.Meta.db]['FILE_PATH']
                else:
    ...
    def update_model_bases(self, name, model_list, db_type):
        ...
        for model_name in model_list:
            ...
            if db_type == 'Sqlite':
                ...
                base_model = models.CSVModel
            elif db_type == 'File':
                base_model = models.FileModel
            ...
            if model.__name__ == 'TablesModel':
                self.models.insert(0, model)
            else:
                self.models.insert(1, model)

    def update_models_from_db(self):

        db_types = ['File', 'Sqlite', 'Csv', 'Mysql', 'Mongodb']
