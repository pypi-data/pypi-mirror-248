...
class API:
    ...
    @classmethod
    def instantiate(cls: t.Type[TAPI], host: str, token: t.Optional[str] = None, validate_connection: bool = False) -> TAPI:
        headers = {'Authorization': f'Basic {token}'} if token else None
        return cls(session=httpx.Client(base_url=host, headers=headers, timeout=60), validate_connection=validate_connection)
    ...
    def __init__(self, session: httpx.Client, validate_connection: bool = False):
        ...
        try:
            ...
            raise Exception(f'Not able to compare backend and client versions, '
                            f'backend or client use incorrect or legacy versioning schema.') from ex
    ...
    def load_openai_data(self, data: t.List[t.Dict[str, t.Any]]) -> t.Optional[httpx.Response]:
        if Tag.INPUT not in self._tags:
            logger.warning("OpenAI latest input message will be used as input data. Set input data manually as tag (Tag.INPUT) to set your exact input")
    ...
    def annotate(self, user_interaction_id: str, version_id: int,
    ...
    def log_interaction(self,
        ...
        data = {"user_interaction_id": user_interaction_id,
                "input": input,
                "output": output,
    ...
    def get_interactions_csv(self, application_version_id: int, return_topics: bool,
                             return_output_props: bool, return_annotation_data: bool,
                             return_custom_props: bool, return_input_props: bool) -> str:
        return maybe_raise(
