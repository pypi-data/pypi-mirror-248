...
class ThinkTrader(base1x.Singleton):
    """
    迅投XtQuant-miniQMT交易
    """
    __xt_trader = None
    ...
    def set_trader(self, qmt_dir: str = '', session_id: int = 0) -> int:
        ...
        self.__xt_trader = XtQuantTrader(qmt_dir, session_id)
        # 启动交易线程
    ...
    def single_available(self, num: int) -> float:
        ...
        logger.warning('账户资金, 可用: {}, 市值: {}, 预留: {}, 可买: {}, 可卖: {}', acc_cash, acc_value, reserve_cash, available, can_use_amount)
        # 7. 如果当日可用金额大于资金账户的可用金额, 输出风险提示
        if available > acc_cash:
            logger.warning('!!! 持仓占比[{}%], 已超过可总仓位的[{}%], 必须在收盘前择机降低仓位, 以免影响下一个交易日的买入操作 !!!', base1x.float_round(100*(acc_value / acc_total)),
