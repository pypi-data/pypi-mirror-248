...
class QlikSense:
    ...
    def get_tasks(self, task_filter: str = None):
        return [QlikSenseTask(t, self.qs_repository) for t in self.qs_repository.get_tasks(task_filter=task_filter)]

    def get_failed_tasks(self, since=None):
        failed_filter = f"operational.lastExecutionResult.status eq 8"
        if since is not None:
            str_timestamp = since.strftime("%Y-%m-%d %H:%M:%S")
            failed_filter += f" and operational.lastExecutionResult.stopTime ge '{str_timestamp}'"
        failed_tasks = self.qs_repository.get_tasks(task_filter=failed_filter)

        return [QlikSenseTask(t, self.qs_repository) for t in failed_tasks]
