...
class QlikApp:
    ...
    def set_script(self, script):
        result = self.__host_server.set_app_script(self.id, script)
        return result
    def get_script(self):
        script = self.__host_server.get_app_script(self.id)
        return script

    def evaluate_expression(self, expression):
        result = self.__host_server.evaluate_expression(self.id, expression)
        return result

    def get_hypercube_data(self, cube, x, y):
        result = self.__host_server.get_hypercube_data(self.id, cube, x, y)
        return result
