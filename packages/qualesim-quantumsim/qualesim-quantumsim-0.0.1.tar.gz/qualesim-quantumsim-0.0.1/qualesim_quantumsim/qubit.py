...
class Qubit:
    ...
    def measure(self, method="random", measure_mod="final_state"):
        ...
        if self.qs_ref is not None:
            ...
            if method == "random":
                ...
                if p0 < 1.0e-20:
                    raise ValueError(
                        "cannot project to 0, probability too low at %f" % p0
                    )
                self.classical = 0
            elif method == 1:
                if p1 < 1.0e-20:
                    raise ValueError(
                        "cannot project to 1, probability too low at %f" % p1
                    )
                self.classical = 1
            else:
            ...
            if trace < 1e-10:
                self.qsi.debug(
                    "renormalizing state density matrix, trace was {}...", trace
                )
    ...
    def ensure_in_sdm(self):
        ...
        if self.qs_ref is None:
            ...
            try:
                ...
                raise RuntimeError(
                    "Too many qubits in use! Max is currently fixed to {}".format(
                        self.qsi.MAX_QUBITS
                    )
                )
