...
def _focal(
    func: Callable[[ndarray], Union[ndarray, List[ndarray]]],
    buffer: int,
    circle: bool,
    grid: Grid,
) -> Tuple[Grid, ...]:
    ...
    return tuple([grid._create(r) for r in result])


def _batch(
    func: Callable[[Grid], Tuple[Grid, ...]],
    buffer: int,
    grid: Grid,
) -> Tuple[Grid, ...]:
    stride = 8000 // buffer
    ...
    for xmin, ymin, xmax, ymax in tiles:
        ...
        for i, grid3 in enumerate(grids3):
            grid4 = grid3.clip(
                (
            ...
            if results:
                results[i] = mosaic(results[i], grid4)
            else:
                results.append(grid4)
