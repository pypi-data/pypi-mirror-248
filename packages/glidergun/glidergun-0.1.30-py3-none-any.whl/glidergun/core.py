...
class Grid:
    ...
    def distance_transform_edt(self, **kwargs: Any):
        return self.local(lambda a: scipy.ndimage.distance_transform_edt(a, **kwargs))

    def focal(
        self, func: Callable[[np.ndarray], Any], buffer: int, circle: bool
    ) -> Tuple["Grid", ...]:
...
def _focal(
    func: Callable[[ndarray], Any], buffer: int, circle: bool, *grids: Grid
) -> Tuple[Grid, ...]:
    ...
    if len(grids) == 1:
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", category=RuntimeWarning)
            result = func(np.transpose(array, axes=(1, 2, 0, 3, 4))[:, :, :, mask])
