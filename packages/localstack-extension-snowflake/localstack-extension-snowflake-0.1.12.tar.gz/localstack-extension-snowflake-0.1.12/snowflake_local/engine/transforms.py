...
def remove_create_or_replace(expression,query):
	I='replace';E=query;A=expression
	if not isinstance(A,exp.Create):return A
	try:D=get_db_engine()
	...
	if A.args.get(I):
		...
		if D and F in(_H,'FUNCTION'):
			G=str(B.this.this);C=Query(query=f"DROP {F} IF EXISTS {G}");C.session=E.session;C.database=E.database;H=G.split('.')
...
def convert_function_args_to_lowercase(expression,**H):
	A=expression
	if isinstance(A,exp.Create)and isinstance(A.this,exp.UserDefinedFunction):
		D=A.args[_G].expressions;B=[A for A in D if isinstance(A,exp.LanguageProperty)];B=str(B[0].this).lower()if B else _B
...
def pg_track_case_sensitive_identifiers(expression,query):
	...
	if isinstance(B,exp.Create):
		...
		if C in(_O,_M,_H):
			...
			if A.args.get('quoted'):D=A.this if C==_O else query.database;E=A.this if C==_M else _B;F=A.this if C==_H else _B;G=D,E,F;State.identifier_overrides.entries.append(G)
	return B
def pg_cast_params_for_string_agg(expression,**E):
	C='separator';A=expression
	if isinstance(A,exp.GroupConcat):
		D=A.this
		if not isinstance(D,exp.Cast):B=exp.Cast();B.args[_A]=A.this;B.args['to']=exp.DataType.build(_F);A.args[_A]=B
		if A.args.get(C)is _B:A.args[C]=exp.Literal(this='',is_string=_D)
	return A
def pg_cast_params_for_to_date(expression,**C):
	...
	if isinstance(A,exp.Func)and str(A.this).lower()=='to_date':
		...
		if len(A.expressions)<=1:LOG.info('Auto-detection of date format in TO_DATE(..) not yet supported');A.expressions.append(exp.Literal(this='YYYY/MM/DD',is_string=_D))
	return A
