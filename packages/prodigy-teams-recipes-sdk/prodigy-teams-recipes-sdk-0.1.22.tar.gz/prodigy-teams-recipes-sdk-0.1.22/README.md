...
| `--model-registration` | `bool` | Register Model | `False` |
| `--model-registration.name` | `str` | Model Name: Name to register model with in Prodigy Teams. | `None` |
| `--model-registration.path` | `str` | Model Path: Path to upload model to. | `None` |
| `--model-registration.version` | `str` | Model Version: Semantic version to save model as | `'0.1.0'` |
| `--model-registration.asset-kind` | `str` | Asset Kind: Kind of asset to register model as. Defaults to 'model'. | `'model'` |
