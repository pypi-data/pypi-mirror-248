...
def conc_scomp(t: np.ndarray,
               J: np.ndarray,
               K: int,
               C0=0
               ) -> np.ndarray:
    """Build the residue for a stationary compartment"""
    if K == 0:
        return trapz(t, J)
    T = 1/K
    return C0 + T*expconv(T, t, J)

def conc_nscomp(t: np.ndarray,
                J: np.ndarray,
                K: np.ndarray,
                C0=0
                ) -> np.ndarray:
    """This builds the residue for a non-stationary (ns) compartment where
    K is not a fixed number by an array of the same length as t and J.
...
def res_comp(t: np.ndarray,
             K: int
             ) -> np.ndarray:
    """Residue function of a compartment"""
    return np.exp(-t*K)

def prop_comp(t: np.ndarray,
              K: int
              ) -> np.ndarray:
    """Propagator of a compartment"""
    return K*np.exp(-t*K)

def conc_2cfm(t: np.ndarray,
              J: np.ndarray,
              K01: int,
              K21: int,
              K02: int
...
def prop_chain(t: np.ndarray,
               T: int,
               disp: int
               ) -> np.ndarray: # dispersion in %
