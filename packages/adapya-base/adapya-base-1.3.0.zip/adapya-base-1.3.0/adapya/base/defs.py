...
if sys.hexversion > 0x03010100:
    ...
    def Abuf(init, encoding='utf-8', errors='strict'):
        ...
        class Cbuf(Cchar):
            ...
            def write(self, wstr):
                """ Supports writing string or binary string """
                p1  = self.pos
...
def evals(s):
    ...
    .. note::
        ...
        In PY3 no escaped single bytes can be defined in strings
        - they are mapped to unicode characters.
...
LOGVB=1<<6      #0x40
LOGIB=1<<7      #0x80
LOGMB=1<<8      #0x0100     # Multifetch buffer
LOGPB=1<<9      #0x0200     # Performance buffer
LOGUB=1<<10     #0x0400
LOGAOS=1<<16    #0x10000
LOGRSP=1<<17    #0x20000
LOGBUF=1<<18    #0x40000 LOG all buffers
LOGSP=1<<19     #0x80000 LOG depending on command option (command table option)
LOGTCP=1<<20    #0x0100000 log on TCP level
    ...
    if debug:
        if logopt == 0:
            ...
            logstr = flag_str(logopt, ((LOGCMD,'LOGCMD'),
                ...
                (LOGSP,'LOGSP'),(LOGTCP,'LOGTCP'),
                ))
        adalog.info('Adapya logging: %s\n' % logstr)
