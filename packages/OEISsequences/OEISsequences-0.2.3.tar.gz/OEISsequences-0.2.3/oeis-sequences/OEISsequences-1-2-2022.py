...
from sympy.ntheory.factor_ import digits as sympydigits, udivisor_sigma, sieve, reduced_totient, \
                                  core as numbercore, antidivisors, udivisors
...
from gmpy2 import mpz, fac, popcount, is_prime, is_square, next_prime, c_divmod, lucas2, fib2, \
                  isqrt_rem, iroot_rem, digits as gmpy2digits
...
def A002708_gen(): # generator of terms
    ...
    for n in count(1):
        yield a % n
        a, b = b, a+b
def A003098_gen(): return filter(lambda m:str(m)==str(m)[::-1],(n*(n+1)//2 for n in range(10**5)))
...
def A236437_gen(): return (p for n in range(1,10**6) if A236174(n) == (p:=prime(n)))
...
def A232179(n):
    ...
    for i in range(len(ds)//2-1,-1,-1):
        ...
        if b:
            return a
    return -1            
def A232444_gen(): return chain((2,),(n for n in (d**2 for d in range(1,10**4)) if isprime(divisor_sigma(n)) and isprime(divisor_sigma(n**2)))) 
...
def A246044_gen(): # generator of terms
    for n in count(1):
        ...
        for x in permutations(str(p)):
            ...
            yield p
def A246519_gen(): return (p for p in (prime(n) for n in range(1,10**5)) if all(isprime(4+p**z) for z in (1,2,3,5)))
...
def A030668(n):
    ...
    while True:
        ...
        d *= 10
        nd *= 10  
def A031688_gen(): (n for n, s in ((i, continued_fraction_periodic(0,1,i)[-1]) for i in range(1,10**5)) if isinstance(s, list) and len(s) % 2 and s[(len(s)-1)//2] == 100)
...
def A074100_gen(): return (n**3 for n in range(1,10**6) if set(str(n**3)) <= set('12357'))
...
def A113496(n):
    ...
    while gcd(k,m) != 1 or isprime(k):
        k += 1
    return k
def A246338_gen(): return filter(lambda n:'0' not in oct(n)[2:] and n % sum(int(d) for d in oct(n)[2:]) == 0 and n % prod(int(d) for d in oct(n)[2:]) == 0,pal_gen(8))
...
def A245576_gen(): return (p for p in (prime(i) for i in range(1,10**4)) if not(str(p).count('0') or str(p**2).count('0')))      
