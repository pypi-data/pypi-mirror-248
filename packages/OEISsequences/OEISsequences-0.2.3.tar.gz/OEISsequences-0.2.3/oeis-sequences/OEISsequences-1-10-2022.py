...
from sympy import factorint, divisors, integer_nthroot, divisor_sigma, nextprime, Matrix, divisor_count, \
                  ...
                  Poly, hermite, mod_inverse, EulerGamma, digamma, discrete_log
...
def A333196(n):
    ...
    return 1 if len(fs) == 0 else reduce(mul,(p**(fs[p]//n + 1 if fs[p] % n else fs[p]//n) for p in fs)) 
...
def A117790_gen(): # generator of terms
    ...
    while True:
        if isprime(sum(int(d) for d in str(b))):
            yield b
        a, b = b, a+b    
def A118548_gen(): return (n for n in (x**2 for x in range(1,10**6)) if not (str(n).count('0') or n % prod(int(d) for d in str(n))))
...
def A256985(n):
    ...
    while ilist != jlist:
        ...
        jlist = [jlist[-1]]+ jlist[:-1]
    return k
def A257002_gen(): return (p for p in (prime(n) for n in range(1,10**4)) if pow(p, p, p+2) == p)
