...
from gmpy2 import mpz, fac, popcount, is_prime, is_square, next_prime, c_divmod, lucas2, fib2, \
...
def A328967(n):
    ...
    while k1 > 1:
        ...
        j, k1 = j2, (n-1)//j2
    return j-c 
def A329722(n): return sum(int(not (~(n+2*k) & 2*n-k) | (~n & k)) for k in range(n+1)) 
def A329792(n):
    if n % 10:
        m, s = 1, set('12345')
...
def A329720(n): return sum(int(not (~(n-k) & 6*k) | (~n & k)) for k in range(n+1))
