...
from math import factorial, floor, comb, prod, isqrt, gcd, lcm
...
#     def comb(n,k): return binomial(n,k)
...
def A349243gen():
    return filter(
        lambda n: set(str(n * (n + 1) // 2)) <= {"1", "3", "5", "7", "9"}, count(0)
    )
...
def A050251(n):
    return (
        4 * n if n <= 1 else 1 + sum(1 for i in paloddgen((n + 1) // 2) if isprime(i))
    )
...
def A348780(n):
    return sum(islice(A347113gen(), n))
...
def A014137gen():
    return accumulate(A000108gen())


def A014138gen():
    return (x - 1 for x in A014137gen())
...
def A005349gen():
    return filter(lambda n: not n % sum((int(d) for d in str(n))), count(1))
...
def A001694gen():
    return filter(lambda n: n == 1 or min(factorint(n).values()) > 1, count(1))
...
def A052382gen():
    return filter(lambda n: "0" not in str(n), count(1))
...
def A054632gen():
    return accumulate(A007376gen())


def A127353gen():
    return islice(A007376gen(), 2, None, 2)


def A127050gen():
    return islice(A007376gen(), 1, None, 2)


def A127950gen():
    return islice(A007376gen(), 2, None, 8)
...
def A003601gen():
    return filter(lambda n: not sum(divisors(n)) % divisor_count(n), count(1))
...
def A011540gen():
    return filter(lambda n: "0" in str(n), count(0))
...
def A002982gen():
    return filter(lambda n: isprime(factorial(n) - 1), count(1))
...
def A010784gen():
    return filter(lambda n: len(set(str(n))) == len(str(n)), count(0))
...
def A007602gen():
    return filter(
        lambda n: not ("0" in str(n) or n % prod(int(d) for d in str(n))), count(1)
    )
...
def A348470(n):
    return 1 if n == 1 else min(primefactors(next(islice(A064413gen(), n - 1, None))))


def A348470gen():
    yield from (min(primefactors(n)) if n > 1 else 1 for n in A064413gen())
...
def A007774gen():
    return filter(lambda n: len(primefactors(n)) == 2, count(1))
...
def A006521gen():
    return filter(lambda n: pow(2, n, n) == n - 1, count(1))


def A124240gen():
    return filter(lambda n: n % reduced_totient(n) == 0, count(1))
...
def A003401gen():
    return filter(lambda n: format(totient(n), "b").count("1") == 1, count(1))
...
def A029967gen():
    return filter(lambda n: palQ(n, 12), palQgen10())


def A029968gen():
    return filter(lambda n: palQ(n, 13), palQgen10())


def A049445gen():
    return filter(lambda n: not n % sum([int(d) for d in bin(n)[2:]]), count(1))
...
def A029731gen():
    return filter(lambda n: palQ(n, 16), palQgen10())


def A029804gen():
    return filter(lambda n: palQ(n, 8), palQgen10())
...
def A034709gen():
    return filter(lambda n: n % 10 and not n % (n % 10), count(1))
...
def A099165gen():
    return filter(lambda n: palQ(n, 32), palQgen10())
...
def A061910gen():
    return filter(lambda n: is_square(sum(int(d) for d in str(n * n))), count(1))
...
def A054377gen():
    return filter(lambda n: sum(n / p for p in primefactors(n)) + 1 == n, count(2))
...
def A057890gen():
    return filter(
        lambda n: bin(n)[2:].rstrip("0") == bin(n)[2:].rstrip("0")[::-1], count(0)
    )
...
def A250408gen():
    return filter(lambda n: palQ(n, 20), palQgen10())
...
def A249156gen():
    return filter(lambda n: palQ(n, 7), palQgen(5))


def A250410gen():
    return filter(lambda n: palQ(n, 25), palQgen10())
...
def A015950gen():
    return filter(lambda n: pow(4, n, n) == n - 1, count(1))
...
def A002796gen():
    return filter(
        lambda n: all((d == "0" or n % int(d) == 0) for d in set(str(n))), count(1)
    )
...
def A054683gen():
    return filter(lambda i: not sum(int(d) for d in str(i)) % 2, count(0))
...
def A055685gen():
    return filter(lambda n: pow(2, n, n - 1) == n - 2, count(2))
...
def A350233gen():
    return filter(
        ...
        filter(lambda n: n % 4 and not n % 5, count(1)),
    )
...
def A350232gen():
    return filter(
        ...
        filter(lambda n: n % 5 and not n % 4, count(1)),
    )
...
def A092221gen():
    return filter(lambda n: not bernoulli(2 * n).p % 59, count(0))


def A281502gen():
    return filter(lambda n: not bernoulli(2 * n).p % 691, count(0))
...
def A110751gen():
    return filter(
        lambda n: primefactors(n) == primefactors(int(str(n)[::-1])), count(1)
    )
...
def A115920gen():
    return filter(lambda n: sorted(str(divisor_sigma(n))) == sorted(str(n)), count(1))


def A115921gen():
    return filter(lambda n: sorted(str(totient(n))) == sorted(str(n)), count(1))
...
def A127936gen():
    return filter(lambda i: isprime(int("01" * i + "1", 2)), count(1))
...
def A171901gen():
    return filter(A171901is_ok, count(0))
