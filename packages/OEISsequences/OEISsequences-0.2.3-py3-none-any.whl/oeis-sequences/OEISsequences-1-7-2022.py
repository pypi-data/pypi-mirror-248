...
For some sequences, e.g. `A269483`, both type of functions `A269483` and `A269483_gen` are provided.
...
from sympy import factorint, divisors, integer_nthroot, divisor_sigma, nextprime, Matrix, divisor_count, \
                  ...
                  sqrt, bernoulli, ff, sin, cos, tan, fibonacci, lucas, pi, hyperexpand, expand, \
                  Poly, sympify, hermite
...
def A047842(n):
    ...
    for i in range(10):
        ...
        if c != '0':
            s += c+y
    return int(s)
def A233466_gen(): return filter(lambda n:2*totient(n) == n-5,count(1,2))
...
def A065197_gen(): return filter(lambda n:n==reduce(lambda m, k: m + (k if (m//k) % 2 else -k),range(n,1,-1),n),count(1))
...
def A029455_gen(): # generator of terms
    ...
    for n in count(1):
        ...
        if not (r % n):
            yield n
def A052045_gen(): return filter(lambda n:not str(n).count('0'),(n**3 for n in count(1)))
def A055941(n):
    s = bin(n)[2:]
...
def A267077(n):
    ...
    while True:
        ...
        v += u+1
        u += 2 
def A267765_gen(): return (int(d,5) for d in (str(i**2) for i in range(10**6)) if max(d) < '5')
def A267766_gen(): return (int(d,6) for d in (str(i**2) for i in range(10**6)) if max(d) < '6')
def A267819_gen(): return (int(d,5) for d in (str(i**2) for i in range(1,10**6)) if max(d) < '5' and isprime(int(d,5)))
def A267820_gen(): return (int(d,6) for d in (str(i**2) for i in range(1,10**6)) if max(d) < '6' and isprime(int(d,6)))
