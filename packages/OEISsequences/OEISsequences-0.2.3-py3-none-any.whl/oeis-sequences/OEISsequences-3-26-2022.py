...
from itertools import islice, count, product, permutations, takewhile, accumulate, combinations_with_replacement, \
        combinations, repeat, groupby, chain, starmap
...
def A000201(n): return (n+isqrt(5*n**2))//2        
def A185381(n): return fibonacci((n+isqrt(5*n**2))//2) 
def A350678(n): return sum(fibonacci((i+isqrt(5*i**2))//2) for i in range(n+1)) 
...
def A002620(n): return n**2//4
def A004526(n): return n//2
...
@lru_cache(maxsize=None)
def A000123(n): return 1 if n == 0 else A000123(n-1) + A000123(n//2)
