...
class Baseline :
	...
	def __init__(self, df_quimicos:pd.DataFrame) -> None:
	...
	def encontrar_ruta(
		...
		# Convierte en df_quimicos con las columnas
		#	- hora
		#	- cluster
		if solucion is not None:
			...
			for i, cluster in enumerate(clusters_ordenados[:-1]):
				...
				hora_ = datetime.datetime.strptime(str(hora), "%H:%M:%S").time()
				hora_en_cluster.append(hora_)
