...
from src.chaparral.modelos.clases import (
	Cluster, 
	Camion, 
	QuimicoEn, 
	Pozo
)
from src.chaparral.modelos.entorno import CampoPozos
from src.chaparral.main.cargar_inventario import estandarizar_nombre
# from chaparral.mapa.convenciones import POZO2CLUSTER 
from src.chaparral.modelos.constantes import (
...
class LectorQuimicos :
	...
	def leer_datos(self) -> None:
		"""Lee datos de archivo y crea el dataframe de info."""
		# Lee datos desde archivo
		self.data = pd.read_json(self.nombre_archivo, lines=True)
		if self.verbose:
			print(f'Datos leídos con éxito desde el archivo {self.nombre_archivo}')
...
class CreadorCampoPozos :
	...
	def __init__(
		...
		self.nombre_archivo_quimicos = nombre_archivo_quimicos
	...
	def generar_clusters_y_quimicos(self) -> List[Cluster]:
		...
		nombre_archivo=self.directorio / Path(self.nombre_archivo_quimicos)
		lector = LectorQuimicos(nombre_archivo)
...
class CampoUtils :
	...
	def cargar_campo_desde_archivo(
		...
		if self.archivo is not None:
			archivo_path = Path.cwd() / Path('..').resolve() / Path('data', 'entornos', self.archivo)
		else:
			archivo_path = Path.cwd() / Path('..').resolve() / Path('data', 'entornos', 'entorno.pkl')
