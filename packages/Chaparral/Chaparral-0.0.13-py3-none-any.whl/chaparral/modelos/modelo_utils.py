...
from chaparral.mapa.rutas import RUTAS
	...
	def actualizar_clusters(
		...
		for cluster in clusters[1:]: # Omitir Bodega
			...
			for pozo in cluster.pozos:
				lista_pozos = self.df_quimicos.punto.unique().tolist()
				# if self.verbose:
				# 	print(f'Actualizando químicos de pozo {pozo.nombre}')
				if pozo.nombre in lista_pozos:
					quimicos_en_cluster = self.df_quimicos.groupby('punto').get_group(pozo.nombre)
					quimicos = list()
...
class CreadorCampoPozos :
	...
	def __init__(
		...
		# -------------------------------
		self.directorio_log = Path('..').resolve() / Path('data', 'logs')
		# self.directorio_log.mkdir(parents=False, exist_ok=True)
		# -------------------------------
		# Registro de datos
		# -------------------------------
	...
	def crear_clusters(self) -> Tuple[List[Cluster], Dict[str,int]] :
		...
		clusters = list()
		data_bodega = self.df_clusters[self.df_clusters.tipo == 'BODEGA']
		...
		data_clusters = self.df_clusters[self.df_clusters.tipo == 'CLUSTER']
		if self.verbose:
			print('')
