...
def metaheuristic_optmizer(setup):
    ...
    if setup['algorithm'] == 'hill_climbing_01':
        ...
        # Multiprocess
        with Pool() as p:
            settings = [[setup, init_population] for init_population in population]
    ...
    # # resume_result process (Time and Excel outputs)
    # STATUS_PROCEDURE = metapyco.SUMMARY_ANALYSIS(best_population_per_rep, setup['N_REP'], setup['n_iterations'])

    # # Command window report
    # BESTBEST = STATUS_PROCEDURE[0]
    # BEST = best_population_per_rep[BESTBEST]
    ...
    return all_results_per_rep, best_population_per_rep
...
def hill_climbing_01(settings):
    ...
    Args:  
        settings (list): [0] setup (dict), [1] initial population (list).
        setup keys:
            ...
            'algorithm parameters' (dict): Algorithm parameters.
                'std (%)' (float): Standard deviation in percentage.
    ...
    Returns:
        ...
        delta_time (float): Time of the algorithm execution.
    """
    ...
    seeds = setup['seed control']

    # algorithm_parameters
    algorithm_parameters = setup['algorithm parameters']
    std = algorithm_parameters['std (%)'] / 100
    ...
    repetition_data = metapyco.resume_process_in_dataframe(x_pop, of_pop, fit_pop,
    ...
    for iter in range(n_iterations):
        ...
        for pop in range(n_population):
            ...
            x_i_temp, of_i_temp, fit_i_temp, neof = metapyco.mutation_01_movement(obj_function,
                                                                                  x_pop[pop],
                                                                                  x_lower, x_upper,
                                                                                  n_dimensions,
                                                                                  pdf, std,
    ...
    return df_all, df_best, delta_time
