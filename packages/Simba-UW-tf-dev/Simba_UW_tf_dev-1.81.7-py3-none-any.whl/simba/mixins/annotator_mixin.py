...
class AnnotatorMixin(ConfigReader):
    ...
    def h_nav_bar(self, parent: Frame,
                  update_funcs: Optional[Callable[[int], None]] = None,
                  store_funcs: Optional[Callable[[], None]] = None,
                  width: int = 700,
    ...
    def v_navigation_pane_targeted(self,
                                   ...
                                   store_funcs: Optional[List[Callable[[], None]]] = None) -> Frame:
    ...
    def update_frame_selection_txt(self, new_frm: Union[int, str]):
    ...
    def update_bout_selection_txt(self, new_start_frm: Union[int, str], new_end_frm: Union[int, str]):
    ...
    def targeted_frames_pane(self, parent: Frame) -> Frame:
        ...
        def set_selection():
            ...
            self.update_bout_selection_txt(new_start_frm=self.start_frm_bx.entry_get, new_end_frm=self.end_frm_bx.entry_get)
            self.change_frame(new_frm_id=self.start_frm_bx.entry_get, update_funcs=[self.update_frame_selection_txt])
        ...
        self.save_button = Button(pane, text=f"SAVE ANNOTATION CLIP \n START FRAME: {self.start_frm_bx.entry_get}, \n END FRAME: {self.end_frm_bx.entry_get}", fg='green', font=("Helvetica", 14, "bold"), command= lambda: self.__targeted_annotations_frames_save())
    ...
    def __targeted_annotations_frames_save(self):
        """ When performing targeted annotations by-frame, save the data """
