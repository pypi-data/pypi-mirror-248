...
from simba.utils.enums import Options, Formats, Keys, Links, TagNames, MetaKeys
...
class MachineModelSettingsPopUp(PopUpMixin, ConfigReader):
    ...
    def __init__(self,
        ...
        self.n_features_bars_entry_box.grid(row=6, column=0, sticky=NW)
        self.feature_permutation_cb.grid(row=7, column=0, sticky=NW)
        self.learning_curve_cb.grid(row=8, column=0, sticky=NW)
        self.learning_curve_k_splits_entry_box.grid(row=9, column=0, sticky=NW)
        self.learning_curve_data_splits_entry_box.grid(row=10, column=0, sticky=NW)
    ...
    def save_global(self):
        ...
        self.config.set('create ensemble settings', 'save_train_test_frm_idx', str(self.create_train_test_log))
    ...
    def save_config(self):
        ...
        meta = {'RF_n_estimators': self.n_estimators,
                ...
                'save_train_test_frm_idx': self.create_train_test_log}
    ...
    def load_config(self):
        ...
        if MetaKeys.SAVE_TRAIN_TEST_FRM_IDX.value in self.meta.keys():
            if self.meta[MetaKeys.SAVE_TRAIN_TEST_FRM_IDX.value]:
                self.create_train_test_log_var.set(value=str(self.meta[MetaKeys.SAVE_TRAIN_TEST_FRM_IDX.value]))
        else:
