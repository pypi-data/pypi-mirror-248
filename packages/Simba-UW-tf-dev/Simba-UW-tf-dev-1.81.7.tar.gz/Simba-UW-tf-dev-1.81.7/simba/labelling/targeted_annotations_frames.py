...
class TargetedAnnotatorFrames(AnnotatorMixin, ConfigReader):
    ...
    def __init__(self,
        ...
        self.nav_bar = self.h_nav_bar(parent=self.main_frm, update_funcs=[self.update_frame_selection_txt], store_funcs=[self.store_targeted_annotations_frames])
        self.selection_pane = self.targeted_frames_pane(parent=self.main_frm)

        self.play_video_frame = self.v_navigation_pane_targeted(parent=self.main_frm, update_funcs=[self.update_frame_selection_txt], store_funcs=[self.store_targeted_annotations_frames])
