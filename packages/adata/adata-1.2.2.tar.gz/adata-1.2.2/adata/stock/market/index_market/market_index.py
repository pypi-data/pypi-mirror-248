...
class StockMarketIndex(object):
    ...
    def __init__(self) -> None:
        self.ths = StockMarketIndexThs()
        self.east = StockMarketIndexEast()

    def get_market_index(self, index_code: str = '000001', start_date='2020-01-01', k_type: int = 1):
        """
        获取指数行情
        """
        res_df = self.east.get_market_index(index_code=index_code, start_date=start_date, k_type=k_type)
        if res_df.empty:
            res_df = self.ths.get_market_index(index_code=index_code, start_date=start_date, k_type=k_type)
        return res_df
    ...
    def get_market_index_min(self, index_code='000001'):
        ...
        """
        res_df = self.east.get_market_index_min(index_code=index_code)
        if res_df.empty:
            res_df = self.ths.get_market_index_min(index_code=index_code)
        return res_df
    ...
    def get_market_index_current(self, index_code: str = '000001'):
        ...
        """
        return self.east.get_market_index_current(index_code=index_code)
