...
from enginefaas.common.http.request import ApiRequest
from enginefaas.common.http.request import RequestInternal
from enginefaas.common.profile.client_profile import ClientProfile
from enginefaas.common.sign import Sign

warnings.filterwarnings("ignore", module="enginefaas", category=UserWarning)
...
LOGGER_NAME = "enginefaas_sdk_common"
...
class AbstractClient(object):
    ...
    _endpoint = ''
    _service = ''
    _sdkVersion = 'SDK_PYTHON_%s' % enginefaas.__version__
    ...
    def _format_params(self, prefix, params):
        ...
        raise SDKException("ClientParamsError", "some params type error")
    ...
    def _build_req_inter(self, action, params, req_inter, options=None):
        ...
        if options.get('SkipSign'):
            ...
            raise SDKException("ClientError", "Invalid signature method.")
    ...
    def _build_req_without_signature(self, action, params, req, options=None):
        ...
        if req.method == "GET" and content_type == _multipart_content:
            raise SDKException("ClientError",
                           "Invalid request method GET for multipart.")

        endpoint = self._get_endpoint()
        timestamp = int(time.time())
        req.header["Host"] = endpoint
        req.header["X-FAAS-Action"] = action[0].upper() + action[1:]
        req.header["X-FAAS-RequestClient"] = self._sdkVersion
        req.header["X-FAAS-Timestamp"] = str(timestamp)
        req.header["X-FAAS-Version"] = self._apiVersion
        if self.profile.unsignedPayload is True:
            req.header["X-FAAS-Content-SHA256"] = "UNSIGNED-PAYLOAD"
        if self.profile.language:
            req.header['X-FAAS-Language'] = self.profile.language
    ...
    def _check_status(self, resp_inter):
        if resp_inter.status_code != 200:
            raise SDKException(code=resp_inter.status_code, message="ServerNetworkError: "+ str(resp_inter.content))
    ...
    def _check_error(self, resp):
        ...
        try:
            ...
            if "Error" in data.get("Response"):
                ...
                raise SDKException(code, message, reqid)
            if "DeprecatedWarning" in data.get("Response"):
                import warnings
    ...
    def _call(self, action, uri, params, options=None, headers=None):
        self._requestPath = uri
        ep = self._get_endpoint()
        req = RequestInternal(ep,
    ...
    def call(self, action, uri, params, headers=None, options=None):
        resp = self._call(action, uri, params, options, headers)
        self._check_status(resp)
        # self._check_error(resp)
        return str(resp.content)

    def call_json(self, action, uri, params, headers=None, options=None):
        """
        ...
        :type headers: dict
        :param headers: request header, like {"X-FAAS-TraceId": "ffe0c072-8a5d-4e17-8887-a8a60252abca"}
        :type options: dict
