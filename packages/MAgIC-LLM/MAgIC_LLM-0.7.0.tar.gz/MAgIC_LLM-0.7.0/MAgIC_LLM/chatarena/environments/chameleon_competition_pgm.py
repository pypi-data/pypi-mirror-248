...
class Chameleon_Competition_PGM(Environment):
    ...
    def __init__(self, player_names: List[str], topic_codes: Dict[str, List[str]] = None, competition=None, **kwargs):
        ...
        if self.competition["pgm_pro"]:
            self._view_non_chameleon_templ = view_templ_active_non_chameleon_pro
            self._view_chameleon_templ = view_templ_active_chameleon_pro
            self._clue_templ_non_chameleon = clue_templ_active_non_chameleon_pro
            self._clue_templ_chameleon = clue_templ_active_chameleon_pro

        self.use_text_vote = False
        self.add_only_one_pgm = True  # use the last pgm in the history
        self._is_pgm_show = False 
        self.test_start = False  # A flag control when to start using openai response
        ...
        if self.add_pgm_metric and self.only_pgm_metric: # we fix all the input if we only want to calculate the pgm
    ...
    def reset(self):
        ...
        if self.add_pgm_metric:
            ...
            request_msg = self.prepare_metric_instruct(self._current_phase, self._next_player_idx)
        else:
    ...
    def is_pgm_player(self, player_idx):
        print("is pgm player+++++++++++++++++++++++>", player_idx, self.player_backends)
        print(self.player_backends[self.player_names[player_idx]]["model"].find("pgm") >= 0)
    ...
    def prepare_pgm_instruct(self, stage, next_player_idx, current_clue_giver_idx, next_clue_giver_idx):
        if stage == "pgm":
            mod_text = self._view_chameleon_templ if self.chameleon_idx  == next_player_idx else self._view_non_chameleon_templ
            other_players = [p for p in self.player_names if p != self.player_names[next_player_idx]]
            mod_text = mod_text.format(player=self.player_names[next_player_idx], other_player_1 = other_players[0], other_player_2 = other_players[1], pre_player=self.player_names[current_clue_giver_idx],secret_code=self.code)
        elif stage == "give clues":
            if not self.competition["pgm_pro"]:
                target_player_idx = self._pgm_list[next_clue_giver_idx].get_target()
                if self.chameleon_idx == next_clue_giver_idx: 
                    mod_text = self._clue_templ_chameleon
                    # target_player_tuples=[(target_player_idx, next_clue_giver_idx, -1)]
                else:
                    mod_text = self._clue_templ_non_chameleon
                    # target_player_tuples = [(i, target_player_idx, 1) for i in range(self._num_players) if i not in [target_player_idx, next_player_idx]]
                target_player=self.player_names[target_player_idx]
            else:
                mod_text = None
                target_player = ""
            if mod_text is not None:
                mod_text = mod_text.format(player=self.player_names[next_player_idx], target_player=target_player)
        else:
            print("do not support stage, ", stage)
            mod_text = None
        return mod_text

    def prepare_metric_instruct(self, stage, next_player_idx):
        if stage =="metric_consistency":
            mod_text = self.metric_consistency_template
        elif stage == "metric_pgm":
            mod_text = self.metric_pgm_template["chameleon"] if next_player_idx == self.chameleon_idx else self.metric_pgm_template["non-chameleon"]
            mod_text = mod_text.format(player=self.player_names[next_player_idx])
        else:
            print("do not support stage, ", stage)
            mod_text = None
        return mod_text
    ...
    def step(self, player_name: str, action: str) -> TimeStep:
        ...
        if self._current_phase == "give clues":
            ...
            if self._next_player_idx is not None:
                ...
                request_msg = self.prepare_pgm_instruct(self._current_phase, self._next_player_idx, self._current_clue_player_idx, self._next_clue_player_idx)
            ...
            if self._next_player_idx < len(self.player_names)-1:
                ...
                if not accuse_correct:
                    if even_vote:
                        ...
                        self._current_turn += 1
                        print(f"The most-voted player is {self.player_names[max_vote]}. The accusation is incorrect. "
                              f"{self.chameleon_name} is the chameleon. {self.chameleon_name} won the game!")
                        self._win_group = 2
            ...
            if self._next_player_idx < len(self.player_names) - 1:
                ...
                if is_pgm:
                    ...
                    #     target_player=self.player_names[self._target_player_idx]
                    # else:
                    #     target_player=""

                    # mod_text = mod_text.format(player=self.player_names[self._next_player_idx], target_player=target_player)
                    request_msg = self.prepare_pgm_instruct(self._current_phase, self._next_player_idx, self._current_clue_player_idx, self._next_clue_player_idx)
