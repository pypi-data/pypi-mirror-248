...
from llm_agent.prompts.prisoner_prompt import *
DEFAULT_TOPIC = """
...
class Prinsoner_Dilemma_PGM(Environment):
    ...
    def _scoring(self, decision):
        ...
        def extract_dec(dec):
            prompt = [{'role':'user','content':"help me to analyze which decision ( \"defect\" or \"cooperate\") the player made in the following text, '"+ dec + "' \n. Please answer with one of the word from defect or cooperate."}]
            # 调用OpenAI API
            ...
            result = extract_dec_re(api_result)
            if result is None:
                if "defect" in api_result:
                    return "defect"
                else:
                    return "cooperate"
            else:
        ...
        def extract_dec_re(dec):
            reg_dec = re.sub(r'\.<eos>$|\.$', '', dec.lower())
            if reg_dec == "cooperate":
                return 'cooperate'
            elif reg_dec == "defect":
                return 'defect'
            else:
        ...
        for player, dec in decision.items():
            ...
            result = extract_dec_re(dec)
            if  result is None:
                result = extract_dec(dec)
            processed_decisions[player] = result
