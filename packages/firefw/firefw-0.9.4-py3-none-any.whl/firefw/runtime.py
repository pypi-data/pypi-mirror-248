...
def pickup_outputs(outs, package) -> List[fire.Value]:
    ...
    # The Value bounded to such object will be unbined here to enable gc to
    # collect the object even though the Value sill referenced by others such
    # as a cache sotred in a user frame created by frame.f_locals.
    #
    # Since such object should be never used by a user program any more, and
    # **in the current implemetation** the Value referenced from the object
