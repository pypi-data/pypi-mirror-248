...
from aryahelpers.appconfig import CONFIG, CANDIDATE_TABLES, JOB_METADATA_COLS
...
class AryaSourcedContenders():
    ...
    def _collect_contds_metadata(self):
        ...
        try:
            ...
            contds_metadata = {_key: merge_dicts((toprocess_info[_key], _info)) 
    ...
    def access_contdsinfo(self, save_path=False, save_type='.json'):
        ...
        try:
            ...
            save_metadata = self.arya_sourcedcontds if save_type == '.json' else pd.DataFrame(
                [{k: v for k, v in d.items() if k != self.expl_key} for idx, d in enumerate(self.arya_sourcedcontds)])
    ...
    def group_samecontdsinfo(self, save_path=False, save_type='.json'):
        ...
        try:
            seg_jobs, sourced_contds, common_guids = {"qa": [], "staging": []}, [], {}
            ...
            save_data = self.arya_groupcontds if save_type == '.json' else {_gname: pd.DataFrame([
                group_contds := {k: v for k, v in d.items() if k != self.expl_key} for idx, d in enumerate(
                    _group)], columns=list(group_contds.keys())) for _gname, _group in self.arya_groupcontds.items()}
