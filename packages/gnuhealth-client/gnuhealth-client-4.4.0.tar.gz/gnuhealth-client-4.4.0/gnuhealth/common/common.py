...
class IconFactory:
    ...
    def register_icon(cls, iconname):
        ...
        to_load = slice(max(
            0, idx - cls.batchnum // 2), idx + cls.batchnum // 2)
        ids = [e[0] for e in cls._gnuhealth_icons[to_load]]
        try:
            icons = rpc.execute(
                'model', 'ir.ui.icon', 'read', ids,
    ...
    def get_pixbuf(cls, iconname, size=16, color=None, badge=None):
        ...
        if iconname in (
                "tryton-open", "tryton-launch", "tryton-cancel",
...
class ModelAccess(object):
    ...
    def __getitem__(self, model):
        ...
        to_load = slice(
            max(0, idx - self.batchnum // 2), idx + self.batchnum // 2)
        try:
            access = rpc.execute(
...
class ModelHistory(object):
    ...
    def load_history(self):
        ...
        try:
            self._models.update(
                rpc.execute('model', 'ir.model', 'list_history', rpc.CONTEXT))
        except GNUHealthServerError:
            pass
...
class ViewSearch(object):
    ...
    def load_searches(self):
        try:
            self.searches = rpc.execute(
                'model', 'ir.ui.view_search', 'get_search', rpc.CONTEXT)
        except GNUHealthServerError:
            self.searches = {}
...
class MessageDialog(UniqueDialog):
    ...
    def build_dialog(
        self, parent, message, msg_type=Gtk.MessageType.INFO,
