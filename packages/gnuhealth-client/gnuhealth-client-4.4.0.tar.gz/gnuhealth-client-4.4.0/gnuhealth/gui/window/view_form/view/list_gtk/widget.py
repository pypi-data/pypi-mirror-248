...
from gnuhealth.common import get_gdk_backend
...
def send_keys(renderer, editable, position, treeview):
    ...
    editable.editing_done_id = editable.connect(
        'editing_done', treeview.on_editing_done, renderer)
...
class GenericText(Cell):
    ...
    def open_remote(
        self, record, create, changed=False, text=None,
            callback=None):
...
class M2O(GenericText):
    ...
    def open_remote(
        self, record, create=True, changed=False, text=None,
            callback=None):
        field = record.group.fields[self.attrs['name']]
        model = self.get_model(record, field)
        ...
        screen = Screen(
            model, domain=domain, context=context,
    ...
    def search_remote(self, record, field, text, callback=None):
        ...
        def search_callback(found):
            ...
            if callback:
                callback()
        win = WinSearch(
            model, search_callback, sel_multi=False,
    ...
    def set_completion(self, entry, path):
        ...
        completion.connect(
            'match-selected', self._completion_match_selected,
            record, field, model)
        completion.connect(
            'action-activated',
