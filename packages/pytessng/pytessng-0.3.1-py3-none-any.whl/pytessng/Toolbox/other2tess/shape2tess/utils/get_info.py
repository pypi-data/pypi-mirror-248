...
from .config import default_lane_width
from .config import join_point_distance
from .config import threshold_length
from .config import threshold_distance_max
from .config import threshold_distance_min
from .config import min_width

from .functions import calculate_center, generate_random_string, m2p
...
# 获取路段信息
def get_links_info(folder_path, is_use_lon_and_lat, is_use_center_line, laneFileName="lane"):
    # 文件路径
    ...
    lon_0 = np.mean(lon_list)
    lat_0 = calculate_center(min(lat_list), max(lat_list), lon_0)
    
    global p
    if is_use_lon_and_lat:
        proj_info = f'+proj=tmerc +lon_0={lon_0} +lat_0={lat_0} +ellps=WGS84'
        other_info = {"proj_info": proj_info, "proj_center_coord": {"lon_0": lon_0, "lat_0": lat_0}}
        p = Proj(proj_info)
    else:
        p = lambda x, y: (x, y)
        other_info = {}
    ...
    # 读取数据，并组合dbf数据和shp数据
    for index, lane_data_shp in enumerate(all_data_shp_lane):
        # 获取该路段的dbf数据
        ...
        road_id = lane_data_dbf['roadId']
        # TODO
        # road_id = lane_data_dbf['FZID']
    ...
    for road_id in list(links_info.keys()):
...
def get_connector_info(folder_path, lanes_info, laneConnectorFileName="laneConnector"):
    ...
    # 读取数据，并组合dbf数据和shp数据
    for index, laneConnector_data_shp in enumerate(all_data_shp_laneConnector):
        # 获取该路段的dbf数据
        ...
        lane_width = laneConnector_data_dbf.get('Width') or default_lane_width[lane_type_from]
...
def get_info(folder_path, is_use_lon_and_lat, is_use_center_line, laneFileName, laneConnectorFileName):
    # 读取路段数据
    links_info, lanes_info, other_info = get_links_info(folder_path, is_use_lon_and_lat, is_use_center_line, laneFileName)
