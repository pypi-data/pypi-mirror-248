...
ox.settings.overpass_rate_limit = False # 取消API限速
...
class OSMData:
    ...
    def _get_osm_graph_object_by_osm_file_path(self)->None:
        ...
        try:
            ...
            lons, lats = [], []
            for u, v, key, data in tqdm(self.G.edges(keys=True, data=True)):
                for node_id in [u, v]:
        ...
        self.other_info["proj_center_coord"] = {"lon_0": lon_0, "lat_0": lat_0}
...
class Network(NetworkData):
    ...
    def _init_network(self):
        ...
        # 三、创建交叉口对象
        for nodes_id in tqdm(group.values()):
            if len(nodes_id) <= 4:
                try:
...
class NetworkCreator:
    ...
    def _create_link(self):
        ...
        # 报错的路段数
        error_count = 0
        for link_id, link in tqdm(network_links.items()):
            # 路段点位
    ...
    def _create_conn(self):
        ...
        # 创建连接段
        for conn in tqdm(network_conns):
