...
from PySide2.QtWidgets import QFileDialog, QMessageBox, QProgressDialog, QHBoxLayout, QVBoxLayout, QGroupBox, QButtonGroup
...
class NetworkEditSimplify(QWidget):
    ...
    def simplify_network(self):
        confirm = Tools.show_confirm_dialog({"content": "执行简化路网操作前需要先保存路网", "yes": "保存并简化路网"})
        ...
        # 1.执行简化
        state, message = link_edit.simplifyTessngFile(netiface, netFilePath, angle, length)
        # 2.显示信息
        if state:
