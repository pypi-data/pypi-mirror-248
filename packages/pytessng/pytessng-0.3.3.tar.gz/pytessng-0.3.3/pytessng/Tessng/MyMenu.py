...
from PySide2.QtWidgets import QWidget, QMenu, QAction, QLabel, QLineEdit, QComboBox, QRadioButton, QCheckBox, QPushButton
from PySide2.QtWidgets import QDialog, QFileDialog, QMessageBox, QProgressDialog, QHBoxLayout, QVBoxLayout, QGroupBox, QButtonGroup
from PySide2.QtCore import Qt, QRegExp, QCoreApplication
...
class Tools():
    # ico图标
    icon_path = os.path.abspath(Path(__file__).resolve().parent / ".." / "Files" / "Ico" / "TESSNG.ico")
    ...
    default_path = os.path.abspath(Path(__file__).resolve().parent / ".." / "Files" / "Examples") # 样例文件夹
    ...
    def network_import(widget, params):
        Tools.default_path = params.get("folder_path") or os.path.dirname(params["file_path"])
        ...
        # 3.执行转换
        try:
            other2tess(netiface, params, widget.mode)
            info, mode = "导入成功", "info"
        except:
    ...
    def network_export(widget):
        ...
        if hasattr(widget, 'checkBox'):
            ...
            isChecked = False
        if isChecked and widget.radio_proj_custom.isChecked():
        ...
        # 5.执行转换
        params = {**proj, "file_path": file_path}
...
class MyMenu(QMenu):
    def __init__(self, *args, extension=False):
        super().__init__(*args)
        # 文件路径
        self.instruction_path = os.path.abspath(Path(__file__).resolve().parent / ".." / "Files" / "Doc" / "TESSNG路网编辑功能说明书.docx")
    ...
    def network_import_openstreetmap(self):
        ...
        self.msg_box.setWindowTitle("导入OpenStreetMap")
        self.msg_box.setWindowIcon(QIcon(Tools.icon_path))
        self.msg_box.setText("请选择导入离线文件或获取在线地图")
        self.msg_box.setStandardButtons(QMessageBox.Yes | QMessageBox.No | QMessageBox.Cancel)
        self.msg_box.button(QMessageBox.Yes).setText("导入离线文件")
        self.msg_box.button(QMessageBox.No).setText("获取在线地图")
...
class NetworkImportOpenstreetmap(QWidget):
    ...
    def set_layout(self):
       ...
       # 第二行：按钮
        ...
        # 总体布局
        layout = QVBoxLayout()
        layout.addLayout(horizontal_layout_offline)
        layout.addWidget(self.button)
        self.setLayout(layout)
...
class NetworkEditSimplify(QWidget):
    ...
    def set_layout(self):
        # 第一行：文本、输入框
        self.label_angle = QLabel('最大简化角度设置（°）：')
        ...
        # 第二行：文本、输入框
        self.label_length = QLabel('最大简化距离设置（m）：')
    ...
    def monitor_state(self):
        ...
        # 按钮状态
        enabled_button = bool(angle) and bool(length) and (0<float(angle)<5) and (0<float(length)<200)
    ...
    def simplify_network(self):
        ...
        # 1.执行简化
        state, message = link_processing.simplifyTessngFile(netiface, angle, length)
        # 2.显示信息
        if state:
