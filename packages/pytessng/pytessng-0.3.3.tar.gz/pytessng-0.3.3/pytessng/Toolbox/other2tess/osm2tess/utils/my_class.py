...
from .config import road_type, default_lane_count
...
class OSMData:
    ...
    def _get_osm_graph_object_by_osm_file_path(self)->None:
        try:
            pgd().update_progress(0, 1, "数据解析中（0/6）")
            self.G = ox.graph_from_xml(self.params["osm_file_path"])
            pgd().update_progress(1, 1, "数据解析中（0/6）")
        ...
        # 投影
        self.other_info["scene_proj"] = f'+proj=tmerc +lon_0={lon_0} +lat_0={lat_0} +ellps=WGS84'
        self.other_info["proj_center_coord"] = self.other_info["scene_center"]
        self.p = Proj(self.other_info["scene_proj"])
        # 场景尺寸
    ...
    def _get_road_type(self):
        # 只有高速公路
        if "高速公路" in self.params["road_type"] and "城市道路" not in self.params["road_type"]:
            road_type = {"custom_filter": '["highway"~"motorway"]'}
        # 只有城市道路
        elif "高速公路" not in self.params["road_type"] and "城市道路" in self.params["road_type"]:
            road_type = {"custom_filter": '["highway"~"trunk|primary|secondary|tertiary"]'}
        # 高速公路和城市道路都有
        else:
    ...
    def _get_osm_json_data(self)->None:
        ...
        for u, v, key, data in pgd.progress(self.G.edges(keys=True, data=True), '数据解析中（1/6）'):
            for node_id in [u, v]:
            ...
            if laneCount is None:
                laneCount = default_lane_count.get(str(highway), 1)
...
class Link:
    ...
    def get_offset_width(lane_count=3)->float:
        offset_x = lane_width * lane_count / 2
        return offset_x
...
class NetworkData:
    ...
    def _init_(self):
        ...
        # 转换点
        for node_id, node in pgd.progress(self._nodes.items(), '数据解析中（2/6）'):
            node_id = str(node_id)
            loc = node["loc"]
        ...
        # 转换边
        for link_id, link in pgd.progress(self._links.items(), '数据解析中（3/6）'):
            link_id = str(link_id)
            name = link.get("name", "")
            _type = link["highway"]
...
class Network(NetworkData):
    ...
    def _init_network(self):
        ...
        # 三、创建交叉口对象
        for nodes_id in pgd.progress(group.values(), '数据解析中（4/6）'):
            if len(nodes_id) <= 4:
                try:
...
class NetworkCreator:
    def __init__(self, netiface, network: Network):
        ...
        # 场景宽度
        self.network_scene_width = 600
        # 场景高度
        self.network_scene_height = 400
        # 路段映射
    ...
    def _create_TESSNG_network(self):
        ...
        # 设置场景宽度和高度
        self._netiface.setSceneSize(self.network_scene_width * 2 + 20, self.network_scene_height * 2 + 20)  # m
        # 设置其他信息
        self._netiface.setNetAttrs("OpenStreetMap Network", otherAttrsJson=self.network_other)
        # print(self._netiface.netAttrs().otherAttrs())
    ...
    def _create_link(self):
        ...
        # 报错的路段数
        error_count = 0
        for link_id, link in pgd.progress(network_links.items(), '路段创建中（5/6）'):
            # 路段点位
            ...
            self.network_scene_width = max([abs(p[0]) for p in link_points] + [self.network_scene_width])
            self.network_scene_height = max([abs(p[1]) for p in link_points] + [self.network_scene_height])
    ...
    def _create_conn(self):
        ...
        # 创建连接段
        for conn in pgd.progress(network_conns, '连接段创建中（6/6）'):
