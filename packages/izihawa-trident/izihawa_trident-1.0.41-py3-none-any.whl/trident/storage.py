...
class Storage:
    def __init__(self, config: confuse.Configuration):
        self._nodes = {
            node_config["name"]: {
                "instance": FileShard(node_config["name"], Path(node_config["path"])),
    ...
    async def store(
        ...
        return {
            "file_shards": [
                writing_file_shard.path / key for writing_file_shard in writing_file_shards
            ]
        }
    ...
    async def exists(self, key: str) -> Path:
        for point in self._hash_ring.range(
            key, self._config["replicas"].get(int), unique=True
        ):
            file_shard: FileShard = point["instance"]
            if await file_shard.exists(key):
                return file_shard.path / key
