...
SAVE_PATH_OLD = utils_file.join_path("/home/work_nfs6/xlgeng/data", "pachong_ximalaya_2", "wav_files")
...
def handle_alum(category, album_id: str | int):
    ...
    if if_json_data_is_none(json_data):
        return
    try:
        total_num = json_data['data']['trackTotalCount']
    except Exception as e:
    ...
    for i in range(1, total_page + 1):
        ...
        if if_json_data_is_none(json_data):
            continue
        try:
            data_list = json_data['data']['tracks']
