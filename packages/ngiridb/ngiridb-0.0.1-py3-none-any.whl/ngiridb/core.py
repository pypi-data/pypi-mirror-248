...
from ngiridb.importer import import_from_string
from ngiridb.interfaces import DatabaseBackend, Record, TransactionBackend
...
logger = logging.getLogger("ngiridb")


_ACTIVE_TRANSACTIONS: ContextVar[
    typing.Optional["weakref.WeakKeyDictionary['Transaction', 'TransactionBackend']"]
] = ContextVar("ngiridb:active_transactions", default=None)


class Database:
    SUPPORTED_BACKENDS = {
        "postgresql": "ngiridb.backends.postgres:PostgresBackend",
        "postgresql+aiopg": "ngiridb.backends.aiopg:AiopgBackend",
        "postgres": "ngiridb.backends.postgres:PostgresBackend",
        "mysql": "ngiridb.backends.mysql:MySQLBackend",
        "mysql+asyncmy": "ngiridb.backends.asyncmy:AsyncMyBackend",
        "sqlite": "ngiridb.backends.sqlite:SQLiteBackend",
    }
