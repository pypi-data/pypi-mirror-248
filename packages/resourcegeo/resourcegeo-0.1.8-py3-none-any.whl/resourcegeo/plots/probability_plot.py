...
def probability_plot(data,figsize=None,xscale='log',fontsize=10,ms=1,ax=None,
    ...
    Args:
        data(np.array, pd.Series): values
    ...
    **Examples:**
    ...
    #get rid of nulls. probscale counts them to get the pplot.
    if not isinstance(data, np.ndarray):
        try:
            data = np.array(data)
        except:
            raise ValueError('Could not coerce to np.array')
    data = data[~np.isnan(data)].copy()
