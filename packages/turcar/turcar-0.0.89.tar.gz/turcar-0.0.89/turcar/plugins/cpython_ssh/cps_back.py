...
logger = getLogger("turcar.plugins.cpython_ssh.cps_back")
...
class SshCPythonBackend(BaseBackend, SshMixin):
    ...
    def _start_main_backend(self) -> RemoteProcess:
        env = {"turcar_USER_DIR": "~/.config/turcar", "turcar_FRONTEND_SYS_PATH": "[]"}
        self._main_backend_is_fresh = True

        cp_launcher_file = (
            self._get_remote_program_directory()
            + "/"
            + "turcar/plugins/cpython_backend/cp_launcher.py"
        )
        args = [
    ...
    def _get_remote_program_directory(self):
        return f"/tmp/turcar-backend-{turcar.get_version()}-{self._user}"
    ...
    def _upload_main_backend(self):
        ...
        if self._get_stat_mode_for_upload(launch_dir) and not turcar.get_version().endswith("-dev"):
            # don't overwrite unless in dev mode
            return

        ensure_posix_directory(
            launch_dir + "/turcar/plugins/cpython_backend",
            self._get_stat_mode_for_upload,
            self._mkdir_for_upload,
        )

        import turcar.ast_utils
        import turcar.backend
        import turcar.jedi_utils
        import turcar.plugins.cpython_backend.cp_back

        # Don't want to import cp_back_launcher and cp_tracers

        local_context = os.path.dirname(os.path.dirname(turcar.__file__))
        for local_path in [
            turcar.__file__,
            turcar.common.__file__,
            turcar.ast_utils.__file__,
            turcar.jedi_utils.__file__,
            turcar.backend.__file__,
            turcar.plugins.cpython_backend.__file__,
            turcar.plugins.cpython_backend.cp_back.__file__,
            turcar.plugins.cpython_backend.cp_back.__file__.replace("cp_back.py", "cp_launcher.py"),
            turcar.plugins.cpython_backend.cp_back.__file__.replace("cp_back.py", "cp_tracers.py"),
        ]:
