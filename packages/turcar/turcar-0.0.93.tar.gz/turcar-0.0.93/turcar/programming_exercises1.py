...
class MainFrame(tk.Frame):
    ...
    def __init__(self, root):
        ...
        # 设置 MainFrame 在 root 中的位置和大小
        self.grid(row=0, column=0, sticky="nsew")

        # 创建 NavigationBar
        self.navigation_bar = NavigationBar(self)
        # 设置 NavigationBar 在 MainFrame 中的位置
        self.navigation_bar.grid(row=0, column=0, sticky="ew")

        # 创建 BrowserFrame
        self.browser_frame = BrowserFrame(self, self.navigation_bar)
        # 设置 BrowserFrame 在 MainFrame 中的位置和大小
...
def init():
    get_workbench().add_view(MainFrame, '练习', "se", visible_by_default=False)
