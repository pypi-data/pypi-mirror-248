...
from turcar.ui_utils import select_sequence, show_dialog
...
class Runner:
    ...
    def _init_commands(self) -> None:
        ...
        try:
            import turcar.plugins.debugger  # @UnusedImport
        ...
        if not running_on_mac_os():  # on Mac Ctrl+C is not used for Copy.
            ...
            if isinstance(widget, tk.Text):
                ...
                try:
                    ...
                    # i.e. it may be from another application when turcar has nothing selected
                    ...
                    if isinstance(selection, str) and len(selection) > 0:
                        ...
                        # ie. there may be no selection in turcar actually.
    ...
    def _handle_backend_termination(self, returncode: int) -> None:
        ...
        try:
            faults_file = os.path.join(turcar_USER_DIR, "backend_faults.log")
    ...
    def _check_alloc_console(self) -> None:
        if sys.executable.endswith("pythonw.exe"):
            ...
            try:
                ...
                # See https://bitbucket.org/plas/turcar/issues/508/cant-allocate-windows-console-when
                # Without flush the console window becomes visible, but turcar can be still used
...
class BackendProxy(ABC):
    ...
    def destroy(self, for_restart: bool = False):
        """Called when turcar no longer needs this instance
        (turcar gets closed or new backend gets selected)
        """
        ...
...
class SubprocessProxy(BackendProxy, ABC):
    ...
    def _get_environment(self):
        ...
        # see https://github.com/turcar/turcar/issues/808
        env["PYTHONUNBUFFERED"] = "1"

        env["PYTHONSAFEPATH"] = "1"

        # Let back-end know about plug-ins
        env["turcar_USER_DIR"] = turcar_USER_DIR
        env["turcar_FRONTEND_SYS_PATH"] = repr(sys.path)

        env["turcar_LANGUAGE"] = get_workbench().get_option("general.language")
        env["turcar_VERSION"] = get_version()

        if turcar.in_debug_mode():
            env["turcar_DEBUG"] = "1"
        elif "turcar_DEBUG" in env:
            del env["turcar_DEBUG"]
        return env
        ...
        while True:
            ...
            if data == "":
                ...
                try:
                    ...
                    # then the suffix can be turcar message
...
def get_environment_overrides_for_python_subprocess(target_executable):
    ...
    # exe is bundled for turcar.
    # In remaining cases it is user's responsibility to avoid
    # calling turcar with environment which may be confusing for
    ...
    if os.path.samefile(
        ...
        # (If they are set then it's most likely because current exe is in turcar bundle)
