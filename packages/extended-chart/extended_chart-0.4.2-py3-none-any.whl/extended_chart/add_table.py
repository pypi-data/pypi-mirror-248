...
def add_signal_table(chart, data, height=1, width=0.1, position='right', color_style=color_style,
                     chunks=3, start_time_offset=pd.offsets.Hour(1)):
    data = data.reset_index(drop=True)
    if 'end_time' not in data.columns:
        data['end_time'] = data['start_time'] + start_time_offset
    ...
    sig_table = chart.create_table(width=width, height=height, return_clicked_cells=True, headings=headings,
                                   heading_background_colors=heading_color, alignments=alignments,
    ...
    for i, x in enumerate(list_df, 1):
        row = sig_table.new_row(*list(x.index.values))
...
def move_pnl_table(table, footer_index, page_number, total_page_count, list_of_pnls):
    ...
    for x in _table.itertuples():
        row = table.new_row(x.Index, x.entry_time, x.exit_time, x.direction, x.entry_price, x.exit_price, x.quantity,
                            x.pnl_with_commission, x.mfe, x.mae, x.time_to_live)
