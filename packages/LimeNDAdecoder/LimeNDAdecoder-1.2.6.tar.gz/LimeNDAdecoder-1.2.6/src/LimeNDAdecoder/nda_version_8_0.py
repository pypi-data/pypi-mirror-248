...
def validate_timegap(df):
    ...
    df.loc[
        (
            ...
            & ((df["step_ID"] == df["prev_step"]) & (df["time_in_step"] != 0))
        ),
        "Validated",
    ] = True
...
def main_validator(df, capacity_nom):
    ...
    if (df["energy_mWh"].min() < 0) and (df[df["step_name"] == "SIM"].shape[0] == 0):
        logging.info(
            "Negative Energy error. Min energy is: " + str(df["energy_mWh"].min())
        )
        return False

    if (df["capacity_mAh"].min() < 0) and (df[df["step_name"] == "SIM"].shape[0] == 0):
        logging.info(
            "Negative Capacity error. Min capacity is: " + str(df["capacity_mAh"].min())
        )
        return False

    if df["capacity_mAh"].max() > (1500 * capacity_nom):
        logging.info(
            "Max Capacity error. Max capacity is: " + str(df["capacity_mAh"].max())
        )
        return False
...
def _bytes_to_list(byte_stream):
    ...
    [Status, Jump, Time] = struct.unpack(
        "<BBQ", byte_stream[12:22]
    )  # state_dict[Status] is step_name
...
def nda_in_df_out(file, include_step_number: bool = False, include_aux: bool = False):
    ...
    with open(file, "rb") as f:
        ...
        if mm.find(b"BTS Client") == -1:
            raise ValueError(
                f"{file} does not appear to be a correctly downloaded Neware file."
            )
        ...
        while mm.tell() < mm_size:
            ...
            if len(bytes) == record_len:
                ...
                if bytes[0:2] == b"\x55\x00" and bytes[82:87] == b"\x00\x00\x00\x00":
                    output.append(_bytes_to_list(bytes))

                # Check for an auxiliary record
                elif bytes[0:1] == b"\x65" and bytes[82:87] == b"\x00\x00\x00\x00":
    ...
    df.loc[((df["prev_cur"] == 0) & (df["current_mA"] != 0)), "DCIR(mOhm)"] = (
        abs((df["voltage_V"] - df["prev_vol"]) / (df["current_mA"] - df["prev_cur"]))
        * 1000000
    )
    df["DCIR(mOhm)"] = df["DCIR(mOhm)"].astype("float64")
