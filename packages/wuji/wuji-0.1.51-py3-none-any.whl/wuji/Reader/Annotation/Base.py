...
def add_not_scored_into_sleep_stages(sleep_stages):
    new_df = pd.DataFrame(columns=sleep_stages.columns)

    # 遍历原始 DataFrame
    for i, row in sleep_stages.iterrows():
        # 如果不是第一行且当前行的 Start 与上一行的 End 不相等
        if i > 0 and row['Start'] != new_df.iloc[-1]['End']:
            # 插入新行
            new_row = {'Type': 'NotScored', 'Start': new_df.iloc[-1]['End'],
                       'Duration': row['Start'] - new_df.iloc[-1]['End'], 'End': row['Start']}
