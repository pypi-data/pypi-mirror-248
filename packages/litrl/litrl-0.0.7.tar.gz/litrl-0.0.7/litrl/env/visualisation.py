import dash
from dash import Input, Output, callback, callback_context, html, dcc
...
def get_server(env: StaticOpponentWrapper, n_actions: int, opponents: dict[OpponentId, Agent]):
    ...
    def render_env(*args, **kwargs):
        ...
        return [image, turn, player_color, description_hidden, winner_div_hidden, winner_name]
            ...
            children=[
                ...
                html.Div(
                    [
                        ...
                        html.Span(PLAYER_COLORS[env.unwrapped.agent_selection], id="player-color")
                    ],
    ...
    opponent_dropdown = dcc.Dropdown(opponent_names, opponent_names[0], id='opponent-dropdown')
...
if __name__ == "__main__":
    ...
    env = make("ConnectFour-v3", opponent=opponents["MCTS"], render_mode="rgb_array")
    env.reset(seed=123);
    app = get_server(env, n_actions=7, opponents=opponents)
    app.run(debug=True)