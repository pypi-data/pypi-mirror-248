...
def compute_meteor(referencess, candidates):
    score_list = []
    for i in range(len(candidates)):
        if type(referencess[i]) is list:
            ref_list = referencess[i]
        else:
            ref_list = [referencess[i]]
...
def compute_sent_bleu(referencess, candidates):
    bleu1 = 0.0
    bleu2 = 0.0
    bleu3 = 0.0
    bleu4 = 0.0
    ...
    for i in range(len(candidates)):
        ...
        if type(referencess[i]) is list:
            tmp = []
            for ref in referencess[i]:
                tmp.append(word_tokenize(ref))
            ref_list.append(tmp)
        else:
            ref_list.append([word_tokenize(referencess[i])])
...
def compute_rouge(referencess, candidates):
    rouge = Rouge()
    scores = rouge.get_scores(candidates, referencess)
...
def compute_chrf(referencess, candidates):
    chrf = CHRF(word_order=2)
    return round(chrf.corpus_score(candidates, [referencess]).score, 4)


def compute_sacre_corpus_bleu(referencess, candidates):
    bleu = BLEU()
    return round(bleu.corpus_score(candidates, [referencess]).score, 4)


def compute_sacre_sent_bleu(referencess, candidates):
    bleu = []
    for i, a_gold in enumerate(referencess):
...
def get_bert_score(referencess, candidates, device):
    # scorer = BertScorer()
...
def compute_corpus_bleu(referencess, candidates):
    bleu1 = 0.0
    bleu2 = 0.0
    bleu3 = 0.0
    bleu4 = 0.0
...
def compute_f1(referencess, candidates, nlp=None):
    """
...
def get_eval_metrics(eval_inputs: Union[Dataset, pd.DataFrame, dict] = None, 
    ...
    Args:
        outputs (Union[Dataset, pd.DataFrame, dict], optional): 模型的输出，必需包含 “model_responses” 和 “references” 两列
    ...
    if "meteor" in metrics:
        ...
        meteor_score = compute_meteor(references, model_responses)
    ...
    if "hf_chrf" in metrics:
        ...
        try:
            ...
            chrf = chrf_computer.compute(
                predictions=model_responses, referencess=references)['score']
    ...
    if "chrf" in metrics:
        try:
            chrf = compute_chrf(references, model_responses)  # 偏低3个点
        except Exception as e:
    ...
    if "f1_space_split" in metrics:
        ...
        f1_references = eval_inputs["f1_references"]
        f1 = compute_f1(f1_references, model_responses)
    ...
    if "f1_nlp_split" in metrics:
        ...
        f1_references = eval_inputs["f1_references"]
        f1 = compute_f1(f1_references, model_responses, nlp)
    ...
    if "hf_google_bleu" in metrics:
        ...
        try:
            ...
            google_bleu_score = google_bleu.compute(
                predictions=model_responses, referencess=references
            )["hf_google_bleu"]
    ...
    if "sacrebleu_sent" in metrics:
        ...
        try:
            bleu = compute_sacre_sent_bleu(
                candidates=model_responses, referencess=references)
        except Exception as e:
    ...
    if "dist" in metrics:
        ...
        dist1 = distinct_ngram(model_responses, n=1)
        dist2 = distinct_ngram(model_responses, n=2)
        test_result.add(
            dist1=dist1,
            dist2=dist2,
        )
    ...
    if "hf_rouge" in metrics:
        ...
        try:
            ...
            rouge_results = rouge.compute(predictions=model_responses,
            ...
            print_error_info(e)
            log.info("hf_rouge 无法计算，references可能为空，请检查生成数据！")

    if "rouge" in metrics:
        log.info("计算 ROUGE ing...")
        try:
            rouge_1, rouge_2, rouge_l = compute_rouge(references, model_responses)
            test_result.add(rouge_1=rouge_1)
            test_result.add(rouge_2=rouge_2)
            test_result.add(rouge_l=rouge_l)
            ...
            print_error_info(e)
            log.info("Rouge 无法计算，references可能为空，请检查生成数据！")
    ...
    if "hf_bert_score" in metrics:
        ...
        bert_score_references = eval_inputs["bert_score_references"]
        try:
            bertscore = load("bertscore")
            bert_score = mean(bertscore.compute(predictions=model_responses,
                                                referencess=bert_score_references, lang="en", rescale_with_baseline=True)['f1'])
            bert_score = round(bert_score, 4)
            test_result.add(bert_score=bert_score)
            log.info(f"hf_bert_score = {str(bert_score)}")
        except ValueError:
            log.info("hf_bert_score 无法计算，references可能为空，请检查生成数据！")

    if "bert_score" in metrics:
        log.info("计算 Bert score ing...")
        bert_score_references = eval_inputs["bert_score_references"]
        try:
            bert_score = get_bert_score(bert_score_references, model_responses, device)
            test_result.add(bert_score=bert_score)
            log.info(f"bert_score = {str(bert_score)}")
        except ValueError:
            log.info("bert_score 无法计算，references可能为空，请检查生成数据！")
    ...
    if "hf_ppl" in metrics:
        ...
        try:
            ppl = perplexity.compute(predictions=model_responses, model_id="gpt2")[
                "mean_perplexity"
            ]
        except Exception as e:
            log.error("计算 PPL 失败")
    ...
    if "q_squared" in metrics:
        ...
        try:
            df, steps_df = q2_step_one(infile=pd.DataFrame(eval_inputs)[["model_responses", "references", "q2_references"]].rename(columns={
