...
class Linear(NetworkLayer):
    ...
    def __init__(self, input_size  : int,
        ...
        self._B = initialize_weights((self._output_size, 1), self._input_size)
...
class Conv2D(NetworkLayer):
    ...
    def __init__(self, input_channels  : int,
        ...
        # (CC, 1)
        self._B = initialize_weights((self._output_channels, 1), N)
    ...
    def forward(self, x : np.ndarray) -> np.ndarray:
        ...
        Z = ConvTools.apply_convolution(x, self._W, self._strides) + self._B.reshape((1, self._output_channels, 1, 1))
    ...
    def backward(self, gradient : np.ndarray) -> np.ndarray:
        ...
        dW = ConvTools.transform_gradient_for_weights(self._X, dZ, self._W, self._strides)

        # (B, C, H, W) <= (B, CC, OH, OW) x (CC, C, CH, CW)
        dX = ConvTools.transform_gradient(dZ, self._W, self._strides)
