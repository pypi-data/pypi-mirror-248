...
class Voice:
    ...
    def _get_sentence_info(self, words: List[Word]) -> Sentence:
        ...
        """
        no_modifier_words = [
            Word(word=word.word, voice=word.voice, is_punctuation=word.is_punctuation)
            for word in words
        ]
    ...
    def generate_audio_from_array(
        self, words: List[Word], dry_run=False, save_to_db=True
    ) -> Sentence:
    ...
    def get_sayable_unsayable(self, words: List[Word]) -> Tuple[List[Word], List[Word]]:
        ...
        sayable_words_set.update(
            [
                ...
                for punct in PUNCTUATION_TIMING
            ]
        )
    ...
    def _create_audio_segments(self, word_array: List[Word]) -> List[AudioSegment]:
        ...
        for word in word_array:
            if word.is_punctuation:
                words_audio.append(AudioSegment.silent(PUNCTUATION_TIMING[word.word]))
            else:
...
class MultiVoice(Voice):
    ...
    def __init__(
        self, voices: Dict[str, SingleVoice], database: Optional[DatabaseConnection]
    ):
        """
        Args:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate a sentence using a voice")
    parser.add_argument(
        "-s",
        "--voice-dir",
        type=str,
