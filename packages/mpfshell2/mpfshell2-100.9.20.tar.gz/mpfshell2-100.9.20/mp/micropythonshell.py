...
class MicropythonShell:
    ...
    def __do_folder_diff(self, directory_local, files_to_skip) -> Tuple[List[str], List[str], List[str]]:
        """
        ...
        files_remote = self.__up_listfiles_remote()
        for filename_remote, sha256_remote in files_remote:
        ...
        for file_remote, _ in files_remote:
            directory, _, _ = file_remote.rpartition("/")
            if directory == "":
                continue
            directories_to_create.discard(directory)

        return sorted(files_to_delete), sorted(files_to_download), sorted(directories_to_create)
    ...
    def sync_folder(self, directory_local, files_to_skip=FILES_TO_SKIP):
        ...
        files_to_delete, files_to_download, directories_to_create = self.__do_folder_diff(
