...
class RibLeft8Dataset:
    '''
    This class is for the rib left 8 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibLeft8Dataset
    
    rib_left_8_dataset = RibLeft8Dataset()
    rib_left_8_dataset.info()  # Print dataset information
    rib_left_8_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1Eivta1OgkjAxo29YW4dv3yo2CV8aXJDX',
            '200': 'https://drive.google.com/uc?id=1SfMKAFftSEflRce5FXiY1d4OFPjn7fpg',
            '400': 'https://drive.google.com/uc?id=10B-9wKiNlYyKR1ZBadCflev-7sucdD9x',
            'all': 'https://drive.google.com/uc?id=1YK-Dg7fQPQvExUs4HFmNtoQs05zEfgYA'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Left 8 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_left_8{subgroup}.zip')
