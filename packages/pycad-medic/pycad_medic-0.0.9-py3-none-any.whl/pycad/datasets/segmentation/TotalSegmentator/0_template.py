...
class kbirDataset:
    '''
    This class is for the sghir segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import kbirDataset
    
    sghir_dataset = kbirDataset()
    sghir_dataset.info()  # Print dataset information
    sghir_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=100',
            '200': 'https://drive.google.com/uc?id=200',
            '400': 'https://drive.google.com/uc?id=400',
            'all': 'https://drive.google.com/uc?id=all'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"kbir Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'sghir{subgroup}.zip')
