...
class InLieuTests(unittest.TestCase):
    def test_in_lieu_should_be_holiday(self):
        self.assertTrue(all(japanese_calendar.is_holiday(_) for _ in japanese_calendar.in_lieu_days))

    def test_in_lieu_should_equal_workdays_amount(self):
        min_year, max_year = min(japanese_calendar.in_lieu_days.keys()).year, max(japanese_calendar.holidays.keys()).year
        for year in range(min_year, max_year + 1):
            in_lieu_days_amount = len(
                list(filter(lambda date: date.year == year, japanese_calendar.in_lieu_days.keys()))
            )
            workdays_amount = len(list(filter(lambda date: date.year == year, japanese_calendar.workdays.keys())))
            if year == 2006:
    ...
    def test_in_lieu_should_be_weekday(self):
        for in_lieu_day in japanese_calendar.in_lieu_days.keys():
