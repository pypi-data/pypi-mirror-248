...
def main():
    ...
    if args.verbose >=1:
        print("ISLA Version 1.0; A 10-K Item Segmentation Tool")
    ...
    for ares in res_files:
        if os.path.exists(ares) == False:
            print(f"Cannot fild resource file {crf_model_fn}." 
    ...
    if method == "isla":
        ...
        ckpt = torch.load(best_model_name)  # load model
    ...
    if len(par1m1) == 0:
        print("Cannot find critial tags (SEC-DOCUMENT to SEC-HEADER). Stop")
        urltype = "HTML"                
    else:
    ...
    if args.verbose >= 1:
        print(f"Output files to {args.outputdir}")
