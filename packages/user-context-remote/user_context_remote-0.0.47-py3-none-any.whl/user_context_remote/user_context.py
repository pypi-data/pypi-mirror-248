...
class UserContext:
    ...
    def _authenticate_by_user_identification_and_password(self, user_identifier: str, password: str) -> str:
        ...
        try:
            ...
            data = {"userIdentifier": user_identifier, "user_identifier": user_identifier, "password": password}
            ...
            # TODO token -> userToken
            self.user_jwt = output.json()["data"]["userJwt"]
    ...
    def _authenticate_by_user_jwt(self, user_jwt: str) -> str:
        ...
        output = requests.post(
            url=authentication_login_validate_user_jwt_url, data=json.dumps(data, separators=(",", ":")), headers=headers
        )
        if output.status_code != HTTPStatus.OK:
            MiniLogger.error(
        ...
        MiniLogger.end(_AUTHENTICATE_BY_USER_JWT_METHOD_NAME,
                       object={"validate_user_jwt_response": output})
        return output
