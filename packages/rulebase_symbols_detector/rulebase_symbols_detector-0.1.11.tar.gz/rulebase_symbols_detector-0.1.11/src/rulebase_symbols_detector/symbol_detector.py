...
import re
import datetime
from .init_data import JSON_DATA, ADDITIONAL_DATA_SYMBOLS
...
class SymbolDetector:
    ...
    def normalize_company_name2symbol(self, info):
        ...
        if '- CTCP' in info["Full Name"]:
            full_name = info["Full Name"].replace('- CTCP','').strip()
        elif 'CTCP Tập' in info["Full Name"]:
            full_name = info["Full Name"].replace('CTCP Tập','Tập').strip()
        else:
            full_name = info["Full Name"].strip()
...
#     d = SymbolDetector()
#     d.update_derivative_symbols([('VN30F2305', 'VN30F1M', 'VN30'), ('VN30F2306', 'VN30F2M', 'VN30'), ('VN30F2309', 'VN30F1Q', 'VN30'), ('VN30F2312', 'VN30F2Q', 'VN30')])
#     content = """Sở Giao dịch Chứng khoán Thành phố Hồ Chí Minh thông báo về việc niêm yết và giao dịch chứng chỉ quỹ ETF thay đổi niêm yết như sau:

# Tài liệu đính kèm:
# 20230802_20230802 - FUESSV30 - TB thay doi NY do GD hoan doi (tang).pdf
# """
#     print(d.detect(content))
