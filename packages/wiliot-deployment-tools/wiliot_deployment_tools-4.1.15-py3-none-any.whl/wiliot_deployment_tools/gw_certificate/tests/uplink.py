...
from wiliot_deployment_tools.interface.if_defines import BRIDGES, DEFAULT_DELAY
...
from wiliot_deployment_tools.gw_certificate.tests.static.generated_packet_table import TEST_UPLINK, UplinkRunData
...
class GenericUplinkStage(GenericStage):
    def __init__(self, mqttc:MqttClient, ble_sim:BLESimulator, gw_capabilities:GWCapabilities,
                 stage_name, pass_criteria:Literal['all', '90%', 'none'] = 'all', **kwargs):
    ...
    def generate_stage_report(self, pass_criteria:Literal['all', '90%', 'none']='all'):
        ...
        report.append((('Number of unique packets sent'), num_pkts_sent))
        report.append((('Number of unique packets received'), num_pkts_received))
    ...
    def generate_stage_report(self, pass_criteria:Literal['all', '90%', 'none'] = 'all', **kwargs):
        ...
        for message in self.all_messages_in_test:
            ...
            if self.gw_capabilities.geoLocationSupport:
                validation = validate_message(MESSAGE_TYPES.DATA_GEOLOCATION, message)
            else:
...
STAGES = [InitStage, BrgDataStage, TagPacketStage, ApiValidationStage]
# STAGES = [InitStage]
