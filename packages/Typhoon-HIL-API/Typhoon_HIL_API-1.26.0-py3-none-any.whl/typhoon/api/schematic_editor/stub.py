...
from typhoon.api.schematic_editor.const import ITEM_HANDLE, DATA_FRAME
        ...
        try:
            ...
            def wrapper(*args, **kwargs):
                ...
                if name in old_functions and result is None and error is None:
                    ...
                    result = pandas.read_json(result["data"])
                    ...
                    for item in result:
                        if isinstance(item, dict) and ITEM_HANDLE in item:
                            ...
                            # -- The data frame is serialized with dataframe.to_json()
                            res.append(pandas.read_json(item["data"]))
                        else:
                            res.append(item)
                    result = res
