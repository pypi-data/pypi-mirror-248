...
def add_messages(
    ...
    roles: str | list[str] = "user",
    contents: str | list[str] = "",
) -> list[dict[str, str]]:
    messages = copy.deepcopy(messages)
    if not is_list(roles):
        roles = [roles]
    if not is_list(contents):
        contents = [contents]
...
def chroma_retriever(
    ...
    **kwargs,
    ...
    query_res = collection.query(query_texts=query_texts, n_results=n_results)
    ...
    if "documents" in retrieval_res and "ids" in retrieval_res:
        ...
        if len(info_data["documents"]) > 0:
            messages = add_messages(
                messages=messages,
                roles="user",
                contents=rag_template(info=info_data["documents"]),
            )
