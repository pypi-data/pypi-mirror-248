...
class DebugDiagnostic(Diagnostic):
    ...
    def __init__(
        self, position: CodePosition, filepath: Path, message: str
    ) -> None:
...
def analyze_attr(
    ...
    pool = analyzer.get_pool()
    if not pool:
        return
    if model.type == "model":
        if attr not in model.fields or "relation" not in model.fields[attr]:
            return
        field = model.fields[attr]
        target = pool.get(field["relation"])
        if field["type"] == "many2one":
            analyzer._node_mappings[node] = target
        else:
            analyzer._node_list_mappings[node] = target
    elif model.type == "wizard":
        if attr not in model.states:
            return
        analyzer._node_mappings[node] = pool.get(
            model.states[attr]["relation"]
        )


def generate_completions_from_model(
    model: "PoolModel",
) -> list[CompletionItem]:
    if model is None:
        return []
    completions = []
    for key, info in model.get_completions().items():
        documentation = ""
        if info["type"] == "field":
