...
class Git:
    ...
    def clone(self) -> None:
        ...
        Command(
            cmd=(
                'git', 'clone', ('--bare' if self.is_bare else '--recursive'),
                self.repository, self.home,
            ), check=True,
        ).run_with_spinner()
    ...
    def from_dir(cls, directory: str | None = None) -> Self:
        ...
        git_home = find_up_dir(predicate=lambda p: os.path.isdir(os.path.join(p, '.git')), start_dir=directory) or find_up_dir(
            predicate=lambda p: os.path.isfile(os.path.join(p, 'packed-refs')), start_dir=directory,
        )
        if not git_home:
