...
def compute_positive_rep(val,var2=var2,var3=var3,msg=False):
	...
	if notint:
		...
		tuples = [(i,j) for i in range(n1) for j in range(n2)]
		avars = [sympy.Symbol(f"a_{i}_{j}")  for i,j in tuples]
		avarsid = [varsimp2[i] - varsimp3[j] for i,j in tuples]
		
		
		base_vectors = []
		base_monoms = []
		base_sympy = []
		ideal = [sympy.sympify(avars[i] - avarsid[i]) for i in range(len(avars))]
		
		G = sympy.groebner(ideal,*var22,*var33,*avars)
		print(f"{G=}")
		...
		#t = time.time()
		#vec = poly_to_vec(val)
		vec = poly_to_vec(sympify(sympy.reduced(val,G,wrt=var22+var33)[1]))
		...
		for mn1 in mn1L:
			...
			for i in range(len(comblistmn1)):
				b1 = comblistmn1[i]
				vec0 = poly_to_vec(sympify(sympy.reduced(b1,G,wrt=var22+var33)[1]),vec)
				if vec0 is not None:
					base_vectors += [vec0]
					base_monoms += [b1]
					base_sympy += [sympy.sympify(b1)]
		...
		#G = sympy.groebner(base_sympy,*var22,*var33)
		#dimen = len(G)
		#for i in range(len(base_sympy)):
		#	b1 = base_sympy[i]
		#	#print(f"{sympy.reduced(b1,G)=}")
		#	base_vectors[i] = sympy.reduced(b1,G)[0]
		#vc = sympy.reduced(sympify(val),G,wrt=var22+var33)[1]
		#print(f"{vc=} {len(sympy.expand(vc).args)=}")
