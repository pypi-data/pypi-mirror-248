...
def posify(val,u2,v2,w2,var2=var2,var3=var3,msg=False):
	...
	if (will_formula_work(v,u) or dominates(u,w)):
		...
		diff_index = [i for i in range(len(w)) if i>=len(u) or w[i]!=u[i]]
		diff_index_u = [diff_index[i] for i in range(len(diff_index)) if i<len(u)]
		wslice = [w[i] for i in diff_index]
		uslice = [u[i] for i in diff_index_u]
		neww = tuple(permtrim([len([i for i in wslice if i<j])+1 for j in wslice]))
		newu = tuple(permtrim([len([i for i in uslice if i<j])+1 for j in uslice]))
		#newu = tuple(permtrim(sorted(id2,key=lambda i: -u[diff_index_u[i-1]])))
		#neww = [diff_index.index(w.index(w[i])) f
		print(f"{u=} {v=} {w=} {diff_index=} {newu=} {neww=} {uslice=} {wslice=}")
		#print(f"{code(newu)=} {code(neww)=}")
		arr = [[[],v]]
		d = -1
		for i in range(len(v)-1):
			if v[i]>v[i+1]:
		...
		for i in range(d):
			...
			if i in diff_index:
				continue
			i2 = 1
			i2 += len([aa for aa in diff_index if i>aa])
			for vr, v2 in arr:
				dpret = pull_out_var(i2,[*v2])
				for v3r, v3 in dpret:
					arr2 += [[vr + [(v3r,i+1)],v3]]
		...
		for L in arr:
			v3 = [*L[-1]]
			iv = inverse(newu)
			print(f"{code(v3)=} {code(newu)=} {code(neww)=} {iv=}")
			
			coeff = sympify(posify(schubmult({newu: 1},tuple(permtrim([*v3]))).get(neww,0),newu,tuple(permtrim([*v3])),neww,var2,var3,msg)).subs({var2[i+1]: var2[iv[i]] for i in range(len(iv))})
			print(f"{coeff=}")
			toadd = 1
			for i in range(len(L[0])):			
				var_index = L[0][i][1]
				...
				for j in range(len(oaf)):				
					toadd*= var2[yv] - var3[oaf[j]]
			varo = [0,*[var2[w[diff_index[k]]] for k in range(3)]]
			print(f"{varo=}")
			subs_dict3 = {var2[i]: varo[i] for i in range(len(varo))}
			toadd*=sympify(coeff).subs(subs_dict3)
			print(f"{toadd=}")
			val += toadd		
	else:
		c01 = code(u)
