		...
		for mn1 in mn1L:
			...
			while len(stack) > 0:
				...
				for i in range(n1,n1+n2):
					if mno[i]!=0:
						mn1_2 = tuple(list(mno[:i])+[0]+list(mno[i+1:]))
						for mm0 in mn:
							cmb1 = cmb
							if mm0[n1:] == mn1_2[n1:]:
								good = True
								...
								if good:
									...
									for k in range(n1):
										if mm0[k] - mn1_2[k] > 1:
											bad = True
											break
										elif mm0[k] - mn1_2[k] == 1:
											cmb1 *= varsimp2[k] - varsimp3[i - n1]
									if not bad:
										stack += [mm0]
										comblist += [cmb1]
						break
			print(len(comblistmn1))
			for b1 in comblistmn1:
				if b1 not in base_monoms:
