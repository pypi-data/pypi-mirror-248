...
def compute_positive_rep(val,var2=var2,var3=var3,msg=False):
	...
	if notint:
		...
		for mn1 in mn1L:
			...
			#	if vec0 is not None:
			#		base_vectors += [vec0]					
			base_monoms += comblistmn1
		
		
		base_vectors = Parallel(n_jobs=-1)(delayed(poly_to_vec)(b1,vec) for b1 in base_monoms)
		
		for i in range(len(base_vectors)-1,-1,-1):
			if base_vectors[i] is None:
				base_vectors.pop(i)
				base_monoms.pop(i)
					
		#base_buckets.sort(key=lambda p: str(base_monoms[p]))
		#base_vectors.sort(key=lambda p: base_buckets[p])
		#base_monoms.sort()
		#base_vectors2 = [base_vectors[
		#np.random.shuffle(base_buckets)
		#base_vectors = [base_vectors[i] for i in base_buckets]
		#base_monoms = [base_monoms[i] for i in base_buckets]
