...
class Object:
    def __init__(self, df):
        ...
        self.liste_flux['StreamType'] = np.where(self.liste_flux['Ti'] > self.liste_flux['To'], 'HS', 'CS')

        # Créer de nouvelles colonnes pour les températures décalées
        self.liste_flux['STi'] = np.where(self.liste_flux['StreamType'] == 'HS',
                                                  self.liste_flux['Ti'] - self.liste_flux['dTmin2'],
                                                  self.liste_flux['Ti'] + self.liste_flux['dTmin2'])

        self.liste_flux['STo'] = np.where(self.liste_flux['StreamType'] == 'HS',
                                                  self.liste_flux['To'] - self.liste_flux['dTmin2'],
                                                  self.liste_flux['To'] + self.liste_flux['dTmin2'])

         # Calculer T_shifted directement dans la classe
        T_shifted = np.concatenate([self.liste_flux['STi'].values, self.liste_flux['STo'].values])
    ...
    def decomposition_flux(self):
        ...
        for idx, row in self.df_intervals.iterrows():
            ...
            for i in range(self.rowCount):
                ...
                if (self.liste_flux['StreamType'][i] == "CS") and (self.liste_flux['STi'][i] < row['Tsup']) and (self.liste_flux['STo'][i] > row['Tinf']):
                    # Flux dans l'intervalle
                    self.df_intervals.at[idx, 'mCp'].append(-self.liste_flux['mCp'][i])
                    self.df_intervals.at[idx, 'StreamName'].append(self.liste_flux['name'][i])
                    self.df_intervals.at[idx, 'StreamType'].append(self.liste_flux['StreamType'][i])  # Add this line




                # Tester la condition spécifique (StreamType[i] == "HS")
                elif (self.liste_flux['StreamType'][i] == "HS") and (self.liste_flux['STi'][i] > row['Tinf']) and (self.liste_flux['STo'][i] < row['Tsup']):
                    # Flux dans l'intervalle
                    self.df_intervals.at[idx, 'mCp'].append(self.liste_flux['mCp'][i])
                    self.df_intervals.at[idx, 'StreamName'].append(self.liste_flux['name'][i])
                    self.df_intervals.at[idx, 'StreamType'].append(self.liste_flux['StreamType'][i])  # Add this line
    ...
    def plot_streams_and_temperature_intervals(self,figsize=(8, 6)):
      ...
      StreamType = self.liste_flux['StreamType']
      names = self.liste_flux['name']
      STi = self.liste_flux['STi']
      STo = self.liste_flux['STo']
    ...
    def surplus_deficit(self):
      ...
      self.Heating_duty=abs(min(self.df_surplus_deficit['cumulative_delta_H']))
