...
class ColumnWhereExec:
    ...
    def get(self):
        ...
        """
        return self.where_exec.get(self.columns[0])

    def select(self):
        """
        sqlexec.table('person').columns('name', 'age').where(name='李四').select('name', 'age')
        """
        return self.where_exec.select(*self.columns)

    def select_one(self):
        """
        sqlexec.table('person').columns('name', 'age').where(name='李四').select_one('name', 'age')
        """
        return self.where_exec.select_one(*self.columns)

    def query(self):
        """
        sqlexec.table('person').columns('name', 'age').where(name='李四').query('name', 'age')
        """
        return self.where_exec.query(*self.columns)

    def query_one(self):
        """
        sqlexec.table('person').columns('name', 'age').where(name__eq='李四').query_one()
        """
        return self.where_exec.query_one(*self.columns)
    ...
    def to_csv(self, file_name: str, delimiter=',', header=True, encoding='utf-8'):
        ...
        """
        self.where_exec.load(*self.columns).to_csv(file_name, delimiter, header, encoding)

    def to_df(self):
        """
        sqlexec.table('person').columns('name', 'age').where(name__eq='李四').df()
        """
        return self.where_exec.load(*self.columns).to_df()

    def to_json(self, file_name: str, encoding='utf-8'):
        """
        sqlexec.table('person').columns('name', 'age').where(name__eq='李四').json('test.json')
        """
        self.where_exec.load(*self.columns).to_json(file_name, encoding)
