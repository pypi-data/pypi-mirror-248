...
class Tree(Base):
    ...
    def acknowledged(self):
        ...
        for child in self._children.values():
            if child.state is not State.okay and not child.acknowledged:
...
class Leaf(Base):
    ...
    def __init__(self, test, state, message=None, reported=None, acknowledgement=None, stale=False):
        super().__init__(test)
        self._state = state
    ...
    @property
    def acknowledged(self):
        """ Query whether current state is acknowledged. """
        return self._acknowledgement is not None
