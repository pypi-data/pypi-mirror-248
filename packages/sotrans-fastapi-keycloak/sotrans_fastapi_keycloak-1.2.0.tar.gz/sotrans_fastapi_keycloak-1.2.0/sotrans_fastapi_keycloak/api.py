...
from sotrans_fastapi_keycloak._roles import _get_highest_role
...
from sotrans_fastapi_keycloak.model import (
    ...
    KeycloakTokenModel,
    KeycloakUserModel,
    OIDCUserModel, KeycloakRoleInfoModel,
)
...
class FastAPIKeycloak:
    ...
    def get_user_roles_with_hierarchy(self, user_id: str) -> KeycloakRoleInfoModel:
        roles = self.get_user_roles(user_id=user_id)
        if len(roles) == 0:
            return KeycloakRoleInfoModel(roles=[], max_role=None)
        return KeycloakRoleInfoModel(
            roles=roles,
            max_role=_get_highest_role(user_roles=[
                role.name for role in roles
            ], role_hierarchy=self.roles),
        )
