...
speciesDict_v = {11:'hg19',12:'hg38',21:'mm09',22:'mm10'}
...
class Annotate(object):
    ...
    def help(self):
        ...
        print('fromOpen2EpiScanpy(data_path, head_path) : generate anndata from annotation result')
    ...
    def viewParams(self,task_id):
        ...
        protocol = int(task_info[0])
        species = task_info[1]
        cell_type = task_info[2]
        perbase = task_info[3]
        print('Protocol: ' + protocolDict_w[int(protocol)])
        print('Species: ' + speciesDict_v[int(species)])
        if int(species) == 12:
            species = 1
        if int(species) == 22:
            species = 3
        ...
        if int(species) == 21:
            species = 3
        if int(species) == 21 and protocol == 3:
            print('The corresponding cell type was not found. Please reselect the parameters.')
            return
    ...
    def getProgress(self,task_id = -1):
        ...
        if result.status_code == 404:
            ...
            print('You can download result file through data = object.getAnnoResult(result_type, %s , cell_type)'%(task_id))


    
    def getAnnoResult(self,result_type,task_id,cell_type=-1):
        if cell_type==-1:
