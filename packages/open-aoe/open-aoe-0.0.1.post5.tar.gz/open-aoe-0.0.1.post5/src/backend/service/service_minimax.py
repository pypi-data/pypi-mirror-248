from src.backend.model.dto.MinimaxDto import MinimaxChatCompletionReqDto
from src.backend.config.biz_config import mini_max_cfg
...
from src.backend.config.constant import API_MINIMAX
...
def get_req_param(request, req_dto):
    groupid_key = get_request_api_key(request, VENDOR_MINIMAX)
    group_id = groupid_key.split("+")[0]
    jwt = groupid_key.split("+")[1]
    api_url = mini_max_cfg.get("api_url")
    headers = {
        "Authorization": jwt,
    ...
    async def event_generator():
        ...
        while True:
            ...
            try:
                ...
                logger.error(f"{e}")
                stop_flag = True
                record_unhealthy = True
                yield str(e)
            if stop_flag:
                break
        if record_unhealthy:
            set_api_unhealthy(API_MINIMAX)
