...
from src.backend.model.dto.ClaudeDto import ClaudeChatReqDto, ClaudeMessage
from src.backend.model.dto.ClaudeSlackDto import ClaudeSlackChatReqDto
from src.backend.model.dto.ReturnBase import ReturnBase
from src.backend.config.biz_config import claude_slack_cfg, claude_cfg
from src.backend.util.choose_one import choose_one
from anthropic import Anthropic, HUMAN_PROMPT, AI_PROMPT
from src.backend.config.constant import TYPE_BOT, TYPE_USER, TYPE_SYSTEM
from src.backend.util.request_record_util import set_api_unhealthy
from src.backend.config.constant import API_CLAUDE, VENDOR_CLAUDE
...
def claude_chat_by_slack_svc(req_dto: ClaudeSlackChatReqDto):
    if not req_dto.token or not req_dto.appid:
        slack_infos = choose_one(claude_slack_cfg.get("slack_infos"))
        slack_user_token = slack_infos.split("+")[0]
        bot_user_id = slack_infos.split("+")[1]
    else:
        slack_user_token = req_dto.token
        bot_user_id = req_dto.appid
...
def claude_chat_anthropic(request: Request, req_dto: ClaudeChatReqDto):
    api_key = get_request_api_key(request, VENDOR_CLAUDE)
...
def claude_chat_stream_svc(request, req_dto: ClaudeChatReqDto):
    ...
    async def stream():
        try:
            ...
            # record api unhealthy
            set_api_unhealthy(API_CLAUDE)
