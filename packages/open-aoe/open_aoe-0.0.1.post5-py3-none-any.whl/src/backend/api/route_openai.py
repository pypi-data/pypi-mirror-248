...
from src.backend.service.service_openai import chat_completion, completions, chat_completion_stream, chat_completion_v2, \
    openai_check, proxy_files, proxy_assistants, proxy_threads, proxy_messages, proxy_runs
from src.backend.model.dto.OpenaiDto import OpenaiChatCompletionReqDto, OpenaiCompletionReqDto, OpenaiChatStreamReqDto, \
...
async def openai_chat(request: Request,
                      ...
                      token: str = Header(alias="alles-apin-token")):
    """
...
async def openai_chat_v2(request: Request, body: OpenaiChatCompletionV2ReqDto = Body(
    openapi_examples=openai_chat_completion_v2_examples()), token: str = Header(alias="alles-apin-token")):
    """
    OpenAI ChatCompletion
    """
    # ts = get_current_ts_ms()
...
@router.post("/v2/check", include_in_schema=False, tags=["OpenAI"])
async def openai_health_check(body: OpenaiCheckReqDto, token: str = Header(alias="alles-apin-token")):
    """
    OpenAI completion
    """
    ret = openai_check(body)
    return ret
