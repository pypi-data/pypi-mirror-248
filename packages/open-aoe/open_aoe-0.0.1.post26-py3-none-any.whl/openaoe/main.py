...
app_fe.mount("/static", StaticFiles(directory=STATIC_RESOURCE_DIR), name="static")


@app_fe.get("/", response_class=HTMLResponse)
@app_fe.get("/home", response_class=HTMLResponse)
async def server():
    return FileResponse(f"{STATIC_RESOURCE_DIR}/index.html")


@app_fe.get("/assets/css/{path:path}")
async def build_resource(path: str):
    build_file = safe_join(CSS_PATH_LIB, path)
    return FileResponse(build_file)


@app_fe.get("/{path:path}")
async def build_resource(path: str):
