...
to their literal name, and one function to set text display attributes:
...
Enum class Color defined constants of some common-used colors and grayscales,
...
class TextAttr(enum.IntEnum):
    """Enumerates attributes of text characters.

    Most of terminal support the first 8 attributes, but the last one
    CROSSED_OUT is rarely supported.
    """
    NORMAL = 0
    BOLD = 1
    FAINT = 2
...
def textattr_seq(attr: TextAttr):
    """Returns a sequence string that can set attributes of text.
    """
    return f'{_CSI_LEAD}{attr}m'


def set_textattr(attr: TextAttr):
    """Sets following text attribute to 'attr'.
    """
    print(textattr_seq(attr), end='')
