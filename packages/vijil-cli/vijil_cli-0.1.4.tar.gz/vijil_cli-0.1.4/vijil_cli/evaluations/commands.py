...
@click.command()
@click.option('--model-type', prompt='Enter the model type', type=str)
@click.option('--model-name', prompt='Enter the model name', type=str)
@click.option('--probes', prompt='Enter the probe group', type=str)
@click.option('--generations', prompt='Enter the number of generations', type=click.IntRange(1, 100))
def run(model_type, model_name, probes, generations):
    """Run the evaluation."""
