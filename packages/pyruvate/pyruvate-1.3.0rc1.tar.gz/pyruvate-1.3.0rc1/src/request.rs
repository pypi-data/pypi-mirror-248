...
impl WSGIRequest {
    ...
    pub fn wsgi_environ(&self, globals: SharedWSGIOptions, py: Python) -> PyResult<PyObject> {
        let io = &globals.io_module;
        let environ = globals.wsgi_environ.extract::<&PyDict>(py)?.copy()?;
        for (k, v) in self.http_headers.iter() {
            environ.set_item(k, v)?;
        }
        environ.set_item(&globals.peer_addr_key, &self.peer_addr)?;
        let input = io.getattr(py, "BytesIO")?.call(py, (), None)?;
        if self.content_length > 0 {
            environ.set_item(&globals.content_length_key, self.content_length)?;
            input.call_method(
                py,
                "write",
                PyTuple::new(
            ...
            input.call_method(py, "seek", PyTuple::new(py, [(0_i32)]), None)?;
        }
        environ.set_item(&globals.wsgi_input_key, input)?;
        Ok(environ.into_py(py))
    }
...
mod tests {
    ...
    fn test_environ_dict() {
        Python::with_gil(|py| {
            ...
            req.append(raw);
            req.parse_data();
            let binding = req.wsgi_environ(g, py).unwrap();
            let got: &PyDict = binding.extract(py).unwrap();
    ...
    fn test_post_simple_form() {
        Python::with_gil(|py| {
            ...
            let input = got.get_item("wsgi.input").unwrap().unwrap();
            debug!("{:?}", input);
            let input = input.call_method("read", (), None).unwrap();
            debug!("{:?}", input);
            assert!(
                input.extract::<&PyBytes>().unwrap().as_bytes() == b"field1=value1&field2=value2"
            );
        });
    }
    ...
    fn test_post_multipart_formdata() {
        Python::with_gil(|py| {
            ...
            let got = input.extract::<&PyBytes>().unwrap();
