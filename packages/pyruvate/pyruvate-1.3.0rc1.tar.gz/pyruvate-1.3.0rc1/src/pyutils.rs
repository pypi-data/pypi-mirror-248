...
use log::{self, error, set_boxed_logger, set_max_level, Level, LevelFilter, Log, Record};
use pyo3::exceptions::{PyTypeError as TypeError, PyValueError as ValueError};
pub use pyo3::ffi::PyThreadState;
use pyo3::ffi::{PyEval_RestoreThread, PyEval_SaveThread, PyGILState_Ensure, PyGILState_Release};
use pyo3::types::PyDict;
use pyo3::{PyErr, PyObject, PyResult, Python};
use std::cell::RefCell;
use std::thread::spawn;

pub fn with_python_thread<F, R>(mut code: F) -> R
where
    F: FnMut(Python) -> R,
{
...
fn set_python_threadinfo(py: Python, thread_name: &str) {
    if let Ok(threading) = py.import("threading") {
        ...
        if locals.set_item("threading", threading).is_ok() {
            ...
            if let Err(e) = py.run(&pycode, None, Some(locals)) {
                error!("Could not set thread info: {:?}", e);
            }
        }
    };
}
...
fn setup_python_logger(
    ...
    let crit = pylogging.getattr("CRITICAL")?.extract()?;
    ...
    match level.extract::<u8>(py) {
        ...
        Err(_) => Err(PyErr::new::<TypeError, _>(format!(
            "Expected u8, got {level:?}"
        ))),
    }
}
...
impl AsyncPythonLogger {
    ...
    fn create_logging_thread(pylog: PyObject) -> PyResult<Sender<LogRecordData>> {
                ...
                py.allow_threads(|| {
                    while let Ok(record) = rx.recv() {
                        ...
                        Python::with_gil(|py| {
                            ...
                            if pylog
                                .call_method(py, "log", (record.0 .0, &record.0 .1), None)
                                .is_err()
                            {};
                        });
                    }
    ...
    ($L: ident, $py: ident, $name: ident) => {
        match $L::new($py, $name) {
            Ok(logging) => match set_boxed_logger(Box::new(logging)) {
                Ok(_) => Ok(()),
                Err(_) => Err(PyErr::new::<ValueError, _>(format!(
...
mod tests {
    ...
    use pyo3::ffi::{PyEval_RestoreThread, PyEval_SaveThread, PyThreadState_Get};
    use pyo3::types::PyDict;
    ...
    use crate::pyutils::{
        clear_pyerr, init_python_threadinfo, with_python_thread, AsyncPythonLogger,
        SyncPythonLogger,
    };
