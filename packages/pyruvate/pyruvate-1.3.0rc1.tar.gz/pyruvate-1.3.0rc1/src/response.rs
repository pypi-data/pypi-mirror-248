use log::{debug, error};
use pyo3::exceptions::{PyTypeError as TypeError, PyValueError as ValueError};
use pyo3::ffi::PyObject_GetIter;
use pyo3::types::{PyBytes, PyDict, PyIterator, PyString};
use pyo3::{IntoPy, Py, PyErr, PyObject, PyRef, PyResult, Python};
...
fn check_content_length_exceeds_data(start_response: &StartResponse, py: Python) -> bool {
    if let Some(cl) = start_response.content_length(py) {
        if start_response.content_bytes_written(py) < cl {
            // check for HEAD request
            match start_response.environ.borrow().downcast::<PyDict>(py) {
                Ok(environ) => {
                    if let Ok(Some(method)) = environ.get_item(REQUEST_METHOD) {
                        match method.downcast::<PyString>() {
                            // HTTP method names are always upper case, see
                            // https://datatracker.ietf.org/doc/html/rfc7231#section-4.1
                            Ok(methodstr) if methodstr.to_string_lossy() == "HEAD" => return false,
                            _ => (),
                        }
                    }
                }
                Err(e) => {
                    error!("Could not extract environ dict: {e}");
                }
            }
...
impl<C: Connection> WSGIResponse<C> {
    ...
    pub fn set_pyobject(&mut self, pyobject: PyObject, py: Python) {
        self.iterable = match wsgi_iterable(&pyobject, py) {
            Ok(pyiter) => Some(pyiter.into_py(py)),
            Err(e) => {
                debug!("Could not create iterator: {:?}", e);
                None
            }
        };
        if let Ok(fw) = pyobject.extract::<PyRef<'_, FileWrapper>>(py) {
            if fw.has_file() {
    ...
    pub fn render_next_chunk(&mut self, py: Python) -> PyResult<()> {
        if let Some(pyob) = self.start_response.as_mut() {
            let start_response: PyRef<'_, StartResponse> = pyob.extract(py)?;
            debug!(
                "render_next_chunk start_response refcount: {:?}",
                pyob.get_refcnt(py)
            );
            let close_conn = self.connection.expired();
            match self.iterable.as_mut() {
                Some(pyobject) => {
                    match pyobject.downcast::<PyIterator>(py) {
                        Ok(mut iter) => match iter.next() {
                            None => {
                                ...
                                self.iterable = None;
                                self.pyobject = None;
                                if check_content_length_exceeds_data(&start_response, py) {
                                    // From the PEP:
                            ...
                            Some(Ok(any)) => match any.downcast::<PyBytes>() {
                                Ok(cont) => {
                                    start_response.write(
                                        cont.as_bytes(),
                                        &mut self.current_chunk,
                                ...
                                Err(_) => {
                                    return Err(TypeError::new_err(format!(
                        ...
                        Err(_) => {
                            return Err(TypeError::new_err(format!(
                                "Could not create iterator from {pyobject:?}"
                            )))
                        }
                    }
                }
            ...
            Err(PyErr::new::<ValueError, _>("StartResponse not set"))
        }
    }
    ...
    fn write_chunk(&mut self, py: Python) -> io::Result<bool> {
        ...
        if self.sendfileinfo {
            if let Some(ob) = self.pyobject.as_mut() {
                self.last_chunk_or_file_sent = match ob.extract::<PyRef<'_, FileWrapper>>(py) {
                    Ok(fw) => {
                    ...
                    Err(e) => {
                        ...
                        debug!("Could not extract FileWrapper: {e}");
                        true
                    }
                }
            }
    ...
    if req.is_bad_request() {
        ...
        match req.wsgi_environ(globals, py) {
            Ok(envpy) => {
                ...
                resp.start_response = match StartResponse::new(envpy.clone_ref(py), Vec::new()) {
                    Ok(sr) => {
                        match Py::new(py, sr) {
                            Ok(pysr) => {
                                let srobj: PyObject = pysr.into_py(py);
                                debug!(
                                ...
                                let result =
                                    application.call(py, (&envpy, srobj.clone_ref(py)), None); // call the object
                                debug!(
                                    "Refcounts after call; application: {:?} start_response: {:?}",
                                    application.get_refcnt(py),
                                    srobj.get_refcnt(py),
                                );
                                match result {
                            ...
                            Err(e) => {
                                error!("Could not wrap start_response: {e:?}");
                                resp.set_error_500();
                                None
                            }
                        }
...
mod tests {
    ...
    use pyo3::exceptions::PyTypeError as TypeError;
    use pyo3::types::{PyDict, PyTuple, PyType};
    use pyo3::{IntoPy, Py, PyAny, PyObject, PyRefMut, Python};
    ...
    fn handle_test_request(
        ...
        handle_request(&app, g, &req, &mut resp, py);
        resp
    }
    ...
    fn test_create() {
        Python::with_gil(|py| {
            ...
            match py.run(
                ...
                Ok(_) => {
                    let pylist = locals.get_item("li").unwrap().unwrap();
                    ...
                    let mut resp = WSGIResponse::new(dummy_connection(), false);
                    resp.set_pyobject(pylist.into(), py);
                    resp.start_response = Some(sr.into_py(py));
    ...
    fn test_iterator() {
        ...
        Python::with_gil(|py| {
            ...
            match py.run(
                ...
                Ok(_) => {
                    let pyit = locals.get_item("it").unwrap().unwrap();
                    let environ = PyDict::new(py);
                    let headers = vec![(
                        "200 OK".to_string(),
                        vec![("Content-type".to_string(), "text/plain".to_string())],
                    ...
                    let sr = StartResponse::new(environ.into(), headers).unwrap();
                    let mut resp = WSGIResponse::new(dummy_connection(), false);
                    resp.set_pyobject(pyit.into(), py);
                    resp.start_response = Some(sr.into_py(py));
    ...
    fn test_set_pyobject() {
        Python::with_gil(|py| {
            ...
            match py.run(r#"li = [b"Hello", b"world", b"!"]"#, None, Some(&locals)) {
                Ok(_) => {
                    let pylist: Py<PyAny> = locals.get_item("li").unwrap().unwrap().into();
                    let mut resp = WSGIResponse::new(dummy_connection(), false);
    ...
    fn test_write_chunk_sendfile() {
        Python::with_gil(|py| {
            ...
            let connection = TcpStream::connect(addr).expect("Failed to connect");
            let mut r = WSGIResponse::new(dummy_persistent_connection(connection), false);
            r.set_pyobject(fw.into_py(py), py);
            r.current_chunk = b"Foo 42".to_vec();
            r.last_chunk_or_file_sent = true;
            r.sendfileinfo = true;
...
app = simple_app"#,
            ...
            match app {
                Ok(_) => {
                    ...
                    let app = locals.get_item("app").unwrap().unwrap();
                    ...
                    req.append(raw);
                    req.parse_data();
                    let mut resp = handle_test_request(&app.into(), g, &req, py);
...
app = simple_app"#, None, Some(&locals));
            match app {
                Ok(_) => {
                    ...
                    assert!(resp.last_chunk_or_file_sent);
                    if let Some(pyob) = resp.start_response.as_mut() {
                        let start_response: PyRefMut<'_, StartResponse> = pyob.extract(py).unwrap();
                        assert!(start_response.content_complete());
                    } else {
                        assert!(false);
                    }
                }
...
app = simple_app"#,
            ...
            match app {
                Ok(_) => {
                    ...
                    assert!(resp.last_chunk_or_file_sent);
                    if let Some(pyob) = resp.start_response.as_mut() {
                        if let Ok(start_response) = pyob.extract::<PyRefMut<'_, StartResponse>>(py)
                        {
                            assert!(!start_response.content_complete());
                        } else {
                            assert!(false);
...
app = simple_app"#,
            ...
            match app {
                Ok(_) => {
                    ...
                    let resp = handle_test_request(&app.into(), g, &req, py);
    ...
    fn test_render_next_chunk_no_iterator() {
        Python::with_gil(|py| {
            ...
            match resp.render_next_chunk(py) {
                Err(e) => assert!(e.get_type(py).is(PyType::new::<TypeError>(py))),
                _ => assert!(false),
            }
        });
    }
...
f = FL()"#,
                ...
                Ok(_) => {
                    ...
                    let fwany = fwtype.call(PyTuple::new(py, &[filelike]), None).unwrap();
                    let environ = PyDict::new(py);
                    let headers = vec![(
                        "200 OK".to_string(),
                        vec![("Content-type".to_string(), "text/plain".to_string())],
                    ...
                    let mut resp = WSGIResponse::new(dummy_connection(), false);
                    resp.start_response = Some(sr.into_py(py));
                    resp.set_pyobject(fwany.into(), py);
                    match resp.render_next_chunk(py) {
                        Ok(_) => {
                            let expected = b"HTTP/1.1 200 OK\r\nContent-type: text/plain\r\nVia: pyruvate\r\nConnection: keep-alive\r\n\r\nHello world!\n";
...
f = FL()"#,
    ...
    fn test_content_length_gt_data_len_sendfile() {
        Python::with_gil(|py| {
            ...
            let sr = StartResponse::new(environ.into(), headers).unwrap();
            let mut r = WSGIResponse::new(dummy_persistent_connection(connection), false);
            r.start_response = Some(sr.into_py(py));
            r.set_pyobject(fw.into_py(py), py);
