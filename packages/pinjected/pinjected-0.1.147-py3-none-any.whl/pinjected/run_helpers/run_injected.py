...
def load_user_default_design():
    ...
    :return:
    """
    dot_pinjected = Path(".pinjected.py")
    if dot_pinjected.exists():
        logger.debug(f"loading default design from {dot_pinjected}")
        design_path = str(Path(".pinjected.py").absolute()) + ":default_design|"
    else:
        logger.debug(f"{dot_pinjected.absolute()} is not found.")
        design_path = ""
