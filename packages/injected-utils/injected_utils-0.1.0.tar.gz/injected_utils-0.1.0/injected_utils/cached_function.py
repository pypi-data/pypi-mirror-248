...
class KeyEncoder(IKeyEncoder):
    ...
    def get_key(self, *args, **kwargs):
        # logger.warning(f"calculating key for {args},{kwargs}")
        bound = self.signature.bind(*args, **kwargs)
        bound.apply_defaults()

        key = self._encode_key(frozendict(bound.arguments))
        # logger.warning(f"calculated key for {args},{kwargs} => {encoded}")
        # logger.warning(f"key_encoder:{self}")
        assert key is not None, f'key is None. {args},{kwargs}'
        # logger.debug(f"calculated key for {args},{kwargs} => {key}")
        return key
