...
def validateValueTendency(registerAmount,object:DataFrame,column,referenceColumn,method,entity,threshold):
    ...
    if referenceColumn is not None :
        data = object.groupBy(referenceColumn).agg(mean(column).alias("mean"),stddev(column).alias("stddev")).collect()
        result_list = []
        errorDf = object
        for row in data:
            if method == "chebyshev":
                k = Two
                reference_column_value = row[referenceColumn]
            ...
            result_list.append([reference_column_value, l_bound, u_bound])
            filteredDf = object.filter((col(column)>l_bound) & (col(column)<u_bound))
            errorDf = errorDf.exceptAll(filteredDf)
        errorDf = errorDf.withColumn("error", concat(lit(errorDesc),referenceColumn))
    else:
