...
def load(name, format, driver=None, query=None, options=None):
    ...
    heimdallClient = spark._jvm.HeimdallClient.Builder().url(get_env_variable(constants.HEIMDALL_BASE_URL)).apikey(g_dataos_token).build()
    data_govern_jvm = spark._jvm.DataGovernor.getInstance(heimdallClient)
    # user = generic_utils.authorize_user(spark, heimdallClient, g_dataos_token)
