...
class RecipeInstance(Resource):
    ...
    def put(self, new_recipe: Recipe, user_info: User, recipe_id=''):
        old_recipe = Recipe.objects(Q(_id=recipe_id) & Q(
            owner=str(user_info.id))).get_or_404()
...
class RecipeIngredientInstance(Resource):
    ...
    def get(self, user_info: User, recipe_id: str, ingredient_id: str):
        ...
        recipe = Recipe.objects(Q(_id=recipe_id) & Q(owner=str(user_info.id)) & Q(
            ingredients__ingredient=ingredient_id)).get_or_404()

        for ing_doc in recipe.ingredients:
            if str(ing_doc.ingredient.id) == ingredient_id:
                return ing_doc, 200

        raise NotFound(description='can\'t find ingredient with id {} in recipe {}'.format(
    ...
    def delete(self, user_info: User, recipe_id='', ingredient_id=''):
        ...
        recipe.ingredients = [
            recipe_ingredient for recipe_ingredient in recipe.ingredients if recipe_ingredient.ingredient.id != ingredient_id]
    ...
    def put(self, user_info: User, recipe_id: str, ingredient_id: str, recipe_ingredient: RecipeIngredient):
        ...
        Recipe.objects(Q(_id=recipe_id) & Q(owner=str(user_info.id)) & Q(
            ingredients__ingredient=ingredient_id)).update(set__ingredients__S=recipe_ingredient)
