...
class DkLangEtcUtils(object):
    ...
    @classmethod
    def f_remove_comments_v1(cls, py: str, *, _removeAstSentence=False, _removeATVast=False, _removeSingleQuoteMuilti=True) -> str:
        '''
        行尾的注释需要保留

        缺省情况下 单引号多行文本会被删除 
        _removeSingleQuoteMuilti参数可以切换是单引号还是双引号
    ...
    @classmethod
    def find_words(cls, texts: List[str], keyFun, *, minLen=6) -> Set[str]:
    ...
    @classmethod
    def f_replace_text_simplea(cls, filepath: str, fcb):
    ...
    @classmethod
    def handle_files_100(cls, fs, keyFun):
        '''
        即使在字符串常量中 也会替换的
        '''
...
def _f_main():
    ...
    rs = DkLangEtcUtils.find_words([ta, ta], '2', minLen=2)
    iprint(rs)
    ...
    ta2 = DkLangEtcUtils.sub_word([ta], fromWord='里', toWord='东西2')
    print(ta2)
