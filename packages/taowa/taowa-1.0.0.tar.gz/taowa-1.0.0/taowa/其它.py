...
def 皮肤_加载(皮肤文件名: str='guohe',密码: str='',色调:int =1,饱和度:int =1,亮度:int =1):
    ...
    skinh_dll = ctypes.CDLL(路径 + '\\TaoWa\\Skin\\Skin.dll')
    skinh_dll.SkinH_AttachEx.argtypes = [ctypes.c_char_p, ctypes.c_char_p, ctypes.c_int, ctypes.c_int, ctypes.c_int]
    skinh_dll.SkinH_AttachEx.restype = ctypes.c_int
    皮肤路径 = 路径 + '\\TaoWa\\Skin\\'+ 皮肤文件名
    skinh_dll.SkinH_AttachEx(皮肤路径.encode('utf-8'), 密码.encode('utf8'),色调,饱和度,亮度)



def 皮肤_加载_从字节集(皮肤: bytes=b'',密码: str='',色调:int =1,饱和度:int =1,亮度:int =1):
    '''
    :param 皮肤: 二进制数据
    :param 密码:
    :param 色调: 1-360
    ...
    路径 = sys._MEIPASS if getattr(sys, "frozen", False) else os.path.abspath(__file__).replace('\\TaoWa\\其它.py', '')
    skinh_dll = ctypes.CDLL(路径 + '\\TaoWa\\Skin\\Skin.dll')
    skinh_dll.SkinH_AttachRes.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_char_p, ctypes.c_int, ctypes.c_int,ctypes.c_int]
    skinh_dll.SkinH_AttachRes.restype = ctypes.c_int
    if not 皮肤:
        with open(路径+'\\TaoWa\\Skin\\guohe', 'rb') as a:
            皮肤 = a.read()
    skinh_dll.SkinH_AttachRes(皮肤, len(皮肤), 密码.encode('utf8'),色调,饱和度,亮度)

def 皮肤_卸载():
    global skinh_dll
    skinh_dll.SkinH_Detach.restype = ctypes.c_int
    skinh_dll.SkinH_Detach()
