...
from bastion_integration.bastion_web_socket.bastion_web_socket_dto import DepartmentDto, PassDto, \
    BastionWebSocketConfig, BastionV3PersonDto, \
    PersonalInfo
from bastion_integration.bastion_web_socket.core import CoreWebSocket
from bastion_integration.bastion_web_socket.custom_error_handler import BastionIntegrationError
from bastion_integration.bastion_web_socket.response_dto import OrganizationResponseDto, PersonResponseDto
...
class CoreWebAsync:
    ...
    def __init__(self, config: BastionWebSocketConfig):
        self.config = config
        self.core = CoreWebSocket(self.config)
    ...
    async def _check_connection(self, only_ping: bool = True, **kwargs):
        await self.core._check_connection(only_ping=only_ping, kwargs=kwargs)
    ...
    async def create_pass(self, dto: PersonDto.Create):
        pass_info = await self.core.create_pass(dto)
        return pass_info
