...
from bastion_integration.base_info.bastion_dto import PersonDto
from bastion_integration.bastion_web_socket.bastion_web_socket_dto import HandShakeDto, PassDto, PersonalInfo, \
    BastionWebSocketConfig, BastionV3PersonDto
from bastion_integration.bastion_web_socket.command_info import WebSocketCommand
from bastion_integration.bastion_web_socket.custom_error_handler import BastionV3Error
from bastion_integration.bastion_web_socket.errors_type import get_bastion_error, BastionErrorTypes
...
class CoreWebSocket:
    ...
    def __init__(self, config: BastionWebSocketConfig):
        self.config = config
    ...
    async def _bastion_read_message_(self):
        async for message in self.bastion_socket:
            ...
            try:
                ...
                raise BastionV3Error(**get_bastion_error(BastionErrorTypes.read_message_error, add_message=e))
    ...
    async def _bastion_send_message(self, message: dict, custom_web_socket: websockets.WebSocketClientProtocol = None):
        ...
        try:
            ...
            raise BastionV3Error(**get_bastion_error(BastionErrorTypes.send_message_error, add_message=e))
        ...
        if self.send_messages:
            try:
                ...
                raise BastionV3Error(**get_bastion_error(BastionErrorTypes.integration_not_enabled_error))
                ...
                for response in self.response_info:
                    if replay := response.get("reply"):
                        if replay.get("exception"):
                            ...
                            raise BastionV3Error(**get_bastion_error(BastionErrorTypes.response_error, custom_message=
                            f"{error_in} - {replay.get('exception').get('message')} - "
                            f"{replay.get('exception').get('comments')}"))
    ...
    async def init(self):
        ...
        await self.handshake(dto=HandShakeDto(roleType=self.config.handshake.role_type,
                                              hostName=self.config.handshake.host_name,
                                              socketId=self.config.handshake.socket_id))
    ...
    async def handshake(self, dto: HandShakeDto):
    ...
    async def _check_connection(self, only_ping: bool = True, **kwargs):
        ...
        try:
            ...
            await self._bastion_send_message(self.command.net_center_status(), custom_web_socket=web_socket)
            await asyncio.sleep(self.sleep_time)
            ...
            if not check_host_and_port_answer.get("body").get("state") == "Normal":
                logger.critical(f"Check host and port. Connection error. Status: {check_host_and_port_answer.get('body').get('state')}")
                return False
            await self._bastion_send_message(self.command.login(login=login, password=password), custom_web_socket=web_socket)
            await asyncio.sleep(self.sleep_time)
            ...
            if not check_login_and_password_answer.get("body"):
                logger.critical(f"Check login and password. Connection error. Status: {check_login_and_password_answer.get('body')}")
                return False
            await self._bastion_send_message(self.command.handshake(dto=HandShakeDto(roleType=role_type, hostName=host_name, socketId=socket_id)), custom_web_socket=web_socket)
            await asyncio.sleep(self.sleep_time)
            check_handshake_answer = await self._get_response_info(RequestName.HANDSHAKE)
        except Exception as e:
            logger.warning(e)
            return False
    ...
    async def get_organization(self, organization_name: str = None):
        ...
        for organization in organizations:
            ...
            items.append({"organization_name": organization.get("organization_name"), "departments": departments_list})
    ...
    async def _check_department(self, name) -> bool:
        ...
        if type(departments) == list:
            for dep in departments:
                # departments_name.append(dep.id)
                # if name in departments_name:
                if name == dep.name:
                    raise BastionV3Error(**get_bastion_error(BastionErrorTypes.department_exist_error))
        else:
            if name == departments.name:
                raise BastionV3Error(**get_bastion_error(BastionErrorTypes.department_exist_error))
        # if department_id:
        #     if department_id not in _ids:
        #         raise BastionV3Error(**get_bastion_error(BastionErrorTypes.department_not_found_error))
    ...
    async def update_organization(self, old_name: str, new_name: str) -> OrganizationResponseDto:
        ...
        if old_name not in organizations_names:
            raise BastionV3Error("Организация с таким именем не существует")
    ...
    async def create_department(self, department_name: str, organization_name: str) -> DepartmentResponseDto:
        ...
        if depart := departments.get(department_name):
            if depart[1] == _id:
                raise BastionV3Error("Департамент с таким именем уже существует")
    ...
    async def update_department(self, old_name: str, new_name: str, organization_name: str,
        ...
        for dep in depart:
            ...
            if dep.get("name") == old_name:
                dep_id = dep.get("id")
        if not dep_id:
            raise BastionV3Error("Департамент с таким именем не существует")
    ...
    async def create_person(self, dto: PersonDto.Create) -> PersonResponseDto:
        ...
        if not organization:
            raise Exception("Организация не найдена")
        ...
        if not department:
            raise Exception("Отдел не найден")
        ...
        _dto = BastionV3PersonDto.Create(name=dto.last_name,
                                               ...
                                               "$values": [
                                                   {
                                                       "$type": "ESprom.Taurus.Roles.NetCenter.Add`1[["
                                                                ...
                                                                "ESprom.Taurus.NetCenter.Common.Persons]], "
                                                                "ESprom.Taurus.NetCenter.Common",
                                                       "target": _dto.model_dump(exclude_none=True)
                                                   }
                                               ]
                                           },
                                           "id": 0
    ...
    async def update_person(self, dto: BastionV3PersonDto.Update):
                                                   ...
                                                   "$values": [
                                                       {
                                                           "$type": "ESprom.Taurus.Roles.NetCenter.Update`1[["
                                                                    ...
                                                                    "ESprom.Taurus.NetCenter.Common.Persons]], "
                                                                    "ESprom.Taurus.NetCenter.Common",
                                                           "target": dto.model_dump(exclude_none=True)
                                                       }
                                                   ]
                                               },
                                               "id": 0
    ...
    async def find_person(self, dto: BastionV3PersonDto.Get):
        request = {"$type": "ESprom.Taurus.Roles.NetCenter.Persons.FindPersonRequest, "
                            "ESprom.Taurus.NetCenter.Common.Persons",
                   "id": 0}
        request.update(dto.model_dump(exclude_none=True))
        await self._send_message(data={"request_name": RequestName.FIND_PERSON.name,
                                       "$type": REQUEST_COMMAND,
                                       "request": request})
    ...
    async def create_pass(self, _dto: PersonDto.Create):
        ...
        find_person_dto = BastionV3PersonDto.Get(name=_dto.last_name, firstName=_dto.first_name,
                                                 secondName=_dto.middle_name)

        if not (person := await self.find_person(find_person_dto)):
            raise BastionV3Error("Person not found")
        ...
        if dto.passCategoryId not in categories_ids:
            raise BastionV3Error(**get_bastion_error(BastionErrorTypes.pass_category_not_found_error))
        ...
        if dto.accessLevelId not in categories_ids:
            raise BastionV3Error(**get_bastion_error(BastionErrorTypes.access_level_not_found_error))
