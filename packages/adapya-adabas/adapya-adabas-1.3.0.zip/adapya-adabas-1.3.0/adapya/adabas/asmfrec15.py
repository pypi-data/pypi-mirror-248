#
# adasmf SMF record structures V1.5 for Adabas V8.3 with zIIP support
#
...
class Asbase(Datamap):
    ...
    Int2('rlen'), # Record length
    Uint2('seg'),          # Segment descriptor
    Bytes('flg',1),        # System indicator flags
    ...
    Bytes('rty',1),  # Adabas record type
    Uint4('tme',ppfunc=dtime100), # Time since midnight when record was
                                  # moved into smf buffer in 1/100 sec

    Packed('dte',4,ppfunc=idate), # Date when record was moved into smf
                                  # buffer as 0cyydddf

    String('sid',4), # System identifier (smfprmxx sid)
    String('ssi',4), # Subsystem identifier
    Uint2('sty',ppfunc=Asbase.assty_str), # Subtype
...
ASBASELN=0x18+0xa8 # Length of ASBASE

ASSMFVC = b'\x01\x03' # current version
...
class Aspid(Datamap):
    ...
    #q Assmfvc   equ   assmfv15             current version: 1.5
    ...
    #q AsSMFV15  equ     x'0105'            version 1.5 - Adabas V8.3 zIIP
...
class Asparm(Datamap):
  def __init__(self, **kw):
    ...
    # additional parameters added for adabas v8.3 (asmfrec v1.3)

    Uint1('aswt1',caption='ASSOSpacewarn Threshold 1',pos=0x0432), #v83
    ...
    Uint1('dswt1',caption='DATASpacwarn Threshold 1',pos=0x043c), #v83
    ...
    byteOrder=NETWORKBO,ebcdic=1,**kw)
ASPARML=0X476 # Length of parameter area
...
#
# ARM file exclusion table
#
class Aspcft(Datamap):
  def __init__(self, **kw):
        Datamap.__init__(self, 'ASPCFT',
    ...
    byteOrder=NETWORKBO,ebcdic=1,**kw)
#
# Cache RABN Entry Table
#
class Aspcrt(Datamap):
    def __init__(self, **kw):
        Datamap.__init__(self, 'ASPCRT',
...
#                                      and adabas AZP835 (asmfrec v1.5)
...
class Asunknown(Datamap):
    def __init__(self, **kw):
        Datamap.__init__(self, 'Unknown Structure',
    byteOrder=NETWORKBO,ebcdic=1,**kw)
