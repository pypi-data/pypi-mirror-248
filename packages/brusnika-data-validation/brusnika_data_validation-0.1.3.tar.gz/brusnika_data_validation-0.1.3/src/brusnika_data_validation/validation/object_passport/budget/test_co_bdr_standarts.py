...
from .... utils.universal import get_abs_diff, get_relative_diff, set_diff, compare_diff_with_threshold
...
def _get_add_dataset_area_statistics(
    ...
    contours_df['region_code'] = contours_df.countour_code.apply(
...
class Schema(pa.DataFrameModel):
    # Здесь поля называются так же, как в параметрической системе и в основной схеме по стакану
    # Но единица измерения не руб, а руб / м2
    ...
    fa_costs_land: Series[float] = pa.Field(nullable=True)
    fa_costs_facilities: Series[float] = pa.Field(nullable=True) # Расходы на инфраструктуру >=0
    fa_costs_warranty: Series[float] = pa.Field(nullable=True)
    # Расходы на ввод и содержание (пока отсутствуют в парам. системе le=0)
    fa_costs: Series[float] = pa.Field(nullable=True)

    fa_gross_margin: Series[float] = pa.Field(nullable=True)

    fa_sga: Series[float] = pa.Field(nullable=True)
    fa_sga_developer_management: Series[float] = pa.Field(nullable=True)
    fa_sga_design_management: Series[float] = pa.Field(nullable=True)
    fa_sga_marketing: Series[float] = pa.Field(nullable=True)
    fa_sga_sales_management: Series[float] = pa.Field(nullable=True)
    fa_sga_brokerage: Series[float] = pa.Field(nullable=True)
    fa_sga_hci: Series[float] = pa.Field(nullable=True)

    fa_internal_expenses: Series[float] = pa.Field(nullable=True)

    fa_ebitda: Series[float] = pa.Field(nullable=True)

    #fa_interest 	# % банку (отсутсвует в сете)
    fa_expenses: Series[float] = pa.Field(nullable=True)

    fa_taxes_income_tax: Series[float] = pa.Field(nullable=True)
    fa_net_profit: Series[float] = pa.Field(nullable=True)
