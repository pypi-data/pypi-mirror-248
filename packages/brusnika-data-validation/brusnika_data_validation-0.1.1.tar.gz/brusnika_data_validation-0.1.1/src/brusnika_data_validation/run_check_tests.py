...
BUDGET_2024_VERSION = 'f42eb656-5747-49d0-8e4b-c26ff1c467fd'
...
def pipeline(
    ...
    object_names = get_param_system_object_names(mapping_dict['object_types'], tokens['parameters'], WORKING_VERSION)
    ...
    for test_method in CALL_LIST:
        check_result = run_test(test_method, tokens, mapping_dict, object_names, WORKING_VERSION)
        if check_result is not None:
            save_method(check_result, **write_params['history'])
    
    for test_method in CALL_LIST:
        check_result = run_test(test_method, tokens, mapping_dict, object_names, BUDGET_2024_VERSION)
        if check_result is not None:
            save_method(check_result, **write_params['history'])
