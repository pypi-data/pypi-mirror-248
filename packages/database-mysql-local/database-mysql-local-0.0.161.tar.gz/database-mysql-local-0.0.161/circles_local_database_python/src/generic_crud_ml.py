...
class GenericCRUDML(GenericCRUD):
    ...
    def add_value(self, table_id: int = None, lang_code: LangCode = user_context.get_effective_profile_preferred_lang_code(),
                  data_json: dict = None, data_ml_json: dict = None,
                  table_name: str = None, ml_table_name: str = None,
                  view_name: str = None) -> tuple[int, int]:
        logger.start(object={"table_id": table_id, "lang_code": lang_code, "data_json": data_json,
                             "data_ml_json": data_ml_json, "table_name": table_name,
        ...
        data_ml_json["lang_code"] = str(lang_code)
    ...
    def get_values_tuple(self, table_id: int, lang_code: LangCode = user_context.get_effective_profile_preferred_lang_code(),
        ...
        result = self.select_one_tuple_by_where(where=f"{id_column_name}=%s AND lang_code=%s",
                                                params=(table_id, str(lang_code)))
        logger.end(object={"result": result})
        return result

    def get_values_dict(self, table_id: int, lang_code: LangCode = user_context.get_effective_profile_preferred_lang_code(),
    ...
    # TODO: add is_main column to test_mysql_ml_table and test_mysql_ml_view and test this method
        ...
        try:
            ...
            try:
                self.delete_by_where(table_name=TEST_ML_TABLE_NAME,
                                     where="`name`=%s AND lang_code=%s", params=(name, str(lang_code)))
            except Exception:
                logger.warn("delete_by_where was not successful for name and lang_code",
                            object={"name": name, "lang_code": lang_code})
                logger.end()
