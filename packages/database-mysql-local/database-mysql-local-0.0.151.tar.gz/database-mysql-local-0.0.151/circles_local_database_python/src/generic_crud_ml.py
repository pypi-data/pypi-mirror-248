...
class GenericCRUDML(GenericCRUD):
    ...
    def get_values_tuple(self, id: int,
                         lang_code: LangCode = user_context.get_effective_profile_preferred_lang_code()) -> tuple:
        logger.start(object={"id": id, "lang_code": lang_code})
        if lang_code is None:
            lang_code = 'en'

        id_column_name = re.sub(r'(_table)$', '_id', self.default_table_name)
        result = self.select_one_tuple_by_where(where=f"{id_column_name}=%s AND lang_code=%s",
                                                params=(id, lang_code))
        logger.end(object={"result": result})
        return result

    def get_values_dict(self, id: int, lang_code: LangCode = user_context.get_effective_profile_preferred_lang_code()) -> dict:
        logger.start(object={"id": id, "lang_code": lang_code})
        if lang_code is None:
            lang_code = 'en'
    ...
    def get_id_by_name(self, name: str, lang_code: LangCode = user_context.get_effective_profile_preferred_lang_code()) -> int:
    ...
    def get_ml_id_by_name(self, name: str,
                          lang_code: LangCode = user_context.get_effective_profile_preferred_lang_code()) -> int:
        logger.start(object={"name": name, "lang_code": lang_code})
        if lang_code is None:
            lang_code = 'en'

        ml_id_column_name = re.sub(r'(_table)$', '_id', self.default_ml_table_name)
        try:
