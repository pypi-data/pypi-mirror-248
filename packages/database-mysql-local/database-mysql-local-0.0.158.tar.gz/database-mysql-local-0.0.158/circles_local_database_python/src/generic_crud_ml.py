...
class GenericCRUDML(GenericCRUD):
    """A class that provides generic CRUD functionality for tables with
    ...
    def sql_in_list_by_entity_list_id(self, schema_name: str, entity_name: str,
        ...
        """Example: select group_id from group.group_list_member_table WHERE
        group_list_id=1"""
    ...
    def add_value(self, id: int = None,
        ...
        if id is None:
            ...
            if id is None:
                logger.error("Error inserting data_json",
                             object={"data_json": data_json})
                logger.end()
                return None
        ...
        ml_id = self.insert(data_json=data_ml_json, table_name=ml_table_name,
                            ignore_duplicate=True)

        logger.end(object={"id": id, "ml_id": ml_id})
        return id, ml_id

    def get_values_tuple(self, id: int,
                         lang_code: LangCode = user_context.get_effective_profile_preferred_lang_code(),  # noqa501
        ...
        try:
            id_column_name = id_column_name or re.sub(r'(_table)$', '_id',
                                                      self.default_table_name)
        except Exception:
            message = "id_column_name was not provided and could not be generated from default_table_name"  # noqa501
            logger.error(message)
            logger.end()
            raise Exception(message)
        result = self.select_one_tuple_by_where(where=f"{id_column_name}=%s AND lang_code=%s",  # noqa501
                                                params=(id, lang_code))
        logger.end(object={"result": result})
        return result

    def get_values_dict(self, id: int,
                        lang_code: LangCode = user_context.get_effective_profile_preferred_lang_code(),  # noqa501
        ...
        try:
            ...
            logger.end()
            raise Exception(message)
        result = self.select_one_dict_by_where(where=f"{id_column_name}=%s AND lang_code=%s",  # noqa501
                                               params=(id, lang_code))
        logger.end(object={"result": result})
        return result
    ...
    def get_id_by_name(self, name: str,
        ...
        try:
            ...
            raise Exception(message)
        try:
            result = self.select_one_tuple_by_where(select_clause_value=id_column_name,  # noqa501
                                                    where="title=%s AND lang_code=%s",  # noqa501
                                                    params=(name, lang_code))
        except Exception:
            logger.warn("select_one_tuple_by_where was not successful for name and lang_code",  # noqa501
                        object={"title": name, "lang_code": lang_code})
            try:
                result = self.select_one_tuple_by_where(select_clause_value=id_column_name,  # noqa501
                                                        where="`name`=%s AND lang_code=%s",  # noqa501
                                                        params=(name, lang_code))  # noqa501
            except Exception:
                logger.warn("select_one_tuple_by_where was not successful for name and lang_code",  # noqa501
    ...
    def get_ml_id_by_name(self, name: str,
        ...
        try:
            ml_id_column_name = id_column_name or re.sub(r'(_table)$', '_id',
                                                         self.default_ml_table_name)  # noqa501
        except Exception:
            message = "id_column_name was not provided and could not be generated from default_table_name"  # noqa501
            logger.error(message)
            logger.end()
            raise Exception(message)
        try:
            result = self.select_one_tuple_by_where(select_clause_value=ml_id_column_name,  # noqa501
                                                    where="title=%s AND lang_code=%s",  # noqa501
                                                    params=(name, lang_code))
        except Exception:
            logger.warn("select_one_tuple_by_where was not successful for name and lang_code",  # noqa501
    ...
    def delete_by_name(self, name: str,
                       lang_code: LangCode = user_context.get_effective_profile_preferred_lang_code()) -> None:  # noqa501
        logger.start(object={"name": name, "lang_code": lang_code})
        if lang_code is None:
            lang_code = 'en'
        ...
        try:
            ...
            logger.warn("delete_by_where was not successful for name and lang_code",  # noqa501
                        object={"title": name, "lang_code": lang_code})
            try:
                self.delete_by_where(table_name=TEST_ML_TABLE_NAME,
    ...
    # The follwing methods may be irrelevant since GenericCRUD has equivalent methods  # noqa501
    ...
    def select_one_by_value_with_none_option(
        ...
        select_query = f"SELECT {select_column_name} " f"FROM {self.schema_name}.{view_table_name} " + (  # noqa501
            f"WHERE `{condition_column_name}` = \"{condition_column_value}\" " if (condition_column_name and  # noqa501
                                                                                   condition_column_value)  # noqa501
    ...
    def select_multi_tuple_by_value_with_none_option(self,
        ...
        view_table_name = view_table_name or self.default_view_table_name
        # Returns the row id if select_column_name is None
        select_column_name = f"`{select_clause_value}`" if select_clause_value != "*" else "*"  # noqa501
        """Selects a column from the table based on a WHERE clause and returns
