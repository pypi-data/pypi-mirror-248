    ...
    def connect(schema_name: str):
        ...
        if schema_name in connections_pool:
            if connections_pool[schema_name].connection.is_connected():
                ...
                # reconnect
                connections_pool[schema_name]._connect_to_db()  # changed from reconnect  # noqa501
                if connections_pool[schema_name].connection.is_connected():
                    logger.end("Reconnected successfully", object={
                        'connections_pool': str(connections_pool[schema_name])})  # noqa501
                    return connections_pool[schema_name]
                else:
                    connector = Connector(schema_name)
                    connections_pool[schema_name] = connector
                    logger.error("Reconnect failed, returning a new connection",  # noqa501
                                 object={'connections_pool': str(connections_pool[schema_name])})  # noqa501
