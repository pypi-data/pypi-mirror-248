...
class Client:
    ...
    def request(self, print_response=True, **kwargs) -> requests.Response:
        headers = {
            'Content-Type': 'application/json',
            'Authorization': f'Bearer {self.api_key}'
        }

        API_PAYLOAD["method"] = kwargs.get('method')

        r = requests.post(self.url, headers=headers, json=API_PAYLOAD)
