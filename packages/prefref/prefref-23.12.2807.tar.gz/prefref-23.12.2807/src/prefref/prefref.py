...
class Config:
  ...
  def add_parser_arg(self, parser: argparse.ArgumentParser, option: Config_Option) -> argparse.ArgumentParser:
    if 'bool' == option.value_type:
      if option.arg_short_key:
        parser.add_argument(option.arg_short_key, option.arg_key if option.arg_key else f'--{option.name}', action='store_true', dest=option.name, 
                            default=option.value,
                            help=f'{option.help_text}{" (default: " + str(option.default_value) + ")" if option.default_value else ""}')
      else:
        parser.add_argument(option.arg_key if option.arg_key else f'--{option.name}', action='store_true', dest=option.name, 
                            default=option.value,
                            help=f'{option.help_text}{" (default: " + str(option.default_value) + ")" if option.default_value else ""}')
    
    elif 'int' == option.value_type:
      if option.arg_short_key:
        parser.add_argument(option.arg_short_key, option.arg_key if option.arg_key else f'--{option.name}', nargs='?', type=int, dest=option.name, 
                            default=option.value,
                            help=f'{option.help_text}{" (default: " + str(option.default_value) + ")" if option.default_value else ""}')
      else:
        parser.add_argument(option.arg_key if option.arg_key else f'--{option.name}', nargs='?', type=int, dest=option.name, 
                            default=option.value,
                            help=f'{option.help_text}{" (default: " + str(option.default_value) + ")" if option.default_value else ""}')
    else: # string
      if option.arg_short_key:
        parser.add_argument(option.arg_short_key, option.arg_key if option.arg_key else f'--{option.name}', nargs='?', type=str, dest=option.name, 
                            default=option.value,
                            help=f'{option.help_text}{" (default: " + str(option.default_value) + ")" if option.default_value else ""}')
      else:
        parser.add_argument(option.arg_key if option.arg_key else f'--{option.name}', nargs='?', type=str, dest=option.name, 
                            default=option.value,
                            help=f'{option.help_text}{" (default: " + str(option.default_value) + ")" if option.default_value else ""}')
  ...
  def decrypt(self, option_name: str) -> 'typing.any':
