...
from sqlton.ast import With, Select, SelectCore, Insert, Replace, Operation, operators, Table, Index, All, Column, Alias, Values, CommonTableExpression
...
class Parser(_Parser):
    ...
    def select_core(self, p):
        kwargs = dict((key.lower(), getattr(p, key))
                      for key in p._namemap.keys()
                      if key not in ('SELECT', 'FROM', 'DISTINCT', 'ALL'))
    ...
    def set_operator(self, p):
        return ' '.join(p[index]
                        for index
                        in range(len(p))).upper()
    
    @_('DISTINCT',
       'ALL',
       '')
    def reduction(self, p):
        return (self.p[0].upper() if len(p) else None)
    ...
    def join_operation(self, p):
        return ((p.join_operator, p.join_constraint), p.table)
    ...
    def expr(self, p):
        return Operation(p.unary_operator, 0, p[-1])
