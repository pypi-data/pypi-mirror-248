...
from lm_datahandler.functions.sleep_staging import sleep_staging_with_features, load_model
...
from lm_datahandler.plots.sleep_staging_plot import plot_spectrogram, plot_avg_diff_acc, plot_sleep_staging_result_5c, \
...
class DataHandler(object):
    ...
    def save_features_to_parquet(self, mat_path):
        if self.features is None:
            self.logger.info("Please do feature extraction first.")
        self.features.to_parquet(path=mat_path)
    ...
    def sleep_staging_with_input_model(self, model_path, staging_mode='offline'):
        if self.eeg is None:
    ...
    def compute_sleep_variables(self, hypno=None):
        ...
        if sl == -1:
            ...
            arousal_count, arousal_time = arousal_time_compute(hypno, self.class_count, self.epoch_len, sleep_range=[sl, gu])
    ...
    def export_analysis_result_to_xlsx(self, save_path, sw_results=True, sp_results=True, sleep_variables=True):
        ...
        if sleep_variables:
            sleep_variables_df = {
                ...
                "N3_SW_Count": [self.sleep_variables["n3_sw_count"]] if self.sleep_variables.__contains__("n3_sw_count") else None,
                "N12_SW_Count": [self.sleep_variables["n12_sw_count"]] if self.sleep_variables.__contains__("n12_sw_count") else None,
                "SP": [self.sp_df.shape[0]] if self.sp_df is not None else None,
                "N3_SP_Count": [self.sleep_variables["n3_sp_count"]] if self.sleep_variables.__contains__("n3_sp_count") else None,
                "N12_SP_Count": [self.sleep_variables["n12_sp_count"]] if self.sleep_variables.__contains__("n12_sp_count") else None,
                "Sti_Count": [
