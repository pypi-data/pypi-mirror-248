...
def upgrade_legacy_db_hamster_applet(db_path):
    ...
    def verify_legacy_version(curs):
        ...
        if len(versions) > 1:
            raise Exception(_("Found more than 1 version entry: {}").format(versions))
            ...
            if db_version != 9:
                raise Exception(
                    _("ERROR: Expected Legacy DB Version “9”, but found: {}").format(
                        db_version
                    )
                )
    ...
    def rename_old_tables(curs):
        for table in table_names:
            curs.execute(
                "ALTER TABLE {table} RENAME TO temp_{table}".format(table=table)
            )
    ...
    def drop_tmp_tables(curs):
        for table in reversed(table_names):
            curs.execute("DROP TABLE temp_{table}".format(table=table))
