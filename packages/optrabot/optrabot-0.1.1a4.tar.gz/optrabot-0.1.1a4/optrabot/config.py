...
def ensureInitialConfig() -> bool:
	...
	data = dict(general=dict(apikey=confAPIKey, agentid=confAgentId, hub=defaultHubHost),
				tws=dict(host=confTWSHost, port=int(confTWSPort), clientid=int(confTWSClientID), marketdata=confTWSMarketData, 
			 			account=confTradingAccount, adjustmentstep=float(confTradingPriceIncement), minimumPremium=float(confTradingMinimumPremium))
			 )
