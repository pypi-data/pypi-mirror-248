...
NETBOX_URL = os.getenv("MANISBLE_NETBOX_API_URL")
NETBOX_TOKEN = os.getenv("MANISBLE_NETBOX_TOKEN")
...
def create_manufacturer(manufacturer_name):
    ...
    # Mandatory fields
    manufacturer_name = os.environ.get('MANISBLE_MANUFACTURER_NAME')
    if manufacturer_name == None:
        manufacturer_name = "default"

    manufacturer_slug = os.environ.get('MANISBLE_MANUFACTURER_SLUG')
    if manufacturer_slug == None:
        manufacturer_name = manufacturer_name.lower()
    manufacturer_description = os.environ.get('MANISBLE_MANUFACTURER_DESCRIPTION')
    if manufacturer_description == None:
        manufacturer_description = ""
    manufacturer_comments = os.environ.get('MANISBLE_MANUFACTURER_COMMENTS')
    if manufacturer_comments == None:
        manufacturer_comments = ""
    manufacturer_custom_fields = os.environ.get('MANISBLE_MANUFACTURER_CUSTOM_FIELDS')
...
def create_device_type(device_type_name):
    ...
    device_type_name = os.environ.get('MANISBLE_DEVICE_TYPE_NAME')
    if device_type_name == None:
        device_type_name = "default"

    device_type_model = os.environ.get('MANISBLE_DEVICE_TYPE_MODEL')
    if device_type_model == None:
        device_type_model = "default"
    device_type_slug = os.environ.get('MANISBLE_DEVICE_TYPE_SLUG')
    if device_type_slug == None:
        device_type_slug = device_type_name.lower()
    device_type_description = os.environ.get('MANISBLE_DEVICE_TYPE_DESCRIPTION')
    if device_type_description == None:
        device_type_description = ""
    device_type_comments = os.environ.get('MANISBLE_DEVICE_TYPE_COMMENTS')
    if device_type_comments == None:
        device_type_comments = ""
    device_type_custom_fields = os.environ.get('MANISBLE_DEVICE_TYPE_CUSTOM_FIELDS')
    if device_type_custom_fields == None:

        device_type_custom_fields = {}
    # Optional fields
    device_type_part_number = os.environ.get('MANISBLE_DEVICE_TYPE_PART_NUMBER')
    if device_type_part_number == None:
        device_type_part_number = ""
    device_type_is_full_depth = os.environ.get('MANISBLE_DEVICE_TYPE_IS_FULL_DEPTH')
    if device_type_is_full_depth == None:
        device_type_is_full_depth = True
    device_type_is_console_server = os.environ.get('MANISBLE_DEVICE_TYPE_IS_CONSOLE_SERVER')
    if device_type_is_console_server == None:
        device_type_is_console_server = False
    device_type_is_pdu = os.environ.get('MANISBLE_DEVICE_TYPE_IS_PDU')
    if device_type_is_pdu == None:
        device_type_is_pdu = False
    device_type_is_network_device = os.environ.get('MANISBLE_DEVICE_TYPE_IS_NETWORK_DEVICE')
    if device_type_is_network_device == None:
        device_type_is_network_device = False
    device_type_subdevice_role = os.environ.get('MANISBLE_DEVICE_TYPE_SUBDEVICE_ROLE')
    if device_type_subdevice_role == None:
        device_type_subdevice_role = "" 
    device_type_interface_ordering = os.environ.get('MANISBLE_DEVICE_TYPE_INTERFACE_ORDERING')
    if device_type_interface_ordering == None:
        device_type_interface_ordering = None
    device_type_tags = os.environ.get('MANISBLE_DEVICE_TYPE_TAGS')
    if device_type_tags == None:
        device_type_tags = []
    device_type_manufacturer = os.environ.get('MANISBLE_DEVICE_TYPE_MANUFACTURER')
    ...
    if device_type_manufacturer_id == None:
        ...
        device_type_manufacturer_id = get_manufacturer_id(device_type_manufacturer)
    device_type_height = os.environ.get('MANISBLE_DEVICE_TYPE_HEIGHT')
...
def create_device():
    ...
    # Mandatory fields
    device_name = os.environ.get('MANISBLE_DEVICE_NAME')
    if device_name == None:
        device_name = "default"
    device_type = os.environ.get('MANISBLE_DEVICE_TYPE')
    if device_type == None:
        device_type = "default"
    device_role = os.environ.get('MANISBLE_DEVICE_ROLE')
    if device_role == None:
        device_role = "default"
    device_site = os.environ.get('MANISBLE_DEVICE_SITE')
    if device_site == None:
        device_site = "default"
    device_status = os.environ.get('MANISBLE_DEVICE_STATUS')
    if device_status == None:
        device_status = "active"
    device_comments = os.environ.get('MANISBLE_DEVICE_COMMENTS')
...
def create_role():
    ...
    role_name = os.environ.get('MANISBLE_ROLE_NAME')
    if role_name == None:
        role_name = "default"
    ...
    role_slug = os.environ.get('MANISBLE_ROLE_SLUG')
    if role_slug == None:
    ...
    role_description = os.environ.get('MANISBLE_ROLE_DESCRIPTION')
    ...
    role_comments = os.environ.get('MANISBLE_ROLE_COMMENTS')
...
def create_platform(platform_name):
    ...
    # Mandatory fields
    platform_name = os.environ.get('MANISBLE_PLATFORM_NAME')
    if platform_name == None:
        platform_name = "default"
    platform_slug = os.environ.get('MANISBLE_PLATFORM_SLUG')
    if platform_slug == None:
        platform_slug = platform_name.lower()
    platform_description = os.environ.get('MANISBLE_PLATFORM_DESCRIPTION')
    if platform_description == None:
        platform_description = ""
    platform_comments = os.environ.get('MANISBLE_PLATFORM_COMMENTS')
...
def create_site(site_name):
    ...
    # Mandatory fields
    site_name = os.environ.get('MANISBLE_SITE_NAME')
    if site_name == None:
        site_name = "default"
    site_slug = os.environ.get('MANISBLE_SITE_SLUG')
    if site_slug == None:
        site_slug = site_name.lower()
    site_description = os.environ.get('MANISBLE_SITE_DESCRIPTION')
    if site_description == None:
        site_description = ""
    site_comments = os.environ.get('MANISBLE_SITE_COMMENTS')
    if site_comments == None:
        site_comments = ""
    site_custom_fields = os.environ.get('MANISBLE_SITE_CUSTOM_FIELDS')
...
def create_type(type_name):
    ...
    manufacturer = os.environ.get("MANISBLE_NETBOX_MANUFACTURER")
...
def create_platform(platform_name):
    ...
    if platform_comments == None:
        platform_comments = ""
    platform_napalm_driver = os.environ.get('MANISBLE_PLATFORM_NAPALM_DRIVER')
    if platform_napalm_driver == None:
        platform_napalm_driver = ""
    platform_napalm_args = os.environ.get('MANISBLE_PLATFORM_NAPALM_ARGS')
...
def create_virtual_machine(vm_name):
    ...
    # Mandatory fields
    vm_name = os.environ.get('MANISBLE_VM_NAME')
    if vm_name == None:
        vm_name = "default"

    vm_cluster = os.environ.get('MANISBLE_VM_CLUSTER')
    if vm_cluster == None:
        vm_cluster = "default"
    vm_tenant = os.environ.get('MANISBLE_VM_TENANT')
    if vm_tenant == None:
        vm_tenant = "default"
    vm_role = os.environ.get('MANISBLE_VM_ROLE')
    if vm_role == None:
        vm_role = "default"
    vm_status = os.environ.get('MANISBLE_VM_STATUS')
    if vm_status == None:
        vm_status = "active"
    vm_comments = os.environ.get('MANISBLE_VM_COMMENTS')
    if vm_comments == None:
        vm_comments = ""
    vm_custom_fields = os.environ.get('MANISBLE_VM_CUSTOM_FIELDS')
    if vm_custom_fields == None:
        vm_custom_fields = {}
    vm_type = os.environ.get('MANISBLE_VM_TYPE')
    if vm_type == None:
        vm_type = "default"
...
def create_iprange(iprange_description = "default"):
    ...
    iprange_description = os.environ.get('MANISBLE_IPRANGE_DESCRIPTION')
...
def create_ip4(ip4_address = "default"):
    ...
    # Mandatory fields
    ip_address = os.environ.get('MANISBLE_IP_ADDRESS')
    if ip_address == None:
        ip_address = "127.0.0.1"
    ip4_status = os.environ.get('MANISBLE_IP_STATUS')
    if ip_status == None:  
        ip_status = "active"
    ip4_role = os.environ.get('MANISBLE_IP4_ROLE')
    if ip_role == None:
        ip_role = "loopback"
    tenant = os.environ.get('MANISBLE_NETBOX_TENANT')   
    if ip_tenant == None:
        ip_tenant = "default"
...
def create_virtual_interface(interface_name):
    ...
    # Mandatory fields
    interface_name = os.environ.get('MANISBLE_VINTERFACE_NAME')
    if interface_name == None:
        interface_name = "default"
    interface_type = os.environ.get('MANISBLE_INTERFACE_TYPE')
    if interface_type == None:
        interface_type = "virtual"
    interface_mac_address = os.environ.get('MANISBLE_INTERFACE_MAC_ADDRESS')
    if interface_mac_address == None:
        interface_mac_address = ""
    interface_mtu = os.environ.get('MANISBLE_INTERFACE_MTU')
    if interface_mtu == None:
        interface_mtu = ""
    interface_description = os.environ.get('MANISBLE_INTERFACE_DESCRIPTION')
    if interface_description == None:
        interface_description = ""
    interface_mode = os.environ.get('MANISBLE_INTERFACE_MODE')
...
def create_cluster(cluster_name):
    ...
    # Mandatory fields
    vmtype = os.environ.get('MANISBLE_VM_TYPE')
    if vmtype == None:
        vmtype = "default"
...
def create_tenant(tenant_name):
    ...
    # Mandatory fields
    tenant_name = os.environ.get('MANISBLE_TENANT_NAME')
    if tenant_name == None:
        tenant_name = "default"
    tenant_slug = os.environ.get('MANISBLE_TENANT_SLUG')
    if tenant_slug == None:
        tenant_slug = tenant_name.lower()
    tenant_description = os.environ.get('MANISBLE_TENANT_DESCRIPTION')
    if tenant_description == None:
        tenant_description = ""
    tenant_comments = os.environ.get('MANISBLE_TENANT_COMMENTS')
    if tenant_comments == None:
        tenant_comments = ""
    tenant_custom_fields = os.environ.get('MANISBLE_TENANT_CUSTOM_FIELDS')
...
def add_vm():
    ...
    if manufacturer == "QEMU":
        ...
        is_virtual = False
    if not is_virtual:
      servername = os.environ.get("MANISBLE_SERVER_NAME")
      if servername == None:
    ...
    if is_virtual:    
      vmname = os.environ.get('MANISBLE_VM_NAME')
      if vmname == None:
        vmname = os.environ.get('HOSTNAME')
      if vmname == None:  
        raise Exception("No VM name provided (MANISBLE_VM_NAME)")
      vmcluster = os.environ.get('MANISBLE_VM_CLUSTER')
      if vmcluster == None:
        raise Exception("No VM cluster provided (MANISBLE_VM_CLUSTER)")
      # Optional fields
      vmsite = os.environ.get('MANISBLE_VM_SITE')
      if vmsite == None:
        vmsite = "default"
      vmrole = os.environ.get('MANISBLE_VM_ROLE')
      if vmrole == None:
        vmrole = "default"
      vmtype = os.environ.get('MANISBLE_VM_TYPE')
      if vmtype == None:
        vmtype = "default"
      vmplatform = os.environ.get('MANISBLE_VM_PLATFORM')
      if vmplatform == None:
        vmplatform = "default"
      vmtentant = os.environ.get('MANISBLE_VM_TENANT')
      if vmtentant == None:
        vmtentant = "default"
      vmdevice = os.environ.get('MANISBLE_VM_DEVICE')
      if vmdevice == None:
        vmdevice = "default"
      vmstatus = os.environ.get('MANISBLE_VM_STATUS')
      if vmstatus == None:
        vmstatus = "active"
      vmdisk = os.environ.get('MANISBLE_VM_DISK')
      if vmdisk == None:
        vmdisk = 1
      vmcpus = os.environ.get('MANISBLE_VM_CPU')
      if vmcpus == None:
        vmcpus = 1
      vmmemory = os.environ.get('MANISBLE_VM_MEMORY')
      if vmmemory == None:
        vmmemory = 1
      vmip = os.environ.get('MANISBLE_VM_IP') 
      if vmip == None:
        vmip = ""
      vmip6 = os.environ.get('MANISBLE_VM_IP6') 
      if vmip6 == None:
        vmip6 = ""
      vmdescription = os.environ.get('MANISBLE_VM_DESCRIPTION')
      if vmdescription == None:
        vmdescription = ""

      vmcomments = os.environ.get('MANISBLE_VM_COMMENTS')
      if vmcomments == None:
        vmcomments = ""

      vmlocal_context_data = os.environ.get('MANISBLE_VM_LOCAL_CONTEXT_DATA')
      if vmlocal_context_data == None:
        vmlocal_context_data = {}
    
      vmtags = os.environ.get('MANISBLE_VM_TAGS')
      if vmtags == None:
        vmtags = []
      vmcustom_fields = os.environ.get('MANISBLE_VM_CUSTOM_FIELDS')
...
def fix_url(apiurl):
    nburl=os.environ.get('MANISBLE_NETBOX_URL')
    if nburl == None:
        print("No MANISBLE_NETBOX_URL provided")
        return False
...
def sshconfig(args):    
    ...
    MYHOME = os.getenv("HOME")
    configfile = os.path.expanduser(MYHOME + "/.ssh/conf.d/manisble.conf")
...
def serve():
    ...
    if NETBOX_URL == None:
        ...
        return False
    prettyllog("manage", "netbox", "serve", "000", "Read the /etc/manisble/manisble_netbox.json file")
    data = {}
    with open('/etc/manisble/manisble_netbox.json') as json_file:
...
def inventory_upload():
    ...
    inventory_file = os.environ.get("MANISBLE_INVENTORY_FILE")
    ...
    for item in items:
        ...
        os.environ["MANISBLE_DEVICE_NAME"] = item
        add_device()
