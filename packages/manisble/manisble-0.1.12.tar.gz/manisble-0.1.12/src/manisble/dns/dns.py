...
def env_check():
  domain = os.getenv('MANISBLE_DNS_DOMAIN')
  url=os.getenv('MANISBLE_DNS_URL')
  dns_type=os.getenv('MANISBLE_DNS_TYPE')
  token=os.getenv('MANISBLE_DNS_TOKEN')
  if domain is None:
    print("Error: MANISBLE_DNS_DOMAIN is not set")
    exit(1)
  if url is None:
    print("Error: MANISBLE_DNS_URL is not set")
    exit(1)
  if dns_type is None:
    print("Error: MANISBLE_DNS_TYPE is not set")
    exit(1)
  if token is None:
    print("Error: MANISBLE_DNS_TOKEN is not set")
    exit(1)
...
def set_env():
  ...
  if token is None:
    print("Error: MANISBLE_DNS_TOKEN is not set")
    exit(1)
  os.environ['MANISBLE_DNS_DOMAIN'] = domain
  os.environ['MANISBLE_DNS_URL'] = url
  os.environ['MANISBLE_DNS_TYPE'] = dns_type
  os.environ['MANISBLE_DNS_TOKEN'] = token
  return True
...
def get_dns_record(record):
  ...
  token=os.getenv('MANISBLE_DNS_TOKEN')
  if token == None:
    print("You need to setup MANISBLE_DNS_TOKEN")  
...
def register():
  ...
  if os.environ.get("MANISBLE_DNS_TYPE") == "cloudflare":
...
def virtlightning():
  ...
  set_env()
  vlfile = os.getenv('MANISBLE_VIRT_LIGHTNING_FILE')
  if vlfile == None:
    print("You need to setup MANISBLE_VIRT_LIGHTNING_FILE")  
    exit(1)
  vldir = os.getenv('MANISBLE_VIRT_LIGHTNING_DIR')
  if vldir == None:
    print("You need to setup MANISBLE_VIRT_LIGHTNING_DIR")  
    exit(1)
