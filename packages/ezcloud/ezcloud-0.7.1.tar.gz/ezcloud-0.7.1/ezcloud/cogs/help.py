...
from ..internal.dc import ezcloud, discord, slash_command
...
async def pass_checks(command: discord.SlashCommand, ctx) -> bool:
    """Returns True if the current user passes all checks (ezcloud only)."""
    passed = True
...
class Help(Cog, hidden=True):
    ...
    async def help(self, ctx):
        ...
        if embed is None:
            ...
            interaction = ctx.interaction if ezcloud else ctx
        ...
        for name, cog in self.bot.cogs.items():
            ...
            if len(cog.get_commands()) == 0 and ezcloud:
                continue
            ...
            for command in cog_cmds:
                ...
                if self.bot.help.permission_check:
                    if ezcloud and not await pass_checks(command, ctx):
                        continue
