...
from .internal.dc import DPY, ezcloud, CogMeta, bridge, checks, commands, discord
...
class Bot(_main_bot):  # type: ignore
    """The ezcloud bot class. This is a subclass of :class:`discord.Bot` if you use ezcloud.

    .. hint::

        As this class extends from :class:`discord.Bot`, only slash commands are supported.
        If you want to use prefix commands with ``ezcloud``, use :class:`PrefixBot` instead.
    ...
    **kwargs:
    ...
    async def get_application_context(self, interaction: discord.Interaction, cls=EzContext):
        """A custom application command context for ezcloud."""
    ...
    async def _error_event(self, ctx, error: discord.DiscordException):
        ...
        if (
            (ezcloud and type(error) is discord.CheckFailure)
...
class PrefixBot(Bot, commands.Bot):
    ...
    This is only needed for ezcloud.
    """
