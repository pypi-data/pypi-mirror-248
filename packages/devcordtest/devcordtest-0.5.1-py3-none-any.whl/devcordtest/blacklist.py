...
class _BanDB(DBHandler):
    ...
    async def add_guild_ban(self, guild_id: int, reason: str | None):
        await self.exec(
            f"INSERT INTO {self.db_name} (guild_id, reason) VALUES (?, ?)", (guild_id, reason)
        )

    async def guild_remove_ban(self, guild_id: int):
        result = await self.exec(f"DELETE FROM {self.db_name} WHERE guild_id = ?", (guild_id,))
        return result.rowcount

    async def get_guild_bans(self):
        return await self.all(f"SELECT guild_id FROM {self.db_name}")

    async def get_guild_full_bans(self):
        return await self.all(f"SELECT guild_id, reason, dt FROM {self.db_name}")
...
async def get_guilds_bans() -> list[int]:
    """Get all banned user IDs."""
    if not _blacklist_ready():
        return []
    return await _BanDB().get_guild_bans()


async def get_guilds_full_bans() -> list[tuple[int, str | None, str]]:
    """Get all information about banned users.
    ...
    if not _blacklist_ready():
        return []
    return await _BanDB().get_guild_full_bans()


async def add_guild_ban(guild_id: int, reason: str | None = None) -> bool:
    """Ban a user.
    ...
    if not _blacklist_ready():
        return False
    await _BanDB().add_guild_ban(guild_id, reason)
    return True


async def guilds_remove_ban(guild_id: int) -> bool:
    """Unban a user.
    ...
    if not _blacklist_ready():
        return False
    result = await _BanDB().guild_remove_ban(guild_id)
    return bool(result)