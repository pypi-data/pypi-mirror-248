...
class Blacklist(Cog, hidden=True):
    ...
    if PYCORD:
        admin = discord.SlashCommandGroup(
            t("admin_group"),
            description="devcordtest admin commands",
...
async def get_or_fetch_guild(bot, guild_id: int):
...
async def _check_guild_blacklist(interaction: discord.Interaction) -> bool:
    bans = await _db.get_guild_bans()
    if interaction.user.id in bans and EzConfig.blacklist:
        if EzConfig.blacklist.raise_error:
            raise Blacklisted()
        else:
...
class Blacklist(Cog, hidden=True):
    ...
    async def cog_check(self, ctx):
        if EzConfig.blacklist.owner_only:
            if not await self.bot.is_owner(ctx.guild):
                if DPY:
                    return False
                else:
                    raise commands.CheckFailure()
    ...
    async def guild_blacklist_add(
        ...
        if guild.id == ctx.guld.id:
            return await emb.error(ctx, "You can't ban yourself.")

        try:
            await _db.add_guild_ban(guild.id, reason)
        except aiosqlite.IntegrityError:
            return await emb.error(ctx, "This user is already banned.")
        await ctx.response.send_message(
            f"The user was banned successfully.\n- **Name:** {guild}\n- **ID:** {guild.id}",
    ...
    @blacklist.command(name="remove", description="Remove a member from the blacklist")
    # @discord.option("user", description="The user to ban/unban")
    async def guild_blacklist_remove(self, ctx, guild: int):
        rowcount = await _db.guild_remove_ban(guild.id)
        if rowcount == 0:
            return await emb.error(ctx, "This user is not banned.")
        await ctx.response.send_message(
            f"The user **{guild}** was unbanned successfully.", ephemeral=True
        )

    @blacklist.command(name="show", description="Show the bot blacklist")
    async def guild_show_blacklist(self, ctx):
        await ctx.response.defer(ephemeral=True)
        bans = await _db.get_guild_full_bans()
        desc = ""

        for guild_id, reason, _ in bans:
            if not reason:
                reason = "No reason provided"

            guild = await get_or_fetch_user(self.bot, guild_id)
            name = f"{guild.name} ({guild.id})" if guild else guild_id
            desc += f"{name} - {reason}\n"
    ...
    @admin.command(description="Show all bot servers")
    async def guild_show_servers(self, ctx):
        await ctx.response.defer(ephemeral=True)
        longest_name = max([guild.name for guild in self.bot.guilds], key=len)
        sep = f"<{len(longest_name)}"
