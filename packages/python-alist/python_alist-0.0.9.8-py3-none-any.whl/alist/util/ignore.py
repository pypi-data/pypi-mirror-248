...
def parse(
    ...
    if show_all:
        return None
    predicate_mime: Optional[Callable] = None
    if check_mimetype and (mimetype_shows or mimetype_ignores):
        def predicate_mime(path: str, /):
            if path.endswith("/"):
                return False
    ...
    predicate_ignore: Optional[Callable] = None
    if not preds:
        return predicate_mime
    elif len(preds) == 1:
        predicate_ignore = preds[0]
    else:
        predicate_ignore = lambda path: all(p(path) for p in preds)
    if predicate_mime is None:
        return predicate_ignore
    return lambda path: predicate_mime(path) or predicate_ignore(path)
