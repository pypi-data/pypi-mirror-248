...
def download_weights(model):
    weights = {
        ...
        'FastSAM': ['https://drive.google.com/u/0/uc?id=1m1sjY4ihXBU1fZXdQ-Xdj-mDltW-2Rqv&export=download&confirm=t&uuid=bacf82f5-6461-44a5-8594-e534dd1b0509&at=AB6BwCD_YbFjLvGr0XiZVPtCQieh:1703072969660', 'FastSAM.pt']
    }
...
def choose_SAM_model(SAM_model):
    ...
    if SAM_model == 'auto':
        ...
        if any(fname.startswith(checkpoint_file) for fname in available_SAM_models):
            return os.path.join(directory, f'sam_vit_{model_types.get(SAM_model)[1]}.pth'), model_types.get(SAM_model)[0]
        else:
            _input = input(f'The SAM weight {model_types[SAM_model][0]} was not found. Do you want to download it? y/n: ')
            if _input.lower() == 'y':
