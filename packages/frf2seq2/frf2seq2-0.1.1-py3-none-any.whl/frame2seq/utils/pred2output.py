...
def output_fasta(model_outs, fasta_dir):
    ...
    with open(f"{fasta_dir}/{pdbid}_{chain}_seq{sample}.fasta", "w") as f:
        f.write(f">pdbid={pdbid} chain_id={chain} recovery={recovery*100:.2f}% score={avg_neg_pll:.2f} temperature={temp}\n")
        f.write(f"{seq}\n")
...
def output_csv(scores, csv_dir):
    ...
    df = pd.DataFrame(list(zip(res_idx, neg_pll)), columns=['Residue index', 'Negative pseudo-log-likelihood'])
