...
class frame2seq(pl.LightningModule):  
    def __init__(self, config): 
        ...
        for i in range(ipa_depth):
            ...
            if self.st_mod_tsit_factor > 1:
                self.layers.append(nn.ModuleList([ipa, ipa_dropout, layer_norm_ipa, pre_transit, transition, post_transit, edge_transition]))
            else:
                self.layers.append(nn.ModuleList([ipa, ipa_dropout, layer_norm_ipa, transition, edge_transition]))
