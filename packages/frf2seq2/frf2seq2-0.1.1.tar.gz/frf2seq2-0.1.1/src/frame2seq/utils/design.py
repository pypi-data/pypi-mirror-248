...
def design(self, pdb_file, chain_id, temperature, num_samples, save_neg_pll, verbose):
    ...
    with torch.no_grad():
        ...
        for sample in tqdm(range(num_samples)):
            ...
            if verbose:
                ...
                print(f"Average negative pseudo-log-likelihood : {avg_neg_pll:.2f}")
            ...
            if save_neg_pll: # save per-residue negative pseudo-log-likelihoods
                ...
                scores['neg_pll'] = [neg_pll[i].item() for i in range(len(sampled_seq_i))]
