...
class Frame2seqRunner():
    ...
    def design(self, pdb_file, chain_id, temperature, num_samples, save_neg_pll=True, verbose=True):
        """
        Design sequences for a given PDB file and chain ID.
        """
        start_time = time()
        design(self, pdb_file, chain_id, temperature, num_samples, save_neg_pll, verbose)
        if verbose:
