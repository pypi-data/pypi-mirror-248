...
from apixunit.parseexcel import ParseExcel
from apixunit.apivalid import Validations
from apixunit.ghelper import update_values_nested
...
def generate_test_api(generate_test_api, sheetname=None, exeLine=None, sTitle=None, hTitle=None):
    sLine, eLine = 2, None
    ...
    with open(pathdata_file, 'w') as f:
        ...
        f.write('from apixunit.api import apicall\n')
        f.write('from cdxg.logging import log\n')
        f.write('from pathlib import Path\n')
        f.write('from configparser import ConfigParser\n')
        f.write('from apixunit.common import get_tags, Create_New_Report, get_results, getepoint, get_sheetnames_excel, excel_row_data\n')
        f.write('from apixunit.apiexecutor import apiSteps\n\n')
        f.write('project_path = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))\n')
        f.write('mypath = Path.cwd()\n')
        # f.write('mypath = mypathchild.parent\n')
        ...
        for getxl in getallxm:
            if getxl != 'Usage':
                ...
                f.write('\t\t\t\t\t\t\texresults, scvarib, ctype, results, rtype, case, depend, sprints, incidents):\n')
                f.write('\t\t"""*"""\n')
                f.write('\t\tlog.info("***" + str(tcdef) + "_" + str(testcase) + "***")\n')
                f.write("\t\tcfilex = config.read(os.path.join(project_path, 'utils', 'endpointapi.ini'))\n")
                f.write('\t\tprint(xendpoint)\n')
                f.write('\t\tprint(project_path)\n')
                f.write('\t\tprint(xendpoint)\n')
                f.write("\t\tif '/' in xendpoint:\n")
                f.write("\t\t\txpoint = str(xendpoint).split('/')\n")
                f.write('\t\t\tprint(xpoint)\n')
                f.write("\t\t\trolepoint = getepoint(xpoint)  # xpoint[len(xpoint)-1]\n")
                f.write('\t\t\tprint(rolepoint)\n')
                f.write('\t\telse:\n')
