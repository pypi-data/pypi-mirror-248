...
def create_excel_sheet_for_testcase(file_path):
    ...
    # Headers for the Excel sheet
    headers = ['Test_Case', 'TC_No', 'EndPoint', 'Method', 'Payload', 'Params', 'Test_Data', 'API_Response',
               'Schema Variables', 'SType|Format|Validation', 'Expected_Results', 'RunType', 'Case', 'Depend',
...
# print(mypath / 'reports' / 'result.html')
gen_file_path = mypath / 'test_data' / 'api_test_data.xlsx'
if __name__ == '__main__':
    ...
    if gen_file_path.is_file():
        # print('Test data file exists...@ Path : ' + str(gen_file_path))
        test_case_data = cObject.get("test_data_xl", "test_case_data_xl")
        generate_test_ap = mypath / test_case_data
        sheetname = args.s
        exeLine = args.l
        generate_test_api(generate_test_ap, sheetname, exeLine, reporting['title'], baseUrl['bUrl'])

    cdxg.main(path="./test_dir/test_onefms_api.py",
...
[test_data_xl]
test_case_data_xl = {replace_test_casedata_path} #./test_data/api_test_data.xlsx
...
apiconf_requires = """[apiEndpoint] # API Endpoints listed here to be used for api calls like ex: nof = NetsOFF [nof(apiEndpoint or api) = NetsOFF(suffix excelsheet name)]
nof = NetsOFF

[pdefinedget]
# Co-relation of dependency endpoints with above apiEndpoints #
"""

test_requires ="""apixunit==1.0
"""

readme_requires = """# oneFMS API
