...
async def deepseek_playwright_run_sign_in(  # todo
        user: str = "313303303@qq.com",
        password: str = "8643188a",
    ...
    for storage_state in Path(Path(storage_state).parent).glob(Path(storage_state).name):
        async with async_playwright() as playwright:
            ...
            await page.goto(url)
            await page.get_by_placeholder("请输入手机号/邮箱地址").fill(user)  # todo: 账号密码
...
def prun(
    ...
    storages = asyncio.run(playwright_run(url, headless, storage_state))
    logger.debug(storages)
