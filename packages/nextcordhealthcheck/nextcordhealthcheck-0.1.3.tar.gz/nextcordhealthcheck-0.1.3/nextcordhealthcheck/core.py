...
class _ClientContext:
    ...
    def __init__(self, client: nextcord.Client, bot_max_latency: float):
        self.client = client
...
def start(
    client: nextcord.Client, port: int = 40404, bot_max_latency: float = 0.5
) -> Awaitable[asyncio.base_events.Server]:
    """Starts the health check server.
    Usually a good place to put this is in the bots setup_hook method.

    Args:
        client: The nextcord client object to monitor
