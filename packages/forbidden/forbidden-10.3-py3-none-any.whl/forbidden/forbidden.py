...
default_user_agent = "Forbidden/10.3"
...
class Forbidden:
	...
	def __init__(self, url, ignore_qsf, ignore_curl, tests, force, values, paths, evil, ignore, content_lengths, threads, sleep, user_agents, proxy, debug):
		...
		self.__user_agents_len = len(user_agents)
		self.__proxy           = proxy
		self.__debug           = debug
		# --------------------------------
...
class MyArgParser(argparse.ArgumentParser):
	def print_help(self):
		print("Forbidden v10.3 ( github.com/ivan-sincek/forbidden )")
		print("")
...
class Validate:
	...
	def __init__(self):
		...
		self.__parser.add_argument("-u"   , "--url"                             , required = True , type   = str         )
		self.__parser.add_argument("-iqsf", "--ignore-query-string-and-fragment", required = False, action = "store_true")
		self.__parser.add_argument("-ic"  , "--ignore-curl"                     , required = False, action = "store_true")
		self.__parser.add_argument("-t"   , "--tests"                           , required = True , type   = str.lower   )
		self.__parser.add_argument("-f"   , "--force"                           , required = False, type   = str.upper   )
		self.__parser.add_argument("-v"   , "--values"                          , required = False, type   = str         )
		self.__parser.add_argument("-p"   , "--path"                            , required = False, type   = str         )
		self.__parser.add_argument("-e"   , "--evil"                            , required = False, type   = str         )
		self.__parser.add_argument("-i"   , "--ignore"                          , required = False, type   = str         )
		self.__parser.add_argument("-l"   , "--content-lengths"                 , required = False, type   = str.lower   )
		self.__parser.add_argument("-th"  , "--threads"                         , required = False, type   = str         )
		self.__parser.add_argument("-s"   , "--sleep"                           , required = False, type   = str         )
		self.__parser.add_argument("-a"   , "--user-agent"                      , required = False, type   = str         )
		self.__parser.add_argument("-x"   , "--proxy"                           , required = False, type   = str         )
		self.__parser.add_argument("-o"   , "--out"                             , required = False, type   = str         )
		self.__parser.add_argument("-dbg" , "--debug"                           , required = False, action = "store_true")
