...
class rk_model(model):
    ...
    def _fix_shape(self, pdf):
        ...
        return pdf
    #---------------------------------------------------------------
    def _get_combinatorial(self, preffix, fix_shape=False):
    ...
    def _add_combinatorial(self, l_pdf, preffix):
        ...
        #if   trig == 'ETOS' and self._kind != 'cmb_ee:use_etos':
        #    return l_pdf
        #elif trig == 'ETOS' and self._kind == 'cmb_ee:use_etos':
        #    log.warning(f'For {preffix}, using alternative combinatorial model: {self._kind}')

        cbkg = self._get_combinatorial(preffix, fix_shape= trig=='ETOS')
        l_pdf.insert(0, cbkg)
    ...
    def _get_pdf(self, preffix='', nent=None):
        ...
        if   preffix.startswith('ee_'):
            ...
            l_pdf = [erbp, erbd, erbs, eprc, emis, esig]
        elif preffix.startswith('mm_'):
            l_pdf = [esig]
        else:
            log.error(f'Invalid preffix: {preffix}')
