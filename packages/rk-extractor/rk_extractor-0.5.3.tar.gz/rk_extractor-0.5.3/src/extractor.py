...
class extractor:
    ...
    def _initialize(self):
        ...
        self._check_fix_pars()
        self._fix_pars()

        self._initialized = True
    #--------------------------------
    def _check_fix_pars(self):
        if self._l_fix is None:
            return

        l_ck = [ par_name for par_name in self._l_fix if par_name.startswith('ck')]
        if l_ck != ['ck']:
            log.error(f'CK parameter introduced multiple times: {l_ck}')
            raise
    #--------------------------------
