...
def test_fix_pars():
    ...
    mod          = model(preffix='const', d_eff=d_eff, d_nent=d_rare_yld, l_dset=['2018_TOS'])
    d_mod        = mod.get_model()
    d_dat        = mod.get_data()
...
def test_real():
    ...
    mod          = model(preffix='real', d_eff=d_eff, d_nent=d_rare_yld, l_dset=['2018_TOS', '2018_TIS'])
    #mod.out_dir  = 'tests/extractor/real/model'
    d_mod        = mod.get_model()
    d_dat        = mod.get_data()
    d_val, d_var = mod.get_cons()

    obj          = ext(dset=['2018_TOS', '2018_TIS'])
    obj.rjpsi    = d_rjpsi
    obj.eff      = d_eff
    obj.cov      = cv_sys + cv_sta
    obj.data     = d_dat
