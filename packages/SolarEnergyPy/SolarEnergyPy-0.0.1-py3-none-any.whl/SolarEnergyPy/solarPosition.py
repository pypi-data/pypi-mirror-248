...
class solarTime(site):
    ...
    def __init__(self, *args, **kwargs):
        ...
        dt        = kwargs['dt']        if 'dt' in kwargs else None         
        longitude = kwargs['longitude'] if 'longitude' in kwargs else 112
        latitude  = kwargs['latitude']  if 'latitude'  in kwargs else 31
        hour      = kwargs['hour']      if 'hour'      in kwargs else 0
        minute    = kwargs['minute']    if 'minute'    in kwargs else 0
        second    = kwargs['second']    if 'second'    in kwargs else 0
        tz        = kwargs['tz']        if 'tz'        in kwargs else 'Asia/Shanghai'

        super().__init__(longitude,latitude)
        self._tz = tz
        self.clocktime = pendulum.now(self.tz)
...
def demo():
    ...
    print(str)
    print('date-time is {}'.format(sp.datetime))
