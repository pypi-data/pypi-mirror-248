...
def _sec2time(sec, date, preday=False):
    return np.datetime64(date,"ns")-np.timedelta64(int(preday), "D")+np.timedelta64(int(sec), "s")


def read_refl_snr(filename, time=datetime.datetime.today(), linenum=1000, interval=15):
    ...
    # seclist = list(set(data["sec"]))
    prnlist = [_code2prn(_) for _ in codelist]

    time0 = _sec2time(data["sec"][0], time, data["sec"][0]>preday_max_sec)
    time1 = _sec2time(data["sec"][-1], time, False)
    ...
    for iline, (code, ele, azi, sec, eler, s6, s1, s2, s5, s7, s8) in enumerate(zip(
        ...
        time_epoch = _sec2time(sec, time, (sec>preday_max_sec)&(iline<linenum))
