...
class GatewayV1(BaseGateway):
    ...
    def has_nft(self, address: str, token_id: str):
        ...
        for nft in nfts:
            nft_token_id = nft.get("tokenId")
            if nft.get("tokenId") == str(token_id):
                return True
        return False
    ...
    def get_nft(self, token_id: str):
        path = 'nfts/detail/{collectionType}/{tokenId}'.format(collectionType=CollectionType.SUPERCLUB.value, tokenId=str(token_id))
    ...
    def get_nft_transactions_by_nft(self, token_id: str):
        path = 'nfts/history/{collectionType}/{tokenId}'.format(collectionType=CollectionType.SUPERCLUB.value, tokenId=str(token_id))
    ...
    def check_nft(self, request_id):
        ...
        if result and result['result'] and result['result']['tokenId']:
            instance.token_id = result['result']['tokenId']
