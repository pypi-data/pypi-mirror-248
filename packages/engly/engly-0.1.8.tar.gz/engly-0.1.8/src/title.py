...
class SampleFrame(wx.Frame):
    ...
    def __data_setup(self):
        if not os.path.exists(self.home_directory + "/.engly_data"):
            ...
            os.makedirs(fig_dir)
            os.makedirs(api_dir)
            with open(res_json, "w", encoding='utf-8') as file:
                json.dump({"ChatGPT":[]}, file, indent=2)
            with open(his_json, "w", encoding='utf-8') as file:
                json.dump({}, file, indent=2)
            with open(word_json, "w", encoding='utf-8') as file:
                json.dump({}, file, indent=2)
    ...
    def reading(self, event):
        ...
        with open(path("apikey", "api"), "r", encoding='utf-8') as f:
            key = f.read().strip()
    ...
    def save(self, event):
        ...
        if dlg.ShowModal() == wx.ID_YES:
            with open(path("apikey", "api"), "w", encoding='utf-8') as f:
