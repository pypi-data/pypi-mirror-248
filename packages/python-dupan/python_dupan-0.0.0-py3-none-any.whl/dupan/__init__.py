...
__all__ = ["Error", "Client", "ShareList"]
...
def recognize_vcode(_ocr=DdddOcr(beta=True), /) -> VCodeResult:
    "识别百度网盘的验证码"
    url = "https://pan.baidu.com/api/getcaptcha?prod=shareverify&web=1&clienttype=0&bdstoken="
    while True:
        resp = get(url).json()
        for i in range(10):
...
class Client:
    ...
    def __init__(self, cookie: Optional[str] = None):
        ...
        # BDUSS, STOKEN
        if not cookie or "BAIDUID=" not in cookie:
    ...
    def login_with_qrcode(self, /, scan_in_console=True):
        "扫描二维码登录"
        gid = str(uuid4()).upper()
    ...
    def filemanager(
        ...
        if isinstance(payload, str):
            payload = {"filelist": dumps([payload])}
    ...
    def transfer(
        ...
        /, 
        params: dict, 
        payload: int | str | list | tuple | dict, 
    ) -> dict:
        ...
        if isinstance(payload, (int, str)):
            payload = {"fsidlist": "[%s]" % payload, "path": "/"}
        elif isinstance(payload, (list, tuple)):
            payload = {"fsidlist": "[%s]" % ", ".join(map(str, payload)), "path": "/"}
        return self.request(api, "POST", params=params, payload=payload)
        ...
        while True:
            with session.post(api, params=params, data=data) as resp:
                ...
                if errno == -62:
                    ...
                    raise OSError(json)
        randsk = self.randsk = json["randsk"]
        ...
        while True:
            with session.get(url) as resp:
                ...
                if b'"verify-form"' in content:
                    ...
                    self.share_uk = data["share_uk"]
                    self.share_id = data["shareid"]
                    file_list = data["file_list"]
                    if file_list is None:
                        raise OSError("没有找到下载文件，可能是链接失效、分享被取消等原因")
                    if file_list:
                        self.root = file_list[0]["path"].rsplit("/", 1)[0]
                    else:
                        self.root = "/"
                    return file_list
