...
class FocasController:
    ...
    def __enter__(self):        #  -> Focas: gives an Error, so leave it!
    ...
    def connect(self):
        ...
        if ret != 0:
            raise ConnectionError(
                f"Unable to connect to Focas (IP: {self.ip}, Port:"
                f" {self.port})"
            )
    ...
    def read_axis(self, category: int, index: int) -> int:
        ...
        example
        >>> url="192.168.1.10" 
        >>> with FocasController(url) as controller:
        ...     # 1...position, 0...absolute
        ...     axisdata = controller.read_axis(1,0)
        >>> len(axisdata)
        5
        >>> type(axisdata[0])
        <class 'axdata'>
