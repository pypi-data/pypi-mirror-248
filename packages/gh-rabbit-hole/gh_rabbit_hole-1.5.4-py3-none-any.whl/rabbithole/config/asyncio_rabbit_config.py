...
class AsyncioRabbitManager:
    ...
    def on_listening_queue_declareok(self, frame):
        ...
        if self.listening_queue:
            self.__logger.debug(
                RabbitLogMessages.START_CONSUMING_NO_QUEUE.format(
                    "on_listening_queue_declareok", self.listening_queue
                )
            )
    ...
    def send_message(
        ...
        try:
            ...
            self.__logger.info(
                RabbitLogMessages.MESSAGE_SENT.format(routing_key, message)
            )
        except Exception as e:
