def OptimalDataSelector2(predictor=None,target=None,combination=10,random_state=0,train_size=0.7,bs_problem='class',time_forecast=True,scaling=None,acc_forecast=True,boost=None,randomness=True,load_path=None,active_checkpoint=False):
    ...
    try:
        ...
        per=input("Are you giving the permission to install pandas package ?[press 'y' for yes or press any key for no] : ")
        if per in('Yes','YES','yes','y','Y'):
            !pip install pandas
        else:
            raise ModuleNotFoundError("This function needs pandas package to perform task but it seems like the package is missing")
    try:  
        import pandas as pd
    except Exception as ex:
        print(ex)
        raise ModuleNotFoundError("Package pandas is not installed properly")
    ...
    try:
        ...
        per1=input("Are you giving the permission to install scikit-learn package ?[press 'y' for yes or press any key for no] : ")
        if per1 in('yes','Yes','YES','Y','y'):
            !pip install scikit-learn
        else:
            raise ModuleNotFoundError("This function needs sklearn package to perform task but it seems like the package is missing")
    try:
        import sklearn
    except Exception as x:
        print(x)
        raise ModuleNotFoundError("Package scikit-learn is not installed properly")
    ...
    try:
        ...
        per2 = input("Are you giving the permission to install scipy package ?[press 'y' for yes or press any key for no] : ")
        if per2 in('Yes','yes','YES','Y','y'):
            !pip install scipy
        else:
            raise ModuleNotFoundError("This function needs scipy package to perform task but it seems like the package is missing")
    try:
        import scipy
    except Exception as exx:
        print(exx)
        raise ModuleNotFoundError("Package scipy is not installed properly")
    import time
    import warnings
