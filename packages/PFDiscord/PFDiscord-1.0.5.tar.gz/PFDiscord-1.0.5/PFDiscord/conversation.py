# Provides PFDiscord a way to store and retrieve conversation messages by the OpenAI model
import os, re
...
class Conversation:
    def __init__(self, user_name: str, assist_name: str, system_content: str, system_model="gpt-3.5-turbo"):
        ...
        # Orient to the base app path for PFDiscord no matter what sub folder I'm coming from
        if not os.environ.get('PFDISCORD_APP_PATH'):
            current_path = os.getcwd()
            app_index = current_path.find("PFDiscord")
            base_path = current_path[:app_index + len("PFDiscord")]
            os.environ['PFDISCORD_APP_PATH'] = base_path

        # Define the directory where to store this conversation with the log file name
        self.interaction_dir = os.path.join(os.environ['PFDISCORD_APP_PATH'], 'sessionlogs')
