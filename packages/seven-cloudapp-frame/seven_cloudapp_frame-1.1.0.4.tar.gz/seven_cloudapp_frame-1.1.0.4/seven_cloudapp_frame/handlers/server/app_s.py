...
@LastEditTime: 2023-12-25 17:12:31
...
class MultipleInstantiateAppHandler(ClientBaseHandler):
    """
    :description: 实例化小程序,支持同时实例化多个C端模板
    """
    def get_async(self):
        """
        :description: 实例化小程序,支持同时实例化多个C端模板
        ...
        if not app_id and user_nick:
            ...
            if len(app_template_dict_list) > 0:
                ...
                app_info_dict_list = app_info_model.get_dict_list("store_user_nick=%s", field="template_id", params=[store_user_nick])
                for app_template_dict in app_template_dict_list:
                    if query(app_info_dict_list).count(lambda x: x["template_id"] == app_template_dict["template_id"]) > 0:
                        continue
                    other_invoke_result_data = top_base_model.instantiate("", user_nick, access_token, app_key, app_secret, is_log, main_user_open_id, app_name=app_template_dict["product_name"], description=app_template_dict["product_desc"], icon=app_template_dict["product_icon"], template_id=app_template_dict["template_id"], template_version=app_template_dict["client_ver"])
                    if other_invoke_result_data.success == False:
                        self.logging_link_error(f"模板ID：{app_template_dict['template_id']}实例化失败，原因：{other_invoke_result_data.error_message}")
                    else:
                        app_relation = AppRelation()
                        app_relation.app_id = other_invoke_result_data.data["app_id"]
                        app_relation.parent_app_id = invoke_result_data.data["app_id"]
...
class MultipleVersionUpgradeHandler(ClientBaseHandler):
    """
    :description: 前端版本更新,支持同时更新多个C端模板
    """
    def get_async(self):
        """
        :description: 前端版本更新,支持同时更新多个C端模板
        ...
        app_info_model = AppInfoModel(context=self)
        app_info_list = app_info_model.get_list("store_user_nick=%s", params=[store_user_nick])
        app_info = query(app_info_list).first_or_default(None, lambda x: x.app_id == app_id)
        if not app_info:
        ...
        top_base_model = TopBaseModel(context=self)
        app_key, app_secret = self.get_app_key_secret()
        if client_ver != app_info.client_ver:
        ...
        if len(app_info_list) > 0:
            ...
            for app_info in app_info_list:
                app_template_dict = query(app_template_dict_list).first_or_default(None, lambda x:x["template_id"] == app_info.template_id) 
                if app_template_dict and app_template_dict['client_ver'] != app_info.client_ver:
                    other_invoke_result_data = top_base_model.version_upgrade(app_info.app_id, app_template_dict['template_id'], app_template_dict['client_ver'], access_token,app_key, app_secret, app_info, is_log)
                    if other_invoke_result_data.success == False:
                        self.logging_link_error(f"模板ID：{app_template_dict['template_id']}更新失败，原因：{other_invoke_result_data.error_message}")
