...
class DingtalkChatbot(object):
    ...
    def send_markdown_img(self, title, text, is_at_all=False, at_mobiles=[], at_dingtalk_ids=[], is_auto_at=True):
        """
        markdown类型
        :param title: 首屏会话透出的展示内容
        :param text: markdown格式的消息内容
        ...
        if all(map(is_not_null_and_blank_str, [title, text])):
            # 给Mardown文本消息中的跳转链接添加上跳转方式
            text = re.sub(r'(?<!!)\[.*?\]\((.*?)\)', lambda m: m.group(0).replace(m.group(1), self.msg_open_type(m.group(1))), text)
            data = {"msgtype": "markdown", "markdown": {"title": title, "text": "####  \n> \n> ![screenshot](" + text + ")\n> "}, "at": {}}
            if is_at_all:
                data["at"]["isAtAll"] = is_at_all
...
if __name__ == '__main__':
    import doctest
    doctest.testmod()
    webhook = "https://oapi.dingtalk.com/robot/send?access_token=7823f3682963d60828b772cd8d08206b219bcbd51bb823045bf4e8e607a34fad"
    secret = "SECa5ec4cb87b097f4b48d0249be3b3505e6dd8c29a8fb2bb68639e09348ef8de93"
    dd = DingtalkChatbot(webhook=webhook, secret=secret)
    res = dd.send_text(msg="订购,哈喽，老铁们早", is_at_all=False)  #test OK

    #res = dd.send_link(title="链接消息", text="#### 乔布斯 20 年前想打造的苹果咖啡厅", message_url="https://static.dingtalk.com/media/lAPPD2P17UFxY7MozQHC_450_40.bmp")
    #send link ok

    # res = dd.send_feed_card(links="https://static.dingtalk.com/media/lAPPD2P17UFxY7MozQHC_450_40.bmp")

    print(res)

    # res2 = dd.send_image("@lADPKHQpXAGfNezNBQDNAkA")  #测试成功，不过图片有点小。
    # print(res2)
    #res3 = dd.send_markdown_img("图片", "@lADPKHQpXAGfNezNBQDNAkA")  #测试成功。
