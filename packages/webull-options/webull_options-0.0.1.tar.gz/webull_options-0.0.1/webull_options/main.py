...
from webull_options.helpers import process_candle_data, get_human_readable_string
...
class WebullOptions:
    ...
    async def option_chart_data(self, derivative_id, timeframe:str='1m'):
        ...
        async with aiohttp.ClientSession(headers=sdk.headers) as session:
            async with session.get(url) as resp:
                data = await resp.json()
    ...
    async def get_volume_analysis(ticker):
        
        data, _, option_data = await sdk.all_options(ticker)
        derivative_data = await sdk.db_manager.query_derivative_ids_in_price_range(ticker, current_price=data.close)
        async with aiohttp.ClientSession(headers=sdk.headers) as session:
                ...
                async with session.get(url) as resp:
                    if resp.status == 200:
                        ...
                        for date, data in zip(dates, datas):
                            ...
                            await sdk.db_manager.insert_volume_analysis(option_id=id, option_symbol=option_symbol, underlying_symbol=underlying_symbol, strike_price=strike_price, expiry=expiry, call_put=call_put, buy=buy, sell=sell, price=price, ratio=ratio, date=date)
    ...
    async def filter_options(self, **kwargs):
        ...
        query = f"SELECT * FROM public.options WHERE "
        params = []
        param_index = 1
        ...
        query = "SELECT * FROM public.options WHERE open_interest > 0"
...
async def filter():
    filtered_options = await sdk.filter_options(theta_max=0.01, theta_min=-0.04)
