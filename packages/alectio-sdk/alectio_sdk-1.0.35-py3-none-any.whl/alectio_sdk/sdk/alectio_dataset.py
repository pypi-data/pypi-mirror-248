...
class AlectioDataset:
    ...
    def get_datamap_csv(self, tenant_schema, project_id):
        filename = "datamap.csv"
        to_path = "data"
        url = f"/api/v2/datasets/user_dataset/{tenant_schema}/{project_id}"
        response = self.api_client.DOWNLOAD_FILE(to_path=to_path, filename=filename, url=url)

    def get_latest_labels(self, project_id, experiment_id):
        filename = "labels.csv"
        to_path = "data"
        
        url = f"/api/v2/labeling_task/latest_labels/{project_id}/{experiment_id}"
        response = self.api_client.DOWNLOAD_FILE(to_path=to_path, filename=filename, url=url)
