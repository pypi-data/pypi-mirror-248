...
# TODO Reanem the class to LocationsLocal
# TODO Rename the file to locations_local

# TODO Create LocationsLocal.get_country_id_by_location_id() and call it from importer-local-python-package  # noqa501
...
class LocationsLocal(GenericCRUD):
    def __init__(self):
        ...
        super().__init__(default_schema_name=LocationLocalConstants.LOCATION_SCHEMA_NAME,  # noqa501
                         default_table_name=LocationLocalConstants.LOCATION_TABLE_NAME,  # noqa501
                         default_view_table_name=LocationLocalConstants.LOCATION_VIEW_NAME,  # noqa501
                         default_id_column_name=LocationLocalConstants.LOCATION_ID_COLUMN_NAME)  # noqa501

        logger.end("end init LocationLocal")

    def get_location_ids(self, neighborhood_name: str, county_name: str,
                         region_name: str, state_name: str, country_name: str, city_name: str) -> tuple([int, int, int, int, int, int]):  # noqa501
        logger.start("start get location ids",
                     object={'neighborhood_name': neighborhood_name, 'county_name': county_name,  # noqa501
                     'region_name': region_name, 'state_name': state_name, 'country_name': country_name, 'city_name': city_name})  # noqa501
        ...
        logger.end("end get location ids",
                   object={"neighborhood_id": neighborhood_id, "county_ids": county_id, "region_ids": region_id,  # noqa501
                           "state_ids": state_id, "country_id": country_id,
                           'city_id': city_id})
        return (neighborhood_id, county_id, region_id, state_id, country_id, city_id)  # noqa501

    def insert(
            self, data: Dict[str, any],
            lang_code: LangCode = user_context.get_effective_profile_preferred_lang_code(),  # noqa501
            is_approved: bool = True,
            new_country_data: Dict[str, any] = None) -> int:

        logger.start("start insert location",
                     object={'data': data, 'lang_code': lang_code,
                     'is_approved': is_approved, 'new_country_data': new_country_data})  # noqa501 

        (neighborhood_id, county_id, region_id, state_id, country_id, city_id) = self._check_details_and_insert_if_not_exist(  # noqa501
            data.get("coordinate"),
        ...
        location_json = {
                key: value for key, value in {
                    'coordinate': data.get("coordinate"),   # noqa501
                    'address_local_language': data.get("address_local_language"),  # noqa501
    ...
    def update(self, location_id: int, data: Dict[str, any],
        ...
        (neighborhood_id, county_id, region_id, state_id, country_id, city_id) = self._check_details_and_insert_if_not_exist(  # noqa501
            data.get("coordinate"), (data.get("neighborhood"), data.get("county"),  # noqa501
            data.get("region"), data.get("state"), data.get("country"), data.get("city")),  # noqa
    ...
    def read(self, location_id: int):
        ...
        result = super().select_one_dict_by_id(id_column_value=location_id,
                                                select_clause_value=LocationLocalConstants.LOCATION_TABLE_COLUMNS)  # noqa501
    ...
    def _check_details_and_insert_if_not_exist(
            ...
            new_country_data: Dict[str, any] = None) -> tuple(
            [int, int, int, int, int, int]):
        logger.start("start _check_details_and_insert_if_not_exist",
                     object={'coordinate': coordinate, 'location_details': location_details,  # noqa501
                     'lang_code': lang_code, 'new_country_data': new_country_data})  # noqa501
        ...
        if neighborhood_id is None and neighborhood_name is not None:
            ...
            neighborhood_id = neighborhood_object.insert(coordinate=coordinate,
                                                         neighborhood=neighborhood_name,  # noqa501
                                                         lang_code=lang_code,
                                                         title_approved=is_approved)  # noqa501
        ...
        if country_id is None and country_name is not None:
            ...
            country_id = country_object.insert(coordinate=coordinate,
                                               country=country_name,
                                               lang_code=lang_code,
                                               title_approved=is_approved,
                                               new_country_data=new_country_data)  # noqa501
        ...
        logger.end("end _check_details_and_insert_if_not_exist",
                   object={'neighborhood_id': neighborhood_id, 'county_id': county_id,  # noqa501
                   'region_id': region_id, 'state_id': state_id, 'country_id': country_id, 'city_id': city_id})  # noqa501
