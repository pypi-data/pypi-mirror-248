...
namespace ampspy
{
  namespace shims
  {
    ...
    bool init(PyObject* module_)
    {
      ...
      PySys_SetObject(const_cast<char*>("exitfunc"), PyDict_GetItemString(PyModule_GetDict(module_), const_cast<char*>(g_shimExitFuncName)));
      ...
      else if (!getSymbol("_Py_IsFinalizing", &ampspy::shims::Py_IsFinalizing))
      {
        ...
        PyErr_SetString(PyExc_RuntimeError, "Error locating _Py_Finalizing or _Py_IsFinalizing; cannot load AMPS.");
        return false;
      }

      if (!getSymbol("_PyThreadState_UncheckedGet", &ampspy::shims::PyThreadState_UncheckedGet))
      {
        PyErr_SetString(PyExc_RuntimeError, "Error locating _PyThreadState_UncheckedGet; cannot load AMPS.");
        return false;
      }
      if (!getSymbol("PyUnicode_AsUTF8AndSize", &ampspy::shims::PyUnicode_AsUTF8AndSize))
      {
        PyErr_SetString(PyExc_RuntimeError, "Error locating PyUnicode_AsUTF8AndSize; cannot load AMPS.");
        return false;
      }
#endif
      return true;
