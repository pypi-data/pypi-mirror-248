...
class DataCoef:
    ...
    def __check_xy_dim(self, x, y, axis):
        ...
        if axis is None:
            ...
            return x, y
        if x.ndim < y.ndim:
            x = np.broadcast_to(np.expand_dims(x, tuple(np.delete(np.arange(y.ndim), axis))), y.shape)
        else:
            y = np.broadcast_to(np.expand_dims(y, tuple(np.delete(np.arange(x.ndim), axis))), x.shape)
    ...
    def coef(self):
        self.cc = np.clip(self.mean(self.norm_x * self.norm_y, axis=self.axis, keepdims=True), 0, 1)
        self.t = self.cc * np.sqrt(self.df)/np.sqrt(1 - np.clip(self.cc, 0, 0.9999) ** 2)
