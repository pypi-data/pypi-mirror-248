...
"""
Core implementation of ezysqlite proxies
"""
...
LOG = logging.getLogger("ezysqlite")
...
class Connection(Thread):
    def __init__(
        ...
        if loop is not None:
            warn(
                "ezysqlite.Connection no longer uses the `loop` parameter",
                DeprecationWarning,
            )
    ...
    async def cursor(self) -> Cursor:
        """Create an ezysqlite cursor wrapping a sqlite3 cursor object."""
    ...
    async def backup(
        ...
        *,
        ...
        Takes either a standard sqlite3 or ezysqlite Connection object as the target.
        """
...
def connect(
    ...
    **kwargs: Any,
    ...
    if loop is not None:
        warn(
            "ezysqlite.connect() no longer uses the `loop` parameter",
            DeprecationWarning,
        )
