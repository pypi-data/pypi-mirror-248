...
"""
Simple perf tests for ezysqlite and the asyncio run loop.
"""
import string
import tempfile
import time
...
class PerfTest(TestCase):
    ...
    async def test_connection_memory(self):
        while True:
            yield
            async with ezysqlite.connect(TEST_DB):
                pass
    ...
    async def test_connection_file(self):
        with tempfile.NamedTemporaryFile(delete=False) as tf:
            ...
            async with ezysqlite.connect(path) as db:
                await db.execute(
            ...
            while True:
                yield
                async with ezysqlite.connect(path):
                    pass
