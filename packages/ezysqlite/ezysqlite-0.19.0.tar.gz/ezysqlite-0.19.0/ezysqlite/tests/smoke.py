...
class SmokeTest(TestCase):
    ...
    async def test_connection_await(self):
        db = await ezysqlite.connect(TEST_DB)
        self.assertIsInstance(db, ezysqlite.Connection)
    ...
    async def test_connection_context(self):
        async with ezysqlite.connect(TEST_DB) as db:
            self.assertIsInstance(db, ezysqlite.Connection)
    ...
    async def test_connection_locations(self):
        ...
        for loc in locs:
            async with ezysqlite.connect(loc) as db:
                cursor = await db.execute("select * from foo")
    ...
    async def test_multi_loop_usage(self):
        ...
        async with ezysqlite.connect(":memory:") as db:
    ...
    async def test_connection_properties(self):
        async with ezysqlite.connect(TEST_DB) as db:
            ...
            async with db.cursor() as cursor:
                cursor.row_factory = ezysqlite.Row
                self.assertEqual(cursor.row_factory, ezysqlite.Row)
                await cursor.execute("select * from test_properties")
                row = await cursor.fetchone()
                self.assertIsInstance(row, ezysqlite.Row)
            ...
            self.assertEqual(db.row_factory, ezysqlite.Row)
    ...
    async def test_connect_error(self):
        ...
        with self.assertRaisesRegex(OperationalError, "unable to open database"):
            async with ezysqlite.connect(bad_db) as db:
                self.assertIsNone(db)  # should never be reached

        with self.assertRaisesRegex(OperationalError, "unable to open database"):
            db = await ezysqlite.connect(bad_db)
            self.assertIsNone(db)  # should never be reached
    ...
    async def test_backup_ezysqlite(self):
        def progress(a, b, c):
            print(a, b, c)

        async with ezysqlite.connect(":memory:") as db1, ezysqlite.connect(
            ":memory:"
        ) as db2:
    ...
    async def test_backup_sqlite(self):
        async with ezysqlite.connect(":memory:") as db1:
