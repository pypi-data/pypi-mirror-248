...
class AsyncOpcuaClient:
    ...
    async def __aexit__(self, *args) -> None:
        ...
        self._connection_status = ConnectionStatus.TERMINATE
    ...
    def persistent_connect(self) -> None:
        self._connection_thread = threading.Thread(target=asyncio.run, args=(self._persistent_connect_thread(),))
    ...
    def _log_connection_status(self, level=logging.INFO) -> None:
        logger.log(level, f'Connection status: {self._connection_status.name}')
    ...
    async def _persistent_connect_thread(self) -> None:
        ...
        while True:
            ...
            elif self._connection_status == ConnectionStatus.TERMINATE:
                break
    ...
    async def subscribe_all(
        ...
        if not self._connection_status == ConnectionStatus.CONNECTED:
            logger.warning(
