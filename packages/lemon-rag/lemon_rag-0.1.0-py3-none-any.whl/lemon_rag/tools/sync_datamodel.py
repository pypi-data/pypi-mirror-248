...
with open(os.path.join(BASE_DIR, "lemon_rag", "lemon_models", "models_template.py"), "r") as template_file:
...
class DesignClient(BaseModel):
    ...
    def list_document(self) -> List[Document]:
...
def assemble_data_models_code(model_document: ModelDocument) -> str:
    ...
    for model in model_document.models:
        ...
        for relationship in out_relationships:
            ...
            lines.append(f"    {relationship.frontref}: Union[peewee.ForeignKeyField, '{target_model_type}']")
...
def generate_data_model_stub():
    last_model_version: Optional[Document] = None
    client = DesignClient(**config.lemon.dict())
    while True:
        res = client.list_document()
        data_model_document = extract_data_model_document_uuid(res)

        if last_model_version is None or last_model_version.document_version < data_model_document.document_version:
            model_document: ModelDocument = client.get_data_model(data_model_document.document_uuid)
            with open(os.path.join(BASE_DIR, "lemon_rag", "lemon_models", "models.py"), "w") as f:
                f.write(base_model_file_template)
                f.write(assemble_data_models_code(model_document))
        last_model_version = data_model_document

        time.sleep(1)


if __name__ == '__main__':
    generate_data_model_stub()
