...
class Bot:
	def __init__(self,login:bool=False,auth=None,key=None):
		self.auth = auth
		self.key = key
		self.print = chop
		if login == False:
	...
	async def post_data_ssesion(self,data):
				...
				async with ClientSession() as session:
						async with session.post(url=f"https://shadmessenger{randint(1,300)}.iranlms.ir/",data=json.dumps({"api_version":"6","tmp_session":session_tmp,"data_enc":enc.encrypt(json.dumps(data))})) as res:
	...
	async def post_data(self,data):
				...
				async with ClientSession() as session:
						async with session.post(url=f"https://shadmessenger{randint(1,300)}.iranlms.ir/",data=jso) as res:
							result = await res.json()
