...
def typed_dict_defaults(__dct: T, *, __sig: Signature | None = None, **kwargs: TypedDict) -> T:
...
def typed_dict_func(__dct: T, __sig: Signature | None = None) -> Callable[[T, Signature], Callable[P, T]]:
    return partial(typed_dict_defaults, __dct, __sig=__sig)
...
class typedict(dict):
    ...
    def __init__(self: Self, *args, **kwargs):
        ...
        kwds = typed_dict_defaults(type(self).__dct, __sig=type(self).__sig, **kwargs)
...
def setkeywords(d: TypedDict, **kwargs: TypedDict):
    ...
    def decorator(func: Callable):
        signature = typed_dict_signature(d, **kwargs)
        construct = typed_dict_func(d, signature)
