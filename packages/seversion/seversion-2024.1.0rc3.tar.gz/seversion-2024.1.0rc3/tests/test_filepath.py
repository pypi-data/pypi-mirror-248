...
import seCore.filepath
from seCore import filepath
from seCore.win32 import ERROR_DIRECTORY

symlinkSkip = not True
...
class BytesTestCase(seTestCase):
    """
    ...
    def mktemp(self):
        ...
        """
        return seTestCase.mktemp(self).encode("utf-8")
...
class AbstractFilePathTests(BytesTestCase):
    ...
    def test_dictionaryKeys(self):
        ...
        dic2id = {f1: 3, f1prime: 4}
        self.assertEqual(dic2id[f1], 4)
        self.assertEqual(list(dic2id.keys()), [f1])
        self.assertIs(list(dic2id.keys())[0], f1)
        self.assertIsNot(list(dic2id.keys())[0], f1prime)  # sanity check
        dic2id[f2] = 5
        self.assertEqual(dic2id[f2], 5)
        self.assertEqual(len(dic2id), 2)
    ...
    def test_dictionaryKeyWithString(self):
        ...
        """
        f1 = self.path.child(b"file1")
        dic2id = {f1: "hello", f1.path: "goodbye"}
        self.assertEqual(len(dic2id), 2)
    ...
    def test_newTimesAreFloats(self):
        ...
        for p in self.path, self.path.child(b"file1"):
            ...
            self.assertEqual(type(p.getModificationTime(isNone=True)), float)
            self.assertEqual(type(p.getStatusChangeTime(isNone=True)), float)

    def test_DeviceIsInt(self):
        for p in self.path, self.path.child(b"file1"):
            self.assertEqual(type(p.getDevice()), int)

    def test_InodeNumberIsInt(self):
        for p in self.path, self.path.child(b"file1"):
            self.assertEqual(type(p.getInodeNumber()), int)
            self.assertEqual(type(p.getInodeNumber(isNone=True)), int)

    def test_NumberOfHardLinksIsInt(self):
        for p in self.path, self.path.child(b"file1"):
            self.assertEqual(type(p.getNumberOfHardLinks()), int)

    def test_UserIDIsInt(self):
        for p in self.path, self.path.child(b"file1"):
            self.assertEqual(type(p.getUserID()), int)

    def test_GroupIDIsInt(self):
        for p in self.path, self.path.child(b"file1"):
            self.assertEqual(type(p.getGroupID()), int)

    def test_getPermissions(self):
        for p in self.path, self.path.child(b"file1"):
            self.assertEqual(type(p.getPermissions()), seCore.filepath.Permissions)
            # print()
            # print(p.getPermissions())
            # print(type(p.getPermissions()))
...
class ExplodingFilePath(filepath.FilePath):
    ...
    @ivar: The L{ExplodingFile} instance most recently returned from the
        C{open} method.
    """

    def __init__(self, pathName, originalExploder=None):
        """
        Initialize an L{ExplodingFilePath} with a name and a reference to the FilePath
...
class PermissionsTests(BytesTestCase):
    ...
    def test_repr(self):
        self.assertEqual(repr(filepath.Permissions(0o777)), repr(filepath.Permissions(0o777)))
...
class FilePathTests(AbstractFilePathTests):
    ...
    def test_Touch(self):
        p = self.path.child(b"__init__.py")
        print(p.touch())
    ...
    @skipIf(True, "Need to look into this")
    ...
    def test_openWithExplicitBinaryMode(self):
        ...
        See https://bugs.python.org/issue7686 for details about the bug.
        """
    ...
    def test_whetherBlockOrSocket(self):
        ...
        self.assertFalse(self.path.isBlockDevice(isNone=True))
        self.assertFalse(self.path.isSocket())
        self.assertFalse(self.path.isSocket(isNone=True))

    def test_isAbsolutePath(self):
        self.assertTrue(self.path.isabs())

    def test_splitext(self):
        # self.assertTrue(self.path.splitext())
        print()
        self.assertEqual(type(self.path.splitext()), tuple)
...
class UnicodeFilePathTests(TestCase):
    ...
    #     """
    #     C{siblingExtension} called with L{bytes} on an L{unicode}-mode
    #     L{FilePath} will return an L{bytes}-mode L{FilePath}.
    #     """
    #     fp = filepath.FilePath("./mon\u20acy")
    #     sibling = filepath.FilePath(fp._asTextPath() + ".txt")
    #     sibling.touch()
    ...
    #     """
    #     C{siblingExtension} called with L{unicode} on an L{unicode}-mode
    #     L{FilePath} will return an L{unicode}-mode L{FilePath}.
    #     """
    #     fp = filepath.FilePath("./mon\u20acy")
    #     sibling = filepath.FilePath(fp._asTextPath() + ".txt")
    #     sibling.touch()
    ...
    def test_unicodeChildSearchPreauth(self):
        """
        C{childSearchPreauth} called with L{unicode} on an L{unicode}-mode
...
class CleanUp(seTestCase):

    def test_999_cleanup(self):
        """Remove folder created during testing"""
        oDir = self.gettemp()
        APP_DIR = os.path.join(os.getcwd(), oDir)
        print("")
        print(f'       ROOT: {APP_DIR}')
        for file in glob.glob(f'{APP_DIR}'):
