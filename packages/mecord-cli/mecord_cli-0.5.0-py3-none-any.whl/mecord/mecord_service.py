...
class MecordPackageThread(Thread):
    ...
    def run(self):
        while (os.path.exists(stop_thread_file) == False):
            time.sleep(10)
            if calendar.timegm(time.gmtime()) - self.last_check_time > 30:
                self.last_check_time = calendar.timegm(time.gmtime())
                try:
                    #update widget
                ...
                try:
                    #update cli
                    isProduct = store.is_product()
                    threadNum = store.get_multithread()
                    remote_config = json.loads(xy_pb.GetSystemConfig(xy_pb.supportCountrys(isProduct)[0], "mecord_cli_version"))
    ...
    start /B {restart_command}''')
                            ...
                            if len(self.getCommandResult("which at")) <= 0:
                                def run_subprocess(s):
                                    r = subprocess.run(f"apt-get update", stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
                                    print(f"{r.stdout}\n{r.stderr}")
