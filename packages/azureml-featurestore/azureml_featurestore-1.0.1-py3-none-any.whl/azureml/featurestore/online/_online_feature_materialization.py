...
def materialize_online(feature_set, dataframe_to_store, materialization_version=None):
    ...
    if not feature_set.online_store.target:
        raise Exception(
            f'Featureset "{feature_set.arm_id}" belongs to a featurestore that does not specify an online store'
            " connection."
        )

    if feature_set.online_store.type != OnlineStoreType.REDIS:
        raise Exception(
            f'Featureset "{feature_set.arm_id}" specifies an online store connection of type'
            f' "{feature_set.online_store.type}". Only "redis" online stores are currently supported.'
        )
    ...
    dataframe_to_store.rdd.foreachPartition(lambda x: saveIntoRedisViaRedisScript(x))  # pylint: disable=unnecessary-lambda
