...
class ConfigGenerator:
            ...
            if self.uri.startswith(EConfigType.VMESS.value[1]):
                ...
                if not self.try_parse_new_vmess(config, allow_insecure):
                    if "?" in self.uri:
                        ...
                        vnext.users[0].security = DEFAULT_SECURITY if not vmess_qr_code.get(
                            "scy") else vmess_qr_code.get("scy")
    ...
    def try_resolve_vmess_4_kitsunebi(self, config):
        try:
            ...
            if config.outboundBean and config.outboundBean.settings and config.outboundBean.settings.vnext:
                vnext = config.outboundBean.settings.vnext[0]
    ...
    def try_resolve_sip002(self, config):
        try:
            ...
            if config.outboundBean and config.outboundBean.settings and config.outboundBean.settings.servers:
                server = config.outboundBean.settings.servers[0]
                server.address = uri.hostname
                server.port = uri.port
