...
async def run(
    ...
    # Note: it's possible that function ref is not found here
    running[tag]["ref"] = ref
    ...
    try:
        ...
        if hub.OPT.idem.get("hard_fail"):
            raise
        # In case of exception, keep 'ctx' in call_kwargs so that it is available in future calls
        call_kwargs["ctx"] = chunk["ctx"]
