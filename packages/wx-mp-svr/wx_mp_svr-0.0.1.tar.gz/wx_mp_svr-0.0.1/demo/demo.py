...
def main():
    token = "LFSCAB52r2ZAmRabBnReGANcuG"
    aes_key = "5oxqmuyUdvmuBtSJl69oEE16kblLtQq8iyNAfhgaKE9"
    corp_id = "wx7f20299a005e4acf"
    host = "127.0.0.1"
    port = 5001
    server = Server("wx_svr", host, port, "/wx_mp_svr", token=token, aes_key=aes_key, corp_id=corp_id)
    ...
    def event_handler(req_msg):
        # if req_msg.event_type == 'subscribe':
        rsp_msg = WxMpRspMsg.create_msg("text")
        rsp_msg.to_user_name = req_msg.from_user_name
        rsp_msg.from_user_name = req_msg.to_user_name
        rsp_msg.content = '欢迎关注<a href="">【老白码农在奋斗】</a>'
        return rsp_msg
