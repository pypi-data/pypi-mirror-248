...
class IndustryOverview:
    ...
    def get_all(self):
        ...
        if self.industry_name in ('光伏', '动力电池', '汽车', '半导体'):
            ...
            score_rd['研发增速_分位水平'] = score_rd['IDX'].rolling(5).apply(lambda x: quantile_definition(x, '研发增速', score_rd))
            score_rd['研发投入占比_分位水平'] = score_rd['IDX'].rolling(5).apply(lambda x: quantile_definition(x, '研发投入占比', score_rd))
            score_rd['研发投入_分位水平'] = score_rd[['研发增速_分位水平', '研发投入占比_分位水平']].mean(axis=1)
            score_rd['研发投入'] = score_rd['研发投入_分位水平'].apply(lambda x: 1 if x <= 0.2 else 2 if x <= 0.4 else 3 if x <= 0.6 else 4 if x <= 0.8 else 5 if x <= 1.0 else np.nan)
            ##################
            score_innerleverage = industry_finance[['产业链内部杠杆']].copy(deep=True)
            score_innerleverage['IDX'] = range(len(score_innerleverage))
            score_innerleverage['产业链内部杠杆_分位水平'] = score_innerleverage['IDX'].rolling(5).apply(lambda x: quantile_definition(x, '产业链内部杠杆', score_innerleverage))
            score_innerleverage['内部杠杆'] = score_innerleverage['产业链内部杠杆_分位水平'].apply(lambda x: 1 if x <= 0.2 else 2 if x <= 0.4 else 3 if x <= 0.6 else 4 if x <= 0.8 else 5 if x <= 1.0 else np.nan)
            ##################
            ...
            ##################
            score_data = pd.concat([score_pettm[['PE_TTM']], score_pblf[['PB_LF']], score_sentiment[['拥挤度']], score_fundholding[['公募持仓']], score_earnings[['盈利']], score_growth[['成长']], score_capex[['资本开支']], score_rd[['研发投入']], score_innerleverage[['内部杠杆']], score_cashflow[['现金流']]], axis=1)
