...
def import_tray(widgets):
    ...
    with open(filename, 'r', encoding='utf-8') as infile:
        for line_number, line in enumerate(infile):
            ...
            sipm_definition = line[0].startswith('sipm_')
            tray = line[0].startswith('tray')
            waf_lot = line[0].startswith('lot')
            waf_num = line[0].startswith('wafer_number')
            fail = False
            ...
            if waf_lot:
                lot_number = lot_wafer(line)
                if lot_number is None:
                    fail = True

            elif waf_num:
                wafer_number = lot_wafer(line)
                if wafer_number is None:
                    fail = True

            elif sipm_definition:
                # long form: 5 values
                # short form: 3 values
                ...
                try:
                    ...
                    try:
                        ...
                        # This is likely to be a line starting with sipm_xx
                        # and no following information (an unfilled template
                        # line), so just allow this to be silently ignored.
                        pass
                    else:
                        # short form
                        # sipm_number, wafer_column, wafer_row
            ...
            if fail:
                print_to_console(
                    widgets,
                    f'Check line {line_number + 1}: "{no_comment.strip()}"'
                )
