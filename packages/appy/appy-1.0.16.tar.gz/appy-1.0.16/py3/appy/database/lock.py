...
class Lock:
    ...
    @classmethod
    def hasOn(class_, o, ignoreMine=True):
        '''Returns True if at least one lock is currently set on p_o'''
        ...
        # this is also a technical requirement, because auto-locks may pose
        # problems when transitions are triggered programmatically. Let's take
        # an example. Suppose some object is created from the UI, and, in its
        ...
        if not locks or not ignoreMine: return
        # Return True if at least one lock is found, that is not mine
