...
def set_config(src: Annotated[str, Option()] = ""):
    ConfigUtils.mutate_global_config(src)
    
@cli.command("set-config-section")
def set_config_section(section: Annotated[ConfigSection, "section"] = None, src: Annotated[str, Option()] = ""):
    # @audit No src signifies wiping the old section.
    if not section: return Logger.error("No section provided!")

    ConfigUtils.mutate_global_config_section(section, src)
    ...
