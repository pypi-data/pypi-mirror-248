...
from compiloor.constants.environment import (
    BASE_CONFIG_SCHEMA, BASE_FINDING_MD_TEMPLATE, CONFIG_NAME, CONFIG_SECTION_DEFAULT,
...
def initialize_directory(force: bool = False) -> None:
    ...
    try:
        ...
        if not isfile(ConfigUtils.get_global_config_location()):
            config = dumps(BASE_CONFIG_SCHEMA, indent=4)
            ConfigUtils.mutate_global_config(config, False)
        else:
            config = open(ConfigUtils.get_global_config_location()).read()
        ...
        for section in ConfigSection:
            ...
            _section_file: str = section.value + "_content.md"
            _section_dir: str = ConfigUtils.get_global_config_location().replace(CONFIG_NAME, join("sections",_section_file))
            
            if not isfile(_section_dir):
                print(_section_dir)
                ConfigUtils.mutate_global_config_section(section, CONFIG_SECTION_DEFAULT, False)
                _content = CONFIG_SECTION_DEFAULT
            else:
                print(_section_dir)
                _content = open(_section_dir).read()
            
            open(join(_abs_main_dir, "sections", _section_file), "w").write(_content)
