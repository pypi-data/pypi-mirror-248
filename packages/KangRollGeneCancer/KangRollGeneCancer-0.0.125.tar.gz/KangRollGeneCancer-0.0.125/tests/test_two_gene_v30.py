...
def init_boundary_H3():
    # global Boundary_H3
    Boundary_H3 = np.array([1e-10, 1, 1e-3, 0]) #存储边界值 
    return Boundary_H3
...
def apply_boundary_correction_advanced(H3_ini, delta_combined,Boundary_H3, weight=0.2):
    ...
    if H3_ini[0] > 0.001 and H3_ini[2] < 1e-6:
        ...
        Boundary_H3[0] = (1+0.05)* Boundary_H3[0]  # 边界值动态调整
...
if __name__ == "__main__":
    # 四组不同的损伤对
    damage_pairs = [(0.01, 0.0) ,(0.12, 0.10), (0.15, 0.12) ,(0.2, 0.2)]
    ...
    for index, (v1_brca1_damage, v1_tp53_damage) in enumerate(damage_pairs):
        ...
        H3_ini = np.array([1e-10, 1, 5e-4, 0])
        ...
        for cycle in range(1,cycles):
            ...
            repair_duration = sleep_hours*20

            mutation_duration = 480 - repair_duration
            ...
            # H3_after_genamonic_tf[0] = H3_after_genamonic_tf[0] + effect_on_H3_0 *10* (Boundary_H3[0] - H3_after_genamonic_tf[0] + effect_on_H3_0)
            # H3_after_genamonic_tf[2] = H3_after_genamonic_tf[2] + effect_on_H3_2 *10* (Boundary_H3[2] - H3_after_genamonic_tf[2] + effect_on_H3_2 )
