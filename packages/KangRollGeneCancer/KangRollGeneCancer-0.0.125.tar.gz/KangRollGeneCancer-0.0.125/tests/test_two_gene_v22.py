...
def apply_boundary_correction_advanced(H3_ini, delta_combined, weight=0.5):
    ...
    # 检查边界条件并应用适当的修正
    if H3_ini[0] > 1 and H3_ini[2] < 1e-8:
        ...
        delta_ToSaveLife =np.array([1,0, 0, 0 ])
        # 情况2: 变异超边界，修复未超边界
        print(YELLOW +"情况2: 变异超边界，修复未超边界。进行部分修正。"+ RESET)
        H3_after_correction = H3_ini + weight * delta_ToSaveLife * (Boundary_H3 - H3_ini)

    elif H3_ini[2] < 1e-8:
        delta_ToSaveLife =np.array([0, 0, 0.1, 0 ])
        # 情况3: 变异未超边界，修复超边界
...
# 四组不同的损伤对
damage_pairs = [(0.1, 0.0)] #(0.0, 0.0), (0.01, 0.02) (0.5, 0.5),
...
for index, (v1_brca1_damage, v1_tp53_damage) in enumerate(damage_pairs):
    total_MLN_output = []
    u_ini = 0.01
    cycles = 365*3#365 * 1 # 5年周期
    H3_ini = np.array([1e-4, 5, 1e-2, 0])
