...
def apply_boundary_correction_advanced(H3_ini, delta_combined, weight=0.5):
    """
    ...
    if H3_ini[0] > 1 and H3_ini[2] < 1e-10:
        ...
        # 情况1: 变异超边界，修复超边界
        print("情况1: 变异超边界，修复超边界。需要特别的修正。")
        H3_after_correction = H3_ini + weight * delta_combined * (Boundary_H3 - H3_ini)

    elif H3_ini[0] > 1:
        delta_combined =np.array([0.1,1, 0, 1 ])
        # 情况2: 变异超边界，修复未超边界
        print("情况2: 变异超边界，修复未超边界。进行部分修正。")
        H3_after_correction = H3_ini + weight * delta_combined * (Boundary_H3 - H3_ini)

    elif H3_ini[2] < 1e-10:
        delta_combined =np.array([0,1, 0.1, 1 ])
        # 情况3: 变异未超边界，修复超边界
        print("情况3: 变异未超边界，修复超边界。进行部分修正。")
        H3_after_correction = H3_ini + weight * delta_combined * (Boundary_H3 - H3_ini)
...
def apply_boundary_correction(H3_ini, delta_combined, weight=1):
    """
    应用边界修正到和谐集合H3上。
    ...
    # 检查是否需要应用边界修正
    print('--------------Judge H3,H3_ini[0] > 1 ？',H3_ini[0] > 1 )
    print('--------------Judge H3_ini[2] < 1e-10？',H3_ini[2] < 1e-10)
    if H3_ini[0] > 1 or H3_ini[2] < 1e-10:
        delta_combined =[0.1,1, 0.1, 1 ]
        # 应用边界修正
        print('--Before----!!!!-------!!!! Beyond Boundary the Life is dangreous !!!!-----!!!------Takc Care of your slelf ---and You need Help！！')
        print('H3_ini = ',H3_ini)
        print('Boundary_H3  = ',Boundary_H3 )
        ...
        H3_after_correction = H3_ini + weight * delta_combined * (Boundary_H3 - H3_ini)
        print('--After --!!!!-------!!!! Beyond Boundary the Life is dangreous !!!!-----!!!------Takc Care of your slelf ---and You need Help！！')
        print('H3_after_correction  = ',H3_after_correction )
    else:
        # 不需要边界修正，正常修正
        H3_after_correction = H3_ini + weight * delta_combined * H3_ini
...
def fn011_combined_delta(v2_TP53, v2_BRCA1, u_ini, H3_ini, lay1_iterations, lay2_iterations, debug_mode=False, test_random=True):
    ...
    # delta_combined_example = np.array([-0.01, 0.02, -0.005, 0.0001])
    H3_after_genamonic_tf = apply_boundary_correction_advanced(H3_ini, delta_combined)
...
# 四组不同的损伤对
damage_pairs = [(0.9, 0.9)] #(0.0, 0.0), (0.01, 0.02) (0.5, 0.5),
...
for index, (v1_brca1_damage, v1_tp53_damage) in enumerate(damage_pairs):
    ...
    for cycle in range(1,cycles):
        ...
        # # H3_after_genamonic_tf[0] = H3_after_genamonic_tf[0] + (effect_on_H3_0 *0* H3_after_genamonic_tf[0] )
        # # H3_after_genamonic_tf[2] = H3_after_genamonic_tf[2] + (effect_on_H3_2 *10* H3_after_genamonic_tf[2] )
