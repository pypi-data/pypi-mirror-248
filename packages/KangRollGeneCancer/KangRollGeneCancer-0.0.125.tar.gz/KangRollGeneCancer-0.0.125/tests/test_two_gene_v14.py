...
# 四组不同的损伤对
damage_pairs = [(0.0, 0.0)]#, (0.2, 0.3), (0.4, 0.4), (0.9, 0.9)]
...
for index, (v1_brca1_damage, v1_tp53_damage) in enumerate(damage_pairs):
    total_MLN_output = []
    u_ini = 0.01
    cycles = 365 * 5  # 假设周期为5年，每年365天
    H3_ini_test = np.array([0.001, 1, 0.080, 0])
    ...
    for cycle in range(cycles):
        ...
        repair_duration = 24 - mutation_duration  # 修复周期

        # 突变周期的分析
        delta_combined, MLN_output_V3, NL_final_V4 = fn011_combined_delta(v2_TP53, v2_BRCA1, u_ini, H3_ini_test, mutation_duration, repair_duration, debug_mode=True, test_random=True)
