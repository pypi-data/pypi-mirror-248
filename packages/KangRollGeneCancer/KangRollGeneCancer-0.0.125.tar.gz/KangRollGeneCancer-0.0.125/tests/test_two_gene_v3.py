...
v2_TP53 = np.array([0, 1, 1, 0] + [0]*12 + [0.3])  # TP53 参数
v2_BRCA1 = np.array([0, 1, 1, 0] + [0]*12 + [0.3])  # BRCA1 参数
...
for cycle in range(cycles):
    # TP53的影响
    iterations_per_gene_TP53 = random.randint(100, 300)  # 随机选择迭代次数
    for i in range(iterations_per_gene_TP53):
        random.seed(42 + i)
        H3_ini_test = np.array([random.uniform(0.0003, 0.001), 1, 0.0018, 0])
        MLN_output_TP53, NL_final = fn010_translp(0.3, v2_TP53, u_ini, H3_ini_test, 50, 100)
        u_ini = MLN_output_TP53[-1]  # 更新u_ini为TP53影响后的最后一个值
        total_MLN_output.extend(MLN_output_TP53)

    # BRCA1的影响
    iterations_per_gene_BRCA1 = random.randint(100, 300)  # 随机选择迭代次数
    for i in range(iterations_per_gene_BRCA1):
        random.seed(42 + i + iterations_per_gene_TP53)
        H3_ini_test = np.array([random.uniform(0.0003, 0.001), 1, 0.0018, 0])
...
# 添加图例
plt.legend(["TP53 and BRCA1 Gene Alterations"])
