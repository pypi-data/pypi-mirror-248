...
v2_BRCA1 = np.array([0.1] * 16 + [0.2])  # 示例参数
...
for cycle in range(cycles):
    ...
    for i in range(iterations_per_gene):
        ...
        H3_ini_test = np.array([random.uniform(0.0003, 0.001), 1, 0.0018, 0])
        MLN_output_BRCA1, NL_final = fn010_translp(0.2, v2_BRCA1, u_ini, H3_ini_test, 50, 100)
...
plt.title('Multi-Layer Network Output with TP53 and BRCA1 Impact on Tumor Characteristics')
plt.xlabel('Hours')
plt.ylabel('Neoplastic Load Value')

# 添加图例
plt.legend(["TP53 Damage: 0.2, BRCA1 Damage: 0.1"])
