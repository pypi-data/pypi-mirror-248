...
import aplpy as apl
from fits_handler import FITSFigureV2
import astropy.io.fits as fits
from misc_tools import check_file, read_lst
...
def refine_psf(in_fits, in_lst, out_lst, fits_suffix=""):
    with plt.style.context('dark_background'):
        ...
        window["-STAR LIST-"].bind("<Up>", "up_arrow")
        window["-STAR LIST-"].bind("<Down>", "down_arrow")
        window["-STAR LIST-"].bind("<Left>", "left_arrow")
        window["-STAR LIST-"].bind("<Right>", "right_arrow")
        ...
        def canvas_update_star(star_id):
            ...
            xlims = (max(int(star['X'])-10, 0), int(star['X'])+10)
            ylims = (max(int(star['Y'])-10, 0), int(star['Y'])+10)
        ...
        while True:
            ...
            elif event in ["-STAR LIST-", "-STAR LIST-up_arrow", "-STAR LIST-down_arrow"]:
            ...
            elif event in ["-STAR LIST-left_arrow", "Add", 
                            "-STAR LIST-right_arrow", "Discard"] and len(values["-STAR LIST-"]):

                sel_star_id = int(values["-STAR LIST-"][0].split()[1])
                actual_index = window["-STAR LIST-"].get_indexes()[0]
                opt_list = window["-STAR LIST-"].get_list_values()
                next_index = (actual_index+1)%len(opt_list)
                if event in ["-STAR LIST-left_arrow", "Add"] and sel_star_id in dropped_stars_ids:
                    dropped_stars_ids.remove(sel_star_id)
                elif event in ["-STAR LIST-right_arrow", "Discard"] and sel_star_id not in dropped_stars_ids:
