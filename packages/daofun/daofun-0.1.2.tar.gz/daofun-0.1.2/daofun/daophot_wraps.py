from misc_tools import check_file, read_lst, read_als, find_ID 
import os


def find(in_fits, out_coo, sum_aver="1,1"):
...
def phot(in_fits, in_coo, out_ap, opt_file="photo.opt"):
	filename = os.path.splitext(os.path.basename(in_fits))[0]
...
def pick(in_fits, in_ap, out_lst, stars_minmag="200,20"):
	filename = os.path.splitext(os.path.basename(in_fits))[0]
...
def create_psf(in_fits, in_ap, in_lst, out_psf, out_nei, suffix=""):
	filename = os.path.splitext(os.path.basename(in_fits))[0]
...
def sub_fits(in_fits, in_psf, in_sub, out_fits, in_lst=False):
	filename = os.path.splitext(os.path.basename(in_fits))[0]
...
def allstar(in_fits, in_psf, in_ap, out_als, out_fits, RE=True):
	filename = os.path.splitext(os.path.basename(in_fits))[0]
	out_filename = os.path.splitext(os.path.basename(out_fits))[0]
...
def sync_lst_als(lst_in, als_in, lst_out):
