from daophot_opt import daophot_dict, phot_dict, allstar_dict
import numpy as np
import matplotlib.pyplot as plt
import matplotlib.cm as cm
from fits_handler import FITSFigureV2
from misc_tools import read_als
...
class DaoFun:
    ...
    # SECCION DAOPHOT
    def save_daophot_options(self, values):
...
"""
        with open('daophot.opt', 'w') as file:
            file.write(file_content)

    def save_daophot_advanced_options(self, values):
        for k in values.keys():
            if k in self.daophot_dict.keys():
                self.daophot_dict[k] = values[k]

    def save_phot_options(self, values):
        for k in values.keys():
...
"""
        ...
        with open('photo.opt', 'w') as file:
            file.write(file_content)

    def save_phot_advanced_options(self, values):
        for k in values.keys():
            if k in self.phot_dict.keys():
                self.phot_dict[k] = values[k]

    # COLUMNA ALLSTAR
    def save_allstar_options(self, values):   
        for k in values.keys():
...
"""
        with open('allstar.opt', 'w') as file:
            file.write(file_content)

    def save_allstar_advanced_options(self, values):
        for k in values.keys():
            if k[2:] in self.allstar_dict.keys():
                self.allstar_dict[k[2:]] = values[k]
