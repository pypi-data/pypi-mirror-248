...
class SARSAAgent:
    ...
    def policy(self, state):
        ...
        with torch.no_grad():
            probs_batch = softmax(action_values).detach().numpy()
            
        action = np.random.choice(self.num_actions, p=probs_batch[0])
        return action
    ...
    def agent_step(self, reward, state):
        ...
        if self.replay_buffer.size() > self.replay_buffer.minibatch_size and self.time_step>self.warmup_steps: # and self.episode_steps%self.replay_buffer.minibatch_size==0:
            for _ in range(self.num_replay):
                ...
                loss = optimize_network_sarsa(experiences, self.discount, self.optimizer, self.target_network, self.q_network,self.device)
