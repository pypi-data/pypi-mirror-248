...
class Client:
    ...
    def get_predicted_sessions(self, params: dict) -> List[Session]:
        ...
        response = requests.get(self.__base_url + '/sessions/predict', params=params, headers={'Authorization': 'Bearer ' + token.access_token})
    ...
    def create_session(self, session: Session) -> Session:
        token = self.__auth_client.token()
        create_params = {'satellite': {'id': session.satellite.id}, 'groundStation': {'id': session.ground_station.id}, 'startDateTime': session.start_datetime.isoformat(sep='T', timespec='auto'),
            'endDateTime': session.end_datetime.isoformat(sep='T', timespec='auto'), }
        response = requests.post(self.__base_url + '/sessions', data=json.dumps(create_params), headers={'Content-type': 'application/json', 'Authorization': 'Bearer ' + token.access_token})
