...
from abstract_utilities import safe_json_loads,is_number,safe_json_loads,make_list,filter_json_list_values,recursive_json_list
from abstract_utilities.list_utils import remove_from_list
from abstract_security import get_env_value
import requests
import os
from abstract_gui import AbstractWindowManager,sg
from abstract_utilities.list_utils import 
class ApiGUI:
...
class APIBridge:
    ...
    def update_rpc(self,rpc_mgr=None,rpc_js=None,rpc_gui=False,rpc_network=None,api_key=None,env_key=None):
        if rpc_gui:
    ...
    def make_api_call(self,api_url=None,api_gui=False,api_url_data=None):
        ...
        if api_gui:
            api_gui = ApiGUI(api_mgr=self)#choose_api_gui(address=address,contract_address=contract_address,start_block=start_block,end_block=end_block)
            self.api_url_data=f"{api_gui.api_url}"[:-len('&apikey=')]
    ...
    def _make_request(self, url):
            ...
            try:
                ...
                return None
    def derive_network(self,contract_address=None,api_data_type='sourcecode',multiple=False):
        networks_found = []
        ...
        for list_itteration,list_obj in enumerate([common_chains,rpc_list]):
            for network in list_obj:
                found = False
                if list_itteration == 0:
                    rpc_js = [network]
                self.update_api_call_partameters(rpc_js =rpc_js,address=False,contract_address=contract_address,api_data_type='sourcecode' if api_data_type in ['sourcecode','abi'] else api_data_type)
                if self.response and isinstance(self.response,list):
                    if api_data_type in ['sourcecode','abi'] and self.response[0] and isinstance(self.response[0],dict):
                        if "ContractName" in self.response[0].keys() and self.response[0].get("ContractName"):
                            found = True
                if found:
                    if multiple:
                        networks_found.append({"network":self.rpc_mgr.rpc_js,"contract_data":self.response})
                    else:
                        if isinstance(self.response,dict) and api_data_type == 'abi':
                            return self.response.get('ABI',self.response)
                        return self.response
