...
class ABIBridge:
    ...
    """
    def __init__(self,contract_address:str=None,api_mgr=None,api_gui=False,rpc_mgr=None,rpc_gui=False,rpc_js:dict=None):
        """
        ...
        self.checkSumRefference = create_and_read_json(self.check_sum_dict_path,{})
        self.update_rpc_mgr(rpc_mgr=rpc_mgr,rpc_js=rpc_js)
        self.update_api_mgr(api_mgr=api_mgr)
        self.update_contract_address(contract_address=contract_address)
    ...
    def update_contract_address(self,contract_address=None):
        self.contract_address = self.rpc_mgr.w3.to_checksum_address(contract_address)#self.try_check_sum(contract_address) or self.api_mgr.contract_address
        if self.contract_address:
            self.abi = self.get_abi(contract_address=self.contract_address)
    ...
    def update_rpc_mgr(self,rpc_mgr=None,rpc_js=None):
        ...
        if self.rpc_mgr == None:
            self.rpc_mgr = RPCBridge(rpc_js=self.rpc_js,rpc_gui=rpc_gui)
        elif self.rpc_js:
            self.rpc_mgr.update_rpc_js(rpc_js=self.rpc_js)
    def update_api_mgr(self,api_mgr=None):
        self.api_mgr = api_mgr
        if self.api_mgr == None:
            self.api_mgr= APIBridge(rpc_mgr=self.rpc_mgr,contract_address=self.contract_address,api_gui=api_gui)
    ...
    def check_source_code(self,contract_address):
        if contract_address and isinstance(contract_address,str):
            ...
            if normalized_address in os.listdir(self.source_code_directory):
                ...
                for item in contract_path_directory:
                    contract_path_network_directory = os.path.join(address_path,item)
                    if os.path.isdir(contract_path_network_directory):
                        source_code_path = os.path.join(contract_path_network_directory,'source_code.json')
                        if os.path.isfile(source_code_path):
                            return safe_json_loads(source_code_path)
    ...
    def create_abi_bridge(self,contract_address:str=None,abi:list=None,rpc_mgr=None,rpc_js=None,api_gui=False):
        ...
        contract_bridge = self.rpc_mgr.w3.eth.contract(address=contract_address, abi=self.abi)
