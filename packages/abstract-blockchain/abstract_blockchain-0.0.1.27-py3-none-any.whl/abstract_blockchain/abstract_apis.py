from abstract_webtool import DynamicRateLimiterManager,SafeRequest,UrlManager
from abstract_rpcs import RPCGUIManager,RPCBridge

from abstract_utilities import safe_json_loads,is_number
...
class ApiGUI:
    def __init__(self,rpc_mgr=None,
                 rpc_js:dict=None,
                 rpc_gui:bool=False,
                 address:str=None,
        ...
        if self.api_mgr == None:
            APIBridge(rpc_mgr=rpc_mgr,
                 ...
                 api_key=api_key,
                 env_key=env_key)
        api_keys = self.api_mgr.api_keys or list(apiCallDesc.keys())
        # Main Window Layout
        self.layout = [
        ...
        self.window_name = self.windows_mgr.add_window(title='API Call GUI', layout=layout,close_events=['Exit',"OK"],event_handlers=[self.while_window])
    ...
    def generate_api_gui(self,api_desc):
        ...
        for i,each in enumerate(inputs):
            ...
            layout.append([sg.T(text=f':',key=f"-{i}_TEXT-",visible=False),sg.Push(),
                           sg.Input(each_inp,size=(20,1),key=f'-{i}_INPUT-',visible=False),
                           sg.T(text='example:',key=f"-{i}_EXAMPLE_TEXT-",visible=False),
                           sg.Input(size=(30,1),key=f'-{i}_EXAMPLE_INPUT-',disabled=True,visible=False)])
    ...
    def generate_api_variables(self,values):
        ...
        for each in generated_url.split('?')[-1].split('&'):
            ...
            if api_val not in js_all[api_key]:
                js_all[api_key].append(api_val)
        return js_all
    def make_invisible_unless(self,visible_list,example):
        highest = 0
...
class APIBridge:
    def __init__(self,rpc_mgr=None,
        ...
        self.request_manager = SafeRequest(url_manager = self.url_mgr)
        self.rate_manager = DynamicRateLimiterManager()
        self.api_service_tracker={}
        self.update_rpc(rpc_js=rpc_js,rpc_gui=rpc_gui)
        ...
        self.update_api_call_partameters(address=address,contract_address=contract_address,start_block=start_block,end_block=end_block,api_data_type=api_data_type)
        print(f"self.contract_address = {self.contract_address}")
        print(f"self.api_data_type = {self.api_data_type}")
        print(f"self.address = {self.address}")
        
    def update_rpc(self,rpc_js=None,rpc_gui=False,rpc_network=None,api_key=None,env_key=None):
        if rpc_gui:
        ...
        self.get_api_key(rpc_network=rpc_network,api_key=api_key,env_key=env_key)
        self.service_name=None
        for service_name,api_keys in self.api_service_tracker.items():
            if self.api_key in api_keys:
                self.service_name = service_name
                break
        if self.service_name == None:
            self.service_names = list(self.api_service_tracker.keys())
            self.service_name = f'APIBridge_service_{len(self.service_names)-1}'
            self.api_service_tracker[self.service_name]=[self.api_key]
            self.rate_manager.add_service(service_name=self.service_name)
    ...
    def update_api_call_partameters(self,address=None,contract_address=None,start_block=None,end_block=None,api_data_type=None):
        self.update_address(address=None,contract_address=None)
        ...
        if self.address or self.contract_address:
            if self.api_data_type.lower() in ['source', 'code', 'abi'] and self.contract_address:
                self.api_url_data = f"module=contract&action=getabi&address={self.contract_address}"
            elif self.start_block and self.end_block:
                self.api_url_data = f"module=account&action=tokentx&address={address}&startblock={self.start_block}&endblock={self.end_block}&sort=asc"
            elif self.contract_address:
                self.api_url_data = f"module=account&action=tokenbalance&contractaddress={contract_address}&address={address}&tag=latest"
        elif self.contract_address:
            self.api_url_data = f"module=module=stats&action=tokensupply&contractaddress={contract_address}"
    def get_api_key(self,rpc_network=None,api_key=None,env_key=None):
        self.rpc_network = rpc_network or os.path.splitext(self.rpc_mgr.scanner)[0]
        if api_key:
    ...
    def make_api_call(self,api_url=None,api_gui=False,api_url_data=None):
        ...
        if api_gui:
            self.api_url = choose_api_gui(address=address,contract_address=contract_address,start_block=start_block,end_block=end_block)
...
api_mgr = APIBridge(api_data_type='abi',api_gui=False,contract_address=contract_address)
...
for rpc in rpc_list:
    ...
    api_mgr.update_rpc(rpc_js=rpc)
    api_mgr.make_api_call(api_gui=False)
    # Function to check if address is a contract
    #def is_contract(address, web3):
    #    APIBridge(api_data='abi',api_gui=False,rpc_js='avalanche',rpc_gui=False,contr='0xCf5DA7C09A115c2800249557aA8aa7963B8658B1')
