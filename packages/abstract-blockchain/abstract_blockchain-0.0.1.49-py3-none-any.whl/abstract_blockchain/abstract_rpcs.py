...
from abstract_gui import AbstractWindowManager,sg,make_component,create_row_of_buttons
    ...
    def get_rpc_values(self,values) -> dict or None:
        ...
        for key in ['-NETWORK_NAME-','-SYMBOL-','-CHAINID-','-NETWORK-','-BLOCK_EXPLORER-','-RPC-']:
            ...
            if value:
                ...
                need.append(key)
        new_rpc['rpc']=[{"url":new_rpc['rpc']}]
        new_rpc['explorer']=[{"url":new_rpc['explorer']}]
        return self.rpc_mgr.update_rpc_js(rpc_js=new_rpc)
