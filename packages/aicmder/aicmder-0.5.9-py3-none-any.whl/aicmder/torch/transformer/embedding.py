...
import inspect
from aicmder.torch.transformer.util_class import Elementwise

class SequenceTooLongError(Exception):
    pass

PADDING_INDEX = 0
_position_encoding_default = "default"
_position_encoding_absolute = "absolute"
_position_encoding = [_position_encoding_default, _position_encoding_absolute]


class TokenTypeEncoding(nn.Module):
    
    def __init__(self, type_vocab_size, dim):
        super().__init__()
        self.type_vocab_size = type_vocab_size
        self.dim = dim
        self.token_type_embeddings = nn.Embedding(self.type_vocab_size, self.dim)
        
    def forward(self, emb, token_type_ids, **kwargs):
        assert emb.shape[:-1] == token_type_ids.shape, "Token type should have same dims with inputs"
        token_type_embeddings = self.token_type_embeddings(token_type_ids)
        return emb + token_type_embeddings
        

class LearnPositionalEncoding(nn.Module):

    def __init__(self, dim, max_position_embeddings=512, dropout=0.1):
        super().__init__()
        self.dim = dim
        self.max_position_embeddings = max_position_embeddings
        self.pe = nn.Embedding(max_position_embeddings, self.dim)
        self.register_buffer("position_ids", torch.arange(max_position_embeddings).expand((1, -1)))
        ## do we need dropout?
        self.dropout = nn.Dropout(p=dropout)

    def forward(self, emb, **kwarg):
        # default position_ids is None
        past_key_values_length = 0
        seq_length = emb.size(1)
        if "position_ids" not in kwarg:
            position_ids = self.position_ids[:, past_key_values_length: seq_length + past_key_values_length]
        else:
            position_ids = kwarg["position_ids"] 
        emb = emb + self.pe(position_ids)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, emb, step=None, **kwarg):
        """Embed inputs.
        Args:
...
class Embeddings(nn.Module):
    ...
    def __init__(self, word_vec_size,
                 word_vocab_size,
                 word_padding_idx=PADDING_INDEX,
                 position_encoding=None,  # ["default", "learn"]
        ...
        if type_vocab_size is not None:
            token_type_encoding = TokenTypeEncoding(type_vocab_size, self.embedding_size)
            self.make_embedding.add_module('token_type', token_type_encoding) 


        self.position_encoding = position_encoding

        if self.position_encoding in _position_encoding:
            if self.position_encoding == _position_encoding_default:
                logger.info("default position encoding[sincos positional encoding] size[{}]".format(self.embedding_size))
                pe = PositionalEncoding(dropout, self.embedding_size)
                self.make_embedding.add_module('pe', pe)
            elif self.position_encoding == _position_encoding_absolute:
                logger.info("learn position encoding size[{}, {}]".format(max_position_embeddings, self.embedding_size))
                pe = LearnPositionalEncoding(self.embedding_size, max_position_embeddings=max_position_embeddings, dropout=dropout)
                self.make_embedding.add_module('pe', pe)
    ...
    def forward(self, source, step=None, token_type_ids=None):
        ...
        if self.position_encoding in _position_encoding:
            for i, module in enumerate(self.make_embedding._modules.values()):
                ...
                if len(forward_signature.parameters) > 1:
                    source = module(source, step=step, token_type_ids=token_type_ids)
                else:
                    source = module(source)
                    
        else:
            logger.debug(self.make_embedding)
...
if __name__ == "__main__":
    ...
    init_seeds()
    src_word_embeddings = Embeddings(300, 184866, 0, position_encoding="default")
    # src_word_embeddings = Embeddings(300, 184866, 0, position_encoding=_position_encoding_absolute, type_vocab_size=2)
    # src_word_embeddings = Embeddings(300, 184866, 0)
    sequence = torch.randint(0, 20000, (32, 5))
    
    inputs = sequence.unsqueeze(2)
    print(inputs.shape, sequence.shape, inputs.dim())
    token_type_ids = torch.ones(sequence.shape, dtype=torch.long)
    word_rep = src_word_embeddings(inputs, step=0, token_type_ids=token_type_ids)
    print(word_rep.shape, src_word_embeddings.word_padding_idx)
