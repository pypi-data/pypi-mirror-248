...
def _create_json_proxy_func(source_func: Callable, orm_model_class: Type[Model], add_flag: bool = False):
    """
    创建代理函数
...
def _create_form_proxy_old_func(source_func: Callable, orm_model_class: Type[Model], add_flag: bool = False):
    """
    创建代理函数
    :param source_func: 需要被代理的函数
    :param orm_model_class: orm模型类
...
def _create_param_convert_depend_func(module_code: str, orm_model_class: Type[Model]) -> Callable:
...
def _create_form_proxy_func(module_code: str, source_func: Callable, orm_model_class: Type[Model],
                            add_flag: bool = False):
    """
    创建代理函数
    :param source_func: 需要被代理的函数
...
def create_admin_router(admin_router_model: AdminRouterModel) -> tuple[APIRouter, APIRouter]:
    ...
    orm_model_class: Type[Model] = admin_router_model.orm_model_class
