...
class SettingsKeyEnum(Enum):
    """应用配置枚举"""
    app_name = SettingsKeyItem(name='app.name', comment="应用名", is_required=True)
    app_sub_title = SettingsKeyItem(name='app.sub_title', comment="副标题", is_required=True)
    app_code = SettingsKeyItem(name='app.code', comment="应用编码", is_required=True)
    app_description = SettingsKeyItem(name='app.description', comment="应用描述", is_required=True)
    app_keywords = SettingsKeyItem(name='app.keywords', comment="应用关键字", is_required=True)
    app_db_url = SettingsKeyItem(name='app.db_url', comment="应用数据库",  value_type=Union[str, dict[str, str]])
    ...
    server_port = SettingsKeyItem(name='server.port', comment="服务器监听端口", value_type=int, default=8081,
                                  is_required=True)
    server_origin = SettingsKeyItem(name='server.origin', comment="服务器域信息", value_type=str, is_required=True)
