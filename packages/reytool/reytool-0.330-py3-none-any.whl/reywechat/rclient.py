...
from reytool.rsystem import throw, search_process
...
class RClient(object):
    ...
    def request(
        ...
        api : API name.
        data : Request data.
        success_code : Suceess code, if unsuccessful, throw an exception.
            - `None` : Not handle.
    ...
    def get_contact_table(
        ...
        if type_ in (None, "room"):
            for row in table:
                if (
                    row["name"] == ""
                    and row["id"][-9:] == "@chatroom"
                ):
                    row["name"] = "群聊"
