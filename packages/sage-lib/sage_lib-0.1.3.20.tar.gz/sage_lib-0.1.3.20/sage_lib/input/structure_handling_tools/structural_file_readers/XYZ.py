...
class XYZ(FileManager):
    ...
    def export_as_xyz(self, file_location:str=None, save_to_file:str='w', verbose:bool=False,
                            species:bool=True, position:bool=True, forces:bool=True, charge:bool=True, magnetization:bool=True, lattice:bool=True, pbc:bool=True,
                            position_tag:str='pos', forces_tag:str='forces', charge_tag:str='charge', magnetization_tag:str='magnetization', pbc_tag:str='pbc') -> str:
        """
        ...
        for i in range(self.atomCount):
            ...
            print(self.total_force[i])
            pos = " ".join(map("{:12.6f}".format, self.atomPositions[i])) if position is not None else ''
            force = " ".join(map("{:14.6f}".format, self.total_force[i])) if force is not None else ''  # Assuming that self._total_force is an array (N, 3)
