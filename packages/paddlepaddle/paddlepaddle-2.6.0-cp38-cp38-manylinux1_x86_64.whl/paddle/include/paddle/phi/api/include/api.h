...
PADDLE_API Tensor weight_dequantize(const Tensor& x, const Tensor& scale, const std::string& algo = "weight_only_int8", DataType out_dtype = DataType::FLOAT16);

PADDLE_API Tensor weight_only_linear(const Tensor& x, const Tensor& weight, const paddle::optional<Tensor>& bias, const Tensor& weight_scale, const std::string& weight_dtype, int arch = 80);

PADDLE_API std::tuple<Tensor, Tensor> weight_quantize(const Tensor& x, const std::string& algo = "weight_only_int8", int arch = 80);
...
PADDLE_API std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor, Tensor> batch_norm(const Tensor& x, const Tensor& mean, const Tensor& variance, const paddle::optional<Tensor>& scale, const paddle::optional<Tensor>& bias, bool is_test, float momentum, float epsilon, const std::string& data_layout, bool use_global_stats, bool trainable_statistics);
...
PADDLE_API Tensor full_with_tensor(const Tensor& shape, const Tensor& value, DataType dtype = DataType::FLOAT32);
