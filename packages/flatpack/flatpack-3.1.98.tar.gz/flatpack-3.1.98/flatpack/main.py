...
async def startup_event():
    ...
    model_url = "https://github.com/romlingroup/flatpack-ai/raw/main/simulators/cobot/object_detection/model_fp16.tflite?raw=true"
...
def fpk_process_depth_map_np(image_np: np.ndarray, model_type: str = "DPT_Large") -> (np.ndarray, np.ndarray):
    ...
    return depth_colored, image_np


def fpk_create_detector(model_path, threshold=0.5):
    base_options = mp_python.BaseOptions(model_asset_path=model_path)
    options = mp_vision.ObjectDetectorOptions(base_options=base_options, score_threshold=threshold)
    return mp_vision.ObjectDetector.create_from_options(options)


def fpk_load_and_detect(detector, image_np):
    image = mp.Image.create_from_array(image_np, channel_order=mp.Image.ChannelOrder.RGB)
    return detector.detect(image)


def fpk_visualize(image_np, detection_result):
    for detection in detection_result.detections:
        ...
        text_position = (int(bbox.origin_x + MARGIN), int(bbox.origin_y + MARGIN))
        cv2.putText(image_np, label, text_position, cv2.FONT_HERSHEY_PLAIN, FONT_SIZE, TEXT_COLOR, FONT_THICKNESS)

    return image_np


def fpk_object_detection(image_np: np.ndarray) -> np.ndarray:
    global mp_detector
    detection_result = fpk_load_and_detect(mp_detector, image_np)
    annotated_image = fpk_visualize(image_np, detection_result)
    return annotated_image


@app.post("/process_depth_map_with_detection/")
async def process_depth_map_with_detection(file: UploadFile = File(...), model_type: str = "DPT_Large",
                                           model_path: str = 'model_fp16.tflite'):
    contents = await file.read()
    image = Image.open(io.BytesIO(contents))
    image_np = np.array(image.convert("RGB"))

    depth_map_with_boxes, original_image = fpk_process_depth_map_np(image_np, model_type)
    detection_result = fpk_object_detection(original_image, model_path)

    combined_result = np.where(detection_result.sum(axis=-1, keepdims=True) != 0, detection_result,
                               depth_map_with_boxes)

    _, encoded_img = cv2.imencode('.png', combined_result)
    return StreamingResponse(io.BytesIO(encoded_img.tobytes()), media_type="image/png")
