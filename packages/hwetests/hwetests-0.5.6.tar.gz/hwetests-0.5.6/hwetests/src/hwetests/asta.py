...
def run_experiment(alleles_count, population_amount, alleles_probabilities,
                   observed_probabilities, correction, i_j_to_var_obs_amount_observed, index_to_allele_, should_save_csv_, cutoff_value_):
        ...
        with open(file_name, 'w', newline='') as file:
            ...
            for i in range(alleles_count):
                for j in range(i, alleles_count):
                    ...
                    variance_val = expected_val * correction_val
                    i_j = f'{i}_{j}'
                    if i_j not in variance_val:
                        var_observed_value = '?'
                    else:
                        var_observed_value = i_j_to_var_obs_amount_observed[i_j][0]
...
def full_algorithm(file_path,
    ...
    # the set of all the observed i_j
    all_i_j_set = set()
    ...
    # i_j -> [var_observed(i,j), amount observed]
    i_j_to_var_obs_amount_observed = {}
    # i_j -> mu(i,j)
    i_j_to_mean = {}
    ...
    if should_save_csv:
        ...
        for current_id in id_to_i_j_to_i_j_observation:
            for i_j in id_to_i_j_to_i_j_observation[current_id]:
                if i_j not in i_j_to_mean:
                    i_j_to_mean[i_j] = 0.0
                i_j_to_mean[i_j] += (id_to_i_j_to_i_j_observation[current_id][i_j][2] / population_amount)
        # calculate observed variance
        for current_id in id_to_i_j_to_i_j_observation:
            for i_j in id_to_i_j_to_i_j_observation[current_id]:
                probability = id_to_i_j_to_i_j_observation[current_id][i_j][2]
                if i_j not in i_j_to_var_obs_amount_observed:
                    i_j_to_var_obs_amount_observed[i_j] = [0.0, 0]
                i_j_to_var_obs_amount_observed[i_j][0] += (probability - i_j_to_mean[i_j]) ** 2
                i_j_to_var_obs_amount_observed[i_j][1] += 1
        # some i_j's are observed only by part of the population, we need to add the zeros
        # observations to the variance
        for i_j in all_i_j_set:
            observed_amount = i_j_to_var_obs_amount_observed[i_j][1]
            i_j_to_var_obs_amount_observed[i_j][0] += (population_amount - observed_amount) * (i_j_to_mean[i_j] ** 2)
    ...
    p_value, chi_squared, dof = run_experiment(alleles_count=alleles_count,
                                               ...
                                               observed_probabilities=observed_probabilities,
                                               correction=correction,
                                               i_j_to_var_obs_amount_observed=i_j_to_var_obs_amount_observed,
