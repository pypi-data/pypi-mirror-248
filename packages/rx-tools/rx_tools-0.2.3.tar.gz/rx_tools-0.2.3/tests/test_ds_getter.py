...
def do_test(q2bin='jpsi', year='2016', trig='ETOS', kind='sign'):
    ...
    obj = dsg(q2bin, trig, year, version, data.partition, kind, data.selection)
    if kind == 'cmb' and trig == 'MTOS':
        rdf  = obj.get_df(remove_cuts=['mass', 'acceptance'])
    else:
        rdf  = obj.get_df(remove_cuts=['mass'])

    utnr.df_to_tex(df.cf.df_eff, f'{data.out_dir}/simple/{kind}_{q2bin}_{year}_{trig}.tex', hide_index=False)
    df.cf.df_cut.to_csv(f'{data.out_dir}/simple/{kind}_{q2bin}_{year}_{trig}.csv')
    ctf=rdf.cf
    ctf.to_json(f'{data.out_dir}/{data.tst_dir}/{kind}_{q2bin}_{year}_{trig}.json')
#-------------------------------------------------
def test_ss_data():
    data.tst_dir = 'ss_data'
    do_test(q2bin='jpsi', year='2018', trig='ETOS', kind= 'cmb')
    do_test(q2bin='jpsi', year='2018', trig='GTIS', kind= 'cmb')
