...
def validate_multi_yaml(file_to_validate, path_to_schema):
    ...
    schemas = yaml.load_all(schema_file, Loader=yaml.FullLoader)
    for file in file_content:
        valid = next(try_validate(file, schema) for schema in schemas)
        if not valid:
            raise SchemaValidationError(f"Failed to validate {file['apiVersion']} {file['kind']} {file['metadata']['name']} against any known schema.")
        print(f"Validated {valid}")
        yield file


def try_validate(file, schema):
    logger.info(f"Validating version={file['apiVersion']} kind={file['kind']} name={file['metadata']['name']}\nagainst\n\t{schema}")
    try:
