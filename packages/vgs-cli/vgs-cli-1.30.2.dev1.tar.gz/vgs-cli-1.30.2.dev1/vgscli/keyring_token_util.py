...
class KeyringTokenUtil:
    ...
    def validate_access_token(self):
        if self.get_access_token():
            token_json = jwt.decode(self.get_access_token(), options={"verify_signature": False})
    ...
    def validate_refresh_token(self):
        if self.get_refresh_token():
            token_json = jwt.decode(self.get_refresh_token(), options={"verify_signature": False})
    ...
    def is_access_token_azp_changed(self, azp) -> bool:
        return jwt.decode(self.get_access_token(), options={"verify_signature": False}).get('azp') != azp
