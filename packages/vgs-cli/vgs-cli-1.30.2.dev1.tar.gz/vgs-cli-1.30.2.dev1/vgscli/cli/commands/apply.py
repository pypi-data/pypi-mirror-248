...
def sync_http_route(payload, ctx, vault_id):
    ...
    vault_management_api = create_vaults_api(ctx, vault_id, ctx.obj.env, token_util.get_access_token())
...
HANDLERS = {
    ...
    ('apps.vgs.io/v1beta', 'MftRoute'): sync_mft_route
}
...
@click.pass_context
@handle_errors()
def all_vault_resources(ctx: click.Context, vault: Optional[ResourceId], file, dry_run) -> None:
    """
    Apply all vault resources (routes, preferences, certificates) to a single vault.
    """
    # print(f"{vault} {file} uscker")
    # print(file)
    parsed_resources = validate_multi_yaml(file, 'validation-schemas/vault-resources.yaml')
    for resource in parsed_resources:
        if dry_run:
            print(f"Pretending to send {resource['apiVersion']} {resource['kind']} {resource['metadata']['name']} to server")
        else:
