...
def get_config_path2() -> str:
    ...
    """
    config_path_path = helper.get_file("order.json")
    ...
    print(config_path_path)
    create_order(config_path_path)
    return config_path_path

def reset_config_path() -> None:
    config_path_path = helper.get_file("config_path.txt")
    if os.path.isfile(config_path_path):
        os.remove(config_path_path)
...
def create_order(config_path: Optional[str] = None) -> None:
    if config_path is None:
        config_file = get_config_path2()
    else:
...
EDITOR:
  ...
  SHOW_BAN_WARNING: False # Show a warning when editing bannable items.
...
SAVE_CHANGES:
  SAVE_CHANGES_ON_EDIT: True # Automatically save changes to your save data after using a feature.
...
def edit_config_path(_: Any) -> None:
    ...
    config_path = helper.select_dir(
        "select_config_path", config_path
    )
