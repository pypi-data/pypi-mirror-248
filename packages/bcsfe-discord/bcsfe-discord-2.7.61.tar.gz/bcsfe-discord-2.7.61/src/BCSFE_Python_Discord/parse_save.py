...
def next_int_len(number: int, save_data_g) -> dict[str, int]:
...
def get_length_data(
    length_bytes: int = 4, separator: int = 4, length: Union[int, None] = None, save_data_g: bytes = None
) -> list[int]:
    data: list[int] = []
    if length is None:
...
def get_length_doubles(
    length_bytes: int = 4, length: Union[int, None] = None, save_data_g: bytes = None
) -> list[float]:
    data: list[float] = []
    if length is None:
        length = next_int(length_bytes)
...
def get_double(save_data_g) -> float:
...
def parse_save(
    ...
    save_stats["game_version"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["version"] = country_code

    save_stats["unknown_1"] = next_int_len(1, save_data_g=save_data_g)

    save_stats["mute_music"] = next_int_len(1, save_data_g=save_data_g)
    save_stats["mute_sound_effects"] = next_int_len(1, save_data_g=save_data_g)

    save_stats["cat_food"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["current_energy"] = next_int_len(4, save_data_g=save_data_g)

    old_address = address
    new_address = find_date()
    set_address(old_address)
    extra = new_address - old_address
    save_stats["extra_time_data"] = next_int_len(extra, save_data_g=save_data_g)
    ...
    save_stats["unknown_flags_1"] = get_length_data(length=3, save_data_g=save_data_g)
    save_stats["upgrade_state"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["xp"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["tutorial_cleared"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["unknown_flags_2"] = get_length_data(length=12, save_data_g=save_data_g)
    save_stats["unknown_flag_1"] = next_int_len(1, save_data_g=save_data_g)
    save_stats["slots"] = get_equip_slots()

    save_stats["cat_stamp_current"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["cat_stamp_collected"] = get_length_data(length=30, save_data_g=save_data_g)
    save_stats["unknown_2"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["daily_reward_flag"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["unknown_116"] = get_length_data(length=10, save_data_g=save_data_g)

    save_stats["story_chapters"] = get_main_story_levels()
    save_stats["treasures"] = get_treasures()
    try:
        save_stats["enemy_guide"] = get_length_data(save_data_g=save_data_g)
    except Exception:
        return parse_save(save_data, country_code, not dst)
    if len(save_stats["enemy_guide"]) == 0:
        return parse_save(save_data, country_code, not dst)
    save_stats["cats"] = get_length_data(save_data_g=save_data_g)
    save_stats["cat_upgrades"] = get_cat_upgrades()
    save_stats["current_forms"] = get_length_data(save_data_g=save_data_g)

    save_stats["blue_upgrades"] = get_blue_upgrades()

    save_stats["menu_unlocks"] = get_length_data(save_data_g=save_data_g)
    save_stats["new_dialogs_1"] = get_length_data(save_data_g=save_data_g)

    save_stats["battle_items"] = get_length_data(4, 4, 6, save_data_g=save_data_g)
    save_stats["new_dialogs_2"] = get_length_data(save_data_g=save_data_g)
    save_stats["unknown_6"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["unknown_7"] = get_length_data(length=21, save_data_g=save_data_g)

    save_stats["lock_item"] = next_int_len(1, save_data_g=save_data_g)
    save_stats["locked_items"] = get_length_data(1, 1, 6, save_data_g=save_data_g)
    save_stats["second_time"] = get_time_data(save_stats["dst"])

    save_stats["unknown_8"] = get_length_data(length=50, save_data_g=save_data_g)
    save_stats["third_time"] = get_time_data(save_stats["dst"])

    save_stats["unknown_9"] = next_int_len(6 * 4, save_data_g=save_data_g)

    save_stats["thirty2_code"] = get_utf8_string()
    save_stats["unknown_10"] = load_bonus_hash()
    save_stats["unknown_11"] = get_length_data(length=4, save_data_g=save_data_g)
    save_stats["normal_tickets"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["rare_tickets"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["gatya_seen_cats"] = get_length_data(save_data_g=save_data_g)
    save_stats["unknown_12"] = get_length_data(length=10, save_data_g=save_data_g)
    length = next_int(2)
    ...
    cat_storage_id = get_length_data(2, 4, length, save_data_g=save_data_g)
    cat_storage_type = get_length_data(2, 4, length, save_data_g=save_data_g)
    ...
    save_stats["unknown_15"] = get_length_data(length=38, save_data_g=save_data_g)
    save_stats["unit_drops"] = get_length_data(save_data_g=save_data_g)
    save_stats["rare_gacha_seed"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["unknown_17"] = next_int_len(12, save_data_g=save_data_g)
    save_stats["unknown_18"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["fourth_time"] = get_time_data(save_stats["dst"])
    save_stats["unknown_105"] = get_length_data(length=5, save_data_g=save_data_g)
    save_stats["unknown_107"] = get_length_data(separator=1, length=3, save_data_g=save_data_g)
    ...
    if save_stats["dst"]:
        save_stats["time_stamps"] = get_length_doubles(length=3, save_data_g=save_data_g)
        ...
        save_stats["energy_notice"] = next_int_len(1, save_data_g=save_data_g)
        save_stats["game_version_2"] = next_int_len(4, save_data_g=save_data_g)
    else:
    ...
    save_stats["unknown_111"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["unlocked_slots"] = next_int_len(1, save_data_g=save_data_g)

    length_1 = next_int(4)
    length_2 = next_int(4)
    unknown_20: dict[str, Any] = {}
    unknown_20 = {"Value": get_length_data(4, 4, length_1 * length_2, save_data_g=save_data_g)}
    unknown_20["Length_1"] = length_1
    unknown_20["Length_2"] = length_2
    save_stats["unknown_20"] = unknown_20

    save_stats["time_stamps_2"] = get_length_doubles(length=4, save_data_g=save_data_g)

    save_stats["trade_progress"] = next_int_len(4, save_data_g=save_data_g)

    if save_stats["dst"]:
        save_stats["time_stamps_2"].append(get_double(save_data_g=save_data_g))
        save_stats["unknown_24"] = generate_empty_len(4)
    else:
        save_stats["unknown_24"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["catseye_related_data"] = get_cat_upgrades()
    save_stats["unknown_22"] = get_length_data(length=11, save_data_g=save_data_g)
    save_stats["user_rank_rewards"] = get_length_data(4, 1, save_data_g=save_data_g)

    if not save_stats["dst"]:
        save_stats["time_stamps_2"].append(get_double(save_data_g=save_data_g))

    save_stats["unlocked_forms"] = get_length_data(save_data_g=save_data_g)
    save_stats["transfer_code"] = get_utf8_string()
    save_stats["confirmation_code"] = get_utf8_string()
    save_stats["transfer_flag"] = next_int_len(1, save_data_g=save_data_g)

    lengths = [next_int(4), next_int(4), next_int(4)]
    length = lengths[0] * lengths[1] * lengths[2]

    save_stats["stage_data_related_1"] = {
        "Value": get_length_data(4, 1, length, save_data_g=save_data_g),
        "Lengths": lengths,
    }
    ...
    save_stats["unknown_25"] = next_int_len(1, save_data_g=save_data_g)

    save_stats["backup_state"] = next_int_len(4, save_data_g=save_data_g)

    if save_stats["dst"]:
        save_stats["unknown_119"] = next_int_len(1, save_data_g=save_data_g)
    else:
        save_stats["unknown_119"] = generate_empty_len(1)

    save_stats["gv_44"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["unknown_120"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["itf_timed_scores"] = list(
        helper.chunks(get_length_data(4, 4, 51 * 3, save_data_g=save_data_g), 51)
    )

    save_stats["unknown_27"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["cat_related_data_1"] = get_length_data(save_data_g=save_data_g)
    save_stats["unknown_28"] = next_int_len(1, save_data_g=save_data_g)

    save_stats["gv_45"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["gv_46"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["unknown_29"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["lucky_tickets_1"] = get_length_data(save_data_g=save_data_g)
    save_stats["unknown_32"] = get_length_data(save_data_g=save_data_g)

    save_stats["gv_47"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["gv_48"] = next_int_len(4, save_data_g=save_data_g)

    if not save_stats["dst"]:
        save_stats["energy_notice"] = next_int_len(1, save_data_g=save_data_g)
    save_stats["account_created_time_stamp"] = get_double(save_data_g=save_data_g)

    save_stats["unknown_35"] = get_length_data(save_data_g=save_data_g)
    save_stats["unknown_36"] = next_int_len(15, save_data_g=save_data_g)

    save_stats["user_rank_popups"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["gv_49"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["gv_50"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["gv_51"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["cat_guide_collected"] = get_length_data(4, 1, save_data_g=save_data_g)

    save_stats["gv_52"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["time_stamps_3"] = get_length_doubles(length=5, save_data_g=save_data_g)

    save_stats["cat_fruit"] = get_length_data(save_data_g=save_data_g)
    save_stats["cat_related_data_3"] = get_length_data(save_data_g=save_data_g)
    save_stats["catseye_cat_data"] = get_length_data(save_data_g=save_data_g)
    save_stats["catseyes"] = get_length_data(save_data_g=save_data_g)
    save_stats["catamins"] = get_length_data(save_data_g=save_data_g)

    save_stats["gamatoto_time_left"] = helper.seconds_to_time(int(get_double(save_data_g=save_data_g)))
    save_stats["gamatoto_exclamation"] = next_int_len(1, save_data_g=save_data_g)
    save_stats["gamatoto_xp"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["gamamtoto_destination"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["gamatoto_recon_length"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["unknown_43"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["gamatoto_complete_notification"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["unknown_44"] = get_length_data(4, 1, save_data_g=save_data_g)
    save_stats["unknown_45"] = get_length_data(4, 12 * 4, save_data_g=save_data_g)
    save_stats["gv_53"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["helpers"] = get_length_data(save_data_g=save_data_g)

    save_stats["unknown_47"] = next_int_len(1, save_data_g=save_data_g)

    save_stats["gv_54"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["purchases"] = get_purchase_receipts()
    save_stats["gv_54"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["gamatoto_skin"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["platinum_tickets"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["login_bonuses"] = get_login_bonuses()
    save_stats["unknown_49"] = next_int_len(16, save_data_g=save_data_g)
    save_stats["announcment"] = get_length_data(length=32, save_data_g=save_data_g)

    save_stats["backup_counter"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["unknown_131"] = get_length_data(length=3, save_data_g=save_data_g)
    save_stats["gv_55"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["unknown_51"] = next_int_len(1, save_data_g=save_data_g)

    save_stats["unknown_113"] = get_data_before_outbreaks()

    save_stats["dojo_data"] = get_dojo_data_maybe()
    save_stats["dojo_item_lock"] = next_int_len(1, save_data_g=save_data_g)
    save_stats["dojo_locks"] = get_length_data(1, 1, 2, save_data_g=save_data_g)

    save_stats["unknown_114"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["gv_58"] = next_int_len(4, save_data_g=save_data_g)  # 0x3a
    save_stats["unknown_115"] = next_int_len(8, save_data_g=save_data_g)

    save_stats["outbreaks"] = get_outbreaks()

    save_stats["unknown_52"] = get_double(save_data_g=save_data_g)
    save_stats["item_schemes"] = {}
    save_stats["item_schemes"]["to_obtain_ids"] = get_length_data(save_data_g=save_data_g)
    save_stats["item_schemes"]["received_ids"] = get_length_data(save_data_g=save_data_g)

    save_stats["current_outbreaks"] = get_outbreaks()

    save_stats["unknown_55"] = get_mission_data_maybe()

    save_stats["time_stamp_4"] = get_double(save_data_g=save_data_g)
    save_stats["gv_60"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["unknown_117"] = get_unknown_data()

    save_stats["gv_61"] = next_int_len(4, save_data_g=save_data_g)
    data = get_unlock_popups()
    save_stats["unlock_popups"] = data[0]

    save_stats["unknown_118"] = data[1]

    save_stats["base_materials"] = get_length_data(save_data_g=save_data_g)

    save_stats["unknown_56"] = next_int_len(8, save_data_g=save_data_g)
    save_stats["unknown_57"] = next_int_len(1, save_data_g=save_data_g)
    save_stats["unknown_58"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["engineers"] = next_int_len(4, save_data_g=save_data_g)
    ...
    save_stats["challenge"] = {"Score": next_int_len(4, save_data_g=save_data_g), "Cleared": next_int_len(1, save_data_g=save_data_g)}

    save_stats["gv_67"] = next_int_len(4, save_data_g=save_data_g)  # 0x43

    save_stats["weekly_event_missions"] = get_dict(int, bool)
    save_stats["won_dojo_reward"] = next_int_len(1, save_data_g=save_data_g)
    save_stats["event_flag_update_flag"] = next_int_len(1, save_data_g=save_data_g)

    save_stats["gv_68"] = next_int_len(4, save_data_g=save_data_g)  # 0x44
    ...
    save_stats["stages_beaten_twice"] = get_length_data(save_data_g=save_data_g)
    ...
    total = lengths["total"]
    save_stats["unknown_62"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["unknown_63"] = get_length_data(length=total, save_data_g=save_data_g)

    save_stats["unknown_64"] = get_data_after_uncanny()

    total = save_stats["unknown_64"]["progress"]["Lengths"]["total"]
    save_stats["unknown_65"] = next_int_len(4, save_data_g=save_data_g)
    val_61 = save_stats["unknown_65"]

    save_stats["unknown_66"] = []
    unknown_66: list[Any] = []
    for _ in range(total):
        val_61 = next_int_len(4, save_data_g=save_data_g)
    ...
    save_stats["lucky_tickets_2"] = get_length_data(length=val_54, save_data_g=save_data_g)

    save_stats["unknown_67"] = []
    if 0x37 < val_61["Value"]:
        save_stats["unknown_67"] = get_length_data(4, 4, val_61["Value"], save_data_g=save_data_g)

    save_stats["unknown_68"] = next_int_len(1, save_data_g=save_data_g)

    save_stats["gv_77"] = next_int_len(4, save_data_g=save_data_g)  # 0x4d

    save_stats["gold_pass"] = get_gold_pass_data()

    save_stats["talents"] = get_talent_data()
    save_stats["np"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["unknown_70"] = next_int_len(1, save_data_g=save_data_g)

    save_stats["gv_80000"] = next_int_len(4, save_data_g=save_data_g)  # 80000

    save_stats["unknown_71"] = next_int_len(1, save_data_g=save_data_g)

    save_stats["leadership"] = next_int_len(2, save_data_g=save_data_g)
    save_stats["officer_pass_cat_id"] = next_int_len(2, save_data_g=save_data_g)
    save_stats["officer_pass_cat_form"] = next_int_len(2, save_data_g=save_data_g)

    save_stats["gv_80200"] = next_int_len(4, save_data_g=save_data_g)  # 80200
    save_stats["filibuster_stage_id"] = next_int_len(1, save_data_g=save_data_g)
    save_stats["filibuster_stage_enabled"] = next_int_len(1, save_data_g=save_data_g)

    save_stats["gv_80300"] = next_int_len(4, save_data_g=save_data_g)  # 80300

    save_stats["unknown_74"] = get_length_data(save_data_g=save_data_g)

    save_stats["gv_80500"] = next_int_len(4, save_data_g=save_data_g)  # 80500

    save_stats["unknown_75"] = get_length_data(2, 4, save_data_g=save_data_g)
    ...
    save_stats["unknown_133"] = get_length_data(4, 1, lengths["total"], save_data_g=save_data_g)
    save_stats["legend_quest_ids"] = get_length_data(4, 4, lengths["stages"], save_data_g=save_data_g)

    save_stats["unknown_76"] = get_data_after_leadership()
    save_stats["gv_80700"] = next_int_len(4, save_data_g=save_data_g)  # 80700
    if save_stats["dst"]:
        save_stats["unknown_104"] = next_int_len(1, save_data_g=save_data_g)
        save_stats["gv_100600"] = next_int_len(4, save_data_g=save_data_g)
        if save_stats["gv_100600"]["Value"] != 100600:
            skip(-5)
    else:
        save_stats["unknown_104"] = generate_empty_len(1)
        save_stats["gv_100600"] = generate_empty_len(4)
    save_stats["restart_pack"] = next_int_len(1, save_data_g=save_data_g)
    ...
    save_stats["unknown_77"] = get_length_data(4, 1, lengths["total"], save_data_g=save_data_g)

    save_stats["gv_90300"] = next_int_len(4, save_data_g=save_data_g)  # 90300

    lengths = get_gauntlet_current()
    save_stats["unknown_78"] = lengths
    save_stats["unknown_79"] = get_gauntlet_progress(lengths)

    save_stats["unknown_80"] = get_length_data(4, 1, lengths["total"], save_data_g=save_data_g)
    ...
    save_stats["unknown_84"] = get_length_data(4, 1, lengths["total"], save_data_g=save_data_g)
    ...
    save_stats["unknown_130"] = next_int_len(4 * 5, save_data_g=save_data_g)

    save_stats["gv_90900"] = next_int_len(4, save_data_g=save_data_g)  # 90900

    save_stats["slot_names"] = get_slot_names(save_stats)
    save_stats["gv_91000"] = next_int_len(4, save_data_g=save_data_g)
    save_stats["legend_tickets"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["unknown_87"] = get_length_data(1, 5, save_data_g=save_data_g)
    save_stats["unknown_88"] = next_int_len(2, save_data_g=save_data_g)

    save_stats["token"] = get_utf8_string()

    save_stats["unknown_89"] = next_int_len(1 * 3, save_data_g=save_data_g)
    save_stats["unknown_90"] = next_int_len(8, save_data_g=save_data_g)
    save_stats["unknown_91"] = next_int_len(8, save_data_g=save_data_g)

    save_stats["gv_100000"] = next_int_len(4, save_data_g=save_data_g)  # 100000
    save_stats = check_gv(save_stats, 100100)
    if save_stats["exit"]:
        return save_stats

    save_stats["date_int"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["gv_100100"] = next_int_len(4, save_data_g=save_data_g)  # 100100
    save_stats = check_gv(save_stats, 100300)
    if save_stats["exit"]:
        return save_stats

    save_stats["unknown_93"] = get_length_data(4, 19, 6, save_data_g=save_data_g)

    save_stats["gv_100300"] = next_int_len(4, save_data_g=save_data_g)  # 100300
    ...
    save_stats["platinum_shards"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["unknown_100"] = get_data_near_end_after_shards()

    save_stats["gv_100700"] = next_int_len(4, save_data_g=save_data_g)  # 100700
    ...
    save_stats["unknown_95"] = next_int_len(1 * 2, save_data_g=save_data_g)
    save_stats["unknown_96"] = get_data_after_aku()

    save_stats["gv_100900"] = next_int_len(4, save_data_g=save_data_g)  # 100900
    save_stats = check_gv(save_stats, 101000)
    if save_stats["exit"]:
        return save_stats

    save_stats["unknown_97"] = next_int_len(1, save_data_g=save_data_g)

    save_stats["gv_101000"] = next_int_len(4, save_data_g=save_data_g)  # 101000
    ...
    save_stats["gv_110000"] = next_int_len(4, save_data_g=save_data_g)  # 110000
    ...
    save_stats["unknown_124"] = get_length_data(4, 1, data["total"], save_data_g=save_data_g)

    save_stats["unknown_125"] = next_int_len(1, save_data_g=save_data_g)

    save_stats["gv_110500"] = next_int_len(4, save_data_g=save_data_g)  # 110500
    save_stats = check_gv(save_stats, 110600)
    if save_stats["exit"]:
        return save_stats

    save_stats["unknown_126"] = next_int_len(1, save_data_g=save_data_g)

    save_stats["gv_110600"] = next_int_len(4, save_data_g=save_data_g)  # 110600
    ...
    if save_stats["dst"]:
        save_stats["unknown_128"] = next_int_len(1, save_data_g=save_data_g)
    else:
        save_stats["unknown_128"] = generate_empty_len(1)

    save_stats["gv_110700"] = next_int_len(4, save_data_g=save_data_g)  # 110700
    save_stats = check_gv(save_stats, 110800)
    if save_stats["exit"]:
        return save_stats

    save_stats["shrine_dialogs"] = next_int_len(4, save_data_g=save_data_g)

    save_stats["unknown_129"] = get_110800_data()

    save_stats["dojo_3x_speed"] = next_int_len(1, save_data_g=save_data_g)

    save_stats["unknown_132"] = get_110800_data_2()

    save_stats["gv_110800"] = next_int_len(4, save_data_g=save_data_g)  # 110800
    save_stats = check_gv(save_stats, 110900)
    if save_stats["exit"]:
        return save_stats

    save_stats["unknown_135"] = get_110900_data()
    save_stats["gv_110900"] = next_int_len(4, save_data_g=save_data_g)  # 110900
    save_stats = check_gv(save_stats, 120000)
    if save_stats["exit"]:
        return save_stats

    save_stats["zero_legends"] = get_zero_legends()
    save_stats["unknown_136"] = next_int_len(1, save_data_g=save_data_g)
    save_stats["gv_120000"] = next_int_len(4, save_data_g=save_data_g)  # 120000
    save_stats = check_gv(save_stats, 120100)
    if save_stats["exit"]:
        return save_stats

    save_stats["unknown_137"] = get_120100_data()
    save_stats["gv_120100"] = next_int_len(4, save_data_g=save_data_g)  # 120100
    save_stats = check_gv(save_stats, 120200)
    if save_stats["exit"]:
        return save_stats

    save_stats["unknown_138"] = get_120200_data()
    save_stats["gv_120200"] = next_int_len(4, save_data_g=save_data_g)  # 120200
    save_stats = check_gv(save_stats, 120200)
    if save_stats["exit"]:
        return save_stats

    length = len(save_data) - address - 32
    save_stats["extra_data"] = next_int_len(length, save_data_g=save_data_g)
