"""HTML5 Canvas-based renderer for Sketching.
...
class Sketch2DWeb(sketching.abstracted.Sketch):
    ...
    def _inner_loop(self):
        if js.document.getElementById(self._element_id) is None:
            self._stopped = True
...
class PyscriptMouse(sketching.control_struct.Mouse):
    ...
    def _report_mouse_down(self, event):
        if event.button == 0:
            ...
            if self._press_callback is not None:
                ...
                self._press_callback(button, self)
        elif event.button == 2:
    ...
    def _report_click(self, event):
        ...
        if self._release_callback is not None:
            button = sketching.control_struct.Button(sketching.const.MOUSE_LEFT_BUTTON)
            self._release_callback(button, self)
...
class PyscriptKeyboard(sketching.control_struct.Keyboard):
