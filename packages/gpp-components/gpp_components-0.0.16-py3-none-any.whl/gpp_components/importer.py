...
class Importer:
    ...
    def importData(self):
        table_data = Importer.loadFile(self.jsonStr)
        # DB 操作
        res = Importer.executeDB(self.jsonStr, table_data)
        if res:
    ...
    def loadFile(obj):
        ...
        if file_type == "Value":
            ...
            table_data = Importer.mergeData(
                sheet_name_data, mapping_data, extends_data, header
            )
        return table_data

    def mergeData(sheet_name_data, mapping_data, extends_data, header):
        """
        ...
        if header != 0:
            columns = [
                "COL" + str(item + 1) if item not in extends_data else item
                for item in columns
            ]
