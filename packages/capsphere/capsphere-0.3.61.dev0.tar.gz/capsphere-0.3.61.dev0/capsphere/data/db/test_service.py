...
from capsphere.domain import LoanStatement, LoanRecord
...
class TestExecutor(unittest.TestCase):
    ...
    def test_postgres_executor(self):
        ...
        data = executor_service.execute("SELECT * FROM loan_statements")

        print(get_dataclass_list_from_query(data, LoanStatement))

    def test_loan_records(self):
        executor_service = DbQueryService(self.connector)
        data = executor_service.execute("SELECT * FROM loans")

        print(get_dataclass_list_from_query(data, LoanRecord))
