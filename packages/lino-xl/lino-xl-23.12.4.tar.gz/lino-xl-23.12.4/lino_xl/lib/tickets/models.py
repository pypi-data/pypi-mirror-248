...
class Ticket(UserAuthored, mixins.CreatedModified, TimeInvestment,
    ...
    def quick_assign_to(self, ar):
        ...
        if self.site and self.site.group:
            qs = self.site.group.members.all()
            qs = settings.SITE.user_model.filter_active_users(qs, dd.today(), prefix="user__")
            # members = self.site.group.members.filter(**kw)
            # members = members.filter(
            #     Q(user__start_date__isnull)|Q(user__start_date__lte=dd.today()))
            # members = members.filter(
            #     Q(user__end_date__isnull)|Q(user__end_date__gte=dd.today()))
            for m in qs:
