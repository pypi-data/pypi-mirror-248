...
def test_render_unknown_non_utf8():
    flow = Flow("http", id="1", path="/hello")
    flow.response_mime_type = "application/unknown"
    flow.response_body = b"\x80\x81\x82\x83\x84\x85\x86\x87\x88\x89"
    assert ("unknown", {"content": "Can't decode content, not UTF-8"}) == render(
        flow, flow.response_body
    )
