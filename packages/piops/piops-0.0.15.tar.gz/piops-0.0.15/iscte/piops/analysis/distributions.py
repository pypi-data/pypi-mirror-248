...
class EventLog:
  ...
  def __init__(self, log, format):
    ...
    self.data['start_time'] = pd.to_datetime(self.data['start_time'], format=format)
    self.data['end_time'] = pd.to_datetime(self.data['end_time'], format=format)
  ...
  def distInterval( self ):
    ...
    f = Fitter( df[ 'interval' ], timeout = 120, distributions = get_common_distributions())
    f.fit()
  ...
  def distActivities( self ):
    ...
    for activity in activities_list:
        ...
        f = Fitter( act['duration'], timeout = 120, distributions = get_common_distributions())
        f.fit()
