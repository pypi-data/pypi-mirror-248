...
from drawranflow.servicelogic.handlers.utils import get_gnb_id,get_trgt_gnb_id,get_tmsi
import numpy as np
...
def message_handler(df, item_id):
    ...
    # target_ho_xnap_df = xnap_df[
    #     (xnap_df['_ws.col.info'] == 'HandoverRequest') & ~xnap_df['nap.NR_Cell_Identity'].isnull()]
    #
    # target_ho_xnap_df.loc[:, 'ngap.NR_Cell_Identity'] = target_ho_xnap_df['ngap.NR_Cell_Identity'].map(get_trgt_gnb_id)
    # print(xnap_df[['xnap.NR_Cell_Identity', 'ngap.NR_Cell_Identity']])
    xnap_df.loc[:,'xnap.NR_Cell_Identity'] = xnap_df['xnap.NR_Cell_Identity'].map(get_trgt_gnb_id)
    ...
    identifiers_df = combined_df[
        ['f1ap.C_RNTI',
         ...
         'f1ap.nRCellIdentity',
         'f1ap.pLMN_Identity',
         'nr-rrc.ng_5G_S_TMSI_Part1',
         ]].copy()
    xnap_df_filtered = xnap_df[xnap_df['xnap.NR_Cell_Identity'].isin(unique_gnb_values)]
    ...
    for _, identifier_row in identifiers_df.iterrows():
        if identifier_row['c_rnti'] is not None and not pd.isnull(identifier_row['c_rnti']):
            ...
            logging.error("Preparation of HO calls")
            ...
            logging.debug(
                f"E1AP: Bearer Ctxt setup within 1 second"
                f"gnb_cu_cp_ue_e1ap_id: {updated_e1ap_value}" )
            ...
            logging.debug(
                    f"E1AP: Bearer Ctxt setup Response/Failure within 10s seconds: "
                    f"gnb_cu_up_ue_e1ap_id: {matching_e1ap_setup}")
            ...
            logging.debug(
               f"XNAP: matching_xnap_resp_setup: {matching_xnap_resp_setup}")

            matching_f1ap_req_setup = f1ap_ue_ctxt_req[
                ...
                (f1ap_ue_ctxt_req['f1ap.GNB_CU_UE_F1AP_ID'] == updated_e1ap_value)
                ]

            if not matching_f1ap_req_setup.empty:
                identifiers_df.at[_, 'gnb_cu_ue_f1ap_id'] = matching_f1ap_req_setup.iloc[0]['f1ap.GNB_CU_UE_F1AP_ID']

            logging.debug(f"matching_f1ap_req_setup: {matching_f1ap_req_setup}")

            matching_f1ap_res = f1ap_ue_ctxt_res[
            ...
            if not matching_f1ap_res.empty:
                identifiers_df.at[_, 'gnb_du_ue_f1ap_id'] = matching_f1ap_res.iloc[0]['f1ap.GNB_DU_UE_F1AP_ID']

            logging.debug(f"matching_f1ap_res: {matching_f1ap_res}")

            matching_ngap_req = ngap_path_swith_req[
                ...
                (ngap_path_swith_req['ngap.RAN_UE_NGAP_ID'] == updated_e1ap_value)
                ]

            if not matching_ngap_req.empty:
                identifiers_df.at[_, 'ran_ue_ngap_id'] = matching_ngap_req.iloc[0]['ngap.RAN_UE_NGAP_ID']

            updated_ran_gap_value = identifiers_df.at[_, 'ran_ue_ngap_id']
            logging.debug(f"matching_ngap_req: {matching_ngap_req}")
            matching_ngap_res = ngap_path_swith_res[
                ...
                (ngap_path_swith_res['ngap.RAN_UE_NGAP_ID'] == updated_ran_gap_value)
                ]

            if not matching_ngap_res.empty:
                identifiers_df.at[_, 'amf_ue_ngap_id'] = matching_ngap_res.iloc[0]['ngap.AMF_UE_NGAP_ID']

            logging.debug(f"matching_ngap_res: {matching_ngap_res}")
