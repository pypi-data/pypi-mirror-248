...
def validate(args, image_path, save_name,encoder, decoders,max_values):
    ...
    if args.vis:
        ...
        plot_visualizing_inference_results(image_path,save_name, scores, save_dir, file_names, args)
    
    return img_scores


def start_inference(device,gpu,image_path,save_name,encoder,decoders,input_size,save_heatmap,heatmap_threshold,save_heatmap_path,max_values):
    args = parse_args()
    if device == "GPU":
    ...
    val_sample = Image.fromarray(image_path)
    args.origin_size = val_sample.size
    
    
    img_scores = validate(args, image_path,save_name, encoder, decoders,max_values)
