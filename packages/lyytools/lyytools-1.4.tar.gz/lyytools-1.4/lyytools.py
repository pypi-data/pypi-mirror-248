...
def get_data_from_cache_or_func(cache_file_path, expiry_duration, next_func=None, debug=False):
    ...
    def read_cache(expiry_duration):
        if os.path.isfile(cache_file_path):
            ...
            if time_difference < expiry_duration:
                ...
                if debug: print(f"{cache_file_path} not expired, return it, df=\n", df)
                return df
    ...
    if df is not None:
        ...
        if debug: print(f"enter next_func={next_func}")
        df = next_func()
        df.to_pickle(cache_file_path)
        return df
    else:
        if debug: print(f"cache expired, and no next_func specified, expiry_duration*10, return expied cache")
