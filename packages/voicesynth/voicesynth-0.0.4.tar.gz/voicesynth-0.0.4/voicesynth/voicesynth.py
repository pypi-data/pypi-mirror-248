...
class Model:
    ...
    def __init__(self, model: str, model_path: str = "model.pt"):
        self.speaker = None
        self.model = model
        ...
        try:
            ...
            for key in models.keys():
                error_msg += f"{key}, "
            raise VoiceSynthError(error_msg)
    ...
    def download_model(self, url: str, model_path: str = "model.pt", show_progress: bool = True) -> None:
        """
        ...
        if not os.path.exists(f"{self.model_path}"):
            ...
            torch.hub.download_url_to_file(url, f"{model_path}",
...
class Synthesizer:
    ...
    def say(self, text: str, path: str = "audio.wav", prosody_rate: int = 100) -> None:
        ...
        if not path.endswith(".wav"):
            raise VoiceSynthError("Incorrect audio format! Ensure that path leads to a .wav file")
        ...
        try:
            ...
            raise VoiceSynthError("There was an error playing synthesized audio")

    def synthesize(self, text: str, path: str = "audio.wav", prosody_rate: int = 100) -> None:
        try:
            self.audio_path = path
            self.model_download.save_wav(
                ssml_text=f"<speak><prosody rate='{prosody_rate}%'>{text}</prosody></speak>",
                speaker=self.model.speaker,
                sample_rate=self.sample_rate,
                audio_path=path)  # ssml text supports a lot of parameters, such as intonation, pauses etc.
    ...
    def play(path):
        if os.path.exists(path):
            ...
            raise VoiceSynthError("Specified audio file path does not exist")
