...
class Synthesizer:
    ...
    def synthesize(self, text: str, path: str = "audio.wav", prosody_rate: int = 100) -> None:
        try:
            ...
            synth_thread = threading.Thread(target=self.model_download.save_wav, kwargs={"ssml_text": f"<speak><prosody rate='{prosody_rate}%'>{text}</prosody></speak>",
                                                                                         "speaker": self.model.speaker,
                                                                                         "sample_rate": self.sample_rate,
                                                                                         "audio_path": path})
            synth_thread.start()
            logger.out("Audio synthesyzing started")
        except Exception:
