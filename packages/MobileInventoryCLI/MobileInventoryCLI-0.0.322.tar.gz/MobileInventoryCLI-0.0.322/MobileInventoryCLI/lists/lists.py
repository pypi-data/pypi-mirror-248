from MobileInventoryCLI.error.error import writeError,obj2dict,getConfig
		...
		while True:
			...
			if action.lower() in ["#quit","#6","#q"]:
				...
				with Session(self.engine) as session:
					...
					if len(result) > 0:
						for li in result:
							editLI=input("edit ListItem[y/N/q/b]: ")
							...
							if editLI in ['n',"no"]:
								...
								for k in li.__table__.columns:
									valstring=input("ListItem {name}:{type}:{value} [newValue/q/n/#q#/#next#,#n,#b#,#n#,#back#)]: ".format(
										name=k.name,
										type=k.type,
										value=getattr(li,k.name)))
									if valstring.lower() in ['q','quit','#q','#quit','#quit#']:
										exit("user quit!")
										...
										setattr(editLI,k.name,getattr(li,k.name))
										session.commit()
										session.flush()
										session.refresh(editLI)
								...
								if editCF.lower() in ['yes','y','ye']:
									...
									if len(cfs) < 1:
										...
										for cf in cfs:
											attr=session.query(self.tbl.CustomField).filter(self.tbl.CustomFieldId==cf.CustomFieldId).first()
											if attr:
												edit=input("{liname}:{liid}:{name}:{type}:{current_value}: [new_value/#q#/#b#]: ".format(
													liname=li.Name,
													liid=li.ListItemId,
													name=attr.Name,
													type=attr.Type,
												...
												if edit.lower() in ['#q#','#quit#',]:
													exit("user quit!")
												elif edit.lower() in ['#b#','#back#']:
													break
												if attr.Type == 1:
													edit=float(edit)
													if edit.is_integer():
						...
						print("not found in list... getting from Item Table!")
