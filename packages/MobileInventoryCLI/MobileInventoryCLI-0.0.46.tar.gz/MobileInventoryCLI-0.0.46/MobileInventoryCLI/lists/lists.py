...
class GotoLists:
	...
	def displayListItemMenu(self):
		...
		{row0}#showListTotalPrice -> show total price for list{end}
		...
		{row0}#expub_pkg -> export unknown barcodes to User's Downloads folder as zipfile
			containing:
				...
				unknownBarcodes.csv
				GeneratedImages/
					1 image for each barcode in unknownBarcodes.csv
		{row1}#expub b|c|n|p
				where:
					p : price
					c : code
				...
				once prompted for system:
					...
					location, or overrode with a path+filename, and hit enter
		"""
		msg=msg.format(row0=fg("red"),row1=fg("green"),end=attr(0))
		print(msg)
		return msg
		...
		while True:
			...
			{row0}#zeroAll -> set all qty's to zero in List{end}'''
				...
				with Session(self.engine) as session:
					...
					session.commit()
			elif action.split(" ")[0].lower() == "#showlisttotalprice":
				with Session(self.engine) as session:
	...
	default_export={
		'android':'/storage/emulated/0/Documents/export.csv',
		'linux':str(Path().home()/Path('export.csv')),
	}
			...
			if format == "pkg":
				while True:
					...
					if sys.lower() in self.default_export.keys():
						...
						return
				images=Path(self.default_export[sys]).parent/Path("generatedImages")
