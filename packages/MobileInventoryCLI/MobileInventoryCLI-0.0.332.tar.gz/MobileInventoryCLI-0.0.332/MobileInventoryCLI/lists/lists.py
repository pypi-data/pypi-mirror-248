			...
			if action.lower() in ["#quit","#6","#q"]:
				...
				try:
					if action.startswith("+"):
						...
						if self.like:
							self.searchAndCreateLike(code,listid,icr=icr)
						else:
					...
					if action.startswith("-"):
						...
						code=input("code/q/skip: ")
						if self.like:
							self.searchAndCreateLike(code,listid,dcr=dcr)
						else:
							self.searchAndCreate(code,listid,dcr=dcr)
					else:
						code=action
						if self.like:
							self.searchAndCreateLike(code,listid)
						else:
							...
							if len(cfs) < 1:
								...
								for num,cf in enumerate(cfs):
									...
									if attr:
										while True:
											try:
												edit=input("{liname}:{liid}:{name}:{type}:{current_value}({t}/{total}): [new_value/#q#/#b#/#delete#]: ".format(
													liname=li.ItemName,
													liid=li.ListItemId,
													name=attr.Name,
													type=attr.Type,
