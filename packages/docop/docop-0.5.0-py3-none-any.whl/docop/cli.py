...
def docop(ctx, config):
    ...
    if not config.exists():
        print("No configuration found. Not doing anything.")
        sys.exit()
...
def run(ctx, task_or_pipe, source, content, target, account, extras):
    ...
    if "no implicit accounts" not in ctx.obj.get("options", []):
        account = account or (target[0] if target else None) or (source[0] if source else None)
    if account:
        found = False
        for account_source in ("accounts", "sources", "targets"):
            if found:= ctx.obj.get(account_source, {}).get(account):
                account = found
                break
        if not found:
            print(f"⚠️  [bold red]Account '{account}' not found.")
            return
    ...
    for counter, task in enumerate(pipe["tasks"], start=1):
        ...
        #
        # MAIN LOOP STEP 1: PROCESS EACH SOURCE
        #
        if source_queue:
            while source_queue:
        ...
        #
        # MAIN LOOP STEP 2: PROCESS CONTENT COLLECTIONS OR DOCUMENTS
        #
        ...
        #
        # MAIN LOOP STEP 3: PROCESS EACH TARGET
        #
        if target_queue:
