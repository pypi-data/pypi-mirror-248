...
def configC(language_module):
    ...
    if os.path.exists("./alfred_osint/config/config.ini") == True:
        print(language_module.config1)
    else:
        print(language_module.error5)
        time.sleep(4)
        with open("./alfred_osint/config/version.cfg", "w") as f:
            f.write(language_module.status3)
        exec(open("./alfred_osint/update.py").read())

    if os.path.exists("./alfred_osint/update.py") == True:
    ...
    with open("./alfred_osint/config/version.cfg", "r") as fp:
        version = fp.read()
        return version
...
def configUpdateStuff(config, browser, language_module):
    config.read("./alfred_osint/config/config.ini")
...
===========================================================================
            ...
            with open("./alfred_osint/config/config.ini", "w") as f:
                config.write(f)
            exit(1)
    ...
    if getNum == 3 and config.get("main", "showtips") == "yes":
        ...
        randomTip = random.choice(open("./alfred_osint/config/tips.txt").readlines())
        print(randomTip)
