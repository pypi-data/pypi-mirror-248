...
from dataset_sh_server.config import load_config
from dataset_sh_server.project_file_manager import ProjectFileManager
...
def create_app(frontend_assets=None, config: RepoServerConfig = None):
    ...
    manager = DatasetStorageManager(store_base=config.data_folder)
    ...
    def create_dataset():
        if config.allow_upload:
            ...
            if uploaded_file.filename == 'dataset-sh-file':
                ...
                manager.import_file(fp, user_name, dataset_name)
                return '', 204
