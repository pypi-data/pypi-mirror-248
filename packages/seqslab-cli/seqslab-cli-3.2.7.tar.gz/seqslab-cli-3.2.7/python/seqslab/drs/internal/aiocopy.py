    ...
    for i, sent in enumerate(status):
        ...
        results.append(
            _create_copyresult(
                ...
                checksum=resp_list[i]['checksums']['checksum'], type=checksum_type, status=status))
    return results
...
async def blobdir_to_dir(
    ...
    async with get_factory().load_storage(kwargs.get("workspace")) as store:
        ...
        optargs = {"chunk_size": kwargs.get("chunk_size", 16 * 1024 * 1024),
                   ...
                   'bandwidth': kwargs.get('bandwidth'),
                   'overwrite': kwargs.get('overwrite')}
        total_tasks = sum((size + kwargs["chunk_size"] - 1) // kwargs["chunk_size"] for size in kwargs['size'])
        ...
        while progress < len(srcs):
            ...
            for p in range(progress, progress + count):
                ...
                rel_path = os.path.relpath(srcs[p].path,
                                           f"/{kwargs.get('access_url').user}/{kwargs.get('access_url').path}")
                file = os.path.join(str(dir), kwargs.get('access_url').parent.name, rel_path)
