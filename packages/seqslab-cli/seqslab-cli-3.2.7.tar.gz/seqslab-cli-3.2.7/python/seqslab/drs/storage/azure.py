...
class BlobStorage(BaseBlob):
    ...
    def blob_service_url(self, uri: URL, **kwargs) -> URL:
        ...
        def https_access_url(get_upload_token, dst: str, **kwargs) -> URL:
            dir_path = os.path.dirname(dst).strip("/") if os.path.dirname(dst).strip("/") else "."
            file = os.path.basename(dst)
            if res := get_upload_token(path=dir_path, **kwargs):
                return URL(f"{res['url']}{file}").with_query(res['headers']['Authorization'])
            else:
                raise requests.HTTPError('Get Upload Token Failed: Response payload is incorrect. '
        ...
        if uri.scheme:
            ...
            if uri.scheme in ['abfss', 'abfs'] and uri.host.endswith('.dfs.core.windows.net'):
                if len(parts) < 3 or not parts[1] == 'drs' or not parts[2].startswith('usr_'):
                    ...
                    return https_access_url(self.get_token, '/'.join(parts[3:]), **kwargs)

            elif uri.scheme in ['https', 'http'] and uri.host.endswith('.blob.core.windows.net'):
                if len(parts) < 3 or not parts[2] == 'drs' or not parts[3].startswith('usr_'):
                    raise ValueError('Enter a valid dst in https://storageaccount.blob.core.windows.net/container/'
                                     '/path/')
                else:
                    return https_access_url(self.get_token, '/'.join(parts[4:]), **kwargs)
        ...
        with open(file, 'rb') as f:
            while position < file_size:
                ...
                for pos in range(position, end_chunk, chunk_size):
                    ...
                    base64_message = b64encode(md5(data).hexdigest().encode(), altchars=b"xy").decode('ascii')
                    ...
                    if base64_message in block_list:
                        ...
                        if chunk_size == block_size or (end_chunk - pos) == block_size:
                            ...
                            if progress_bar := kwargs.get('progress_bar'):
                                ...
                                progress_bar.print(f"get_block: Get data block successfully.")
                            continue
                    self.upload_success += 1
    ...
    def get_block_list(self, uri: URL, **kwargs) -> iter:
        ...
        try:
            ...
            if get_block_list.status_code == 200:
                if progress_bar := kwargs.get('progress_bar'):
                    progress_bar.print(f"Get block list successfully.")
                return ElementTree.fromstring(get_block_list.text)
            else:
                if progress_bar := kwargs.get('progress_bar'):
                    progress_bar.print(f"No block list.")
                return
        except Exception as err:
            if progress_bar := kwargs.get('progress_bar'):
                progress_bar.print(f"Get block list failed.(Connection failed.)")
            raise err

    async def put_block(self, uri: URL, data: open,
            ...
            try:
                with BytesIO(data) as buffer:
                    ...
                    await self.request(url, "PUT",
                                       ...
                                       *args, **kwargs)
                if progress_bar := kwargs.get('progress_bar'):
                    progress_bar.print(f"Put block successfully.")
                message = length
                break
            except ClientResponseError as err:
                ...
                retry -= 1
                if retry == 0:
                    if "Make sure the value of Authorization header is formed correctly including the signature." in err.message:
                        message = f"{uri.with_query('')} is already exist in your storage as different type." \
    ...
    async def download(
        ...
        for _ in range(kwargs.get('bandwidth')):
            task = asyncio.create_task(
                self.get_blob(url=uri, pos=queue, file=temp_file,
        ...
        for i, x in enumerate(sorted(self.buffer[temp_file], key=lambda x: x['index'])):
            if isinstance(x, Exception):
                if progress_bar := kwargs.get('progress_bar'):
                    progress_bar.print(f'download: Check block: truncate block.')
                exception = x
