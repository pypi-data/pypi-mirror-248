...
def lucky_draw(ticket_key):
    
    drawed_data_in_json   = request.get_json()
    ...
    try:
        ...
        if lucky_draw_ticket:
            ...
            return create_rest_message('Invalid lucky draw ticket', status_code=StatusCode.BAD_REQUEST)
            
            
    
    except Exception as err:
        logger.error('Failed due to %s', get_tracelog())
        return create_rest_message(str(err), status_code=StatusCode.BAD_REQUEST)
    ...
    try:
        ...
        if lucky_draw_ticket:
            ...
            if lucky_draw_ticket.grabbed:
                return create_rest_message('Prize have been grabbed before', status_code=StatusCode.BAD_REQUEST)
            else:
                with db_client.context():
                ...
                return create_rest_message(status_code=StatusCode.OK, customer_rewards=customer_details_dict)
        else:
            return create_rest_message('Invalid lucky draw ticket', status_code=StatusCode.BAD_REQUEST)
...
def view_lucky_draw(ticket_key):
    ...
    try:
        ...
        if lucky_draw_details:
            ...
            return create_rest_message('Fail to draw', status_code=StatusCode.BAD_REQUEST)    
    except Exception as err:
        logger.error('Failed due to %s', get_tracelog())
        return create_rest_message(str(err), status_code=StatusCode.BAD_REQUEST)    
...
def create_merchant_lucky_draw_ticket(merchant_key):
    ...
    try:
        ...
        if draw_ticket:
            return create_rest_message(status_code=StatusCode.OK, draw_ticket=draw_ticket.to_dict())
        else:
            return create_rest_message('Failed to create lucky draw ticket', status_code=StatusCode.BAD_REQUEST)    
    except Exception as err:
        logger.error('Failed due to %s', get_tracelog())
        return create_rest_message(str(err), status_code=StatusCode.BAD_REQUEST)
