...
class mailmail:
	...
	def read_opened_email(self):
		aaa = self.outlook_program.Inspectors
		item = aaa.ltem(1).Currentltem
		print(aaa.Count)
		bbb = self.get_one_mail_information(item)
		print(bbb)
		return item



	def change_1(self, new_mail, to):
		new_mail.To = to

	def add_text_at_cursor(self, this_mail):
		aaa = this_mail.Getlnspector.WordEditor.Range(Start=0, End=20).Select()