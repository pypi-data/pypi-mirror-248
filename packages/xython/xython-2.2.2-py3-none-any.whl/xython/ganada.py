...
class ganada:
	...
	def font_Bold_in_selection(self, my_range=""):
		if my_range == "": my_range = self.selection
		my_range.Font.Bold = True

	def font_StrikeThrough_in_selection(self, my_range=""):
		if my_range == "": my_range = self.selection
		my_range.Font.StrikeThrough = True


	def font_Italic_in_selection(self, my_range=""):
		if my_range == "": my_range = self.selection
		my_range.Font.Italic = True


	def font_Size_in_selection(self, my_range= "", input_value= ""):
		if my_range == "": my_range = self.selection
		my_range.Font.Size = input_value


	def font_Color_in_selection(self, my_range= "", input_value= ""):
		if my_range == "": my_range = self.selection
		my_range.Font.Color = input_value
	...
	def font_name_in_selection(self, my_range= "", input_value= ""):
		if my_range == "": my_range = self.selection
		my_range.Font.Name = input_value


	def font_underline_in_selection(self, my_range= ""):
		if my_range == "": my_range = self.selection
		my_range.Font.Underline = 1


	def font_underline_color_in_selection(self, my_range= "", color= ""):
		if my_range == "": my_range = self.selection
		my_range.Font.UnderlineColor = color


	def font_space_in_selection(self, my_range= "", input_value=1.5):
		if my_range == "": my_range = self.selection
		my_range.Font.Spacing = input_value


	def font_borderline_left_in_selection(self, my_range="", style="", size="", color=""):
		if my_range == "": my_range = self.selection
		if style != "": my_range.Font.Borders(-2).LineStyle = style
		if size != "": my_range.Font.Borders(-2).LineWidth = size
		if color != "": my_range.Font.Borders(-2).ColorIndex = color

	def font_borderline_right_in_selection(self, my_range="", style="", size="", color=""):
		if my_range == "": my_range = self.selection
		if style != "": my_range. Font.Borders(-4).LineStyle = style
		if size != "": my_range.Font.Borders(-4).LineWidth = size
		if color != "": my_range.Font.Borders(-4).ColorIndex= color

	def font_borderline_top_in_selection(self, my_range="", style="", size="", color=""):
		if my_range == "": my_range = self.selection
		if style != "": my_range.Font.Borders(-1).LineStyle=style
		if size != "": my_range.Font.Borders(-1).LineWidth = size
		if color != "": my_range.Font.Borders(-1).ColorIndex = color

	def font_borderline_bottom_in_selection(self, my_range="", style="", size="", color=""):
		if my_range == "": my_range = self.selection
		if style != "": my_range.Font.Borders(-3).LineStyle = style  # wdLineStyleDouble 7
		if size != "": my_range.Font.Borders(-3).LineWidth = size  # wdLineWidth075pt 6
		if color != "": my_range.Font.Borders(-3).ColorIndex = color  # 7 :yellow


	def font_borderline_all_in_selection(self, my_range="", style="", size="", color=""):
		if my_range == "": my_range = self.selection
		for num in [-1, -2, -3, -4]:
			if style != "": my_range.Font.Borders(num).LineStyle = style  # wdLineStyleDouble 7.
			if size != "": my_range.Font.Borders(num).Lineidth = size  # wdLineWidth075pt
			if color != "": my_range.Font.Borders(num).ColorIndex = color  # 7 :yellow
