...
import mygrid_sample_data_01
import table_data_01
...
class display_cell_class(QAbstractTableModel):
    ...
    def rowCount(self, parent):
        ...
        :return:
        """
        return len(self.grid_cell_values)
    ...
    def columnCount(self, parent):
        ...
        :return:
        """
        return len(self.grid_cell_values[0])
    ...
    def data(self, index, role):
        ...
        cell_values = my_values = table_data_01.write_data()
        ...
        if role == Qt.ForegroundRole:
            ...
            if font_color:
                return QVariant(QColor(font_color[0], font_color[1], font_color[2]))
            pass
...
class display_cell_upgrade_class(QStyledItemDelegate): #모양을 나타내기 위해 만든 것
    ...
    def paint(self, painter, option, index):
        ...
        x = index.row()
        y = index.column()
        cell_values = self.grid_cell_values[x][y].values
        ...
        if cell_values["kind_1"] == "memo":
            print("----------->", x, y)
            painter.setBrush(Qt.red)
            painter.drawEllipse(QPoint(x1-5, y1+5), 4, 4)

        if cell_values["draw_line"] == "yes":
            if cell_values["line_top_dic"]["do"]:
                values = cell_values["line_top_dic"]
                painter.setBrush(Qt.red)
                painter.setPen(QPen(values["color"], values["thick"], values["style"]))
                painter.drawLine(x0, y0, x1, y1)

            if cell_values["line_bottom_dic"]["do"]:
                values = cell_values["line_top_dic"]
                painter.setBrush(Qt.red)
                painter.setPen(QPen(values["color"], values["thick"], values["style"]))
                painter.drawLine(x2, y2, x3, y3)

            if cell_values["line_left_dic"]["do"]:
                values = cell_values["line_top_dic"]
                painter.setBrush(Qt.red)
                painter.setPen(QPen(values["color"], values["thick"], values["style"]))
                painter.drawLine(x0, y0, x2, y2)

            if cell_values["line_right_dic"]["do"]:
                values = cell_values["line_top_dic"]
                painter.setBrush(Qt.red)
                painter.setPen(QPen(values["color"], values["thick"], values["style"]))
                painter.drawLine(x1, y1, x3, y3)

            if cell_values["line_x1_dic"]["do"]:
                values = cell_values["line_top_dic"]
                painter.setBrush(Qt.red)
                painter.setPen(QPen(values["color"], values["thick"], values["style"]))
                painter.drawLine(x0, y0, x3, y3)

            if cell_values["line_x2_dic"]["do"]:
                values = cell_values["line_top_dic"]
                painter.setBrush(Qt.red)
                painter.setPen(QPen(values["color"], values["thick"], values["style"]))
...
class CreateTable(QTableWidget): # QTableWidget
    ...
    def write_button_in_cell(self, xy, action, caption = "abc"):
        ...
        btnRun.setFont(QFont('Times New Roman', 12))
