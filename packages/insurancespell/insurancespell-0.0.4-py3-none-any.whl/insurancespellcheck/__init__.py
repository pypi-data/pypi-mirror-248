...
# templates_file = os.path.join(os.path.dirname(insurancespell.__file__),"sp.model")
# dirpath = 'insurancespellcheck/'
...
def get_correct_word(text:str, dictionary:dict = mydict):
...
def check(text: str,autocorrect: bool=False, correct_word: bool = False):
    ...
    if correct_word:
        ...
        while i<ii:
            if listall[i]=="<คำผิด>":
                output+=get_correct_word(listall[i+1])
                i+=3
            else:
                output+=listall[i]
                i+=1
    if autocorrect:
        ## modify code replace with check in mydict insurance
        output=output.replace("<คำผิด>","|---|<คำผิด>|---|").replace("</คำผิด>","|---|</คำผิด>|---|")
        listall=output.split("|---|")
        i=0
        output=""
