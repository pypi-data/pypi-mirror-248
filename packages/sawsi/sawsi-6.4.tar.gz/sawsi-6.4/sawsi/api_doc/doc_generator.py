...
# html 파일의 경로를 알아냅니다.
html_path = os.path.join(directory_path, 'api_doc_template.html')
...
class DocMaker:
    ...
    def req(self, api_name:str, api_description:str, body:dict, headers:dict, ignore_err=False)->Optional[dict]:
        # 테스트
        response = None
        if self.do_test:
            try:
...
def read_html(api_base_url:str= 'main_api.json'):
    """
...
def read_json(json_file_name:str= 'main_api.json'):
