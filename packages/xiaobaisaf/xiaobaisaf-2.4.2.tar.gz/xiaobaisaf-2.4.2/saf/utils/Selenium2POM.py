...
from saf.utils import SELENIUM_MANAGER_FULL_PATH
...
class PageListener(object):
    def __init__(self, start_url: str = 'https://www.baidu.com', dirname: str = '.', rewrite: bool = True):
        ...
        try:
            ...
            system(f'{SELENIUM_MANAGER_FULL_PATH}/selenium-manager --browser chrome --cache-path {SELENIUM_MANAGER_FULL_PATH}')
            CHROMEDRIVER_FULL_PATH = glob.glob(f'{SELENIUM_MANAGER_FULL_PATH}/**/chromedriver**', recursive=True)[0]
            shutil.copy(CHROMEDRIVER_FULL_PATH, f'{self.PROJECT_PATH}/')
            self.driver = webdriver.Chrome(service=Service(CHROMEDRIVER_FULL_PATH))
    ...
    def converter(self, page_name: str, url: str, input_list: list, button_list: list):
        ...
        function_strings.append('#********************************#')
        function_strings.append(f'')
        function_strings.append('from . import By')
        function_strings.append(f'')
