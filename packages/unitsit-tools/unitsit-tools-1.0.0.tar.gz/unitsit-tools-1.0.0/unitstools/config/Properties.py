    ...
    def _parse_properties(self, config_dict: dict, depth: int):
        for key, value in config_dict.items():
            ...
            if depth > 1 and isinstance(value, dict):
                self.__dict__['__dict__'][key] = PropertiesConfigDict(value, depth=depth - 1)
            else:
        ...
        with open(filepath, 'r') as f:
            for line in f:
                ...
                if key_value_match:
                    ...
                    keys = key.replace(' ', '_').replace('-', '_').rstrip('_').split('.')
                    current_dict = properties

                    for k in keys[:-1]:
                        if '[' in k:
                            list_key, index = re.match(r'(.+)\[(\d+)]', k).groups()
                            index = int(index)
                            current_dict = current_dict.setdefault(list_key, [{}] * (index + 1))
