...
class LogFileParser:
    ...
    def __init__(self, log_format: str = None):
        ...
        :type log_format: str
        """
        self.log_format = log_format or (r'\[(?P<timestamp>.*?)\] '
                                         r'(?P<level>\w+) '
    ...
    def parse_log_line(self, log_line: str, force: bool = False) -> Optional[LogEntry]:
        ...
        if match:
            ...
            return LogEntry(timestamp=timestamp, level=level, filename=filename, lineno=lineno, message=message)
        elif force:
            return LogEntry(timestamp=datetime.now(), level='UNPARSED', filename='', lineno=0, message=log_line)
        else:
            return None
    ...
    def parse_log_file(self, log_file_path: str, force: bool = False) -> List[LogEntry]:
        ...
        if force and unparsable_entries:
            ...
            log_entries.append(
                LogEntry(timestamp=timestamp, level=level, filename=filename, lineno=lineno, message=message))
