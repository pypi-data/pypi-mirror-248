...
class TimedRotatingLog:
    ...
    def __init__(self,
                 ...
                 timer: int = 1440,
                 max_backups: int = 14,
                 log_format: str = None  # Added log_format parameter
                 ) -> None:
        """
        ...
        log_format = log_format or "[%(asctime)s] %(levelname)s [%(filename)s:%(lineno)d] %(message)s"

        self.handler.setFormatter(
            Formatter(log_format, '%Y-%m-%d %H:%M:%S'))
