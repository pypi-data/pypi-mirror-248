...
class AsyncPGCRUD:
    ...
    def _build_select_query(self, conditions=None, columns_clause='*'):
        ...
        where_conditions = ("WHERE " + " AND ".join(
            ...
            if conditions else ''
        return (
            f"SELECT {columns_clause} FROM {self.table_name} {where_conditions}"
        ), list(conditions.values()) if conditions else None
    ...
    def _build_update_query(self, data, conditions):
        ...
        where_conditions = ', '.join([f'{key} = ${i + len(data) + 1}' for i, key in enumerate(conditions)])
    ...
    def _build_delete_query(self, conditions):
        ...
        :rtype: tuple
        """
        where_conditions = ', '.join([f'{key} = ${i + 1}' for i, key in enumerate(conditions)])

        return (
            f"DELETE FROM {self.table_name} WHERE {where_conditions}"
    ...
    async def get_random_item(self):
        ...
        async with self._get_connection_cursor() as connection:
            ...
            if total_rows > 0:
                ...
                select_query = (
                    f"SELECT * FROM {self.table_name} OFFSET $1 LIMIT 1"
                )
                return await connection.fetchrow(select_query, random_offset)
            else:
                return None

    async def execute_query(self, query: str, params: Union[None, tuple] = None, fetch_result: bool = False):
        """
        ...
        async with self._get_connection_cursor() as connection:
            ...
            if fetch_result:
                result = await connection.fetch(query,
                                                *prepared_params) if prepared_params else await connection.fetch(
                    query)
            else:
                result = None
                await connection.execute(query, *prepared_params) if prepared_params else await connection.execute(
                    query)
