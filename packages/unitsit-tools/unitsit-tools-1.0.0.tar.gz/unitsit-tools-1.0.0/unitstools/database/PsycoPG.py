...
class PsycoPGCRUD:
    def __init__(self, connection_params: Dict[str, Union[str, int]], table_name: str, columns: Dict[str, str],
                 binary: bool = False):
        """
    ...
    def _build_select_query(self, conditions: Dict[str, Union[str, int]] = None,
                            columns_clause: sql.SQL = sql.SQL('*')):
        """
        Build SQL SELECT query.

        :param conditions: Dictionary containing column-value pairs for WHERE clause.
        :param columns_clause: SQL fragment for the columns to be selected.
        :return: SQL query.
        ...
        ...     print(result)
        """
        where_conditions = sql.SQL(' WHERE ') + sql.SQL(' AND ').join(
            sql.SQL('{key} = %s').format(key=sql.Identifier(key)) for key in conditions.keys()
        ) if conditions else sql.SQL('')

        return sql.SQL(
            'SELECT {columns} FROM {table}{where_conditions}'
        ).format(
    ...
    def read_columns(self, conditions: Dict[str, Union[str, int]], columns: List[str] = None):
        """
        ...
        if not columns:
            ...
            columns_clause = sql.SQL(', ').join(map(sql.Identifier, columns))
    ...
    def _build_update_query(self, data: Dict[str, Union[str, int]], conditions: Dict[str, Union[str, int]]):
        """
    ...
    def _build_delete_query(self, conditions: Dict[str, Union[str, int]]):
        ...
        return sql.SQL(
            "DELETE FROM {table} WHERE {where_conditions}"
        ).format(
            table=sql.Identifier(self.table_name),
            where_conditions=where_conditions
        )
    ...
    def get_random_item(self):
        ...
        >> print(random_record)
        """
        count_query = sql.SQL("SELECT COUNT(*) FROM {table}").format(table=sql.Identifier(self.table_name))
    ...
    def execute_query(self, query: str, params: Union[None, tuple] = None, fetch_result: bool = False):
        """
