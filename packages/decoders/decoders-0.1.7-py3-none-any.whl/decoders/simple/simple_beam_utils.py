...
class SimpleBeamSearch(object):
    ...
    def add(self, node, finished=False):
        ...
        if finished:
            if node.log_prob == float("-inf"): # this is the only hack in the algorithm. Discard nodes with -inf log prob
                return
    ...
    def prune(self, keep_k_always_alive=False):
        ...
        n_beams_to_keep = self.beam_size if keep_k_always_alive else self.beam_size - finished
        for _ in range(n_beams_to_keep):
            node = self.alive_nodes.get_nowait()
            nodes.put(node)
