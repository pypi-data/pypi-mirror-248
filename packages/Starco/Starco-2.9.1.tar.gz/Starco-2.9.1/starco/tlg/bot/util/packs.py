...
def LoginTlgAccount(self:Conversation):
    ...
    p.db_config={'tlg_accounts':{'id':0,'u_id':0,'status':0,'messaging':0,'spam_check':0,'group':'','fastremove':0,'limit_status':'','limit_time':0,'last_check':0,'active_sessions':0,'time':0}}
    e=Node()
...
def ReloginTlgAccount(self:Conversation):
    ...
    def add_pendding(self: Conversation):
        ...
        time.sleep(1)
        oid = get_number(tlg.number)
        self.db.do('tlg_accounts',{'group':'relogin'},condition=f"id={oid}")
