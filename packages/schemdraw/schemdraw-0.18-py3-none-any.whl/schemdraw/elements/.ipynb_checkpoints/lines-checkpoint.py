...
class Arrow(Line):
    ...
    def __init__(self, *d,
                 ...
                 headlength: Optionalfloat] = None,
                 **kwargs):
        super().__init__(*d, arrowlength=headlength, arrowwidth=headwidth, **kwargs)
        self.double = double
        self.headwidth = headwidth if headwidth else self.defaults.get('headwidth', .15)
        self.headlength = headlength if headlength else self.defaults.get('headlength', .25)
...
class Dot(Element):
    ...
    '''
    def __init__(self, *d, radius: Optional[float] = None, open: bool = False, **kwargs):
        super().__init__(*d, **kwargs)
        radius = radius if radius else self.defaults.get('radius', 0.075)
...
class Arrowhead(Element):
    ...
    '''
    def __init__(self, *d, headwidth: Optional[float] = None, headlength: Opetional[float] = None, **kwargs):
        super().__init__(*d, **kwargs)
        headwidth = headwidth if headwidth else self.defaults.get('headwidth', .15)
        headlength = headlength if headlength else self.defaults.get('headlength', .25)
        self.segments.append(Segment([
            (-headlength, 0), (0, 0)], arrowwidth=headwidth, arrowlength=headlength, arrow='->'))
        self.anchors['start'] = (0, 0)
        self.anchors['center'] = (0, 0)
        self.anchors['end'] = (0, 0)
        self.params['lblofst'] = self.defaults.get('lblofst', .25)
