...
def make(import_path, package_type_opt, name, email, ignore_packaged, ignore_salsa):
    ...
    with MakeGoSession() as session:
        ...
        click.echo("Checking for binaries")
        ...
        package_type: PackageType = (
            ...
            .skip_if(len(all_binaries) == 0, PackageType.LIBRARY)
            .skip_if(
                package_type_opt is not None, PackageType.from_opt(package_type_opt)
            )
            .ask()
        )
...
"""
        ...
        # TODO: instructions on what to do next
    ...
    def prepare_upstream(self, source_package, git_repo):
        with TemporaryDirectory() as t:
            ...
            if len(tags) > 0:
                ...
                version = (f"0.0~git{date}.{hash[:7]}", hash)
                versioned = False
