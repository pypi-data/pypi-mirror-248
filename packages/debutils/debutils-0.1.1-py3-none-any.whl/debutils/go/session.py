...
class GoSession:
    ...
    def get_package_modules(self, package):
        if not (self.gopath / "src" / package).exists():
            # This is a last resort, it may fail due to no Go files in the
            # package root.
    ...
    def go_get(self, package):
        try:
            self.go_get_module(package + '/...')
        except NoGoFilesError:
            for module in self.get_package_modules(package):
                self.go_get_module(module)
