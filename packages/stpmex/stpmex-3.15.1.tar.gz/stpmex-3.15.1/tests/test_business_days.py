...
@pytest.mark.parametrize(
    'utc_current_datetime, expected_date',
    [
        (dt.datetime(2023, 11, 6, 23, 59, 59), dt.date(2023, 11, 6)),
        (dt.datetime(2023, 11, 7, 0, 0, 0), dt.date(2023, 11, 7)),
        (dt.datetime(2023, 11, 10, 23, 59, 59), dt.date(2023, 11, 10)),
        (dt.datetime(2023, 11, 11, 0, 0, 0), dt.date(2023, 11, 13)),
    ],
)
def test_current_working_date(
    utc_current_datetime: dt.datetime, expected_date: dt.date
):
    with freeze_time(utc_current_datetime):
        working_date = get_current_working_day()
    assert working_date == expected_date
