...
class calculator:
    ...
    @calib_dir.setter
    def calib_dir(self, value):
        try:
            os.makedirs(value, exist_ok=True)
        except:
            log.error(f'Cannot make calib directory: {value}')
            raise

        self._calib_dir = value
    #----------------------------------------
    @property
    def evt_mix(self):
    ...
    def _set_dir(self, name):
        if getattr(self, f'_{name}_dir') is not None:
            return

        env_var = {'cache' :        'CASDIR', 'calib' : 'CALDIR'}[name]
        dir_knd = {'cache' : 'cb_calculator', 'calib' : 'scales'}[name]

        try:
            dir_path = os.environ[env_var]
        except:
            log.error(f'{env_var} not found in environment')
            raise

        dir_path = f'{dir_path}/{dir_knd}/{self._vers}'
        setattr(self, f'_{name}_dir', dir_path)

        try:
            os.makedirs(dir_path, exist_ok=True)
        except:
            log.error(f'Cannot make cache directory: {dir_path}')
            raise
    #----------------------------------------
    def _set_cuts(self):
        bdt_cut = rs.get('bdt', self._trig, q2bin=self._q2bin, year = self._dset)
        qsq_cut = rs.get('q2' , self._trig, q2bin=self._q2bin, year = self._dset)

        qsq_cut     = qsq_cut.replace('&&', 'and')
        bdt_cut_sgn = bdt_cut.replace('&&', 'and')
        bdt_cut_inv = bdt_cut_sgn.replace('BDT_cmb >', 'BDT_cmb <')

        self._bdt_cut_inv = bdt_cut_inv
        self._bdt_cut_sgn = bdt_cut_sgn
    ...
    def _remove_spikes(self, df, sample):
        ...
        return df
    #----------------------------------------
    def _filter(self, df, sample, invert_bdt=True):
        ...
        eff_qsq = efficiency(nqsq, arg_tot=nbdt, cut=self._qsq_cut)
        ...
        cfl_path = f'{self._plot_dir}/cfl_{sample}_{invert_bdt}.json'
    ...
    def _replace_mass(self, df):
        ...
        return df
    #----------------------------------------
    def _get_data(self, sample=None, invert_bdt=True):
        key = f'{sample}_{invert_bdt}'
        if key in self._d_data:
            return self._d_data[key]

        proc    = self._d_sample[sample]
        trig    = self._trig[1:]
        dat_dir = f'{self._cache_dir}/{proc}_{self._channel}_{self._dset}_{trig}'
        l_path  = glob.glob(f'{dat_dir}/data*.json')
        ...
        df   = self._filter(df, sample, invert_bdt = invert_bdt)
    ...
    def _fit(self, df, name, title):
        ...
        res=obj.fit(ntries=15, ranges=[(self._low_m, self._low_blind), (self._high_blind, self._high_m)])
    ...
    def _get_quantiles(self, df):
        arr_bdt = df.BDT_cmb.to_numpy()
        arr_qnt = numpy.quantile(arr_bdt, self._l_bdt_quant)
    ...
    def _get_pars(self):
        ...
        d_df_ss = self._bin_df(df_ss)
        d_df_os = self._bin_df(df_os)
    ...
    def _save_pars(self, d_par):
        scale_path = f'{self._calib_dir}/{self._q2bin}_{self._const_pref}_{self._trig}_{self._dset}.json'
