...
void match_track_mtx(PyObject *posix_path, PyObject *paths, PyObject *trees, PyObject *dead, PyObject *paths_sizes, PyObject *trees_sizes, PyObject *dead_sizes, PyObject *matching, PyObject *result)
{
    // Convert PosixPath to string
    PyObject *posix_path_str = PyObject_Str(posix_path);
    if (posix_path_str == NULL)
    {
        PyErr_Print();
        Py_DECREF(posix_path);
        // Handle error
        return;
    }
    // Print the string representation
    FILE *f;
    if ((f = fopen(PyUnicode_AsUTF8(posix_path_str), "r")) == NULL)
    {
        fprintf(stderr, "Error: Cannot open %s!\n", PyUnicode_AsUTF8(posix_path_str));
        exit(1);
    }
    Graph *G;
    int N, EdgeListSize;
    ...
    long N_label = 0;
    ForAllGraphVertices(V, G, P)
        VertexRelabel(V, (VertexData)N_label++);
    ForAllEdges(E, M, P)
    {
        PyObject *the_tuple = PyTuple_New(tuple_length);
        if (the_tuple == NULL)
...
void match_track_mtx_openmp(PyObject *posix_path, PyObject *paths, PyObject *trees, PyObject *dead, PyObject *paths_sizes, PyObject *trees_sizes, PyObject *dead_sizes, PyObject *matching, PyObject *result)
{
    // Convert PosixPath to string
    PyObject *posix_path_str = PyObject_Str(posix_path);
    if (posix_path_str == NULL)
