...

{
   ...
   if ((ret_code = mm_read_mtx_crd_size(stream, N, N, M)) != 0)
      exit(1);
   ...
   G->VertexArray = (Vertex **)Allocate(*N * sizeof(Vertex *));
   if (G->VertexArray == NULL)
      Error("(ReadMTX) Memory allocation failed.");
   ...
   if (mm_is_pattern(matcode))
   {
      for (i = 0; i < *M; i++)
      {
         scanErr = fscanf(stream, "%d %d\n", &a, &b);
         if (scanErr != 2)
            printf("Error reading edge!");
         // fprintf(stdout, "%d %d\n", a, b);
         CreateEdge(G, G->VertexArray[a - 1], G->VertexArray[b - 1], Nil);
      }
   }
