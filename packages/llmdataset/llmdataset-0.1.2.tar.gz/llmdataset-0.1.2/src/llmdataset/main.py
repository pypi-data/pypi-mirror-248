...
class LLMdataset:
    def __init__(
        ...
        if self.dataset_name:
            self.data_types, self.num_data, self.dataset = self._select_dataset()
            print(f"Data types:{self.data_types}")
            print(f"Number of data:{self.num_data}")

    def _select_dataset(self):
        if self.dataset_name == 'gsm8k':
            data_types, data_count, dataset = gsm8k_dataset(data_type='train')
            return data_types, data_count, dataset
        elif self.dataset_name == 'multiarith':
            data_types, data_count, dataset = multiarith_dataset(data_type='train')
            return data_types, data_count, dataset
    ...
    def dataloader(self, data_type = None, batch_size=1, seed=None, max_data=5):
        self.data_list = self._split_dataset(data_type)
        ...
        if max_data is not None:
            data_list = random.sample(self.split_dataset, max_data)
        else:
