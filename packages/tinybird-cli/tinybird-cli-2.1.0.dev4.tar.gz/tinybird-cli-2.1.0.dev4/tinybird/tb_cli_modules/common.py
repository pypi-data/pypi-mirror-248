...
async def deploy_environment(
    config: CLIConfig,
    branch_id: str,
    current_main_workspace: Dict[str, Any],
    wait: Optional[bool],
    verbose: Optional[bool],
    commit: str,
    semver: str,
    data_migrations: Optional[Dict[str, Any]],
    dry_run: bool,
):
    """
    ...
    prev_workspace: str = config["name"]

    try:
        response = await config.get_client().deploy_environment(
            branch_id, semver, commit, data_migrations=data_migrations, dry_run=dry_run
        )
        await switch_workspace(config, current_main_workspace["name"])
        ...
        if wait and not dry_run:
            ...
            await switch_workspace(config, prev_workspace, only_environments=True)
    except Exception as e:
        raise CLIException(FeedbackManager.error_exception(error=str(e)))
