...
@coro
async def deploy_env(semver: str, wait: bool, verbose: bool, dry_run: bool, migrations: str) -> None:
    config = CLIConfig.get_project_config()
    ...
    workspaces: List[Dict[str, Any]] = (await config.get_client().user_workspaces_and_branches()).get("workspaces", [])
    current_ws: Dict[str, Any] = next(
        (workspace for workspace in workspaces if config and workspace.get("id", ".") == config.get("id", "..")), {}
    )

    current_release: Optional[Dict[str, Any]] = current_ws.get("release")
    ...
    data_migrations: Optional[Dict[str, Any]] = None
    if migrations:
        with open(migrations, "r") as file:
            data_migrations = json.loads(file.read())

    await deploy_environment(
        config, config["id"], current_main_workspace, wait, verbose, commit, semver, data_migrations, dry_run
    )
