...
@pytest.fixture
def taxrule_zero(event):
    return event.tax_rules.create(rate=Decimal("0.00"))
...
def _build_order(item, event, testtime, question, taxrule, taxrule_zero, is_business):
    codice_destinatario = (
...
@pytest.fixture
def private_order(event, item, taxrule, taxrule_zero, question):
    ...
    with mock.patch("django.utils.timezone.now") as mock_now:
        ...
        return _build_order(
            item, event, testtime, question, taxrule, taxrule_zero, is_business=False
        )


@pytest.fixture
def business_order(event2, item2, taxrule, taxrule_zero, question):
    testtime = datetime.datetime(2017, 12, 1, 10, 0, 0, tzinfo=datetime.timezone.utc)

    with mock.patch("django.utils.timezone.now") as mock_now:
        mock_now.return_value = testtime
        return _build_order(
            item2, event2, testtime, question, taxrule, taxrule_zero, is_business=True
        )
