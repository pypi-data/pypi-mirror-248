    ...
    def get_entity_code(self, reduced=True, reduce_empty_lines=False, include_line_number=False,
        ...
        if reduced:
            for nested_entity in nested_entities:
                ...
                if include_line_number:
                    enumerated_entity_code += self.enumerate_code_lines(code_to_add, last_line, file_end_line)
                else:
                    enumerated_entity_code += code_to_add
                # Add the nested entity's function definition line
                code_to_add = ''.join(lines[nested_start:nested_declaration_end + 1])
                entity_code += code_to_add
                if include_line_number:
                    enumerated_entity_code += self.enumerate_code_lines(code_to_add, nested_start + 1,
                                                                        file_end_line)  # +1 here is not tested
                else:
                    enumerated_entity_code += code_to_add
                # Add the description as a comment, replacing the function body
                enumerated_entity_code += f"{indentation}    # {description}\n"
        ...
        if include_line_number:
            enumerated_entity_code += self.enumerate_code_lines(actual_code, last_line, file_end_line)
        else:
            enumerated_entity_code += actual_code

        if reduce_empty_lines and not include_line_number:
            entity_code = remove_empty_lines(entity_code)
            enumerated_entity_code = remove_empty_lines(enumerated_entity_code)
        return entity_code, enumerated_entity_code
    ...
    def get_code_from_path(self, start_line=None, code_for_prompt=None, show_nested=True, include_line_number=False,
        ...
        if start_line:
            ...
            if not code_for_prompt:
                if entity:
                    self.current_entity = entity
                    entity_code, enumerated_entity_code = self.get_entity_code(file_path=file_path, reduced=not show_nested,
                                                                               include_line_number=include_line_number, file_type=file_type,
                                                                               file_end_line=file_end_line)
                    code_for_prompt = enumerated_entity_code
                    ...
                    for entity_info in entity_lines:
                        self.set_bookmark(entity_info)
                        entity_code, enumerated_entity_code = self.get_entity_code(reduced=not show_nested, include_line_number=include_line_number)
                        code_for_prompt += enumerated_entity_code
                        code_for_hash += entity_code
