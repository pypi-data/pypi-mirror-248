...
def download_and_extract_repo(https_link, branch=None, headers=None):
    ...
    with tempfile.TemporaryDirectory() as temp_dir, \
        ...
        for root, _, files in os.walk(temp_dir):
            for file in files:
                ...
                if exclude_files(file_path, ignore_files):
                    continue
                ...
                formatted_content = indent_code(file_content)
...
def print_repo_content_json(repo_content, root_dir, ignore_files):
    json_structure = convert_to_json(repo_content, root_dir, ignore_files)
    formatted_json = json.dumps(json_structure, indent=2, separators=(',', ': '), sort_keys=True)
    return formatted_json

def get_github_repo_branches(repo_link, headers=None):
...
def harvest_github_repo(repo_link, branch=None, access_token=None):
    ...
    ignore_files = read_ignore_files(ignore_file_path)

    if not branch:
        branches = get_github_repo_branches(repo_link, headers=headers)
        if not branches:
    ...
    repo_content = download_and_extract_repo(f"{repo_link}/archive/{branch}.tar.gz", headers=headers)
    if not repo_content:
        print("Failed to download and extract repository content.")
        return None

    root_directory = os.getcwd()
    repo_content = print_repo_content_json(repo_content, root_directory, ignore_files)
