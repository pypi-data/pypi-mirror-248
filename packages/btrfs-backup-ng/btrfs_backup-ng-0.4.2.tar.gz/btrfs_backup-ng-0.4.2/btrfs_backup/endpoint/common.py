...
class Endpoint:
    ...
    def prepare(self):
        logging.info(f"Preparing endpoint {self} ...")
    ...
    def snapshot(self, readonly=True, sync=True):
        ...
        logging.info(f"{self.source} -> {snapshot_path}")
    ...
    def list_snapshots(self, flush_cache=False):
        ...
        if self.__cached_snapshots is not None and not flush_cache:
            logging.debug(
                f"Returning {len(self.__cached_snapshots)} cached snapshots for {self}."
            )
            return list(self.__cached_snapshots)

        logging.debug(f"Building snapshot cache of {self} ...")
        snapshots = []
        ...
        logging.debug(
            f"Populated snapshot cache of {self} with {len(snapshots)} items."
        )
    ...
    def set_lock(self, snapshot, lock_id, lock_state, parent=False):
        ...
        logging.debug(
            f"Lock state for {snapshot} and lock_id {lock_id} changed to {lock_state} (parent = {parent})"
        )
    ...
    def delete_snapshots(self, snapshots, **kwargs):
        ...
        logging.info(f"Removing {len(to_remove)} snapshot(s) from {self}:")
        for snapshot in snapshots:
            if snapshot in to_remove:
                logging.info(f"  {snapshot}")
            else:
                logging.info(f"  {snapshot} - is locked, keeping it")
    ...
    def _build_snapshot_cmd(self, source, destination, readonly=True):
    ...
    def _read_locks(self):
        ...
        try:
            ...
            logging.error(f"Error on reading lock file {path}: " "{e}")
    ...
    def _write_locks(self, lock_dict):
        ...
        path = self._get_lock_file_path()
        try:
            logging.debug(f"Writing lock file: {path}")
            with open(path, "w") as f:
                f.write(util.write_locks(lock_dict))
        except OSError as e:
            logging.error(f"Error on writing lock file {path}: {e}")
            raise util.AbortError()
