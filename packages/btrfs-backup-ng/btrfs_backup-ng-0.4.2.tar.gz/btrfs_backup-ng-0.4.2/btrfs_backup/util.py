...
def exec_subprocess(cmd, method="check_output", **kwargs):
    ...
    try:
        ...
        logging.error(f"Error on command: {cmd}")
        raise AbortError()
...
def date2str(timestamp=None, fmt=None):
...
def str2date(time_string=None, fmt=None):
...
def is_btrfs(path):
    ...
    logging.debug(f"Checking for btrfs filesystem: {path}")
    best_match = ""
    best_match_fs_type = ""
    logging.debug(f"  Reading mounts file: {MOUNTS_FILE}")
    for line in open(MOUNTS_FILE):
        try:
            mount_point, fs_type = line.split(" ")[1:3]
        except ValueError as e:
            logging.debug(f"  Couldn't split line, skipping: {line}")
            continue
        ...
        if (path == mount_point or path.startswith(mount_point_prefix)) and len(
            ...
            logging.debug(
                f"  New best_match with filesystem type {best_match_fs_type}: {best_match}"
            )
    result = best_match_fs_type == "btrfs"
    logging.debug(
        f"  -> best_match_fs_type is {best_match_fs_type}, result is {result}"
    )
    return result
...
def is_subvolume(path):
    ...
    if not is_btrfs(path):
        return False
    logging.debug(f"Checking for btrfs subvolume: {path}")
    # subvolumes always have inode 256
    st = os.stat(path)
    result = st.st_ino == 256
    logging.debug(f"  -> Inode is {st.st_ino}, result is {result}")
    return result
...
def read_locks(s):
    ...
    try:
        ...
        logging.error(f"Lock file couldn't be parsed: {e}")
