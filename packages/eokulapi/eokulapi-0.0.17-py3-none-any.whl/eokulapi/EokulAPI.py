...
"""API base URL"""
api_fixed_password = "#9dP^4fG7tL2wKq3zE*rY6vA!5uJxIhS&1mC0oX8bZ$N;W~Q"
...
class EokulAPI:
    ...
    def _update_lesson_schedule(self, student: EokulStudent) -> dict:
        ...
        try:
            ...
            student.lesson_schedule = self.__create_object_from_dict(
                LessonSchedule, resp
            )
            return resp.json()
        except RuntimeError as e:
            if "Table 0" in str(e):
                self.__logger.info("lesson schedule query returned an error")
                self.__logger.debug(e, exc_info=True)
                student.lesson_schedule = LessonSchedule.empty()
                return {}
