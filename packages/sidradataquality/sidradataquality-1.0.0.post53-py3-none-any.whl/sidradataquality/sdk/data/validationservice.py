...
class ValidationService():
    ...
    def execute_validations_for_entity_asset(self, entity_id, asset_id):
        ...
        entity_suite_name = self.metadata_entity_service.get_entity_suite(entity.item_id)

        # If there is no suite for the entity, we skip the execution
        if entity_suite_name is None:
            return False
        ...
        table_name = f"{provider.provider_name}.{entity.name}"
        ...
        # Get/Create validation context for entity
        context = self.get_entity_context(const.STORAGE_DEFAULT_CONTAINER, provider.provider_name, entity.name)
        run_id = str(asset.id)
        self.metadata_validation_service.set_asset_results(provider.provider_name, entity.name, entity.item_id, asset.item_id, const.ASSET_VALIDATION_STATUS_PENDINGVALIDATION)

        # Check all data vs sampled data
        self.metadata_validation_service.set_asset_results(provider.provider_name, entity.name, entity.item_id, asset.item_id, const.ASSET_VALIDATION_STATUS_INPROGRESS)
        check_full_asset, error_count = self.check_sampled_data_if_required(context, entity.name, entity_suite_name, entity_validations, table_name, asset.id, f"{run_id}-sampled")
        ...
        if check_full_asset is False:
            ...
            self.metadata_validation_service.set_asset_results(provider.provider_name, entity.name, entity.item_id, asset.item_id, const.ASSET_VALIDATION_STATUS_ERROR, error_count)
        else:
            # Check all data
            self.logger.debug(f"[Validation Service][execute_validations_for_entity][Asset_{asset.id}] Execute validations for all data")
            df_to_validate = self.get_validation_df(context, entity.name, table_name, asset.id)
            validation_result_data, validation_result = self.execute_validations(context, df_to_validate, entity_suite_name, entity_validations, run_id, entity_pk_columns)
                    
            storage_entity_path = f'{provider.provider_name}/{entity.name}/{datetime.now().strftime("%Y/%m/%d")}'
            # Save HTML report in storage account
            file_name = f"{run_id}.html".lower()
            file_path = f'{const.STORAGE_REPORTS_FOLDER}/{storage_entity_path}'.lower()
            self.logger.debug(f"[Validation Service][execute_validations_for_entity][Asset_{asset.id}] Report path: {file_path}")
            self.storage_service.upload_file_to_data_quality_storage(file_name, file_path, validation_result_data[const.VALIDATION_RESULT_DATA_REPORT], const.STORAGE_DEFAULT_CONTAINER)
            # Save JSON results in storage account
            file_name = f"{run_id}.json".lower()
            file_path = f'{const.STORAGE_RESULTS_FOLDER}/{storage_entity_path}'.lower()
            self.logger.debug(f"[Validation Service][execute_validations_for_entity][Asset_{asset.id}] Results path: {file_path}")
            self.storage_service.upload_file_to_data_quality_storage(file_name, file_path, str(validation_result), const.STORAGE_DEFAULT_CONTAINER)
            ...
            self.metadata_validation_service.set_asset_results(provider.provider_name, entity.name, entity.item_id, asset.item_id, const.ASSET_VALIDATION_STATUS_VALIDATED, validation_result_data[const.VALIDATION_RESULT_DATA_ROWS_FAILED_COUNT], file_name)
            self.metadata_validation_service.set_entity_delta(entity.item_id)
        return True

    def get_entity_context(self, container_name, provider_name, entity_name):
        return gx.get_context(context_root_dir=f"/dbfs/{container_name}/{provider_name}/{entity_name}/great_expectations/")
