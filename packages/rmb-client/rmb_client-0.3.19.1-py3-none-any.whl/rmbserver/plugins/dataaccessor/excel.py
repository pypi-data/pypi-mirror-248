...
from rmbcommon.tools import gen_base62_uuid
...
2. 表名需要像这样`table_name`引起来。"""
    ...
    def create_file_with_query(self, struc_query: StrucQuery,
                               meta_data: MetaData, file_type='excel') -> str:
        if file_type != 'excel':
            raise ValueError(f"file_type must be 'excel', but got {file_type}")

        file_name = f"{gen_id_for_excel_file()}.xlsx"

        # 将DataFrame保存到一个内存中的文件对象
        excel_file = BytesIO()
        rst = self.query(struc_query, meta_data=meta_data)
        rst.content.to_excel(excel_file)
        excel_file.seek(0)  # 重置文件指针到开始

        # 上传到OSS
        oss_bucket.put_object(file_name, excel_file)
        # 设置URL的过期时间（例如，1小时）
        expire_time = timedelta(hours=1)
        url = oss_bucket.sign_url('GET', file_name, expire_time.total_seconds())
