...
class Asset(BaseModel):
    ...
    def to_disk(self, folder: Path):
        ...
        try:
            ...
            msg.warn(f"Skip phrase construction: {self.id=}")

        try:
            self.matchers_to_disk(folder)
        except Exception:
            msg.warn(f"Skip pattern construction: {self.id=}")
    ...
    def query(
        ...
        for fts_expr in randomize_fts_expr(q, chunk_limit):
            params = {"fts_expr": fts_expr, "num_segments": results_per_query}
    ...
    def split(
        ...
        msg.info(f"Num Train Docs: {len(train_docs)}")
        msg.info(f"Num Dev Docs: {len(dev_docs)}")
