...
class Asset(BaseModel):
    ...
    def query(
        ...
        sql_footnotes = """--sql
            ...
            where opinion_footnotes_fts match :fts_expr and os.char_count between 50 and 2000
            limit :num_results -- prevent Republic v. CA and other common titles to bias results
        ;"""
    ...
    def split(
        ...
        """Split the docs that have been converted by `parse` into two categories: dev and train.
    ...
    def bin(
        ...
        """Combine `.split()` with `.parse()`:

        1. Query database text that match q.txt / terms
        2. Convert each text to a spaCy Doc
        3. Store each Doc in a DocBin, serialized to .spacy format
    ...
    def nlp(self, base_model: str = "en_core_web_md", params_file: Path = Path("params.json.gz")) -> Language:
        msg.info(f"Loading {self.id} gzip settings for tokenizer/patterns.")
        ...
        limited_patterns = [p for p in cfg["span_ruler_patterns"] if p["id"] == self.id]
        ruler.add_patterns(limited_patterns)  # type: ignore
        return nlp
